...
function doInit(){
    ...
    initEmEdit(EM_IN_DT, "YYYYMMDD", PK);            //입고신청일자
   
    //콤보 초기화
    initComboStyle(LC_S_STR,DS_O_S_STR, "CODE^0^30,NAME^0^80", 1, PK);          		//점구분(조회)
    initComboStyle(LC_STR,DS_O_STR, "CODE^0^30,NAME^0^80", 1, PK);              		//점구분(등록)
    ...
    registerUsingDataset("mgif116", "DS_IO_MASTER,DS_IO_DETAIL");  

}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}    name="NO"             width=25   align=center</FC>'
    	             + '<FC>id=STR_CD      name="점"             width=100  align=left         EditStyle=Lookup    Data="DS_O_STR:CODE:NAME"  </FC>'
                     + '<FC>id=IN_DT       name="입고일자"       width=100  mask="XXXX/XX/XX"  align=center</FC>'
                     + '<FC>id=IN_SLIP_NO  name="입고번호"       width=100  align=center</FC>'
                     + '<FC>id=GIFT_TYPE_CD  name="상품권종류"   width=100  align=left         EditStyle=Lookup    DATA="DS_O_GIFT_TYPE_CD:CODE:NAME"</FC>'
                     + '<FC>id=IN_QTY_SUM  name="수량"        sumtext=@sum    width=100   align=right</FC>'
                     + '<FC>id=IN_AMT_SUM  name="입고금액"    sumtext=@sum    width=150   align=right</FC>';
                     
    initGridStyle(GD_MASTER, "common", hdrProperies, false);
    
    var hdrProperies1 = '<FC>id={currow}    name="NO"         width=30   align=center</FC>'
        + '<FC>id=GIFT_AMT_TYPE	 name="금종"   sumtext="합계"  	 edit=none width=60 align=center</FC>'
        + '<FC>id=GIFT_AMT_NAME	 name="금종명"   				 edit=none width=80 align=left</FC>'
        + '<FC>id=GIFTCERT_AMT	 name="상품권금액" 			     edit=none width=80 align=right</FC>'
        + '<FC>id=STAT_FLAG		 name="STAT_FLAG"  			     edit=none width=80 align=center  show=false </FC>'
        + '<FC>id=GIFT_HEAD		 name="시작"  			         edit=none width=70 align=right  show=false  </FC>'
        + '<FC>id=GIFT_NO		 name="번호"  			         edit={if(GIFT_HEAD = "", "false", true)} EditLimit=6 edit=numeric  width=90  align=center</FC>'
        + '<FC>id=IN_QTY		 name="수량"   sumtext=@sum      edit={if(GIFT_HEAD = "", "false", true)} EditLimit=7 edit=numeric  width=50  align=right</FC>'
        + '<FC>id=GIFT_S_NO		 name="상품권시작번호"  		 edit=none width=150 align=center</FC>'
        + '<FC>id=GIFT_E_NO		 name="상품권종료번호"			 edit=none width=150 align=center</FC>'
...
function btn_Search() {
	...
	TR_MAIN.Action="/mss/mgif116.mg?goTo=getMaster"+strParams;  
...
function btn_New() {
	 if(DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_SLIP_NO") == ""){
		 if (showMessage(QUESTION , YESNO, "USER-1072") != 1) return;
	 ...
	 DS_IO_MASTER.AddRow(); 
	 LC_STR.Index = 0;
	 EM_IN_DT.Text = strToday;
	 DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_DT") = strToday; 
...
function btn_Save() {
    ...
    if(EM_IN_DT.Text.length != 8){
        showMessage(EXCLAMATION, OK, "USER-1003", "입고등록일자");
        EM_IN_DT.Focus();
        return;
    }  
    
    //if(DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_SLIP_NO") == "" && EM_IN_DT.Text < strToday){
  	if(DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_SLIP_NO") == ""&&EM_IN_DT.Text!=EM_IN_DT.Text){
        showMessage(EXCLAMATION, OK, "USER-1030", "입고등록일자");
        EM_IN_DT.Focus();
        return;
    }
    ...
    for(var i=1;i<=DS_IO_DETAIL.CountRow;i++){
    	if (DS_IO_DETAIL.SysStatus(i) != 0) {
    		if (DS_IO_DETAIL.NameValue(i,"IN_QTY") == "" || DS_IO_DETAIL.NameValue(i,"IN_QTY") == "0"){
   				showMessage(EXCLAMATION, OK, "USER-1000", i + "행에 수량을 입력하세요.");
   		        return;
    		}
    		
    		if (DS_IO_DETAIL.NameValue(i,"GIFT_NO") == ""){
  				showMessage(EXCLAMATION, OK, "USER-1000", i + "행에 번호를 입력하세요.");
  		        return;
       		}
    		
    		strGift_S_No = DS_IO_DETAIL.NameValue(i, "GIFT_S_NO");
    		strGift_E_No = DS_IO_DETAIL.NameValue(i, "GIFT_E_NO");
    	    
    		var strParams = "&strStrCd="         + encodeURIComponent(strStrCd)
    		              + "&strGift_S_No="     + encodeURIComponent(strGift_S_No)
    		              + "&strGift_E_No="     + encodeURIComponent(strGift_E_No);
    		
    		TR_MAIN.Action="/mss/mgif116.mg?goTo=getMasterChk"+strParams;  
    	    TR_MAIN.KeyValue="SERVLET(O:DS_O_CHECKLIST=DS_O_CHECKLIST)"; //조회는 O
    	    TR_MAIN.Post();
    	        	    
    	    if(DS_O_CHECKLIST.CountRow > 0 ) {
  				showMessage(EXCLAMATION, OK, "USER-1000", i + "행의 상품구간의 상품이 이미 존재합니다.");
  	    	    strCurRow = 0;
  		        return;
       		}
        ...
        //strSum = parseInt(strSum,10) + parseInt(DS_IO_DETAIL.NameValue(i,"IN_QTY"),10);
    }
    
    //if(strSum == 0 && DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_SLIP_NO") == ""){
    //	showMessage(EXCLAMATION, OK, "USER-1000", "수량을 입력하세요.");
    //    return;
    //}
    ...
    //var strInDt = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_DT");			변경 20171023 jky
    var strRegDt 	  = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_DT");  // 입고등록일자.			추가 20171023 jky
    var strInDt 	  = trim(EM_IN_DT.text)											// 상품권 채번용 일자.		추가 20171023 jky
    var strGiftTypeCd = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "GIFT_TYPE_CD");
    var strInSlipNo   = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_SLIP_NO");
        
    var strParams = "&strStrCd="      + encodeURIComponent(strStrCd)
                  + "&strInDt="      + encodeURIComponent(strInDt)
                  + "&strGiftTypeCd=" + encodeURIComponent(strGiftTypeCd)
                  + "&strInSlipNo="  + encodeURIComponent(strInSlipNo)
    			  + "&strRegDt="      + encodeURIComponent(strRegDt)  				// 입고등록일자.			추가 20171023 jky
					;	
    TR_MAIN.Action="/mss/mgif116.mg?goTo=save"+strParams;
 ...
 *************************************************************************/
 ...
 function setObject(tf_temp) {
    ...
    EM_IN_DT.Enable = strFlag;
    LC_GIFT_TYPE_CD.Enable = strFlag;
    enableControl(IMG_IN_DT, strFlag);
    enableControl(IMG_SEARCH, strFlag);
 }
 ...
 function getDetail() {
     ...
     TR_DTL.Action="/mss/mgif116.mg?goTo=getDetail"+strParams;  
 ...
 function getNewDetail(){
     ...
     var strGiftTypeCd   = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "GIFT_TYPE_CD");
     //var strIN_DT   = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "IN_DT");
     var strIN_DT   = EM_IN_DT.text
     var strSTRCD   = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "STR_CD");

     var strParams = "&strGiftTypeCd="+encodeURIComponent(strGiftTypeCd)
   				   + "&strIN_DT="    +encodeURIComponent(strIN_DT)
   				   + "&strSTRCD="    +encodeURIComponent(strSTRCD);
     
     TR_DTL.Action="/mss/mgif116.mg?goTo=getNewDetail"+strParams;  
     TR_DTL.KeyValue="SERVLET(O:DS_IO_DETAIL=DS_IO_DETAIL)"; //조회는 O
     TR_DTL.Post();
...
<script language=JavaScript for=DS_IO_DETAIL  event=OnColumnChanged(row,colid)>
	...
	switch(colid)
	{
		...
		case "GIFT_NO" :
		  	if(this.NameValue(row, "GIFT_NO") == ""){
				this.NameValue(row, "GIFT_S_NO") = "";
				this.NameValue(row, "GIFT_E_NO") = "";
				this.NameValue(row, "TOT_AMT") = 0;
				return;
		  	}
		  
			if(this.NameValue(row, "IN_QTY") == 0){
				this.NameValue(row, "GIFT_S_NO") = "";
				this.NameValue(row, "GIFT_E_NO") = "";
				this.NameValue(row, "TOT_AMT") = 0;
				return;
			...
			// DC
			//var strGiftNo = this.NameValue(row, "GIFT_S_NO").substring(0,9);
			//var strSNo = this.NameValue(row, "GIFT_S_NO").substring(9,17);
			//var strQty = this.NameValue(row, colid);
			//var strVal = parseInt(strSNo,10) + parseInt(strQty,10) -1;
			//strVal = lpad(strVal.toString(),8,'0');
		  
			var strGiftHead = this.NameValue(row, "GIFT_HEAD");
			var strGiftNo = this.NameValue(row, "GIFT_NO");
			var strSNo = this.NameValue(row, "GIFT_NO");
			
			var strQty = this.NameValue(row, "IN_QTY");
			    	      	  
			var strVal = parseInt(strSNo,10) + parseInt(strQty,10) -1;
			
			strVal = lpad(strVal.toString(),6,'0'); 
			
			this.NameValue(row, "GIFT_S_NO") = strGiftHead + lpad(strGiftNo.toString(),6,'0');
			this.NameValue(row, "GIFT_E_NO") = strGiftHead + strVal;
			this.NameValue(row, "TOT_AMT") = strQty * this.NameValue(row, "GIFTCERT_AMT");
			break;
...
<comment id="_NSID_">
<object id="DS_O_CHECKLIST" classid=<%=Util.CLSID_DATASET%>></object>
</comment>
<script>_ws_(_NSID_);</script>
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>	
						...
						<td colspan=3 width=1400><comment id="_NSID_"> <object
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0"
								...
								<tr>
									<td><comment id="_NSID_"><OBJECT id=GD_MASTER
										width=100% height=210 classid=<%=Util.CLSID_GRID%>>
							...
							<tr>
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0"
									...
									<tr>
										...
										<td width="130"><comment id="_NSID_"> <object id=EM_IN_DT
											classid=<%=Util.CLSID_EMEDIT%>
											onblur="javascript:checkDateTypeYMD(this);" width=100
											tabindex=1 align="absmiddle"> </object></comment><script>_ws_(_NSID_);</script>
										<img src="/<%=dir%>/imgs/btn/date.gif" id=IMG_IN_DT
											onclick="javascript:openCal('G',EM_IN_DT)" align="absmiddle" />
										</td><th width="80" class="point">상품권종류</th>
										<td ><comment id="_NSID_"> <object
											...
											onclick="javascript:getNewDetail();" align="absmiddle" />
											<font color="red">* 발생일자는 상품권 코드 3~6번째 자리 년월 참고. (ex : 1702 일경우 17년 2월 발행)</font>
										</td>
									</tr>
								</table>
								</td>
						...
						<table width="100%" border="1" cellspacing="0" cellpadding="0"
							...
							<tr>
								<td><comment id="_NSID_"><OBJECT id=GD_DETAIL
									width=100% height=530 classid=<%=Util.CLSID_GRID%>>
