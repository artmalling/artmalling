<!-- 
/*******************************************************************************
 * 시스템명 : 상품권관리 > 상품권 판매/회수> 상품권 판매등록(일괄)
 * 작 성 일 : 2011.01.18
 * 작 성 자 : 
 * 수 정 자 : 
 * 파 일 명 : mgif3190.jsp
...
function doInit(){
    ...
    registerUsingDataset("mgif319", "DS_IO_MASTER");   
}
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}            name="NO"         width=25   align=center</FC>'
    	             ...
    	             + '<FC>id=GIFT_TYPE_NAME      name="상품권종류" SumText="합계" edit=none width=100   align=left</FC>'
                     + '<FC>id=ISSUE_TYPE_NAME     name="발행형태"   edit=none  width=70   align=left</FC>'
...
function btn_Save() {
    ...
    if(EM_SALE_DT.Text != strToday){
        showMessage(EXCLAMATION, OK, "USER-1000", "판매일자는 당일 일자로만 가능합니다.");
        EM_SALE_DT.Text = strToday;
        EM_SALE_DT.Focus();
        return false;
    }
 ...
 // 마감체크 (common.js) : 월마감
    ...
    TR_MAIN.Action="/mss/mgif319.mg?goTo=save"+strParam;  
...
function getSaleInfo(){
    ...
    var parameters = "&strGiftSNo=" + encodeURIComponent(EM_GIFT_S_NO.Text) 
                   ...
                   + "&strStrCd="   + encodeURIComponent(LC_STR.BindColVal) ; 
    TR_MAIN.Action   = "/mss/mgif319.mg?goTo=" + goTo + parameters;
...
function delRow(){
	...
	for(var i=1;i<=strCnt;i++){
    	if(DS_IO_MASTER.NameValue(i,"FLAG") == "T"){
    		strDelCnt += 1;
...
//if(DS_IO_MASTER.CountRow == 0) return true;
