...
function doInit(){
	...
	//Master 그리드 세로크기자동조정  2013-07-17
	 var obj   = document.getElementById("GD_AMT_MASTER"); 
    ...
    DS_SEL_GIFT_AMT_MST.setDataHeader('<gauce:dataset name="H_GIFT_AMT_MST"/>');
    ...
    DS_O_SALE_FLAG.Filter();     //판매구분, 01,02,03,04  만셋팅
...
function gridCreate(){
    var hdrProperies = '<FC>id={currow}     name="NO"          width=25   align=center</FC>'
                     ...
                     + '<FC>id=SALE_SLIP_NO name="순번"        width=50   align=center</FC>'
                     + '<FC>id=SALE_NO      name="상품권매출번호" width=150   align=center</FC>'
                     + '<FC>id=SALE_FLAG_NM name="판매구분"     width=90   align=left</FC>'
                     + '<FC>id=CSTN_NO      name="품의서번호"   width=150   align=center</FC>'
                     + '<FC>id=SALE_QTY_SUM name="수량"         width=80   align=light sumtext=@sum </FC>'
                     + '<FC>id=SALE_AMT_SUM name="판매금액"     width=100   align=light sumtext=@sum </FC>'
                     + '<FC>id=REMARK       name="적요"         width=250   align=left    </FC>'
                     ;
    initGridStyle(GD_MASTER, "common", hdrProperies, false);
    
    var hdrProperies1 = '<FC>id={currow}     name="NO"          width=25   align=center</FC>'
                        + '<FC>id=GIFT_TYPE_CD       name="상품권종류코드"       width=30  SHOW=FALSE  </FC>'
                        + '<FC>id=GIFT_TYPE_NAME       name="상품권종류"          width=80  align=left sumtext="합계" </FC>'
                        + '<FC>id=GIFT_AMT_TYPE      name="금종코드"     width=80   align=center   SHOW=FALSE</FC>'
                        + '<FC>id=GIFT_AMT_NAME     name="금종"        width=60   align=left </FC>'
                        + '<FC>id=GIFTCERT_AMT      name="상품권금액"  width=70   align=light </FC>'
                        + '<FC>id=SALE_QTY_SUM      name="수량"     width=50   align=light sumtext=@sum</FC>'
                        + '<FC>id=SALE_AMT_SUM      name="판매금액"     width=70   align=light sumtext=@sum</FC>'
                        ;
    initGridStyle(GD_AMT_MASTER, "common", hdrProperies1, false);

    var hdrProperies2 = '<FC>id={currow}     name="NO"          width=25   align=center</FC>'
				        + '<FC>id=GIFT_AMT_NAME       name="상품권명"       width=80   align=left sumtext="합계" </FC>'
				        + '<FC>id=GIFTCARD_NO       name="상품권코드"          width=130  align=center </FC>'
				        + '<FC>id=SALE_QTY      name="수량"     width=52   align=right   sumtext=@sum</FC>'
...
function btn_Search() {
	 if(LC_STR_CD.BindColVal.length == 0){ 
	  ...
	  TR_MAIN.Action="/mss/mgif305.mg?goTo="+goTo+parameters;  
...
function btn_New() {
    ...
    //LC_SALE_FLAG.BindColVal = "%";
    LC_SALE_FLAG.index = 0;
    LC_STR_CD.index = 0;
}
...
function btn_Excel() {
	  ...
	  var parameters = "판매점="+strStrCd
          + "판매시작일자="+strSaleDtFrom
          + "판매종료일자"+strSaleDtTo
          + "판매구분="+strSaleFlag;
		  
	var ExcelTitle = "상품권판매내역조회"
 ...
 */
function btn_Print() {
	var params   = "&strSaleNo="+ DS_SEL_GIFT_SALE_MST.NameValue(DS_SEL_GIFT_SALE_MST.RowPosition, "SALE_NO");
...
function getGiftAmtMst(strStrCd,strSaleDt,strSaleSlipNo) {
    ...
    TR_MAIN2.Action="/mss/mgif305.mg?goTo="+goTo+parameters;  
...
function getGiftDtlAmtMst(strStrCd,strSaleDt,strSaleSlipNo,strGiftAmtType) {
    ...
    TR_MAIN3.Action="/mss/mgif305.mg?goTo="+goTo+parameters;  
...
<script language=JavaScript for=DS_O_SALE_FLAG event=OnFilter(row)>
	if (DS_O_SALE_FLAG.NameValue(row, "CODE") == "%"  || DS_O_SALE_FLAG.NameValue(row, "CODE") == "01" 
			|| DS_O_SALE_FLAG.NameValue(row, "CODE") == "02"
				|| DS_O_SALE_FLAG.NameValue(row, "CODE") == "03"
					|| DS_O_SALE_FLAG.NameValue(row, "CODE") == "04"
						 || DS_O_SALE_FLAG.NameValue(row, "CODE") == "10" 
								|| DS_O_SALE_FLAG.NameValue(row, "CODE") == "11"
									|| DS_O_SALE_FLAG.NameValue(row, "CODE") == "13"
										|| DS_O_SALE_FLAG.NameValue(row, "CODE") == "14")  {// 정상,특판,교환,고객포인트 판매/취소
	    return true;
	}
	return false;
</script>
...
<script language=JavaScript for=DS_SEL_GIFT_AMT_MST event=OnRowPosChanged(row)>
if(row > 0 && old_Row1 != row ){
    //점코드
    var strStrCd = DS_SEL_GIFT_SALE_MST.NameValue(DS_SEL_GIFT_SALE_MST.RowPosition, "STR_CD");
    var strSaleDt = DS_SEL_GIFT_SALE_MST.NameValue(DS_SEL_GIFT_SALE_MST.RowPosition, "SALE_DT");
    var strSaleSlipNo = DS_SEL_GIFT_SALE_MST.NameValue(DS_SEL_GIFT_SALE_MST.RowPosition, "SALE_SLIP_NO");
    var strGiftAmtType = DS_SEL_GIFT_AMT_MST.NameValue(row,"GIFT_AMT_TYPE");
    //상품권 정보
    getGiftDtlAmtMst(strStrCd,strSaleDt,strSaleSlipNo,strGiftAmtType);
}
old_Row1 = row;
</script>
...
<!-- =============== Input용 -->
<!-- ===============- Output용 -->
<comment id="_NSID_"><object id="DS_O_SALE_FLAG"  classid=<%= Util.CLSID_DATASET %>>
...
<comment id="_NSID_"><object id="DS_SEL_GIFT_AMT_MST"  classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr valign="top">
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                    <tr>
                        <td width="100%">
                          <comment id="_NSID_"><OBJECT id=GD_MASTER width=100% height=300 classid=<%=Util.CLSID_GRID%>>
    ...
    <tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                <td class="sub_title"><img src="/<%=dir%>/imgs/comm/ring_blue.gif" class="PR03" align="absmiddle" /> 금종정보</td>
                <td class="sub_title"><img src="/<%=dir%>/imgs/comm/ring_blue.gif" class="PR03" align="absmiddle" /> 상품권정보</td>
            </tr>
            <tr valign="top">
                <td width="400">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                    <tr>
                        <td width="100%">
                          <comment id="_NSID_"><OBJECT id=GD_AMT_MASTER width=100% height=300 classid=<%=Util.CLSID_GRID%>>
					              <param name="DataID" value="DS_SEL_GIFT_AMT_MST">
					              <Param Name="ViewSummary"   value="1" >
					      </OBJECT></comment><script>_ws_(_NSID_);</script>
                        </td>
                    </tr>
