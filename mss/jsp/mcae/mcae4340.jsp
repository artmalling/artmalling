...
function doInit(){
	...
	//Master 그리드 세로크기자동조정  2013-07-17
	 ...
	 obj.style.height  = (parseInt(window.document.body.clientHeight)-top)-150 + "px";
	 
	 var obj   = document.getElementById("GD_SKU_LIST_CARD"); 
	 obj.style.height  = (parseInt(window.document.body.clientHeight)-top)-150 + "px";
	 
	 var obj   = document.getElementById("GD_MEMB_EVENT"); 
	 obj.style.height  = (parseInt(window.document.body.clientHeight)-top)-180 + "px";

	 var obj   = document.getElementById("GD_SKU_LIST_MEMB"); 
	 obj.style.height  = (parseInt(window.document.body.clientHeight)-top)-180 + "px";
	...
	DS_IO_SKU_LIST_MEMB.setDataHeader('<gauce:dataset	name="H_SKU_LIST_MEMB"/>');
	...
	DS_O_MEMB_EVENT.setDataHeader('<gauce:dataset name="H_MEMB_EVENT"/>');
	...
	GD_SKU_LIST_MEMB.TranslateKeyDown = 0;
	GD_SKU_LIST_MEMB.TargetEnterKey = 2;
...
function gridCreate8(){
	...
	initGridStyle(GD_MEMB_EVENT,	"common", hdrProperies,	true);
}
...
function gridCreate32(){
	var	hdrProperies ='<FC>id={currow}		name="NO"				width=30	align=center</FC>'
					...
					+ '<FC>id=GIFTCARD_LIST	name="상품권LIST"     	width=200	align=rigth		edit=none	  </FC>'
					;
	initGridStyle(GD_SKU_LIST_MEMB, "common", hdrProperies,	true);
}
...
function btn_Save() {
	...
	//브랜드 영수증 정보
	if (DS_O_PB_EVENT.CountRow == 0 && DS_O_PAID_EVENT.CountRow == 0 && DS_O_CARD_EVENT.CountRow == 0 && DS_O_MEMB_EVENT.CountRow == 0){
		//저장할 내용이 없습니다
    ...
    // 저장할 내용이 없습니다.
    if (!DS_IO_SKU_LIST_ALL.IsUpdated && !DS_IO_SKU_LIST.IsUpdated && !DS_IO_SKU_LIST_CARD.IsUpdated && !DS_IO_SKU_LIST_MEMB.IsUpdated){
...
//                 }
	...
	// 지급품정보(중복불가) 데이터 체크 시작
	for(var i=1;i<=DS_IO_SKU_LIST_MEMB.CountRow;i++){
		if(DS_IO_SKU_LIST_MEMB.NameValue(i,"CHK") == "T"){
            
			strSKU_GB = DS_IO_SKU_LIST_MEMB.NameValue(i, "SKU_GB");		//사은품종료 =2:상품권
            
            if (strSKU_GB =="2"){             	
                strGiftCardNo = DS_IO_SKU_LIST_MEMB.NameValue(i, "GIFTCARD_NO");		// 상품권번호
                nBUY_COST_PRC  = DS_IO_SKU_LIST_MEMB.NameValue(i, "BUY_COST_PRC");	    // 지급품금액
                nGiftCertAmt  = DS_IO_SKU_LIST_MEMB.NameValue(i, "GIFTCERT_AMT");	    // 상품권 금액
                strGiftCardList = DS_IO_SKU_LIST_MEMB.NameValue(i, "GIFTCARD_LIST");		// 상품권번호LIST
...
//                 }
	...
	TR_MAIN.KeyValue="SERVLET(I:DS_IO_SKU_LIST_ALL=DS_IO_SKU_LIST_ALL,I:DS_IO_SKU_LIST=DS_IO_SKU_LIST,I:DS_IO_SKU_LIST_CARD=DS_IO_SKU_LIST_CARD, I:DS_IO_SKU_LIST_MEMB=DS_IO_SKU_LIST_MEMB)";  
	TR_MAIN.Post();
...
function getPbSaleInfo(){
	...
	TR_MAIN.KeyValue="SERVLET(O:DS_O_PB_EVENT=DS_O_PB_EVENT, O:DS_O_PAID_EVENT=DS_O_PAID_EVENT,  O:DS_O_CARD_EVENT=DS_O_CARD_EVENT,O:DS_O_MEMB_EVENT=DS_O_MEMB_EVENT, I:DS_O_RECEIPT_ADD=DS_O_RECEIPT_ADD)"; //조회는 O	
	TR_MAIN.Post();	
...
function getSkuList() {
	...
	for(var	i=1;i<=DS_O_MEMB_EVENT.CountRow;i++){			
		DS_O_MEMB_EVENT.UserStatus(i) = 1;
	}	
	...
	TR_MAIN.KeyValue="SERVLET(I:DS_O_PB_EVENT=DS_O_PB_EVENT, I:DS_O_PAID_EVENT=DS_O_PAID_EVENT, I:DS_O_CARD_EVENT=DS_O_CARD_EVENT,I:DS_O_MEMB_EVENT=DS_O_MEMB_EVENT, "  
				   + " O:DS_IO_SKU_LIST_ALL=DS_IO_SKU_LIST_ALL, O:DS_IO_SKU_LIST=DS_IO_SKU_LIST, O:DS_IO_SKU_LIST_CARD=DS_IO_SKU_LIST_CARD, O:DS_IO_SKU_LIST_MEMB=DS_IO_SKU_LIST_MEMB)";	//조회는 O
	TR_MAIN.Post();
	...
	for(var i=1;i<=DS_IO_SKU_LIST_MEMB.CountRow;i++){
		...
		var strEventCd_i = DS_IO_SKU_LIST_MEMB.NameValue(i, "EVENT_CD");
		
		if (strEventCd_i != strEventCd) {
			DS_IO_SKU_LIST_MEMB.NameValue(i,"CHK") = "T";			
		}

		var strEventCd = DS_IO_SKU_LIST_MEMB.NameValue(i, "EVENT_CD");
...
function btn_DeleteRow() {
	...
	DS_IO_SKU_LIST_MEMB.ClearData();      // 제휴카드사은행사
...
<script language="javascript" for=GD_SKU_LIST_MEMB event=OnClick(row,colid)>
	//제휴카드 지급내역 선택시
	...
	var strEventCd = DS_IO_SKU_LIST_MEMB.NameValue(row, "EVENT_CD");
	var strEventCd_i = "";
	strChk_i = "";
	
	for(var i=1;i<=DS_IO_SKU_LIST_MEMB.CountRow;i++){
		
		strChk_i = DS_IO_SKU_LIST_MEMB.NameValue(row,"CHK")
		strEventCd_i = DS_IO_SKU_LIST_MEMB.NameValue(i,"EVENT_CD");
		
		if (strEventCd_i == strEventCd && i != row && DS_IO_SKU_LIST_MEMB.NameValue(row,"CHK") == "T" ) {
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCARD_NO") = "";	
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCERT_AMT") = "";
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCARD_CNT") = "";	
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCARD_LIST") = "";		
			DS_IO_SKU_LIST_MEMB.NameValue(i,"CHK") = "F";			
		}
		
		if (strEventCd_i == strEventCd && i == row && DS_IO_SKU_LIST_MEMB.NameValue(row,"CHK") == "F" ) {
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCARD_NO") = "";	
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCERT_AMT") = "";	
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCARD_CNT") = "";	
			DS_IO_SKU_LIST_MEMB.NameValue(i,"GIFTCARD_LIST") = "";	
	...
	switch(colid){
		case 'GIFTCARD_NO':     // 상품권 번호 조회	
			if(DS_IO_SKU_LIST_ALL.NameValue(row,"GIFTCARD_NO").length != 12){
				...
				//회원 대상 행사
				for(var irow=1;irow<=DS_IO_SKU_LIST_MEMB.CountRow;irow++){
	    			
	    			iGIFTCARD_CNT = DS_IO_SKU_LIST_MEMB.NameValue(irow,"GIFTCARD_CNT");
	    			intcol = 0;
		    		for(var i=1;i<=iGIFTCARD_CNT;i++){
		    			var strGIFTCARD_NO = DS_IO_SKU_LIST_MEMB.NameValue(irow,"GIFTCARD_LIST").substring(intcol,12 + intcol);
		    			
		    			if (iItemCd == strGIFTCARD_NO) {
		    	            showMessage(STOPSIGN, OK, "USER-1000", "회원대상 지급정보 " + irow + "행에 이미 입력한 상품권번호 입니다.");
		    	            DS_IO_SKU_LIST_ALL.NameValue(row,"GIFTCARD_NO") = "";
		    	            return;
...
<!-- 데이터셋 값 변경 후 -->
<script language="javascript"  for=DS_IO_SKU_LIST_MEMB event=OnColumnChanged(row,colid)>

	//중복불가 지급내역 선택시
	if(DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_NO").length == 0) {
		//DS_IO_SKU_LIST_CARD.NameValue(row, "GIFTCERT_AMT") = 0;
		return;
	}
	...
	switch(colid){
		case 'GIFTCARD_NO':     // 상품권 번호 조회	
			if(DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_NO").length != 12){
				
			}else{
	    		var iItemCd = DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_NO");
	    		var iGIFTCERT_AMT = DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCERT_AMT");
	    		var iGIFTCARD_CNT = 0;
	    		var strGIFTCARD_LIST = DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_LIST");
				...
				for(var irow=1;irow<=DS_IO_SKU_LIST_CARD.CountRow;irow++){
		    		...
		    		for(var i=1;i<=iGIFTCARD_CNT;i++){
		    			...
		    			if (iItemCd == strGIFTCARD_NO) {
		    	            showMessage(STOPSIGN, OK, "USER-1000", "제휴카드행사 지급정보 "+ irow + "행에 이미 입력한 상품권번호 입니다.");
		    	            setFocusGrid(GD_SKU_LIST_MEMB, DS_IO_SKU_LIST_MEMB, row , "GIFTCARD_NO");
		    	            DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_NO") = "";
		    	            return;
	    		...
	    		//--상품권이 중복이없고 유효한 상품권이경우에 수량, 금액, list에 추가
	    		if (chkGiftCardNo("DS_IO_SKU_LIST_MEMB", row)) {
	    			DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_CNT") = DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_CNT") + 1;
	    			DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCERT_AMT") = DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCERT_AMT") + iGIFTCERT_AMT;
	    			DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_LIST") = DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_NO") + strGIFTCARD_LIST;
	    			DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_NO") = "";
	    			//alert(row+colid);
	    		}
	    		else {
	    			DS_IO_SKU_LIST_MEMB.NameValue(row,"GIFTCARD_NO") = DS_IO_SKU_LIST_MEMB.OrgNameValue(row,"GIFTCARD_NO"); 
					setFocusGrid(GD_SKU_LIST_MEMB, DS_IO_SKU_LIST_MEMB, row , "GIFTCARD_NO");
	    		}
			}
			break;
...
<!-- 데이터셋 값 변경 후 -->
<script language="javascript"  for=GD_SKU_LIST_MEMB event=CanColumnPosChange(Row,Colid)>
	...
	switch(Colid){
		case 'GIFTCARD_NO':     // 상품권 번호 조회	
			if(DS_IO_SKU_LIST_MEMB.NameValue(Row,"GIFTCARD_NO").length != 12){
	    		...
	    		var iItemCd = DS_IO_SKU_LIST_MEMB.NameValue(Row,"GIFTCARD_NO");
	    	    for(var k=i+1; k<=DS_IO_SKU_LIST_MEMB.CountRow; k++) {
	    	    	if (Row != k) {
	    	    		var kItemCd = DS_IO_SKU_LIST_MEMB.NameValue(k,"GIFTCARD_NO");
	    		        if (iItemCd==kItemCd) {
	    		            showMessage(STOPSIGN, OK, "USER-1000", k +"번째행의  상품권번호와 중복됩니다.");
	    		            DS_IO_SKU_LIST_MEMB.NameValue(Row,"GIFTCARD_NO") = DS_IO_SKU_LIST_CARD.OrgNameValue(Row,"GIFTCARD_NO"); 
	    					setFocusGrid(GD_SKU_LIST_MEMB, DS_IO_SKU_LIST_MEMB, Row , "GIFTCARD_NO");
	    					//setFocusGrid(GD_SKU_LIST_CARD, DS_IO_SKU_LIST_CARD, Row , "GIFTCARD_NO");
	    		            return false;
	    		        }
...
<comment id="_NSID_"><object id="DS_O_MEMB_EVENT"		    classid=<%=Util.CLSID_DATASET%>></object></comment><script>_ws_(_NSID_);</script>
...
<comment id="_NSID_"><object id="DS_IO_SKU_LIST_MEMB"	classid=<%=Util.CLSID_DATASET%>></object></comment><script>_ws_(_NSID_);</script>
			...
			<tr	valign="top">
				...
				<table width="100%"	border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<table width="100%"	border="0" cellspacing="0" cellpadding="0" class="BD4A">
							<tr>
								<td><comment id="_NSID_"><OBJECT id=GD_MEMB_EVENT
									width=100%   classid=<%=Util.CLSID_GRID%>>
									<param name="DataID" value="DS_O_MEMB_EVENT">
									<Param Name="ViewSummary" value="1">
								</OBJECT></comment><script>_ws_(_NSID_);</script></td>
							</tr>
						</table>
				...
				<table width="100%"	border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<table width="100%"	border="0" cellspacing="0" cellpadding="0" class="BD4A">
							<tr>
								<td><comment id="_NSID_"><OBJECT id=GD_SKU_LIST_MEMB
									width=100% classid=<%=Util.CLSID_GRID%>>
									<param name="DataID" value="DS_IO_SKU_LIST_MEMB">
								</OBJECT></comment><script>_ws_(_NSID_);</script></td>
							</tr>
						</table>
						</td>
