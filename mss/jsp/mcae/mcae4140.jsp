<!-- 
/*******************************************************************************
 * 시스템명	: 경영지원 > 사은행사관리 >	사은품 지급/취소 > 사은품지급등록(리콜사은)G
 * 작 성 일	: 2011.01.18
 * 작 성 자	: 김슬기
 * 수 정 자	: 홍종영
 * 파 일 명	: MCAE4140.jsp
 ...
 *		  2012.06.14 (홍종영) 프로그램 수정(mcae424 복사)
 ******************************************************************************/
-->
...
function gridCreate1(){
	var	hdrProperies ='<FC>id={currow}		name="NO"			width=30	align=center</FC>'
					...
					+ '<FC>id=CUST_NO		name="회원번호"		width=80	align=left		edit=none	</FC>';
...
function gridCreate2(){
	var	hdrProperies2 ='<FC>id={currow}		name="NO"			width=30	align=center</FC>'
					...
					+ '<FC>id=BUY_COST_PRC	name="매입원가"		width=90	align=rigth		edit=none	show=true</FC>'
...
function btn_Save() {
	...
	TR_MAIN.Action="/mss/mcae414.mc?goTo=save"+strParams; 
...
function getEventCombo(){
	...
	TR_MAIN.Action="/mss/mcae414.mc?goTo=getEventCombo"+parameters;	
...
function getEventInfo() {
	...
	TR_MAIN.Action="/mss/mcae414.mc?goTo=getEventInfo"+parameters;  
...
/* 	if(DS_O_EVENT_INFO.NameValue(1,"EVENT_TYPE") != "03"){
	...
	TR_CARDINFO.Action="/mss/mcae414.mc?goTo=getCardInfo"+parameters;
...
/*	if(DS_O_CARD_INFO.NameValue(1,"RTN_CD")	!= "0"){
		...
		TR_MAIN.Action="/mss/mcae414.mc?goTo=getSaleInfo"+parameters; 
...
function getSaleInfo2(){
	...
	TR_MAIN.Action="/mss/mcae414.mc?goTo=getSaleInfo2"+parameters; 
 ...
 */
	...
	function getSkuList() {
		...
		TR_MAIN.Action="/mss/mcae414.mc?goTo=getSkuList"+parameters;	 
...
function getSkuInfo() {
	...
	TR_MAIN.Action="/mss/mcae414.mc?goTo=getSkuInfo"+parameters;	 
...
function setDivAmt(strFlag) {
	...
	if(CHK_EXCP.checked){
		...
		for(var i=1;i<=DS_IO_SKU_LIST.CountRow;i++){
			if(	DS_IO_SKU_LIST.NameValue(i,"CHK") == "F")
				continue;
...
function chkGiftCardNo(strRow){
	...
	TR_MAIN3.Action="/mss/mcae414.mc?goTo="+goTo+parameters;  
...
function getGiftCardNo() {
	...
	TR_MAIN2.Action="/mss/mcae414.mc?goTo="+goTo;
