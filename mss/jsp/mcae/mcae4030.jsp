<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 > 사은행사관리 > 사은품 지급/취소 > 사은품지급내역조회
 * 작 성 일 : 2011.01.18
 * 작 성 자 : 김슬기
 * 수 정 자 : 
 * 파 일 명 : MCAE4030.jsp
...
function gridCreate2(){ 
    var hdrProperies2 ='<FC>id={currow}             name="NO"         width=30   align=center</FC>'
			        + '<FC>id=EVENT_GIFT_TYPE       name="사은품구분"   width=80  align=center</FC>'
			        + '<FC>id=SKU_CD                name="사은품코드"   width=120   align=center</FC>'
			        + '<FC>id=SKU_NAME              name="사은품명"     width=120  align=left   sumtext="합계" </FC>'
                    + '<FC>id=SAL_COST_PRC          name="단가"        width=70  align=right   </FC>'
                    + '<FC>id=IN_QTY                name="입고"        width=60   align=right sumtext=@sum</FC>'
                    + '<FC>id=RFD_QTY               name="반품"        width=60  align=right  sumtext=@sum</FC>'
                    + '<FG>id=STR_NAME              name="지급"        width=70  align=center'
                    + '<FC>id=PRSNT_QTY             name="정상지급"     width=70   align=right  sumtext=@sum</FC>'
                    + '<FC>id=EXCP_PRSNT_QTY        name="사후지급"     width=80  align=right   sumtext=@sum</FC>'
                    + '<FC>id=RECOVERY_QTY          name="지급취소"     width=70  align=right   sumtext=@sum</FC>'
                    + '<FC>id=POS_NDRAWL_CAN_QTY    name="미회수"       width=70   align=right  sumtext=@sum</FC>'
                    + '<FC>id=ETC_PRSNT             name="기타지급"     width=80  align=right   sumtext=@sum</FC>'
                    + '<FC>id=NQTY                  name="누적지급"     width=70  align=right   sumtext=@sum</FC></FG>'
                    + '<FG>id=STR_FLAG              name="현재고"      width=70   align=center'
                    + '<FC>id=QTY                   name="수량"        width=40  align=right  sumtext=@sum</FC>'
                    + '<FC>id=AMT                   name="금액"        width=70  align=right  sumtext=@sum</FC></FG>';
...
function btn_Excel() {
	...
	strDt   = EM_S_DATE.Text    + " ~ " + EM_E_DATE.Text ;
	 
	var strTitle = "사은품지급내역조회";
...
function openPop(){
    ...
    var returnVal = window.showModalDialog("/mss/mcae403.mc?goTo=openPop",
                                           arrArg,
