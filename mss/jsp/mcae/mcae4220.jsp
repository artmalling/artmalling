<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 > 사은행사관리 > 사은품 지급/취소 > 사은품행사회원매출조회
 * 작 성 일 : 2011.01.18
 * 작 성 자 : 김슬기
 * 수 정 자 : 
 * 파 일 명 : MCAE4220.jsp
...
function gridCreate1(){ 
    var hdrProperies ='<FC>id={currow}          name="NO"          width=40   align=center</FC>'
                    ...
                    + '<FC>id=EVENT_CD          name="행사코드"     width=100  align=center show=true</FC>'
                    ...
                    + '<FC>id=EVENT_DT          name="행사기간"      width=160  align=center Mask="XXXX/XX/XX~XXXX/XX/XX"</FC>'
                    + '<FC>id=EVENT_TYPE        name="행사종류"      width=130  align=left</FC>'
...
function gridCreate2(){ 
    var hdrProperies2 ='<FC>id={currow}          name="NO"           width=30   align=center</FC>'
			        + '<FC>id=SALE_DT            name="행사일"   	     width=100  align=center Mask="XXXX/XX/XX"</FC>'  
			        + '<FG>id=TITLE              name="점전체" 		edit=none 	align=center'  
                    + '<FC>id=TOT_SALE_AMT       name="총매출"      width=100  align=right   sumtext=@sum</FC>'
                    + '<FC>id=CUST_TOT_SALE_AMT             name="회원총매출"      width=110  align=right   sumtext=@sum</FC>'
                    + '<FC>id=NCUST_TOT_SALE_AMT            name="비회원총매출"    width=110  align=right   sumtext=@sum</FC>'
                    + '</FG>'  
			        + '<FG>id=TITLE              name="행사전체" 		edit=none 	align=center'  
                    + '<FC>id=TOT_SALE_AMT1      name="총매출"      width=100  align=right   sumtext=@sum</FC>'
                    + '<FC>id=CUST_TOT_SALE_AMT1       name="회원매출"      width=100  align=right   sumtext=@sum</FC>'
                    + '<FC>id=NCUST_TOT_SALE_AMT1      name="비매출"      width=100  align=right   sumtext=@sum</FC>'
			        + '</FG>'  
                    + '<FG>id=TITLE                         name="회원행사매출" 			edit=none 	align=center'  
                    + '<FC>id=CUST_EVENT_TOTAL_SALE_AMT     name="총매출"      width=110  align=right   sumtext=@sum</FC>'
                    + '<FC>id=CUST_EVENT_DIRECT_SALE_AMT    name="직접행사매출액"   width=110  align=right   sumtext=@sum</FC>'
                    + '<FC>id=CUST_EVENT_INDIRECT_SALE_AMT	name="간접행사매출액"   width=110   align=right  sumtext=@sum</FC>'
                    + '</FG>' 
                    ;
...
function btn_Search() {
    ...
    TR_MAIN.Action="/mss/mcae422.mc?goTo="+goTo+parameters;  
...
function btn_Excel() {
    ...
    //openExcel2(GD_DETAIL, ExcelTitle, params, true );
    openExcel5(GD_DETAIL, ExcelTitle, params, true, "",g_strPid );
}
...
function openPop(){
    ...
    var returnVal = window.showModalDialog("/mss/mcae422.mc?goTo=openPop",
                                           arrArg,
...
<script language=JavaScript for=EM_S_EVENT_CD event=onKillFocus()>
  //  if (EM_O_EVENT_CD.Text.length > 0 ) { 
      if(!this.Modified)
        ...
        setMssEvtMstNmWithoutPop('DS_O_RESULT', 'SEL_STR_EVT_NAME', EM_S_EVENT_CD, EM_S_EVENT_NAME, LC_S_STR_CD.BindColVal, '4/5/6','05');
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr valign="top">                
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
          <tr>
            <td>
                <comment id="_NSID_"><OBJECT id=GD_MASTER width=100% height=385 classid=<%=Util.CLSID_GRID%>>
