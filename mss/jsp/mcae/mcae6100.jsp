...
//     initEmEdit(EM_SUM_POINT,   "NUMBER^9^0",          READ);       //누적+발생포인트
    ...
    registerUsingDataset("mcae610","DS_I_POINT,DS_I_MASTER");
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}         name="NO"                   width=30     align=center </FC>'
                     //+ '<C>id=EXCH_TYPE        name="tka"           width=200    align=center  EditStyle=Lookup Data="DS_I_EXCH_CD:CODE:NAME" </C>'
                     //+ '<C>id=EXCH_CNT     		name="지급갯수"            width=100    align=right  EditStyle=combo </C>'
                     + '<C>id=EXCH_TYPE        name="상품권번호"           width=200    align=center  EditStyle="16" edit="Numeric" </C>'
                     + '<C>id=EXCH_CNT     	   name="상품권금액"           width=100    align=left  ReadOnly =true sumtext=@sum </C>'
                     ;
                     
    initGridStyle(GR_MASTER1, "common", hdrProperies, true);
    GR_MASTER1.ViewSummary = "1";
}
...
function btn_Save() {
    ...
    if(DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition,"EXCH_CNT") != "5000"){
		showMessage(STOPSIGN, OK, "USER-1000", "5000원 상품권이 아닙니다.");
		setFocusGrid(GR_MASTER1, DS_I_MASTER, DS_I_MASTER.CountRow , "EXCH_TYPE");
		return;
	}
...
function saveData(){
    ...
    var goTo        = "chkSave";
    var action      = "I";  //조회는 O
    TR_MAIN2.Action  ="/mss/mcae610.mc?goTo="+goTo;   
 ...
 */
 function realSave(){
    ...
    TR_MAIN.Action  ="/mss/mcae610.mc?goTo="+goTo+parameters;   
...
function check_cnt(){
    ...
    var action       = "O";
    var parameters   = "&strCardNo="    + encodeURIComponent(strCardNo);
    TR_MAIN3.Action  ="/mss/mcae610.mc?goTo="+goTo+parameters;
...
function btn_Add1(){
    ...
    if (DS_O_CNT.NameValue(DS_O_CNT.RowPosition, "GRADE_YN") == 'N') {
        showMessage(StopSign, OK, "USER-1000",  "해당유형의 회원은 지급할 수 없습니다.");
        return;
    }
    ...
    */
    ...
    if(DS_O_CNT.NameValue(DS_O_CNT.RowPosition, "CUR_CNT") != 0) {
		...
		DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "EXCH_TYPE") = "";
		var exch_cnt = "";
...
function chkGiftCardNo(strRow){
	...
	var strGiftCardNo = DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "EXCH_TYPE");
 
	var goTo       = "chkGiftCardNo";
	var	action	   = "O";
	var parameters = "&strGiftCardNo="+ encodeURIComponent(strGiftCardNo);
	
	TR_MAIN3.Action="/mss/mcae425.mc?goTo="+goTo+parameters;  
	...
	TR_MAIN3.Post();
 	...
 	if(DS_O_CHECK.CountRow == 0){
		...
		setTimeout("setFocusGrid(GR_MASTER1, DS_I_MASTER, DS_I_MASTER.CountRow , 'EXCH_TYPE')",50);
		return false;
	} else {
		//setFocusGrid(GD_GIFTCARD_NO, DS_IO_GIFTCARD_NO, strRow , "GIFTCERT_AMT");
		DS_I_MASTER.NameValue(strRow, "EXCH_CNT") = DS_O_CHECK.NameValue(1, DS_O_CHECK.ColumnID(2));
		//return true;
	}
}
/**
...
function btn_Delete(){
    ...
    TR_MAIN4.Action  ="/mss/mcae610.mc?goTo="+goTo+parameters;   
...
<script language=JavaScript for=GR_MASTER1 event=CanColumnPosChange(row,colid)>
	switch(colid){
		case 'EXCH_TYPE':
			if(DS_I_MASTER.NameValue(row,"EXCH_TYPE").length != 16){//강연식
