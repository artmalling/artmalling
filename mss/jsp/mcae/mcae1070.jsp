 ...
 *		  2011.05.02 (김성미) 현업 요구사항	수정 반영
 *		  2012.05.03 (김시헌) 사은품지급주기, 영수증합산여부, 영수증당일여부 추가
 *        2012.05.04 (김시헌) MCAE106 복사  신규 생성
 *                           리콜행사구분,사은품 한정수랑 추가 
 ******************************************************************************/
-->
...
function doInit(){
	...
	initEmEdit(EM_LIMIT_QTY, "NUMBER^3^2", READ);         						//사은품 한정수량	
	...
	initComboStyle(LC_RECALL_CPN_EVENT_TYPE,DS_O_RECALL_CPN_EVENT_TYPE, "CODE^0^30,NAME^0^100", 1, READ);  //리콜행사구분
	
	initComboStyle(LC_DEPT,DS_O_DEPT, "CODE^0^30,NAME^0^80", 1,	READ);							// 팀
	initComboStyle(LC_TEAM,DS_O_TEAM, "CODE^0^30,NAME^0^80", 1,	READ);							// 파트 
	...
	getEtcCode("DS_O_RECALL_CPN_EVENT_TYPE", "D", "M204", "N");
...
function gridCreate1(){
	var	hdrProperies ='<FC>id={currow}		name="NO"			width=25	align=center</FC>'
					...
					+ '<FC>id=EVENT_CD		name="행사코드"		width=85	align=center</FC>'
...
function btn_Search() {
	...
	TR_MAIN.Action="/mss/mcae107.mc?goTo="+goTo+parameters;	 
...
function btn_Save()	{
	 ...
	 //	사은행사 마스터	정보 필수입력사항
	  ...
	  if(DS_IO_STREVT.NameValue(1, "RECP_TODAY_YN")	== "" && 
			  DS_IO_STREVT.NameValue(DS_IO_STREVT.RowPosition, "RECALL_CPN_EVENT_TYPE") == "10"){	// 영수증 당일여부
	  ...
	  if(DS_IO_STREVT.NameValue(1, "EVENT_TYPE") ==	"02" &&	DS_IO_STREVT.NameValue(1, "APP_RATE") == ""){ // 인정율
		  showMessage(EXCLAMATION ,	OK,	"USER-1003", "인정율");
		  EM_APP_RATE.Focus();
		  return;
	  }
	  ...
	  DS_IO_STREVT_COPY.ImportData(strData);
	   ...
	   TR_MAIN.Action="/mss/mcae107.mc?goTo=save&strAllBrand="+encodeURIComponent(CHK_ALLBRAND.checked); 
 ...
 *************************************************************************/
 ...
 function setObject() {
		...
		LC_RECALL_CPN_EVENT_TYPE.Enable	= strFlag;
		
	 	//	리콜행사 인 경우
		if(DS_O_EVTMST.NameValue(DS_O_EVTMST.RowPosition,"EVENT_TYPE") == "7"){ // 리콜행사
			LC_EVENT_TYPE.BindColVal = "03"; // 사은행사유형 : 보너스카드행사
			LC_EVENT_TYPE.Enable	= false;
			LC_CUST_ONLY_YN.BindColVal = "Y"; // 보너스카드 ONLY : Y
			LC_CUST_ONLY_YN.Enable	= false;
			LC_CARD_ONLY_YN.BindColVal = "N"; // 신용카드 ONLY : N
			LC_CARD_ONLY_YN.Enable	= false;
			LC_EVENT_GIFT_CYC.BindColVal = "P"; // 사은품지급주기 : P-기간중1회
...
if(DS_O_EVTMST.NameValue(DS_O_EVTMST.RowPosition, "EVENT_TYPE")	== "7"){
	if (DS_O_EVENT_TYPE.NameValue(row, "CODE") == "03")	{//	단독, D3카드 고객행사
		return true;
	}
}
return false;
...
<script language=JavaScript for=LC_RECALL_CPN_EVENT_TYPE event=OnSelChange()>
    if(  this.BindColVal == '10' && this.Enable ){ // 구매행사
        enableControl(LC_RECP_ADD_YN, false);
        setComboData(LC_RECP_ADD_YN,"Y");  
        enableControl(LC_RECP_TODAY_YN, true);
        setComboData(LC_RECP_TODAY_YN,""); 
        
    }else{
        enableControl(LC_RECP_ADD_YN, false);
        setComboData(LC_RECP_ADD_YN," ");  
...
<script	language="javascript" for=GD_EVTPBN
...
<script	language=JavaScript	for=GD_EVTSTRSKU
...
<comment id="_NSID_"><object id="DS_O_RECALL_CPN_EVENT_TYPE" classid=<%= Util.CLSID_DATASET%>></object></comment><script>_ws_(_NSID_);</script>
	...
	<div id="testdiv" class="testdiv">
		...
		<table width="100%"	border="0" cellspacing="0" cellpadding="0">
			...
			<tr	valign="top">
										...
										<td	class="PT01	PB03"><table width="100%" border="0"
												...
												<tr>
													<td><table width="100%"	border="0" cellpadding="0"
															...
															<tr>
																<th	width="80" class="point">행사주관부서</th>
																<td	colspan=3><comment id="_NSID_">	<object
															...
															<tr>
																<th	width="80" class="point">사은행사유형</th>
																<td><comment id="_NSID_"> <object
															...
															<tr>
																<th	width="80" class="point">보너스카드	ONLY</th>
																<td><comment id="_NSID_"> <object
															...
															<tr>
																<th	width="80" class="point">사은품 지급주기</th>
																<td><comment id="_NSID_"> <object
															...
															<tr>
																<th	width="80" class="point">영수증 당일여부</th>
																<td><comment id="_NSID_"> <object
				...
				<tr>
                    <th width="80" class="point">리콜행사구분</th>
                    <td>
                      <comment id="_NSID_">
                            <object id=LC_RECALL_CPN_EVENT_TYPE classid=<%= Util.CLSID_LUXECOMBO %> height=100 width=140 tabindex=1 align="absmiddle">
                            </object>
                      </comment><script>_ws_(_NSID_);</script>    
                    </td>                    
                    <th width="80" >사은품 한정수량</th>
                    <td>
                      <comment id="_NSID_">
                            <object id=EM_LIMIT_QTY classid=<%= Util.CLSID_EMEDIT %> height=100 width=140 tabindex=1 align="absmiddle">
                            </object>
                      </comment><script>_ws_(_NSID_);</script>    
                    </td>
                  </tr>
															<tr>
																<th	width="80" class="point">DESK담당자</th>
																<td	colspan="3"><comment id="_NSID_"> <object
									...
									<tr>
										<td	class="PT10">
											<div id=TAB	width="100%" height=310	TitleWidth=120
...
<object	id=BO_EVTSTR classid=<%=Util.CLSID_BIND%>>
		...
		<c>col=RECALL_CPN_EVENT_TYPE	ctrl=LC_RECALL_CPN_EVENT_TYPE	param=BindColVal</c>
