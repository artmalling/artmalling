...
function doInit(){
	...
	initEmEdit(EM_PRSNT_CHAR_NM, "GEN",	READ);												//ID
	initEmEdit(EM_PRSNT_CHAR_RNM, "GEN",	NORMAL);										//등록자
	...
	initEmEdit(EM_PRSNT_MEMO, "GEN^15",	NORMAL);										//사유
	...
	initEmEdit(EM_MARIO_NO, "GEN^13",	NORMAL);										//보너스포인트카드번호
	initEmEdit(EM_CARD_NO, "GEN^16",	NORMAL);										//신용카드번호
	initEmEdit(EM_CARDAPP_NO, "GEN^8",	NORMAL);										    //승인번호
	initEmEdit(EM_AMT, "NUMBER^12^0",	NORMAL);										    //승인금액
	...
	setTimeout("EM_MARIO_NO.Focus()",50);
}


function gridCreate1(){
	var	hdrProperies ='<FC>id={currow}		name="NO"			width=30	align=center</FC>'
					+ '<FC>id=CHK			name=""				width=20	align=center	Edit=none	EditStyle=CheckBox	HeadCheckShow="false" </FC>'
					+ '<FC>id=PROVIDE		name="영수증번호"		width=200	align=left		edit=none	sumtext="합계"</FC>'
					+ '<FC>id=SALE_DT		name="입력일자"		width=80	align=center	edit=none	mask="XXXX/XX/XX"</FC>'
					+ '<FC>id=CARD_NOCK		name="카드번호"		width=100	align=left		edit=none	</FC>'
					+ '<FC>id=CARD_APPNO	name="승인번호"		width=80	align=left		edit=none	</FC>'
...
function btn_Save() {
	...
	if (EM_PRSNT_CHAR_RNM.Text =="" ){
		showMessage(STOPSIGN, OK, "USER-1000", "등록자 오류 입니다");
		return;
	}   
	
	if (EM_PRSNT_MEMO.Text =="" ){
		showMessage(STOPSIGN, OK, "USER-1000", "사유 오류 입니다");
		return;
	}   
	...
	*/
	...
	//DS_O_CARD_INFO.NameValue(1,"CUST_ID")
	
	var strParams = "&strPrsntFlag="		+ ""
				  + "&strSkuCd="			+ ""
				  + "&strTrgCd="			+ encodeURIComponent(HD_TRG_CD.Value)
				  + "&strBuyCostPrc="		+ encodeURIComponent(HD_BUY_COST_PRC.Value)
				  + "&strExcpConfId="		+ ""
				  + "&strExcpPrsntRsn="		+ ""
				  + "&strCardCustId="		+ encodeURIComponent(EM_MARIO_NO.Text)
				  ...
				  + "&strCHAR_RNM="			+ encodeURIComponent(EM_PRSNT_CHAR_RNM.Text) 
				  + "&strMEMO="				+ encodeURIComponent(EM_PRSNT_MEMO.Text); 
	
	TR_MAIN.Action="/mss/mcae429.mc?goTo=save"+strParams; 
...
function getEventCombo(){
	...
	TR_MAIN.Action="/mss/mcae429.mc?goTo=getEventCombo"+parameters;	
...
function getEventInfo() {
	...
	TR_MAIN.Action="/mss/mcae429.mc?goTo=getEventInfo"+parameters;  
...
/*	if(DS_O_CARD_INFO.NameValue(1,"RTN_CD")	!= "0"){
		...
		var strSaleDt		= strToday.substring(0,8);
		var strReceiptNo	= EM_RECEIPT_NO.Text;
		var strCardNOCK		= EM_CARD_NO.Text;
		var strMCARD        = EM_MARIO_NO.Text;
		var strCARDAPP		= EM_CARDAPP_NO.Text;
		var strEM_AMT		= EM_AMT.Text;
		...
		var parameters = "&strStrCd="    + encodeURIComponent(strStrCd)
					   ...
					   + "&strCardNOCK=" + encodeURIComponent(strCardNOCK)
					   + "&strMCARD="    + encodeURIComponent(strMCARD)		   
					   + "&strCARDAPP="  + encodeURIComponent(strCARDAPP)
					   + "&strEM_AMT="   + encodeURIComponent(strEM_AMT)
					   + "&strCardNo="   + encodeURIComponent(strCardNo)
					   + "&strCustId="   + encodeURIComponent(strCustId);
		//showMessage(EXCLAMATION ,	OK,	"USER-1000",parameters ); 

		TR_MAIN.Action="/mss/mcae429.mc?goTo=getSaleInfo"+parameters; 
	...
	function getSkuList() {
		...
		TR_MAIN.Action="/mss/mcae429.mc?goTo=getSkuList"+parameters;	 
...
function getSkuInfo() {
	...
	TR_MAIN.Action="/mss/mcae429.mc?goTo=getSkuInfo"+parameters;	 
...
function setDivAmt(strFlag) {
	...
	for(var i=1;i<=eval(strDataSet).CountRow;i++){
		...
		strCnt += 1;
		strSum += eval(strDataSet).NameValue(i,"TOT_SALE_AMT");
		//strSaleAmt[i] =	eval(strDataSet).NameValue(i,"TOT_SALE_AMT");
		//strRosPos[i] = i;
	}
...
function chkGiftCardNo(strRow){
	...
	TR_MAIN3.Action="/mss/mcae429.mc?goTo="+goTo+parameters;  
...
function getGiftCardNo() {
	...
	TR_MAIN2.Action="/mss/mcae429.mc?goTo="+goTo;
...
showMessage(STOPSIGN, OK, "USER-1000", "111111");
...
<script	language=JavaScript	for=EM_RECEIPT_NO event=OnKillFocus()>
	...
	if(this.Text.length	!= 24){
			showMessage(EXCLAMATION	, OK, "USER-1000", "당사 발행 영수증이 아닙니다(영수증체크).");
		}
		getSaleInfo();
	    	    ...
	    	    for(var k=i+1; k<=DS_IO_GIFTCARD_NO.CountRow; k++) {
	    	    	if (row != k) {
	    		        ...
	    		        if (iItemCd==kItemCd) {
	    		            ...
	    		            DS_IO_GIFTCARD_NO.NameValue(row,"GIFTCARD_NO") = "";
	    		            DS_IO_GIFTCARD_NO.NameValue(row,"GIFTCERT_AMT") = "";
	    		            return false;
	    		        }
	    	    	}
	    	    }
			...
			<tr>
				...
				<table width="100%"	border="0" cellpadding="0" cellspacing="0" class="s_table">
					<tr>
						...
						<th	width="100" class="point">ID</th>
							<td>
								<comment id="_NSID_">
						...
						<table width="100%"	border="0" cellpadding="0" cellspacing="0" class="s_table">
							<tr>
								...
								<th	width="110">행사명</th>
									<td>
										<comment id="_NSID_">
											<object id=EM_EVENT_NAME classid=<%=Util.CLSID_EMEDIT%> width="130"	 tabindex=1 align="absmiddle"></object>
										</comment><script>_ws_(_NSID_);</script>
									</td>
							</tr>
							<tr>
								...
								<th	width="110">사은품 지급주기</th>
									<td>
										<comment id="_NSID_">
											<object id=EM_EVENT_GIFT_CYC classid=<%=Util.CLSID_EMEDIT%> width="130"	 tabindex=1 align="absmiddle"></object>
										</comment><script>_ws_(_NSID_);</script>
									</td>
							</tr> 
							<tr>
								...
								<th	width="110">영수증  당일여부</th>
									<td>
										<comment id="_NSID_">
											<object id=EM_RECP_TODAY_YN classid=<%=Util.CLSID_EMEDIT%> width="130"	 tabindex=1 align="absmiddle"></object>
										</comment><script>_ws_(_NSID_);</script>
									</td>
							</tr>							
						</table>
	...
	<tr>
		...
		<table width="100%"	border="0" cellspacing="0" cellpadding="0" class="o_table">
			<tr>
				...
				<table width="100%"	border="0" cellpadding="0" cellspacing="0" class="s_table">
					<tr>
						<th	class="point" width="140">보너스카드번호</th>
						<td	width="200"><comment id="_NSID_"> <object
							id=EM_MARIO_NO classid=<%=Util.CLSID_EMEDIT%>	width=200
							tabindex=1 align="absmiddle"> </object>	</comment><script>_ws_(_NSID_);</script>
						</td>
						<th	class="point" width="140">신용카드번호</th>
						<td	width="140"><comment id="_NSID_"> <object
							id=EM_CARD_NO classid=<%=Util.CLSID_EMEDIT%> width=140
							tabindex=1 align="absmiddle"> </object>	</comment><script>_ws_(_NSID_);</script>
						</td>
						<th	class="point" width="140">거래승인번호</th>
						<td	width="140"><comment id="_NSID_"> <object
							id=EM_CARDAPP_NO classid=<%=Util.CLSID_EMEDIT%> width=140
							tabindex=1 align="absmiddle"> </object>	</comment><script>_ws_(_NSID_);</script>
						</td>
						<th	class="point" width="140">결재금액</th>
						<td	width="140"><comment id="_NSID_"> <object
							id=EM_AMT classid=<%=Util.CLSID_EMEDIT%> width=140
							tabindex=1 align="absmiddle"> </object>	</comment><script>_ws_(_NSID_);</script>
						</td>
					</tr>
				
					<tr>
						<th	class="point" width="140">사유</th>
						<td	width="200"><comment id="_NSID_"> <object
							id=EM_PRSNT_MEMO classid=<%=Util.CLSID_EMEDIT%>	width=200
							tabindex=1 align="absmiddle"> </object>	</comment><script>_ws_(_NSID_);</script>
						</td>
						
						<th	class="point" width="140">영수증번호</th>
						<td	width="200"><comment id="_NSID_"> <object
						 ...
						 <th	width="140">등록자</th>
						<td	width="140"><comment id="_NSID_"> <object
							id=EM_PRSNT_CHAR_RNM classid=<%=Util.CLSID_EMEDIT%> width=140
							tabindex=1 align="absmiddle"> </object>	</comment><script>_ws_(_NSID_);</script>
						</td>	
					</tr>
				</table>
