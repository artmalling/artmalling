...
function gridCreate1(){	
	var	hdrProperies ='<FC>id={currow}				name="NO"			width=30	align=center</FC>'
					...
					+ '<FC>id=EVENT_NAME			name="행사명"			width=200	align=left	 sumtext="합계"	</FC>'
					...
					+ '<FC>id=IN_QTY				name="입고"			width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=RFD_QTY				name="반품"			width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=PRSNT_QTY				name="정상지급"		width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=EXCP_PRSNT_QTY		name="예외지급"		width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=RECOVERY_QTY			name="지급취소"		width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=POS_NDRAWL_CAN_QTY	name="미회수"			width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=ETC_PRSNT				name="기타지급"		width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=NQTY					name="누적지급"		width=100	align=right	sumtext=@sum	</FC>'
					+ '<FC>id=QTY					name="현재고"			width=100	align=right	sumtext=@sum	</FC>';
...
function gridCreate2(){	
	var	hdrProperies2 ='<FC>id={currow}				name="NO"					width=30	align=center				</FC>'
					+ '<FC>id=EVENT_NM				name="행사명"					width=100	align=center				</FC>'
					+ '<FC>id=PUMBUN_CD				name="브랜드코드"				width=80	align=center sumtext="합계"	</FC>'
					+ '<FC>id=PUMBUN_NM				name="브랜드명"				width=110	align=left	 sumtext="합계"	</FC>'
					+ '<C>id=AMT1					name="정상지급 매출금액"		width=120	align=right	 sumtext=@sum	</C>'
					+ '<C>id=AMT2					name="예외지급 매출금액"		width=120	align=right	 sumtext=@sum	</C>'
					+ '<C>id=AMT3					name="정상지급취소 매출금액"		width=140	align=right	 sumtext=@sum	</C>'
					+ '<C>id=AMT4					name="예외지급취소 매출금액"		width=140	align=right	 sumtext=@sum	</C>'
					+ '<C>id=TOT_AMT				name="총 지급 매출금액"			width=120	align=right	 sumtext=@sum	</C>';
...
function btn_Search() {
	...
	TR_MAIN.Action="/mss/mcae411.mc?goTo="+goTo+parameters;	 
...
function btn_Excel() {
	 ...
	 if	(CUR_GD	== 0) {
			var	ExcelTitle = "사은행사 행사별 지급내역 조회"
			//openExcel2(GD_MASTER, ExcelTitle, "", true );
				openExcel5(GD_MASTER, ExcelTitle, "", true , "",g_strPid );

		} else {
			var	ExcelTitle = "사은행사 브랜드별	지급내역 조회"
			//openExcel2(GD_DETAIL, ExcelTitle, "", true );
			openExcel5(GD_DETAIL, ExcelTitle, "", true , "",g_strPid );
		}
}
...
function openPop(){
	...
	var	returnVal =	window.showModalDialog("/mss/mcae411.mc?goTo=openPop",
										   arrArg,
...
if(clickSORT) return;
	if (row	> 0	&& g_select==false)	{  
		if(DS_MASTER.CountRow >	0) setTimeout("getDetail()",50);
