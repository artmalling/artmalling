...
function doInit(){
    ...
    // Output Data Set Header 초기화
     ...
     DS_O_DETAIL.setDataHeader('<gauce:dataset name="H_SEL_MPCALLMGRDETAIL"/>');//상세보기
    ...
    initComboStyle(LC_CALL_FLAG_I,DS_CALL_FLAG_I, "CODE^0^30,NAME^0^80", 1, PK);      //통화구분
    ...
    //initEmEdit(TXT_O_CALL_DESC, "GEN", NORMAL);            //통화내역
    
    
     getStore("DS_O_STR_CD", "Y", "1", "N");            //점코드        
    getEtcCode("DS_O_SCH_FLAG",   "D", "M061", "N", "N");//조회구분
    getEtcCode("DS_PROM_TYPE",   "D", "M021", "Y", "N");//약속유형
    getEtcCode("DS_STR_CD", "D", "M034", "N");
    getEtcCode("DS_CALL_FLAG_I", "D", "M026", "N"); //통화구분
    ...
    LC_CALL_FLAG_I.Enable = "false";
    TEXT_CALL_DESC_I.Enable = "false";    
        
    registerUsingDataset("mpro105","DS_O_MASTER, DS_O_DETAIL");
}
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}     name="NO"             width=30   align=center</FC>'
                     + '<FC>id=CALL_DT      name="전화통화일자"     Edit=None width=130 MASK="XXXX/XX/XX" align=center</FC>'                     
                     + '<FC>id=SEND_NM      name="발신자"          Edit=None width=80   align=left</FC>'
                     + '<FC>id=SEQ_NO       name="순번"            Edit=None width=80   align=center</FC>';
                     ...
                     + '<FC>id=CALL_FLAG    name="통화구분"            width=80 DATA="DS_SEND_TYPE_I:CODE:NAME  align=center SHOW=FALSE </FC>';
...
function btn_New() {
	 if ( DS_O_MASTER.CountRow > 0) {
         ...
         LC_CALL_FLAG_I.Enable = "true";
         TEXT_CALL_DESC_I.Enable = "true";
         LC_CALL_HH.Index = 0;
         LC_CALL_MM.Index = 0;
         LC_CALL_FLAG_I.Index = 0;
         EM_SEND_NM_I.Focus();       
...
function btn_Delete() {
     ...
     TR_MAIN.Action="/mss/mpro105.mp?goTo=save&strFlg=delete"+parameters; 
...
function btn_Save() {
	 ...
	 if ( !DS_O_DETAIL.IsUpdated ) {//변경 데이터가 있을때만 저장
	     ...
	     TR_MAIN.Action = "/mss/mpro105.mp?strFlg=save&goTo=" + goTo + parameters;
 ...
 *************************************************************************/
 function getSearch(){
	  ...
	  TR_MAIN.Action = "/mss/mpro105.mp?goTo=" + goTo + parameters + "&custNm=" + encodeURIComponent(custNm);
...
function getCallDetail(){
    ...
    TR_MAIN.Action = "/mss/mpro105.mp?goTo=" + goTo + parameters;
...
function getCallDetail2(row){
    ...
    TR_S_MAIN.Action = "/mss/mpro105.mp?goTo=" + goTo + parameters;
...
function VailCheckSave(){
	...
	for( i = 0; i <= DS_O_DETAIL.CountRow; i++ ){
		if( DS_O_DETAIL.RowStatus(i) == 1 ||  DS_O_DETAIL.RowStatus(i) == 3 ){
			...
			if( DS_O_DETAIL.NameValue(i, "CALL_FLAG") == "" ){
                showMessage(STOPSIGN, OK, "USER-1003", "통화구분");
                DS_O_DETAIL.RowPosition = i;
                LC_CALL_FLAG_I.Focus();
                return false;
            }
...
if(clickSORT) return;
     ...
     if( DS_O_DETAIL.NameValue(row, "SEND_NM") == "" && DS_O_DETAIL.NameValue(row, "SEQ_NO ") == "" ){
...
<comment id="_NSID_"><object id=DS_CALL_FLAG_I  classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
        ...
        <td >
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
            <tr>
                ...
                <td>
                          <comment id="_NSID_">
                              <object id=EM_SALE_S_DT classid=<%=Util.CLSID_EMEDIT%>  width=85 tabindex=1 align="absmiddle">
                              </object>
                          </comment>
                          ...
                          ~
                          <comment id="_NSID_">
                              <object id=EM_SALE_E_DT classid=<%=Util.CLSID_EMEDIT%>  width=85 tabindex=1 align="absmiddle">
                              </object>
                          </comment>
                          <script> _ws_(_NSID_);</script>
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr valign="top">
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="BD4A">
          <tr>
            <td width="100%">
            <comment id="_NSID_"><OBJECT id=GD_MASTER width=100% height=297 classid=<%=Util.CLSID_GRID%> tabindex=1 >
    ...
    <td class="PT1"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr >
                <td width="350"><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="BD4A">
                    <tr>
                        <td width="100%">
                            <comment id="_NSID_"><OBJECT id=GD_DETAIL width=100% height=150 classid=<%=Util.CLSID_GRID%> tabindex=1 >
                    ...
                    <tr>
                        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="s_table">
                            <tr>
                                ...
                                <td width="80">
                                      <comment id="_NSID_">
                                          <object id=EM_CALL_SEQ_NO_I  classid=<%=Util.CLSID_EMEDIT%>  width=80 tabindex=1 align="absmiddle">
                                          </object>
                                      </comment>
                                      <script> _ws_(_NSID_);</script>
                                </td>
                                ...
                                <td>
                                      <comment id="_NSID_">
                                          <object id=EM_CALL_DT_I  classid=<%=Util.CLSID_EMEDIT%>  width=85 tabindex=1 align="absmiddle">
                                          </object>
                                      </comment>
                                      <script> _ws_(_NSID_);</script>
                            ...
                            <tr>
                                ...
                                <td width="80">
                                      <comment id="_NSID_">
                                          <object id=EM_SEND_NM_I  classid=<%=Util.CLSID_EMEDIT%>  width=80 tabindex=1  align="absmiddle"  >
                                          </object>
                                      </comment>
                                      <script> _ws_(_NSID_);</script>
                                </td>
                                <th class="POINT" width="60">수신자</th>
                                <td >
                                      <comment id="_NSID_">
                                          <object id=EM_RECV_NM_I   classid=<%=Util.CLSID_EMEDIT%>  width=210 tabindex=1 align="absmiddle">
                                          </object>
                                      </comment>
                                      <script> _ws_(_NSID_);</script>
                                </td>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                      <comment id="_NSID_">
                                          <object id=LC_CALL_FLAG_I classid=<%=Util.CLSID_LUXECOMBO%>  width=385 tabindex=1 align="absmiddle">
                                          </object>
                                      </comment>
                                      <script> _ws_(_NSID_);</script>
                                </td>
                            ...
                            <tr>
                                ...
                                <td colspan="3" class="PT02 PB02">
                                    <comment id="_NSID_"> 
                                        <object id=TEXT_CALL_DESC_I   classid=<%=Util.CLSID_TEXTAREA%> tabindex=1 height=70 width=100%
                                         tabindex=1 align="absmiddle" maxLength="2000" 
                                         onkeyup="javascript:textAreaMaxlength(TEXT_CALL_DESC_I, 2000);"
                                         onkeyDown="javascript:textAreaMaxlength(TEXT_CALL_DESC_I, 2000);"
                                         onkeyPress ="javascript:textAreaMaxlength(TEXT_CALL_DESC_I, 2000);" 
                                         > 
                                        </object> 
                                    </comment>
                                    <script>_ws_(_NSID_);</script>
...
<comment id="_NSID_"> <object id=BO_IO_DETAIL
    ...
    <param name=BindInfo
        ...
        <c>Col=CALL_FLAG        Ctrl=LC_CALL_FLAG_I          param=BindColVal</c>
