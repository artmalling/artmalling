...
function doInit(){
    ...
    registerUsingDataset('<%=((SessionInfo) session.getAttribute("sessionInfo")).getPGM_ID()%>'.toLowerCase(), "DS_MASTER,DS_DETAIL");
    ...
    // EMedit에 초기화
    initEmEdit(EM_S_S_DT,        "YYYYMMDD",   READ);       //조회기간FROM (조회)
    initEmEdit(EM_S_E_DT,        "YYYYMMDD",   READ);       //조회기간TO (조회)     
    initEmEdit(EM_S_DONG,        "000",        NORMAL);     //동 (조회)
    initEmEdit(EM_S_HO,          "0000",       NORMAL);     //호 (조회)
    initEmEdit(EM_S_HHOLD_NAME,  "GEN^40",     NORMAL);     //세대주명 (조회) 
    EM_S_HHOLD_NAME.Language = 1;                           //세대주명-한글
    ...
    initEmEdit(EM_I_MOVE_OUT_DT, "YYYYMMDD",   PK);         //전출일  
    ...
    EM_I_MOVE_OUT_DT.Enable = false;
    enableControl(OutDt, false);
}
...
function btn_Search() {
    ...
    var strHname     = EM_S_HHOLD_NAME.Text;       // [조회용]세대주명
    var strSel       = LC_S_SEL.BindColVal;        // [조회용]조회기간구분
    var strStart     = EM_S_S_DT.Text;             // [조회용]조회기간FROM
    var strEnd       = EM_S_E_DT.Text;             // [조회용]조회기간TO
    
    var goTo = "getMaster";
    var parameters = "&strFclCd="   + encodeURIComponent(strFclCd)
                   + "&strDong="    + encodeURIComponent(strDong)
                   + "&strHo="      + encodeURIComponent(strHo)
                   + "&strHname="   + encodeURIComponent(strHname)
                   + "&strSel="     + encodeURIComponent(strSel)
                   + "&strStart="   + encodeURIComponent(strStart)
                   + "&strEnd="     + encodeURIComponent(strEnd);
    TR_MAIN.Action = "/mss/mren306.mr?goTo=" + goTo + parameters;
...
function move_out() {
   if(DS_MASTER.CountRow >= 1) {
	    if (DS_MASTER.NameValue(DS_MASTER.RowPosition, "MOVE_OUT_DT").length == 8) {
	        if (DS_MASTER.NameValue(DS_MASTER.RowPosition, "MOVE_OUT_DT") > getTodayDB("DS_O_RESULT")) {
	            showMessage(INFORMATION, OK, "USER-1000", "전출일은 당일 이전으로만 등록가능합니다.");
	            EM_I_MOVE_OUT_DT.Focus();
	            return false;
	        }
	    }  else {
	        showMessage(INFORMATION, OK, "USER-1003", "전출일");
	        EM_I_MOVE_OUT_DT.Focus();
	        return false;
	    }
       
       if(DS_MASTER.NameValue(DS_MASTER.RowPosition,"MOVE_IN_DT") > DS_MASTER.NameValue(DS_MASTER.RowPosition,"MOVE_OUT_DT")) {
           showMessage(EXCLAMATION, OK, "USER-1000",  "전출일은 전입일 이후로 입력하셔야 합니다.");
           EM_I_MOVE_OUT_DT.Focus();
           return;
       } 
   }
   ...
   //관리비정산
   if( showMessage(INFORMATION, YESNO, "USER-1000", "이미 등록된 정보는 삭제되고 재생성됩니다. 정산을 실행하시겠습니까?") != 1 ){
       return;
   }
   
   var strStrCd = DS_MASTER.NameValue(DS_MASTER.RowPosition,"STR_CD");
   var strHholdId = DS_MASTER.NameValue(DS_MASTER.RowPosition,"HHOLD_ID"); 
   var strILJA = DS_MASTER.NameValue(DS_MASTER.RowPosition,"MOVE_OUT_DT"); 
   g_currRow = DS_MASTER.RowPosition;
   
   var goTo = "procCalculate";
   var parameters = "&strStrCd="    + encodeURIComponent(strStrCd)
                  + "&strHholdId="  + encodeURIComponent(strHholdId)
                  + "&strILJA="     + encodeURIComponent(strILJA);
   TR_SAVE.Action = "/mss/mren306.mr?goTo=" + goTo + parameters;
   TR_SAVE.KeyValue = "SERVLET(O:DS_MASTER=DS_MASTER)" 
   TR_SAVE.Post(); 
}
 ...
 */
function getDetail() { 
	var strOutDt     = DS_MASTER.NameValue(DS_MASTER.RowPosition,"MOVE_OUT_DT");
	var strCntrId    = DS_MASTER.NameValue(DS_MASTER.RowPosition,"HHOLD_ID"); 
	var goTo       = "getDetail" ;    
...
function checkValidateSearch() {
	...
	if (LC_S_SEL.BindColVal != "%") {
		if (EM_S_S_DT.Text == "") {
		    showMessage(INFORMATION, OK, "USER-1003", "조회기간(FROM)");
		    EM_S_S_DT.Focus();
		    return false;
		}
	    
		if (EM_S_E_DT.Text == "") {
		    showMessage(INFORMATION, OK, "USER-1003", "조회기간(TO)");
		    EM_S_E_DT.Focus();
		    return false;
		}
		
		if (EM_S_S_DT.Text > EM_S_E_DT.Text) {
            showMessage(INFORMATION, OK, "USER-1000", "조회기간(TO)는 조회기간(FROM) 이 후로 입력하셔야 합니다.");
            EM_S_E_DT.Focus();
            return false;
		}
	} 
...
<script language=JavaScript for=TR_SAVE event=onSuccess>
    ...
    DS_MASTER.RowPosition = g_currRow;
</script>
...
if(clickSORT) return;
if(DS_MASTER.NameValue(row,"MOVE_OUT_DT") == "") {
	EM_I_MOVE_OUT_DT.Text          = "";   
	EM_I_MOVE_OUT_DT.Enable = true;
	enableControl(OutDt, true);
} else {
	EM_I_MOVE_OUT_DT.Text = DS_MASTER.NameValue(row,"MOVE_OUT_DT");
	EM_I_MOVE_OUT_DT.Enable = false;
	enableControl(OutDt, false);
}
...
if (DS_MASTER.CountRow > 0) {
    if (DS_MASTER.RowStatus(row) == 3) { //데이터 변경시
...
<script language=JavaScript for=LC_S_SEL event=OnSelChange()>
//조회조건초기화
if (LC_S_SEL.BindColVal == "%") {
    EM_S_S_DT.Text = "";
    EM_S_E_DT.Text = "";
    EM_S_S_DT.Enable      = false;           
    EM_S_E_DT.Enable      = false;           
    enableControl(IMG_S_DT,false); // 조회기간 비활성화(팝업)
    enableControl(IMG_E_DT,false); // 조회기간 비활성화(팝업)
} else {
    initEmEdit(EM_S_S_DT,         "SYYYYMMDD",NORMAL); // [조회용]기간S
    initEmEdit(EM_S_E_DT,         "EYYYYMMDD",NORMAL); // [조회용]기간E
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<th width="100">세대주명</th>
						<td><comment id="_NSID_"> <object
							id=EM_S_HHOLD_NAME classid=<%=Util.CLSID_EMEDIT%> width=112
							tabindex=1 align="absmiddle"> </object> </comment><script>_ws_(_NSID_);</script></td>
					</tr>
					<tr>
						<th>조회기간구분</th>
						<td><comment id="_NSID_"> <object id=LC_S_SEL
							classid=<%=Util.CLSID_LUXECOMBO%> width="130" height=120
							tabindex=1 align="absmiddle">
							<param name=CBData value="%^전체,1^입주일,2^퇴거일">
						...
						<object id=EM_S_S_DT classid=<%=Util.CLSID_EMEDIT%> width=79
							tabindex=1 align="absmiddle"
						...
						<img src="/<%=dir%>/imgs/btn/date.gif" align="absmiddle"
							id="IMG_S_DT" onclick="javascript:openCal('G',EM_S_S_DT)" /> ~ <comment
							id="_NSID_"> <object id=EM_S_E_DT
							classid=<%=Util.CLSID_EMEDIT%> width=79 tabindex=1
							align="absmiddle" onblur="javascript:checkDateTypeYMD(this);">
						</object> </comment><script>_ws_(_NSID_);</script> <img
							src="/<%=dir%>/imgs/btn/date.gif" align="absmiddle" id="IMG_E_DT"
							...
							<tr>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0"
									...
									<tr>
										...
										</object> </comment> <img src="/<%=dir%>/imgs/btn/date.gif" id="OutDt"
											onclick="javascript:openCal('G',EM_I_MOVE_OUT_DT)"
											align="absmiddle" /></td>
									</tr>
									<tr>
										<th>면적</th>
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												<td class="sub_title"><img
													...
													align="absmiddle" /> 전출정산 정보</td>
												<td class="right PB03"><img
													src="/<%=dir%>/imgs/btn/move_out.gif" hspace="2"
													onClick="javascript:move_out();" />
											</tr>
										</table>
										</td>
									</tr>
										...
										<table width="100%" border="0" cellspacing="0" cellpadding="0"
											...
											<tr>
												<td><comment id="_NSID_"><OBJECT id=GD_DETAIL
													width=100% height=322 classid=<%=Util.CLSID_GRID%>> tabindex=1 
