...
function doInit(){
	...
	initEmEdit(EM_SALE_DT, "YYYYMMDD", PK); //조회일자
	initEmEdit(EM_COMP_DT, "YYYYMMDD", PK); //조회일자
	EM_SALE_DT.Text = "<%=toDay%>";
    var strCmprdt = searchCmpr();
    EM_COMP_DT.text =  strCmprdt;
...
function btn_Search() {
    ...
    if (isNull(EM_SALE_DT.text)){
        showMessage(EXCLAMATION, OK, "USER-1003", "매출일자"); //(조회일자)은/는 반드시 입력해야 합니다.
        EM_SALE_DT.focus();
        return;
    }
    if (isNull(EM_COMP_DT.text)){
        showMessage(EXCLAMATION, OK, "USER-1003", "대비일자"); //(조회일자)은/는 반드시 입력해야 합니다.
        EM_COMP_DT.focus();
        return;
    }  
    var parameters = "&strStrCd="  + LC_STR_CD.BindColVal //점코드
                   + "&strSaleYmd=" + EM_SALE_DT.text      //조회일자
                   + "&strCompYmd=" + EM_COMP_DT.text      //조회일자
                   ;
    
    IFrame.location.href="/mss/jsp/meis/meis2101.jsp?"+parameters;
}
...
function btn_Print() {
    ...
    var parameters = "&strStrCd="  + LC_STR_CD.BindColVal //점코드
                   ...
                   + "&strCompYmd=" + EM_COMP_DT.text      //조회일자
                   ;
    //window.open("/mss/jsp/meis/meis2101.jsp?"+parameters,"OZPRINT", 1000, 700);	 
    window.open("/mss/jsp/meis/meis2101.jsp?"+parameters,"OZPRINT", 1000, 700);
 ...
 *************************************************************************/
 ...
 function searchCmpr(){
     ...
     var strSaleDtS      = EM_SALE_DT.text;         //매출일자
...
<script language=JavaScript for=EM_SALE_DT event=onKillFocus()>
    ...
    var strCmprdt = searchCmpr();   
    if(strCmprdt == "") EM_COMP_DT.text =  addDate('Y',-1, EM_SALE_DT_S.text);
    else EM_COMP_DT.text =  strCmprdt;
</script>    
<!--*************************************************************************-->
<!--* C. 가우스 이벤트 처리 끝                                                                                                                                   *-->
<!--*************************************************************************-->
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
            ...
            <td width="160">
              <comment id="_NSID_">
                <object id=EM_SALE_DT classid=<%=Util.CLSID_EMEDIT%> width=80 align="absmiddle"></object>
              </comment><script> _ws_(_NSID_);</script>
              <img src="/<%=dir%>/imgs/btn/date.gif" class="PR03"
                onclick="javascript:openCal('G',EM_SALE_DT)" align="absmiddle" />
            </td>
            <th width="80" class="point">대비일자</th>
            <td>
              <comment id="_NSID_">
                <object id=EM_COMP_DT classid=<%=Util.CLSID_EMEDIT%> width=80 align="absmiddle"></object>
              </comment><script> _ws_(_NSID_);</script>
              <img src="/<%=dir%>/imgs/btn/date.gif" class="PR03"
                onclick="javascript:openCal('G',EM_COMP_DT)" align="absmiddle" />
            </td>
          </tr>
        </table></td>
      </tr>
