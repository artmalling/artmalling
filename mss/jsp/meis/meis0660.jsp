<!-- 
/*******************************************************************************
 * 시스템명 : 경영실적 > 경영실적> 경영실적배부이력조회
 * 작 성 일 : 2011.08.16
 * 작 성 자 : 이정식
 * 수 정 자 : 
 * 파 일 명 : meis0660.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 경영실적의 조직별 실적항목의 배부 금액의 이력을 조회한다.
 * 이    력 :
 *        2011.08.16 (이정식) 신규작성
 ******************************************************************************/
-->
...
function gridCreate(){
    ...
    var hdrProperies1 = '<FC>id={currow}      width=25  align=center name="NO"</FC>'
                      ...
                      + '<FC>id=BIZ_CD_NM     width=80  align=left   name="항목명"</FC>'
    ...
    var hdrProperies4 = '<C>id={currow}     width=25  align=center name="NO"</C>'
                      + '<C>id=STR_CD       width=50  align=center name="점코드"</C>'
                      + '<C>id=STR_NAME     width=100 align=center name="점명"</C>'
                      + '<C>id=ORG_CD       width=100 align=center name="조직코드"</C>'
                      + '<C>id=ORG_NAME     width=100 align=left   name="조직명" SumText="합계"</C>'
                      + '<C>id=BIZ_CD       width=90  align=center name="항목코드"</C>'
                      + '<C>id=BIZ_CD_NM    width=110 align=left   name="항목명"</C>'
                      + '<C>id=DIV_RECV_AMT width=80 align=right  name="조정받은금액" SumText=@sum</C>'
                      + '<C>id=REMARK    	width=200 align=left   name="비고"</C>'
                      ;

    initGridStyle(GD_ADJ_RECV, "common", hdrProperies4, false);
    GD_ADJ_RECV.ViewSummary = "1";
    
    var hdrProperies5 = '<C>id={currow}     width=25  align=center name="NO"</C>'
                      + '<C>id=STR_CD       width=50  align=center name="점코드"</C>'
                      + '<C>id=STR_NAME     width=100 align=center name="점명"</C>'
                      + '<C>id=ORG_CD       width=100 align=center name="조직코드"</C>'
                      + '<C>id=ORG_NAME     width=100 align=left   name="조직명" SumText="합계"</C>'
                      + '<C>id=BIZ_CD       width=90  align=center name="항목코드"</C>'
                      + '<C>id=BIZ_CD_NM    width=110 align=left   name="항목명"</C>'
                      + '<C>id=DIV_RECV_AMT width=80 align=right  name="조정금액" SumText=@sum</C>'
                      + '<C>id=REMARK    	width=200 align=left   name="비고"</C>'
                      ;

    initGridStyle(GD_ADJ, "common", hdrProperies5, false);
    GD_ADJ.ViewSummary = "1";


    var adjstHistExcel = '<C>id={currow}     width=25  align=center name="NO"				</C>'
				        + '<C>id=STR_CD       width=50  align=center name="점코드"			</C>'
				        + '<C>id=STR_NAME     width=100 align=center name="점명"				</C>'
				        + '<C>id=ORG_CD       width=100 align=center name="조직코드"			</C>'
				        + '<C>id=ORG_NAME     width=100 align=left   name="조직명"			</C>'
				        + '<C>id=BIZ_CD       width=90  align=center name="항목코드"			</C>'
				        + '<C>id=BIZ_CD_NM    width=110 align=left   name="항목명"			</C>'
				        + '<C>id=TGET_ORG_CD       width=100 align=center name="대상조직코드"	</C>'
				        + '<C>id=TGET_ORG_NAME     width=100 align=left   name="대상조직명"		</C>'
				        + '<C>id=TGET_BIZ_CD       width=90  align=center name="대상항목코드"	</C>'
				        + '<C>id=TGET_BIZ_CD_NM    width=110 align=left   name="대상항목명"		</C>'
				        + '<C>id=ADJST_AMT width=80 align=right  name="조정금액" 			</C>'
				        + '<C>id=REMARK    	width=200 align=left   name="비고"</C>'
				        ;

	initGridStyle(GD_ADJST_HIST_EXCEL, "common", adjstHistExcel, false);
...
function btn_Search() {
    ...
    var parameters = "&strRsltYm=" + EM_RSLT_YM.text       //실적년월
                   ...
                   ;
    DS_ORG.DataID = "/mss/meis066.me?goTo="+goTo+parameters;
    DS_ORG.Reset();
}
...
function subQuery(row){
    ...
    DS_BIZ.DataID = "/mss/meis066.me?goTo="+goTo+parameters;
    DS_BIZ.Reset();	 
}
...
function subQuery2(row){
 	...
 	DS_HIS.ClearData();
 	DS_HIS.DataID   = "/mss/meis066.me?goTo="+goTo+parameters;
...
function btn_Excel() {
    ...
    DS_ADJST_HIST_EXCEL.DataID   = "/mss/meis066.me?goTo="+goTo+parameters;
    DS_ADJST_HIST_EXCEL.SyncLoad = "true";
    DS_ADJST_HIST_EXCEL.Reset();
...
function btn_ExcelDownload() {
	...
	var parameters  = "점명="           		+ DS_ADJST_HIST_EXCEL.NameString(1,"STR_NAME");
		parameters += " , 경영실적년도= "  	+ EM_RSLT_YM.text;
	var ExcelTitle = "경영실적조정이력";
	
	openExcel2(GD_ADJST_HIST_EXCEL, ExcelTitle, parameters, true );
...
<script language=JavaScript for=DS_ADJST_HIST_EXCEL event=OnLoadCompleted(rowcnt)>
    if(!isNull(this.ErrorMsg)) 
        showMessage(INFORMATION, OK, "USER-1000", this.ErrorMsg);

    if(rowcnt > 0) btn_ExcelDownload();
...
<script language=JavaScript for=DS_ADJST_HIST_EXCEL  event=OnLoadError()>
    dsFailed(this);
</script>
<!--------------------- 5. DataSet 이벤트 처리  ------------------------------->
<!--로우변경시 디테일정보 로딩-->
...
<!-- ====================Output용================== -->
    ...
    <object id="DS_ADJST_HIST_EXCEL"	classid=<%= Util.CLSID_DATASET %>></object>
  ...
  <tr>
    <td class="PT05 PB03">
      <div id=TB_NORMAL width="100%" height=242 TitleWidth=120 TitleAlign="center" TitleStyle="" TitleGap=3>
        ...
        <menu TitleName="조정받은이력" DivId="tr_tab3" ClickFunc="fn_changeTab" />
        <menu TitleName="조정이력"     DivId="tr_tab4" ClickFunc="fn_changeTab" />
      </div>             
    </td>
  </tr>
  <tr>
                    ...
                    <td class="PT03"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr valign="top">
                        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
                          <tr>
                            <td>
                              <comment id="_NSID_"><object id=GD_ADJ_RECV width="100%" height=207 classid=<%=Util.CLSID_GRID%>>
                                <param name="DataID" value="DS_HIS">
                              </object></comment><script>_ws_(_NSID_);</script>
                            </td>
                          </tr>
                    ...
                    <td class="PT03"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr valign="top">
                        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
                          <tr>
                            <td>
                              <comment id="_NSID_"><object id=GD_ADJ width="100%" height=207 classid=<%=Util.CLSID_GRID%>>
                                <param name="DataID" value="DS_HIS">
                              </object></comment><script>_ws_(_NSID_);</script>
                            </td>
                          </tr>
...
------------------------------------------------------------------------------>
<comment id="_NSID_">
  <object id=GD_ADJST_HIST_EXCEL width="0" height=0 classid=<%=Util.CLSID_GRID%>>
    <param name="DataID" value="DS_ADJST_HIST_EXCEL">
  </object>
</comment><script>_ws_(_NSID_);</script>
</body>
</html>
