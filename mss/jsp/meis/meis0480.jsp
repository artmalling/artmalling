<!-- 
/*******************************************************************************
 * 시스템명 : 경영실적 > 경영계획> 조직별손익계획상세조회
 * 작 성 일 : 2011.07.01
 * 작 성 자 : 최재형
 * 수 정 자 : 
 * 파 일 명 : meis0480.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 경영계획의 조직별손익계획상세조회을 조회한다.
 * 이    력 :
 *        2011.07.01 (최재형) 신규작성
 ******************************************************************************/
-->
...
function gridCreate(){
	...
	var gd_profitAndLossPlan  = '<FC>id={currow}   	width=25  	align=center	name="NO" 		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} </FC>';
	gd_profitAndLossPlan += '<FC>id=BIZ_CD         	width=80  	align=center 	name="항목코드"	BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} </FC>';
	gd_profitAndLossPlan += '<FC>id=BIZ_CD_NM       width=90  	align=left 		name="항목명"	BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} </FC>';
	gd_profitAndLossPlan += '<C>id=AMT_TOT       	width=100  	align=right 	name="년합계"	BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_01         	width=90  	align=right 	name="1월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_02         	width=90  	align=right		name="2월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_03         	width=90  	align=right 	name="3월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_04         	width=90  	align=right 	name="4월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_05         	width=90  	align=right 	name="5월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_06         	width=90  	align=right 	name="6월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_UPPER_HALF	width=100  	align=right 	name="상반기계"  BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_07         	width=90  	align=right 	name="7월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_08         	width=90  	align=right 	name="8월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_09         	width=90  	align=right 	name="9월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_10         	width=90  	align=right 	name="10월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_11         	width=90  	align=right 	name="11월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_12         	width=90  	align=right 	name="12월"		BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';
	gd_profitAndLossPlan += '<C>id=AMT_LOWER_HALF 	width=100  	align=right 	name="하반기계"  BgColor={decode(BIZ_LVL,"1","#BDDBEF","2","#FFD0D0")} decao=2 </C>';				         
				         
    initGridStyle(GD_PROFIT_AND_LOSS_PLAN_DTL, "common", gd_profitAndLossPlan, false);
    
    //엑셀용
    initGridStyle(GD_PROFIT_AND_LOSS_PLAN_DTL_EXCEL, "common", gd_profitAndLossPlan, false);
...
function btn_Search() {
	...
	//dataset 초기화
	DS_PROFIT_AND_LOSS_PLAN_DTL.ClearData();
	...
	var goTo = "searchProfitAndLossPlanDtl";
	var action     = "O";
	var parameters = "";
    ...
    DS_PROFIT_AND_LOSS_PLAN_DTL.DataID   = "/mss/meis048.me?goTo="+goTo+parameters;
    DS_PROFIT_AND_LOSS_PLAN_DTL.SyncLoad = "true";
    DS_PROFIT_AND_LOSS_PLAN_DTL.Reset();
...
function btn_Excel() {
	...
	if(!lo_searchFlag || DS_PROFIT_AND_LOSS_PLAN_DTL.CountRow == 0){
    ...
    DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL.DataID   = "/mss/meis048.me?goTo="+goTo+parameters;
    DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL.SyncLoad = "true";
    DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL.Reset();
}
...
function btn_ExcelDownload() {
	...
	var parameters  = "조직명="           	+ DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL.NameString(1,"ORG_NAME");
		parameters += " , 경영계획년도= "  	+ lo_PlanY;
	var ExcelTitle = "조직별손익계획";
	
	openExcel2(GD_PROFIT_AND_LOSS_PLAN_DTL_EXCEL, ExcelTitle, parameters, true );
...
<!--------------------- 3. DataSet Success 메세지 처리  ----------------------->
<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN_DTL event=OnLoadCompleted(rowcnt)>
    if(!isNull(this.ErrorMsg)) 
        showMessage(INFORMATION, OK, "USER-1000", this.ErrorMsg);

	lo_searchFlag = true;
    setPorcCount("SELECT", rowcnt);
    GD_PROFIT_AND_LOSS_PLAN_DTL.Focus();
</script>

<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL event=OnLoadCompleted(rowcnt)>
    if(!isNull(this.ErrorMsg)) 
        showMessage(INFORMATION, OK, "USER-1000", this.ErrorMsg);
...
<!--------------------- 4. DataSet Fail 메세지 처리  -------------------------->
<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN_DTL  event=OnLoadError()>
    if (this.ErrorCode == 0 ) showMessage(STOPSIGN, OK, "GAUCE-1010");
    else showMessage(STOPSIGN, OK, "GAUCE-1000", this.ErrorMsg);
</script>

<script language=JavaScript for=for=DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL  event=OnLoadError()>
    if (this.ErrorCode == 0 ) showMessage(STOPSIGN, OK, "GAUCE-1010");
    else showMessage(STOPSIGN, OK, "GAUCE-1000", this.ErrorMsg);
</script>

<!--------------------- 5. DataSet 이벤트 처리  ------------------------------->
<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN_DTL event=OnRowPosChanged(row)>
...
<comment id="_NSID_">
    ...
    <!--[그리드]매출계획분석 -->
    <object id="DS_PROFIT_AND_LOSS_PLAN_DTL"      classid=<%= Util.CLSID_DATASET %>></object>
    <!--[그리드]매출계획분석 엑셀 -->
    <object id="DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL"      classid=<%= Util.CLSID_DATASET %>></object>
                    ...
                    <td>
	                   	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
    	                   	<tr>
        	               		<td  class="BD4A">
            	                    <comment id="_NSID_">
                                    <object id=GD_PROFIT_AND_LOSS_PLAN_DTL width="100%" height=480 classid=<%=Util.CLSID_GRID%>>
                                        <param name="DataID" value="DS_PROFIT_AND_LOSS_PLAN_DTL">
...
------------------------------------------------------------------------------>
<comment id="_NSID_">
  <object id=GD_PROFIT_AND_LOSS_PLAN_DTL_EXCEL width="0" height=0 classid=<%=Util.CLSID_GRID%>>
    <param name="DataID" value="DS_PROFIT_AND_LOSS_PLAN_DTL_EXCEL">
  </object>
</comment><script>_ws_(_NSID_);</script>
