 ...
 * 개    요 : 계정별/조직별/년월별로 비용계획을 등록/확정한다.
 * 이    력 :
 *        2011.05.23 (이정식) 신규작성
 ******************************************************************************/
-->
...
function doInit(){
    ...
    registerUsingDataset("meis0260","DS_ACC,DS_ORG,DS_MONTH" );
}
...
function gridCreate(){
	var hdrProperies = '<FC>id={currow} width=25 align=center name="NO"</FC>'
		             + '<FC>id=ACC_CD   width=80 align=center name="계정항목" edit="AlphaNum" EditStyle=Popup</FC>'
		             + '<C>id=ACC_NM    width=80 align=left   name="항목명"   edit="None" SumText="합계"</C>'
    ...
    var hdrProperies2 = '<C>id=MONTH    width=40 align=center name="월" edit="None" SumText="합계"</C>'
                      + '<C>id=PLAN_AMT width=70 align=right  name="금액" edit=Numeric SumText=@sum</C>'
                      ;

    initGridStyle(GD_MONTH, "common", hdrProperies2, true);
 ...
 */
function btn_Search() {
	if(DS_ACC.IsUpdated || DS_ORG.IsUpdated || DS_MONTH.IsUpdated){
    ...
    lo_StrCd = LC_STR_CD.BindColVal;
    lo_PlanY = EM_PLAN_YEAR.Text;
    DS_ACC.DataID   = "/mss/meis026.me?goTo="+goTo+parameters;
...
function subQuery(row){
    ...
    var parameters = "&strStrCd=" + DS_ACC.NameValue(row, "STR_CD")        //점코드
                   + "&strPlanY=" + DS_ACC.NameValue(row, "BIZ_PLAN_YEAR") //경영계획년도
                   + "&strAccCd=" + DS_ACC.NameValue(row, "ACC_CD")        //계정항목코드
                   ;

    DS_ORG.DataID  = "/mss/meis026.me?goTo="+goTo+parameters;
...
function subQuery2(row){
    ...
    var parameters = "&strStrCd=" + DS_ORG.NameValue(row, "STR_CD")        //점코드
                   ...
                   + "&strAccCd=" + DS_ORG.NameValue(row, "ACC_CD")        //계정항목코드
                   + "&strOrgCd=" + DS_ORG.NameValue(row, "ORG_CD")        //조직코드
                   ;
    
    DS_MONTH.DataID  = "/mss/meis026.me?goTo="+goTo+parameters;
...
function btn_New() {
    ...
    DS_ACC.NameValue(DS_ACC.CountRow, "BIZ_PLAN_YEAR") = lo_PlanY;
    enableControl(IMG_ROW_ADD, false);
    enableControl(IMG_ROW_DEL, false);
...
function btn_Delete() {
	...
	btnSaveClick     = true;
    TR_MAIN.Action   = "/mss/meis026.me?goTo=delete";
 ...
 */
function btn_Save() {
	if(!DS_ORG.IsUpdated && !DS_MONTH.IsUpdated){
        //저장할 내용이 없습니다.
    ...
    for(var i = 1; i<=DS_MONTH.CountRow ; i++){
    	if(DS_MONTH.RowStatus(i) == "3"){
    		if(DS_MONTH.NameValue(i, "PLAN_AMT") < 0){
    			showMessage(INFORMATION, OK, "USER-1020", "금액", "0");
    			setFocusGrid(GD_MONTH,DS_MONTH, i, "PLAN_AMT");
    			return;
    		}
    	}
    }
    ...
    TR_MAIN.Action   = "/mss/meis026.me?goTo=save";  
    TR_MAIN.KeyValue = "SERVLET(I:DS_ACC=DS_ACC,I:DS_ORG=DS_ORG,I:DS_MONTH=DS_MONTH)"; 
    TR_MAIN.Post();
    btnSaveClick     = false;
...
function btn_Conf() {
    ...
    TR_MAIN.Action   = "/mss/meis026.me?goTo=confirm" + parameters;
...
function getExcelPopup(){
    ...
    var returnVal = window.showModalDialog( "/mss/meis026.me?goTo=popup"+parameters
                                          , ""
...
function btn_addRow(){
    ...
    DS_ORG.NameValue(DS_ORG.CountRow, "BIZ_PLAN_YEAR") = DS_ACC.NameValue(DS_ACC.RowPosition, "BIZ_PLAN_YEAR");
...
function btn_delRow(){
    ...
    DS_ACC.NameValue(DS_ACC.RowPosition, "PLAN_AMT") = GD_ORG.SummaryString('PLAN_AMT', 1);
}
 ...
 */
 function getAccData(row, colid , popFlag){
    ...
    var strYear = DS_ACC.NameValue(row,"BIZ_PLAN_YEAR");
    ...
    if(popFlag != "1"){
    	DS_ACC.NameValue(row, "ACC_NM") = "";
        rtnMap = getAccNonPop("2", "DS_I_COMMON", "", strYear, strCd);
    }else{
        rtnMap = getAccPop("2", strYear, strCd);
    }  
...
function setOrgCodeToGrid(evnflag, row){
    ...
    if( rtnMap != null){
    	if(rtnMap.get("STR_CD") == strCd){
            ...
            //조직기타정보와 월별 정보 셋팅
            var goTo         = "searchOrgMon";
            
            var action       = "O";
            var parameters   = "&strStrCd=" + encodeURIComponent(DS_ORG.NameValue(row, "STR_CD")) //점코드
                             + "&strPlanY=" + encodeURIComponent(DS_ORG.NameValue(row, "BIZ_PLAN_YEAR")) //경영계획년도
            ...
            TR_MAIN.Action   = "/mss/meis026.me?goTo="+goTo+parameters;
            TR_MAIN.KeyValue = "SERVLET("+action+":DS_MONTH=DS_MONTH," + action+":DS_ETC=DS_ETC)"; //조회는 O
            TR_MAIN.Post();
...
<script language=JavaScript for=DS_MONTH event=onColumnChanged(row,colid)>
    DS_ORG.NameValue(DS_ORG.RowPosition, "PLAN_AMT") = GD_MONTH.SummaryString('PLAN_AMT', 1);
</script>
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
            ...
            <td width="160">
              <comment id="_NSID_">
                <object id=EM_PLAN_YEAR classid=<%=Util.CLSID_EMEDIT%>  width=80 tabindex=1 align="absmiddle"></object>
              </comment>
            ...
            <td>
              <comment id="_NSID_">
                <object id=LC_CONFIRM classid=<%= Util.CLSID_LUXECOMBO %> width=100 tabindex=1 align="absmiddle"></object>
              </comment><script>_ws_(_NSID_);</script>
            </td>
          </tr>
        </table></td>
                    ...
                    <td class="PT03"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr valign="top">
                        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
                          <tr>
                            <td>
                              <comment id="_NSID_"><object id=GD_MONTH width="100%" height=477 classid=<%=Util.CLSID_GRID%>>
