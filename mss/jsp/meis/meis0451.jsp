...
//	    chart1.renderControl();
	...
	//손익계획 월별추이 차트
	} else if(CHART_GB.equals("IFRAME_PROFIT_AND_LOSS_PLAN_CHART")) {
		...
		for(int i = 0; i < list.size(); i++) {
			...
			for(int j = 0; j < 12; j++) {
				...
				chart1.getData().set(i, j, Long.parseLong((String)tMap.get("plan_amt_" + month)));
		...
		//상위타이틀
		TitleDockable title = new TitleDockable(((Map)list.get(0)).get("org_name") + " 손익계획 월별추이");
		title.setFont(new Font("Malgun Gothic",Font.BOLD,12));
		title.setAlignment(StringAlignment.CENTER);
		chart1.getTitles().add(title);
	    ...
	    chart1.getDataGrid().setHeight((list.size())*17);
