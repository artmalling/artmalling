<!-- 
/*******************************************************************************
 * 시스템명 : 경영실적 > 경영계획> 월별조직별손익계획조회
 * 작 성 일 : 2011.06.26
 * 작 성 자 : 최재형
 * 수 정 자 : 
 * 파 일 명 : meis0450.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 경영계획의 월별조직별손익계획을 조회한다.
 * 이    력 :
 *        2011.06.26 (최재형) 신규작성
 ******************************************************************************/
-->
...
function gridCreate(){
	...
	gd_profitAndLossPlan += '<FC>id=BIZ_CD                 width=60  align=center name="대분류;항목코드"   SubBgColor=' + subSumBgColor + ' </FC>';
	subSumText 	= '{decode(curlevel,9999,"합계",1,"소계")}';
	value		= "{BIZ_CD_NM}";
	gd_profitAndLossPlan += '<FC>id=BIZ_CD_NM  value=' + value + '    SubSumText=' + subSumText + '             width=100 align=left   name="대분류;항목명"     SubBgColor=' + subSumBgColor + ' </FC>';
	gd_profitAndLossPlan += '<G>name="손익계획"';
	...
	value		= '{INCREASE_RATE}';	
	gd_profitAndLossPlan += '<C>id=INCREASE_RATE decao="2"  value=' + value + ' SubSumText=' + subSumText + ' width=80  align=right  name="증감율" SubBgColor=' + subSumBgColor + ' </C>';
	gd_profitAndLossPlan += '</G>';
				         
    initGridStyle(GD_PROFIT_AND_LOSS_PLAN, "common", gd_profitAndLossPlan, false);
    DS_PROFIT_AND_LOSS_PLAN.SubSumExpr  = "total";
    
    var gd_profitAndLossPlanExcelHeader = '<FC>id={currow}          width=25  align=center	name="NO" 		 SubBgColor=' + subSumBgColor + ' </FC>';
    gd_profitAndLossPlanExcelHeader += '<FC>id=BIZ_CD                 width=60  align=center name="대분류;항목코드"   SubBgColor=' + subSumBgColor + ' </FC>';
	subSumText 	= '{decode(curlevel,9999,"합계",1,"소계")}';
	value		= "{BIZ_CD_NM}";
    gd_profitAndLossPlanExcelHeader += '<FC>id=BIZ_CD_NM   value=' + value + '    SubSumText=' + subSumText + '            width=100 align=left   name="대분류;항목명"     SubBgColor=' + subSumBgColor + ' </FC>';
    
    gd_profitAndLossPlanExcelHeader += '<X>name="손익계획"';
	...
	subSumText 	= '{ DECODE(SUBSUM(PRE_RSLT_AMT),0,0, (SUBSUM(PLAN_AMT) - SUBSUM(PRE_RSLT_AMT))/SUBSUM(PRE_RSLT_AMT)*100 )  }';
	value		= '{INCREASE_RATE}';	
    gd_profitAndLossPlanExcelHeader += '<C>id=INCREASE_RATE decao="2" value=' + value + '    SubSumText=' + subSumText + ' width=80  align=right  name="증감율" SubBgColor=' + subSumBgColor + ' </C>';
    ...
    for(var i = 1; i <= 12; i++) {
    	...
    	value		= '{INCREASE_RATE_' + month + '}';	
        gd_profitAndLossPlanExcelHeader += '<C>id=INCREASE_RATE_' + month + ' decao="2" value=' + value + '    SubSumText=' + subSumText + ' width=80  align=right  name="증감율" SubBgColor=' + subSumBgColor + ' </C>';
        gd_profitAndLossPlanExcelHeader += '</G>';
    }
    ...
    //엑셀용
    initGridStyle(GD_PROFIT_AND_LOSS_PLAN_EXCEL, "common", gd_profitAndLossPlanExcelHeader, false);
    DS_PROFIT_AND_LOSS_PLAN_EXCEL.SubSumExpr  = "total";
}
...
function gridCreate_bak(){
	...
	gd_profitAndLossPlan += '<FC>id=BIZ_CD                 width=60  align=center name="대분류;항목코드"   BgColor={decode(GROUP_ID,2,"#BDDBEF",1,"#FFD0D0")} </FC>';
	gd_profitAndLossPlan += '<FC>id=BIZ_CD_NM               width=100 align=left   name="대분류;항목명"     BgColor={decode(GROUP_ID,2,"#BDDBEF",1,"#FFD0D0")} </FC>';
	gd_profitAndLossPlan += '<G>name="손익계획"';
	...
	gd_profitAndLossPlan += '<C>id=INCREASE_RATE width=80  align=right  name="증감율" BgColor={decode(GROUP_ID,2,"#BDDBEF",1,"#FFD0D0")} </C>';
	gd_profitAndLossPlan += '</G>';
    ...
    gd_profitAndLossPlanExcelHeader += '<FC>id=BIZ_CD                 width=60  align=center name="대분류;항목코드"   BgColor={decode(GROUP_ID,2,"#BDDBEF",1,"#FFD0D0")} </FC>';
    gd_profitAndLossPlanExcelHeader += '<FC>id=BIZ_CD_NM               width=100 align=left   name="대분류;항목명"     BgColor={decode(GROUP_ID,2,"#BDDBEF",1,"#FFD0D0")} </FC>';
    ...
    gd_profitAndLossPlanExcelHeader += '<C>id=INCREASE_RATE width=80  align=right  name="증감율" BgColor={decode(GROUP_ID,2,"#BDDBEF",1,"#FFD0D0")} </C>';
    gd_profitAndLossPlanExcelHeader += '</G>';
    ...
    for(var i = 1; i <= 12; i++) {
        ...
        gd_profitAndLossPlanExcelHeader += '<C>id=INCREASE_RATE_' + month + ' width=80  align=right  name="증감율" BgColor={decode(GROUP_ID,2,"#BDDBEF",1,"#FFD0D0")} </C>';
        gd_profitAndLossPlanExcelHeader += '</G>';
    }
...
function btn_Search() {
	...
	//dataset 초기화
	DS_PROFIT_AND_LOSS_PLAN.ClearData();
	...
	var goTo = "searchProfitAndLossPlan";
	var action     = "O";
	var parameters = "";
    ...
    DS_PROFIT_AND_LOSS_PLAN.DataID   = "/mss/meis045.me?goTo="+goTo+parameters;
    DS_PROFIT_AND_LOSS_PLAN.SyncLoad = "true";
    DS_PROFIT_AND_LOSS_PLAN.Reset();
...
function btn_Excel() {
	...
	if(!lo_searchFlag || DS_PROFIT_AND_LOSS_PLAN.CountRow == 0){
    ...
    DS_PROFIT_AND_LOSS_PLAN_EXCEL.DataID   = "/mss/meis045.me?goTo="+goTo+parameters;
    DS_PROFIT_AND_LOSS_PLAN_EXCEL.SyncLoad = "true";
    DS_PROFIT_AND_LOSS_PLAN_EXCEL.Reset();
}
...
function btn_ExcelDownload() {
	...
	var parameters  = "조직명="           	+ DS_PROFIT_AND_LOSS_PLAN_EXCEL.NameString(1,"ORG_NAME");
		parameters += " , 경영계획년도= "  	+ lo_PlanY;
	var ExcelTitle = "조직별손익계획";
	
	openExcel2(GD_PROFIT_AND_LOSS_PLAN_EXCEL, ExcelTitle, parameters, true );
...
function UpdateChart() {	
	...
	IFRAME_PROFIT_AND_LOSS_CHART.location.href = "/mss/meis045.me?goTo="+goTo+parameters+"&CHART_GB=" + CHART_GB;

	//손익계획 월별추이 차트 호출
	CHART_GB = "IFRAME_PROFIT_AND_LOSS_PLAN_CHART";
	IFRAME_PROFIT_AND_LOSS_PLAN_CHART.location.href = "/mss/meis045.me?goTo="+goTo+parameters+"&CHART_GB=" + CHART_GB;		
...
<!--------------------- 3. DataSet Success 메세지 처리  ----------------------->
<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN event=OnLoadCompleted(rowcnt)>
    if(!isNull(this.ErrorMsg)) 
        showMessage(INFORMATION, OK, "USER-1000", this.ErrorMsg);

	lo_searchFlag = true;
    setPorcCount("SELECT", rowcnt);
    GD_PROFIT_AND_LOSS_PLAN.Focus();
    UpdateChart();
</script>

<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN_EXCEL event=OnLoadCompleted(rowcnt)>
    if(!isNull(this.ErrorMsg)) 
        showMessage(INFORMATION, OK, "USER-1000", this.ErrorMsg);
...
<!--------------------- 4. DataSet Fail 메세지 처리  -------------------------->
<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN  event=OnLoadError()>
    if (this.ErrorCode == 0 ) showMessage(STOPSIGN, OK, "GAUCE-1010");
    else showMessage(STOPSIGN, OK, "GAUCE-1000", this.ErrorMsg);
</script>

<script language=JavaScript for=for=DS_PROFIT_AND_LOSS_PLAN_EXCEL  event=OnLoadError()>
    if (this.ErrorCode == 0 ) showMessage(STOPSIGN, OK, "GAUCE-1010");
    else showMessage(STOPSIGN, OK, "GAUCE-1000", this.ErrorMsg);
</script>

<!--------------------- 5. DataSet 이벤트 처리  ------------------------------->
<script language=JavaScript for=DS_PROFIT_AND_LOSS_PLAN event=OnRowPosChanged(row)>
//UpdateChart(false);
</script>
...
<script language=JavaScript for=GD_PROFIT_AND_LOSS_PLAN event=OnClick(row,colid)>
...
<comment id="_NSID_">
    ...
    <!--[그리드]매출계획분석 -->
    <object id="DS_PROFIT_AND_LOSS_PLAN"      classid=<%= Util.CLSID_DATASET %>></object>
    <!--[그리드]매출계획분석 엑셀 -->
    <object id="DS_PROFIT_AND_LOSS_PLAN_EXCEL"      classid=<%= Util.CLSID_DATASET %>></object>
			                    ...
			                    <td>
                                	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
                                    	<tr>
                                    		<td  class="BD4A">
			                                    <comment id="_NSID_">
			                                    <object id=GD_PROFIT_AND_LOSS_PLAN width="100%" height=180 classid=<%=Util.CLSID_GRID%>>
			                                        <param name="DataID" value="DS_PROFIT_AND_LOSS_PLAN">
        ...
        <td>
        	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
            	<tr>
                	<td width="100%">
                   		<!-- 손익계획월별추이 차트 -->
						<iframe id="IFRAME_PROFIT_AND_LOSS_PLAN_CHART" width="100%" height="295" style="overflow:hidden;" 
								src=""></iframe>
                   	</td>
               	</tr>
        	</table>
...
------------------------------------------------------------------------------>
<comment id="_NSID_">
  <object id=GD_PROFIT_AND_LOSS_PLAN_EXCEL width="0" height=0 classid=<%=Util.CLSID_GRID%>>
    <param name="DataID" value="DS_PROFIT_AND_LOSS_PLAN_EXCEL">
  </object>
</comment><script>_ws_(_NSID_);</script>
