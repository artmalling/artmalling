<!-- 
/*******************************************************************************
 * 시스템명 : 경영실적 > 경영계획> 경영계획 조회 및 확정
 * 작 성 일 : 2011.06.03
 * 작 성 자 : 이정식
 * 수 정 자 : 
 * 파 일 명 : meis0420.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 생성된 경영계획을 조회하고 확정한다.
 * 이    력 :
 *        2011.06.03 (이정식) 신규작성
 ******************************************************************************/
-->
...
function doInit(){
    ...
    registerUsingDataset("meis0420","DS_STORE" );
}
...
function gridCreate(){
	var hdrProperies = '<C>id={currow}   width=25  align=center name="NO"</C>'
                     ...
                     + '<C>id=STR_NM     width=75  align=left   name="점명" edit="None"</C>'
                     + '<C>id=CONF_YN    width=65  align=center name="확정여부" edit="None" EditStyle=Lookup data="DS_CONFIRM:CODE:NAME"</C>'
                     ;

    initGridStyle(GD_STORE, "common", hdrProperies, true);
    
    var hdrProperies1 = '<C>id={currow}   width=35  align=center name="NO"</C>'
                      + '<C>id=BIZ_CD     width=95  align=center name="항목코드" SumText="합계"</C>'
                      + '<C>id=BIZ_CD_NM  width=115 align=left   name="항목명"</C>'
                      + '<C>id=PLAN_AMT   width=148 align=right  name="계획금액" SumText=@sum</C>'
                      ;

    initGridStyle(GD_BIZ_CD, "common", hdrProperies1, false);
    GD_BIZ_CD.ViewSummary = "1";
    
    var hdrProperies2 = '<C>id={currow}   width=35 align=center name="NO"</C>'
    	              + '<C>id=ORG_FLAG   width=90 align=center name="조직구분" EditStyle=Lookup data="DS_ORG_FLAG:CODE:NAME"</C>'
                      + '<C>id=ORG_CD     width=90 align=center name="조직코드" SumText="합계"</C>'
                      + '<C>id=ORG_NAME   width=85 align=left   name="조직명"</C>'
                      + '<C>id=PLAN_AMT   width=93 align=right  name="계획금액" SumText=@sum</C>'
                      ;

    initGridStyle(GD_ORG, "common", hdrProperies2, false);
    GD_ORG.ViewSummary = "1";
    
    var hdrProperies3 = '<C>id=MONTH    width=40 align=center name="월"   SumText="합계"</C>'
                      + '<C>id=PLAN_AMT width=90 align=right  name="금액" SumText=@sum</C>'
                      ;
...
function btn_Search() {
    ...
    var parameters    = "&strStrCd="   + LC_STR_CD.BindColVal
                      + "&strYear="    + EM_PLAN_YEAR.text
                      + "&strConfirm=" + LC_CONFIRM.BindColVal;
    DS_STORE.DataID   = "/mss/meis042.me?goTo="+goTo+parameters;
...
function subQuery(row){
    ...
    var parameters     = "&strStrCd="   + DS_STORE.NameValue(row, "STR_CD")
                       + "&strYear="    + DS_STORE.NameValue(row, "BIZ_PLAN_YEAR")
                       + "&strConfirm=" + DS_STORE.NameValue(row, "CONF_YN");
    DS_BIZ_CD.DataID   = "/mss/meis042.me?goTo="+goTo+parameters;
...
function subQuery1(row){
    ...
    var parameters  = "&strStrCd="   + DS_BIZ_CD.NameValue(row, "STR_CD")
                    + "&strYear="    + DS_BIZ_CD.NameValue(row, "BIZ_PLAN_YEAR")
                    + "&strConfirm=" + DS_BIZ_CD.NameValue(row, "CONF_YN")
                    + "&strBizCd="   + DS_BIZ_CD.NameValue(row, "BIZ_CD");
    DS_ORG.DataID   = "/mss/meis042.me?goTo="+goTo+parameters;
...
function subQuery2(row){
    ...
    var parameters    = "&strStrCd="   + DS_ORG.NameValue(row, "STR_CD")
                      + "&strYear="    + DS_ORG.NameValue(row, "BIZ_PLAN_YEAR")
                      + "&strConfirm=" + DS_ORG.NameValue(row, "CONF_YN")
                      + "&strBizCd="   + DS_ORG.NameValue(row, "BIZ_CD")
                      + "&strOrgCd="   + DS_ORG.NameValue(row, "ORG_CD");
    DS_MONTH.DataID   = "/mss/meis042.me?goTo="+goTo+parameters;
...
function btn_Conf() {
    ...
    TR_MAIN.Action   = "/mss/meis042.me?goTo=save";  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr valign="top">
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr valign="top">
        <td width="223"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
        ...
        <td width="150" class="PL10"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td class="PT03"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr valign="top">
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
