...
<%    
    ...
    String strRsltYm  = String2.trimToEmpty(request.getParameter("strRsltYm"));
%>
<html>
<head>
<title>회계마감실적관리 EXCEL UPLOAD</title>
...
function doInit(){
    ...
    DS_EXCEL.setDataHeader('<gauce:dataset name="H_BIZ_UPLOAD_TEMP"/>');
    ...
    registerUsingDataset("meis0611","DS_ACC_RSLT" );
}
...
function gridCreate(){
	var hdrProperies = '<C>id={currow}   width=30  align=center name="NO"</C>'
                     ...
                     + '<C>id=RSLT_YM    width=60  align=center name="년월" edit="None" mask="XXXX/XX"</C>'
...
function btn_DeleteRow(){
    var intRowCount =  DS_ACC_RSLT.CountRow;
    for(var i=intRowCount; i >= 1; i--){
    	if(DS_ACC_RSLT.NameValue(i, "CHECK_FLAG") == 'T') DS_ACC_RSLT.DeleteRow(i);
    }
 ...
 */
function btn_Apply(){
	if (DS_ACC_RSLT.NameValueRow("CHECK_FLAG","T") == 0){
        //저장할 내용이 없습니다
        showMessage(INFORMATION, OK, "USER-1028");
        return;
    }
	
    for(var i = 1; i <= DS_ACC_RSLT.CountRow; i++){
    	if(DS_ACC_RSLT.NameValue( i, "CHECK_FLAG") == "T"){
    		if(DS_ACC_RSLT.NameValue( i, "ETC") != ""){
    			showMessage(INFORMATION, OK, "GAUCE-1000", "선택된 항목에 오류가 있습니다.");
    			setFocusGrid(GD_EXCEL,DS_ACC_RSLT, i, "ETC");
    			return;
    		}
    	}
    }
    
    TR_MAIN.Action   = "/mss/meis061.me?goTo=uploadExcel";  
    TR_MAIN.KeyValue = "SERVLET(I:DS_ACC_RSLT=DS_ACC_RSLT)"; 
    TR_MAIN.Post();
...
function loadExcelData() {
    ...
    DS_EXCEL.ClearData();
    DS_ACC_RSLT.ClearData();
    //Excel파일 DateSet에 저장               
    DS_EXCEL.Do("Excel.Application", strOption);
    DS_EXCEL.SortExpr = "+ACC_CD+ORG_CD";
    DS_EXCEL.Sort();
...
function checkValidation() {
    var parameters = "&strStrCd="  + encodeURIComponent("<%=strStrCd%>")
                   + "&strRsltYm=" + encodeURIComponent("<%=strRsltYm%>")
                   ;
    
	TR_MAIN.Action   = "/mss/meis061.me?goTo=checkValidation"+parameters;  
    TR_MAIN.KeyValue = "SERVLET(O:DS_ACC_RSLT=DS_ACC_RSLT,I:DS_EXCEL=DS_EXCEL)"; 
    TR_MAIN.Post();
}
-->
</script>
...
<script language="javascript"  for=GD_EXCEL event=OnHeadCheckClick(col,colid,bCheck)>
    ...
    if (bCheck == '1'){       // 전체체크
        for(var i=1; i<=DS_ACC_RSLT.CountRow; i++) DS_ACC_RSLT.NameString(i, colid) = 'T';
    }else{                    // 전체체크해제
        for(var i=1; i<=DS_ACC_RSLT.CountRow; i++) DS_ACC_RSLT.NameString(i, colid) = 'F';
    }
    this.ReDraw = "true";       
</script>
<!--*************************************************************************-->
...
<!-- ====================Output용================== -->
    ...
    <object id="DS_ACC_RSLT"    classid=<%= Util.CLSID_DATASET %>></object>
    <!--[그리드 콤보]계정/예산항목 구분코드 -->
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="" class="title">
              ...
              <SPAN id="title1" class="PL03">회계마감실적관리 엑셀 업로드 </SPAN>
            </td>
            <td align="right">
                    ...
                    <tr>
                       <th>파일선택</th>
                        ...
                        <td width="75" align="center">
                          <a href="/mss/samplefiles/ME_ACCRSLT_UPLOAD.xls">
            ...
            <td>
              ...
              <comment id="_NSID_">
                <OBJECT id=GD_EXCEL height="275px" width="100%" classid=<%=Util.CLSID_GRID%>>
                  <param name="DataID" value="DS_ACC_RSLT">
                </OBJECT>
              </comment><script>_ws_(_NSID_);</script>
            </td>
          </tr>  
