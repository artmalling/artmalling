<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 >  협력사EDI >  협력사EDI 커뮤니티 > 협력사 법률자문 Q&A 리스트 상세팝업
 * 작 성 일 : 2011.05.17
 * 작 성 자 : 김정민  
 * 수 정 자 : 
 * 파 일 명 : medi105ㄴ1.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 협력사의 법률자문  질문에 대한 내역을 조회 , 답변을 등록
 * 이    력 :
 *         2011.05.17 (김정민) 신규작성
 ******************************************************************************/
-->
...
function doInit(){
    ...
    DS_MASTER.setDataHeader('<gauce:dataset name="H_POPUPDETAIL"/>');//조직    
...
function getPopUpDatile(){
	...
	TR_MAIN.Action = "/mss/medi105.md?goTo=getPopupDetail&seq_no="+ encodeURIComponent(seq_no) + "&reg_dt=" + encodeURIComponent(reg_dt) +"&strcd=" + encodeURIComponent(strcd);
    TR_MAIN.KeyValue="SERVLET(O:DS_MASTER=DS_MASTER)"; //조회는 O
    TR_MAIN.Post();
    
    EM_TITLE.Text = DS_MASTER.NameValue(1, "TITLE");
    EM_I0_REGISTER.Text = DS_MASTER.NameValue(1, "PUMBUN_NM");
    TXT_CONTENT.Text = DS_MASTER.NameValue(1, "CONTEN");
    TXT_CONTENT_REPEL.Text = DS_MASTER.NameValue(1, "ANS_CONTENT");
}
...
function btn_Save(){ 
	 if( DS_MASTER.IsUpdated ){
         ...
         TR_MAIN.Action = "/mss/medi105.md?goTo="+ goTo +"&seq_no="+ encodeURIComponent(seq_no) + "&reg_dt=" + encodeURIComponent(reg_dt) 
