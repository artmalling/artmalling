...
public class MGif103DAO extends AbstractDAO {
	/**
	 * 협력사 목록 조회
	 * 
	 * @param form
	...
	public int save(ActionForm form, MultiInput[] mi, String userId)
		...
		try {
			...
			while(mi[1].next()){
				sql.close();
				i = 1;
				if(mi[1].IS_INSERT() || mi[1].IS_UPDATE()){ // 수수료 정보 저장 & 수정
					...
					sql.setString(i++, mi[1].getString("JGIFT_PAY_FEE_RATE")); 	// 제휴상품권 지급 수수료율
					sql.setString(i++, mi[1].getString("JCPN_PAY_FEE_RATE"));		// 쿠폰 지급 수수료율
					sql.setString(i++, mi[1].getString("JCPN_REC_FEE_RATE"));		// 쿠폰 수취 수수료율
					sql.setString(i++, userId); 								// 등록자
					res = update(sql);
					if (res != 1) {
						throw new Exception("[USER]"+"데이터의 적합성 문제로 인하여"
				...
				}else if(mi[1].IS_DELETE()){	// 수수료 정보저장 삭제
