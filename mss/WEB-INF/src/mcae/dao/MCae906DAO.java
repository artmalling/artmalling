 ...
 * @caused   by 
 */
public class MCae906DAO extends AbstractDAO {
	/**
	 * 행사 마스터 정보 조회
	 * 
	 * @param form
		...
		try {
			...
			while(mi.next()){
				...
				if (mi.IS_INSERT()) { // 저장
					...
					sql.setString(i++, mi.getString("CAL_YN"));
					sql.setString(i++, mi.getString("PAY_TYPE_CASH"));
					sql.setString(i++, mi.getString("PAY_TYPE_CARD"));
					sql.setString(i++, mi.getString("PAY_TYPE_GIFT"));
					sql.setString(i++, mi.getString("PB_APP_RATE"));
					sql.setString(i++, mi.getString("DBL_PAID_YN"));
					sql.setString(i++, mi.getString("RECP_DISP_YN"));
					sql.setString(i++, mi.getString("MEMB_ONLY_YN"));
