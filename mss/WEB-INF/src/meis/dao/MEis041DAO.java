...
public class MEis041DAO extends AbstractDAO {
	...
	public List searchBizPlan(ActionForm form) throws Exception {
		...
		int i              = 1;
		//파라미터 변수선언
		String strPlanYear = null;

		try {
			//파라미터 값 가져오기
			strPlanYear   = String2.trimToEmpty(form.getParam("strPlanYear"));
	...
	private ProcedureResultSet create(Service svc, MultiInput mi)
		...
		proc.put("MSS.PR_MEBIZPLANRPT", 4);

		proc.setString(++i, mi.getString("STR_CD"));
		proc.setString(++i, mi.getString("PLAN_YEAR"));
