...
public class MEis030DAO extends AbstractDAO {
	...
	public List searchBizPlan(ActionForm form) throws Exception {
		...
		try {
			...
			if("3".equals(strDivFlag)){
				sql.put(svc.getQuery("SEL_BIZ_PLAN_WHERE_PC"));
			} else {
				sql.put(svc.getQuery("SEL_BIZ_PLAN_WHERE_TEAM"));
			}
			
			sql.put(svc.getQuery("SEL_BIZ_PLAN_BOTTOM"));
	...
	private int updateBizPlan(Service svc, MultiInput mi, String strUserId) throws Exception {
		...
		sql.setString(++i, mi.getString("PLAN_AMT"));
