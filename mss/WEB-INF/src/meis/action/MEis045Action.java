 ...
 * @caused by
 */
public class MEis045Action extends DispatchAction {
	
	/*
	 * Java Pattern 에서 지원하는 logger를 사용할 수 있도록 객체를 선언
	 */
	private Logger logger = Logger.getLogger(MEis045Action.class);
	
	/** 
	 * <p>월별조직별손익계획조회 화면로딩</p>
	 */
	 ...
	 * <p>손익계획조회</p>
	 */
	public ActionForward searchProfitAndLossPlan(ActionMapping mapping, ActionForm form,
										HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			list    = dao.searchProfitAndLossPlan(form, false);
			helper.setDataSetHeader(dSet, "H_SEL_PROFIT_AND_LOSS_PLAN");
	...
	public ActionForward chartCall(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			dao     = new MEis045DAO();
			list    = dao.searchProfitAndLossPlan(form, true);
