...
public class MRen309DAO extends AbstractDAO {
	...
	public List getMaster(ActionForm form) throws Exception {
		...
		try {
			...
			String strVenCd 	= String2.nvl(form.getParam("strVenCd"));	// 거래처번호
			String strMntnitem 	= String2.nvl(form.getParam("strMntnitem"));// 관리비항목
			int i = 0;			
			
			strQuery += svc.getQuery("SEL_MR_ITEMAMT") + "\n";	
		...
		try {
			...
			while (mi.next()) {
				...
				if (mi.IS_INSERT()) {
					...
					sql.setString(++i, mi.getString("SUM_QTY"));
					sql.setString(++i, mi.getString("USE_AMT"));
					sql.setString(++i, mi.getString("VAT_AMT"));
					sql.setString(++i, mi.getString("USE_AMT") + mi.getString("VAT_AMT"));
					sql.setString(++i, userID);
