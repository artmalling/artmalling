...
public class MRen402DAO extends AbstractDAO {
	...
	public List getMaster(ActionForm form) throws Exception {
		...
		try {
			...
			String strTaxType 	= String2.nvl(form.getParam("strTaxType"));	// 구분
			int i = 0;
		...
		try {
			...
			while (mi.next()) {
				if (mi.getString("CHK_BOX").equals("T")) { //체크시에만 처리
					...
					psql.setString(i++, "W02"); /*V_WORK_FLAG*/
					psql.setString(i++, mi.getString("STR_CD")); /*V_STR_CD*/
					psql.setString(i++, mi.getString("CNTR_ID")); /*V_VEN_CD*/
					psql.setString(i++, mi.getString("CAL_TYPE")); /*V_CAL_TYPE*/
