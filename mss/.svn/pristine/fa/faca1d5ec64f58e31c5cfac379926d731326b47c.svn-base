<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 > 상품권관리 > 상품권 재고관리 > 상품권마스터 현황 조회
 * 작 성 일 : 2011.03.18
 * 작 성 자 : 신익수
 * 수 정 자 : 
 * 파 일 명 : mgif4100.jsp
...
function btn_Search() {
	if(LC_STR_CD.BindColVal.length == 0){ 
     ...
     TR_MAIN.Action="/mss/mgif410.mg?goTo="+goTo+parameters;  
...
function getDrawlDtl(strDrawStr,strDrawlYmdFrom,strDrawlYmdTo){
    ...
    TR_MAIN1.Action="/mss/mgif410.mg?goTo="+goTo+parameters;  
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				<td><comment id="_NSID_"><OBJECT id=GD_DETAIL
					width=100% height=420 classid=<%=Util.CLSID_GRID%>>
