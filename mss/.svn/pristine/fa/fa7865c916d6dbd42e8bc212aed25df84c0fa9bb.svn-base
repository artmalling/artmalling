...
function doInit(){
    ...
    registerUsingDataset("mgif203", "DS_IO_MASTER,DS_IO_DETAIL"); 
 }
...
function btn_Search() {
    ...
    TR_MAIN.Action="/mss/mgif203.mg?goTo="+goTo+parameters;  
...
function btn_Save() {
    ...
    TR_MAIN.Action="/mss/mgif203.mg?goTo=save"+parameters; 
 ...
 *************************************************************************/
 ...
 function getDetail () {
   ...
   var strStatFlag = DS_IO_MASTER.NameValue(row, "STAT_FLAG");
    ...
    TR_DETAIL.Action="/mss/mgif203.mg?goTo="+goTo+parameters;  
 ...
 function getConf (strFlag) {
    ...
    if(strFlag.id == "EM_GIFT_S_NO"){
        ...
        var parameters = "&strSGiftNo=" + encodeURIComponent(DS_IO_DETAIL.NameValue(DS_IO_DETAIL.CountRow, "GIFT_S_NO"))
                       ...
                       ;
        TR_MAIN.Action="/mss/mgif203.mg?goTo=getCnt"+parameters;  
...
<script language=JavaScript for=DS_S_STR event=OnFilter(row)>
    if (DS_S_STR.NameValue(row, "GBN") == "0") {// 본사점
        return true;
    }
    return false;
</script>
...
<script language=JavaScript for=DS_I_STR event=OnFilter(row)>
	if (DS_I_STR.NameValue(row, "GBN") == "0") {// 본사점
	    return true;
	}
	return false;
</script>
