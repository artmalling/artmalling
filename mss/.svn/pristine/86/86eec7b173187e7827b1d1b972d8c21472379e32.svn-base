<!-- 
/*******************************************************************************
 * 시스템명 : 영업지원 > 임대관리 > 관리비정산관리 > 임대회계전표생성
 * 작 성 일 : 2010.05.11
 * 작 성 자 : 김경은
 * 수 정 자 : 
 * 파 일 명 : mren701.jsp
 ...
 */
 ...
 function doInit(){
     ...
     initEmEdit(EM_YYYYMM,       "THISMN",   NORMAL);              // 매입/매출년월
 ...
 *************************************************************************/
  ...
  */
  ...
  function CloseCheck(){
     ...
     TR_VALCHECK.Action="/mss/mren701.mr?goTo=valCheck"; 
 ...
 function process(){
	  ...
	  //변경또는 신규 내용을 저장하시겠습니까?
        if( showMessage(QUESTION, YESNO, "USER-1204") == 1 ){    // validation 체크
            TR_MAIN.Action="/mss/mren701.mr?goTo=save&strFlag=save"; 
...
<script language=JavaScript for=DS_IO_SEND_FLAG event=OnFilter(row)>
    if (DS_IO_SEND_FLAG.NameValue(row,"REFER_CODE") == "2"){
    	return true;
    }
	else 
		return false;
