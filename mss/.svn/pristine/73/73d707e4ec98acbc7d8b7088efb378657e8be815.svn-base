 ...
 * @caused by
 */
public class MEis046Action extends DispatchAction {

	/*
	 * Java Pattern 에서 지원하는 logger를 사용할 수 있도록 객체를 선언
	 */
	private Logger logger = Logger.getLogger(MEis046Action.class);
	
	/** 
	 * <p>월별조직별판관비계획조회 화면로딩</p>
	 */
	 ...
	 * <p>판관비계획조회</p>
	 */
	public ActionForward searchSaleAndManagePlan(ActionMapping mapping, ActionForm form,
										HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			list    = dao.searchSaleAndManagePlan(form, false);
			helper.setDataSetHeader(dSet, "H_SEL_SALE_AND_MANAGE_PLAN");
	...
	public ActionForward chartCall(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if(CHART_GB.equals("IFRAME_SALE_AND_MANAGE_CHART")) {
				dao     = new MEis046DAO();
				list    = dao.searchSaleAndManagePlan(form, true);
