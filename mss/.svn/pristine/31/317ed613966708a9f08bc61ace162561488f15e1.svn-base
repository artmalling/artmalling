...
try {
	...
	if(CHART_GB.equals("IFRAME_SALES_AMT_CHART")) {
		...
		chart1.getData().set(0, 1, Long.parseLong(PRE_SALES_AMT));	//실적 매출액
		...
		chart1.getAxisX().getLabels().set(1,"전년\n실적");
...
//		chart1.renderControl();
	...
	//매출이익 
		...
		chart1.getData().set(0, 1, Long.parseLong(PRE_SALES_PROFIT_AMT));	//실적 매출이익액
		chart1.getAxisX().getLabels().set(0,"이익계획");
		chart1.getAxisX().getLabels().set(1,"전년실적");
...
//		chart1.renderControl();
	...
	//매출계획
	} else if(CHART_GB.equals("IFRAME_TAB1_SALES_PLAN_CHART")) {
	    ...
	    chart1.getSeries().get(0).setText("매출액");
	    chart1.getSeries().get(1).setText("전년매출액");

	    
		//시리즈, 포인트 , 값
		for(int i = 0; i < 2; i++) {
			String key = "PLAN_SALES_AMT_";
			if(i == 1) key = "PRE_SALES_AMT_";
			for(int j = 0; j < 12; j++) {
		...
		chart1.getSeries().get(0).setColor(new Color(57, 105, 156, 200));		
        ...
        chart1.getDataGrid().setFont(new java.awt.Font("Malgun Gothic",java.awt.Font.PLAIN,10));
...
//        chart1.renderControl();
	...
	//매출이익계획
	} else if(CHART_GB.equals("IFRAME_TAB2_SALES_PROFIT_PLAN_CHART")) {
	    ...
	    //chart1.getSeries().get(0).setText(BIZ_PLAN_YEAR + " 계획");
	    //chart1.getSeries().get(1).setText(PRE_RSLT_YEAR + " 실적");
	    chart1.getSeries().get(0).setText("매출이익액");
	    chart1.getSeries().get(1).setText("전년매출이익");	    
		
		//시리즈, 포인트 , 값
		for(int i = 0; i < 2; i++) {
			String key = "PLAN_SALES_PROFIT_AMT_";
			if(i == 1) key = "PRE_SALES_PROFIT_AMT_";
			for(int j = 0; j < 12; j++) {
				String month = j < 9 ? "0" + (j+1) : (j+1) + "";
				chart1.getData().set(i, j, Long.parseLong(request.getParameter(key + month)));
				if(i == 1) chart1.getAxisX().getLabels().set(j, month +"월");
