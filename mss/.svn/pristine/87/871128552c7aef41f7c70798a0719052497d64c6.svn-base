...
public class MCae429DAO extends AbstractDAO {
	...
	public List[] getSaleInfo(ActionForm form) throws Exception {
		...
		String strCardNOCK = String2.nvl(form.getParam("strCardNOCK"));
		String strMCARD = String2.nvl(form.getParam("strMCARD"));
		String strCARDAPP = String2.nvl(form.getParam("strCARDAPP"));
		String strEM_AMT = String2.nvl(form.getParam("strEM_AMT"));
		...
		sql.setString(i++, strCardNOCK);
		sql.setString(i++, strCARDAPP);
		sql.setString(i++, strEM_AMT);
	...
	public int save(ActionForm form, MultiInput[] mi, String userId)
		...
		try {
			...
			sql.setString(i++, String2.nvl(form.getParam("strMEMO"))); 		// 사유
			sql.setString(i++, userId); 										// ID
			sql.setString(i++, String2.nvl(form.getParam("strCHAR_RNM"))); 	// 실등록자
			sql.setString(i++, String2.nvl(form.getParam("strPrsntDt")));		// 지급일
			sql.setString(i++, mi[0].getString("STR_CD"));						// 점코드
			sql.setString(i++, mi[0].getString("EVENT_CD"));					// 행사코드
			sql.setString(i++, String2.nvl(form.getParam("strCardCustId"))); 	// 카드회원번호
			...
			while (mi[1].next()) {
				if (mi[1].getString("CHK").equals("T")) {
					...
					sql.setString(i++, mi[0].getString("STR_CD")); 				// 점코드
					sql.setString(i++, strPrsntNo); 							// 지급번호
					sql.setString(i++, mi[1].getString("PROVIDE")); 			// 매출일자
					sql.setString(i++, mi[1].getString("CARD_NOCK")); 				// 포스번호
					sql.setString(i++, mi[1].getString("CARD_APPNO"));				// 거래번호
					sql.setString(i++, mi[1].getString("TOT_SALE_AMT"));			// 인정매출금액
					sql.setString(i++, ""); 			// 지분안분금액
