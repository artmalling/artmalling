<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 사은행사관리 > 경품행사 > 경품행사 응모자 등록 
 * 작 성 일 : 2011.01.24
 * 작 성 자 : 김유완
 * 수 정 자 : 
 * 파 일 명 : mcae2020.jsp
...
function doInit() {
    ...
    registerUsingDataset("mcae202","DS_IO_DETAIL" );
}
...
function gridCreate(gdGnb) {
    ...
    //디테일그리드
        ...
        var hdrProperies = ''
            + '<FC>ID={CURROW}      NAME=NO'        
            + '                                     WIDTH=30    ALIGN=CENTER</FC>'
            + '<FC>ID=ENTY_NAME     NAME=*응모자명'       
            + '                                     WIDTH=80    ALIGN=LEFT      LANGUAGE=1</FC>'
            + '<FC>ID=SS_NO         NAME=생년월일'       
...
function btn_Search() {
    ...
    TR_MAIN.Action = "/mss/mcae202.mc?goTo=" + goTo + parameters;
...
function btn_Save() {
    ...
    if (DS_IO_DETAIL.IsUpdated) {//변경 데이터가 있을때만 저장
        ...
        if (!checkDSBlank( GD_DETAIL, "1,3,4,5"))  return;
        //주소 문자열체크
...
<!-- Grid detail event 처리 -->    
<script language=javascript for=GD_DETAIL event=OnKeyPress(keycode)>  
if(keycode == 255 && GD_DETAIL.GetColumn() =="EMAIL"){
	if(DS_IO_DETAIL.CountRow == DS_IO_DETAIL.RowPosition){
		//자동행추가 처리 
		btn_AddRow();
	}
} 
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
            <tr>
                <td><comment id="_NSID_"><object id=GD_DETAIL
                    width="100%" height=460 classid=<%=Util.CLSID_GRID%>>
