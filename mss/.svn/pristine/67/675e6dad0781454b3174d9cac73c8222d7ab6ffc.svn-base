...
function doInit(){
 ...
 //   EM_I_SALE_DT.Text    = getTodayFormat("YYYYMMDD");   // 매출일자
    ...
    initEmEdit(EM_I_GIFT_NO,        "GEN^18", NORMAL);   // 상품권번호
 ...
 // initEmEdit(EM_S_GIFT_NO,        "GEN^18", NORMAL);   // 시작번호
    initEmEdit(EM_S_GIFT_NO,        "GEN^18", READ);     // 시작번호
 // initEmEdit(EM_E_GIFT_NO,        "GEN^18", NORMAL);   // 종료번호
  ...
  //점(조회) 
    ...
    registerUsingDataset("mgif324", "DS_O_MASTER,DS_O_DETAIL");
    
    //showMessage(StopSign, OK, "USER-1000", "이 화면은 POS결제 전 판매등록된 매출에 대한 취소를 하는 화면입니다. 결제건 취소는 상품권 판매등록 메뉴에서 진행하세요.");
...
function gridCreate1(){ 
    var hdrProperies = '<FC>id={currow}         name="NO"           width=25   align=center</FC>'
                     ...
                     + '<FC>id=STAT_FLAG       name="상태정보"         width=70   align=left  Edit=none Show=false </FC>'
                     + '<FC>id=STAT_FLAG_NM       name="상태정보"         width=70   align=center  Edit=none  </FC>';
...
function btn_Search() {
    ...
    // var strStrCd      = LC_S_STR_CD.BindColVal;    // 점코드
     ...
     var strPosSale    = "";   									//EM_I_GIFT_NO.Text;        //판매상품권번호
     ...
     TR_MAIN.Action   = "/mss/mgif324.mg?goTo=" + goTo + parameters;
     ...
     for(var i=1; i < DS_O_MASTER.CountRow; i++) {
         if (DS_O_MASTER.NameValue(i,"STAT_FLAG") != "06") {
        	 showMessage(StopSign, OK, "USER-1000", DS_O_MASTER.NameValue(i,"GIFTCARD_NO") + " 는 반품 불가 상품권 입니다." +i);
...
function btn_Save() {  
	...
	for(var i=1; i <= DS_O_MASTER.CountRow; i++) { 
        if (DS_O_MASTER.NameValue(i,"STAT_FLAG") != "06") {
			showMessage(StopSign, OK, "USER-1000", i + "번째행의 [" + DS_O_MASTER.NameValue(i,"GIFTCARD_NO") + "]는 반품불가 상품권 입니다.");
// 			DS_O_MASTER.ClearData();
// 			DS_O_DETAIL.ClearData();
// 			LC_S_STR_CD.Enable = true;
// 			EM_I_SALE_SLIP_NO2.Text = "";
...
// 			LC_S_STR_CD.Focus();
  ...
  //변경또는 신규 내용을 저장하시겠습니까?
    ...
    var strSaleFlag = DS_O_RESULT.NameValue(1,"SALE_FLAG");
 ...
 *************************************************************************/
 ...
 function getSaleSlip() {
	 ...
	 if(EM_I_GIFT_NO.Text == "" && EM_I_SALE_SLIP_NO.Text == "") {
		 showMessage(INFORMATION, OK, "USER-1000", "상품권번호 또는 원거래 상품권 매출번호를 입력하셔야 합니다.");
         return;
	 }
	 ...
	 if(DS_O_RESULT.CountRow == "0") {
		 ...
		 if(DS_O_RESULT.NameValue(DS_O_RESULT.RowPosition,"CANCEL_GB") == "1") {
             ...
             EM_I_SALE_SLIP_NO2.Text = "";
             EM_I_SALE_DT.Text = ""; 
			 setTimeout("EM_I_GIFT_NO.Focus()",50); 
	         return;
		 }
		 else {
			 // 원거래 조회시 점코드 수정불가 
  ...
  * return값 : void
  */
function getSALE_SLIP() { 
	  var strGiftCardNo = EM_I_GIFT_NO.Text;
	  
	  var goTo       = "getSALE_SLIP";
	  var action     = "O";
	...
	//rt(DS_SELECT.CountRow);
	  ...
	  if(DS_SELECT.CountRow == "0") {
		  ...
		  if(DS_SELECT.NameValue(DS_SELECT.RowPosition,"CANCEL_GB") == "1") {
			  showMessage(INFORMATION, OK, "USER-1000", "이미 반품된 영수증의 상품권번호 입니다.");
// 			  EM_I_GIFT_NO.Text = "";
// 	          setTimeout("EM_I_GIFT_NO.Focus();", 50); 
	          return;
			  ...
			  EM_I_SALE_SLIP_NO.Text = DS_SELECT.NameValue(1,"SALE_SLIP");
		  }
...
<script language=JavaScript for=EM_S_GIFT_NO event=onKillFocus()>
    if(EM_S_GIFT_NO.Text.length > "0") {
    	...
    	if(EM_S_GIFT_NO.Text.length != 13 && EM_S_GIFT_NO.Text.length != 16 && EM_S_GIFT_NO.Text.length != 18) {
    		showMessage(INFORMATION, OK, "USER-1000", "13,16,18자리로 입력 하셔야 합니다.");
            EM_S_GIFT_NO.Text = "";
            setTimeout("EM_S_GIFT_NO.Focus()",50); 
            return;
    	} 
...
<script language=JavaScript for=EM_I_GIFT_NO event=onKillFocus()>

    if (EM_I_GIFT_NO.Text.length == 12 ){
        getSALE_SLIP(); 
    	return;
    } else {
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					class="s_table">
					<tr>
						<th width="120">판매상품권번호</th>
						<td width="230"><comment id="_NSID_"> <object
							id=EM_I_GIFT_NO classid=<%=Util.CLSID_EMEDIT%> width=230
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr> 
						...
						<td valign="top" style="display:none"><!-- 반품내역 -->
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
