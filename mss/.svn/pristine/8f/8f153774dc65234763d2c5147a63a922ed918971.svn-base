<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 > 임대관리 > CESBILLING > 시설구분별관리기준
 * 작 성 일 : 2010.05.31
 * 작 성 자 : 김유완
 * 수 정 자 : 
 * 파 일 명 : MREN6010.jsp
...
function doInit(){
    ...
    getFlc("DS_LC_FCL_FLAG", "C", "2", "Y", "N", "1");      // [조회용]시설구분(주거)
...
function btn_Search() {
    if (DS_IO_MASTER.IsUpdated) {//변경데이터 있을 시 확인
        ...
        if (ret == "1") {
            ...
            TR_MAIN.Action = "/mss/mren601.mr?goTo=" + goTo + parameters;
...
function btn_Save() {
    if (DS_IO_MASTER.NameValue(1, "INSGB") == "Y") {
        if (DS_IO_MASTER.IsUpdated) {//변경 데이터가 있을때만 저장
            ...
            //저장
            var goTo = "save";
            TR_SAVE.Action = "/mss/mren601.mr?goTo=" + goTo;
...
function checkValidate() {
    ...
    /*} else if (DS_IO_MASTER.NameValue(1, "HUSE_FLAG")== "") {
        ...
        return false;
    } else if (DS_IO_MASTER.NameValue(1, "MNTN_CAL_YN")== "") {
    	LC_MNTN_CAL_YN.Focus();
        showMessage(INFORMATION, OK, "USER-1002", "관리비계산여부");
        return false;
    } else if (DS_IO_MASTER.NameValue(1, "CHRG_YN")== "") {
    	LC_CHRG_YN.Focus();
        showMessage(INFORMATION, OK, "USER-1002", "관리비청구내역관리여부");
        return false;
    } else if (DS_IO_MASTER.NameValue(1, "ARR_CAL_YN")== "") {
    	LC_ARR_CAL_YN.Focus();
        showMessage(INFORMATION, OK, "USER-1002", "관리비연체계산여부");
        return false;
    } else if (DS_IO_MASTER.NameValue(1, "PAY_TERM_DD") >  "32") {
    	EM_PAY_TERM_DD.Focus();
        showMessage(INFORMATION, OK, "USER-1000", "[관리비납부기한일자]가 날자형식이 아닙니다.");
        return false;
    }
...
if (DS_IO_MASTER.CountRow == 1) {
	//관리비계산여부
    objectControl(true);
    if (DS_IO_MASTER.NameValue(1, "MNTN_CAL_YN") == "Y") {
        LC_CHRG_YN.Enable      = true; // 관리비청구내역관리여부
    } else {
        LC_CHRG_YN.Enable      = false;// 관리비청구내역관리여부
        LC_ARR_CAL_YN.Enable   = false;// 관리비연체계산여부
        EM_ARR_RATE.Enable     = false;// 연체율
    }
    //관리비청구내역관리여부
    if (DS_IO_MASTER.NameValue(1, "CHRG_YN") == "Y") {
        LC_ARR_CAL_YN.Enable   = true; // 관리비연체계산여부
    } else {
        LC_ARR_CAL_YN.Enable   = false;// 관리비연체계산여부
        EM_ARR_RATE.Enable     = false;// 연체율
    }
    // 관리비연체계산여부
    if (DS_IO_MASTER.NameValue(1, "ARR_CAL_YN") == "Y") {
        EM_ARR_RATE.Enable     = true; // 연체율
    } else {
        EM_ARR_RATE.Enable     = false;// 연체율
    }
}
...
if (DS_IO_MASTER.CountRow == 1) {
    if (colid == "STR_CD") {
        ...
        if (DS_IO_MASTER.NameValue(1, "MNTN_CAL_YN") == "Y") {
            ...
            LC_ARR_CAL_YN.Enable   = false;// 관리비연체계산여부
            EM_ARR_RATE.Enable     = false;// 연체율
            DS_IO_MASTER.NameValue(1, "CHRG_YN")   = "N";
            DS_IO_MASTER.NameValue(1, "ARR_CAL_YN")= "N";
            DS_IO_MASTER.NameValue(1, "ARR_RATE")  = "0"; 
        }
        g_RowStatus = 3;
    } else  if (colid == "CHRG_YN") {
        //관리비청구내역관리여부
        if (DS_IO_MASTER.NameValue(1, "CHRG_YN") == "Y") {
            LC_ARR_CAL_YN.Enable   = true; // 관리비연체계산여부
        } else {
...
<script language=JavaScript for=EM_PAY_TERM_DD event=OnKillFocus()>
//관리비납부기한일자
var tmpTremdd = EM_PAY_TERM_DD.Text;
if (tmpTremdd.length == 1) {
	EM_PAY_TERM_DD.Text = "0" + tmpTremdd;
}
</script>

<script language=JavaScript for=EM_PAY_TERM_DD event=onKeyUp(kcode,scode)>
var tmpTremdd = EM_PAY_TERM_DD.Text;
if (tmpTremdd.length == 2) {
    if (tmpTremdd > 32) {
    	showMessage(INFORMATION, OK, "USER-1000", "입력하신 "+tmpTremdd+"일은 날짜 형식이 아닙니다.");
    	EM_PAY_TERM_DD.Text = "31";
    } 
}
</script>

<script language=JavaScript for=EM_ARR_RATE event=OnKillFocus()>
//단가의 음수제외
if (EM_ARR_RATE.Text=="-"||EM_ARR_RATE.Text=="-0") 
	EM_ARR_RATE.Text = 0;
	return;
	
if (eval(EM_ARR_RATE.Text) < 0 ) 
	EM_ARR_RATE.Text = Math.abs(EM_ARR_RATE.Text);
</script>
			...
			<tr valign="top">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						...
						<th>연체율(%)</th>
						<td ><comment id="_NSID_"> <object
							id=EM_ARR_RATE classid=<%=Util.CLSID_EMEDIT%> width="135"
							tabindex=1> </object> </comment> <script> _ws_(_NSID_);</script></td>
					</tr>
					<tr>
						<th>관리비납부기한일자</th>
						<td colspan="3"><comment id="_NSID_"> <object id=LC_PAY_TERM_TYPE
							classid=<%= Util.CLSID_LUXECOMBO %> width="80" height=100 tabindex=1
