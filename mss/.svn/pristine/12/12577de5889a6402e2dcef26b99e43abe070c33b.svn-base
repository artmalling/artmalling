 ...
 * return값	: void
 */
 var top1 = 105;		//해당화면의 동적그리드top위치
 var top2 = 280;		//해당화면의 동적그리드top위치
 var top3 = 590;		//해당화면의 동적그리드top위치
	function doInit(){
		...
		initEmEdit(EM_S_GIFTCARD_NO, "GEN^18", NORMAL);											//시작 번호
		initEmEdit(EM_E_GIFTCARD_NO, "GEN^18", NORMAL);											//종료번호
	...
	function gridCreate4(){	
 	...
 	function gridCreate5(){
		//영수증정보
		var	hdrProperies = ''
			+ '<FC>ID={CURROW}			NAME="NO1"			WIDTH=30	ALIGN=CENTER	</FC>'
			+ '<FC>ID=SEQ_NO			NAME="NO2"			WIDTH=30	ALIGN=CENTER	</FC>'
			+ '<FC>ID=PUMBUN_CD			NAME="브랜드코드"	WIDTH=80	ALIGN=LEFT		</FC>'
			+ '<FC>ID=RECP_NAME			NAME="브랜드명"		WIDTH=100	ALIGN=LEFT		</FC>'
			+ '<FC>ID=SALE_AMT			NAME="판매금액"		WIDTH=80	ALIGN=CENTER	</FC>'
			+ '<FC>ID=STR_CD			NAME="점"			WIDTH=0		ALIGN=CENTER	SHOW=FALSE</FC>'
			+ '<FC>ID=SALE_DT			NAME="판매일자"		WIDTH=0		ALIGN=LEFT		SHOW=FALSE</FC>'
			+ '<FC>ID=POS_NO			NAME="회수POS"		WIDTH=0		ALIGN=CENTER	SHOW=FALSE</FC>'
			+ '<FC>ID=TRAN_NO			NAME="거래번호"		WIDTH=0		ALIGN=CENTER	SHOW=FALSE</FC>'
			;
...
function btn_Search() {
	...
	TR_MAIN.Action="/mss/mgif408.mg?goTo="+goTo+parameters;	 
 ...
 *************************************************************************/
 ...
 function getGiftTypeCd	() {
	 TR_MAIN.Action	   = "/mss/mgif408.mg?goTo=getGiftTypeCd";
...
function getGiftMst(strGiftCardNo) {
	...
	TR_MAIN1.Action="/mss/mgif408.mg?goTo="+goTo+parameters;  
...
function getSaleMst(strGiftCardNo) {
	...
	TR_MAIN2.Action="/mss/mgif408.mg?goTo="+goTo+parameters;  
...
function getGiftDrawl(strGiftCardNo){
	...
	TR_MAIN3.Action="/mss/mgif408.mg?goTo="+goTo+parameters;  
...
function getGiftReuse(strGiftCardNo){
	...
	TR_MAIN4.Action="/mss/mgif408.mg?goTo="+goTo+parameters;  
...
function getReceipt(strGiftCardNo){
	...
	TR_MAIN5.Action="/mss/mgif408.mg?goTo="+goTo+parameters;  
  ...
  *	return값 :
  */
 function receiptPopUp(strStrCd,strSaleDt,strPosNo,strTranNo){
	    var rtnMap  = new Map();
	    var arrArg  = new Array();

	    arrArg.push(rtnMap);
		arrArg.push(strStrCd);
		arrArg.push(strSaleDt);
		arrArg.push(strPosNo);
		arrArg.push(strTranNo);

		var returnVal = window.showModalDialog("/mss/mgif408.mg?goTo=receiptPopUp",
						arrArg,
			            "dialogWidth:372px;dialogHeight:512px;scroll:no;" +
...
<!-- 그리드 더블클릭시	-->
<script language=JavaScript for=GD_MASTER5 event=OnDblClick(row,colid)>
var strStrCd  = DS_O_MASTER5.NameValue(row,"STR_CD");
var strSaleDt = DS_O_MASTER5.NameValue(row,"SALE_DT");
var strPosNo  = DS_O_MASTER5.NameValue(row,"POS_NO");
var strTranNo = DS_O_MASTER5.NameValue(row,"TRAN_NO");

receiptPopUp(strStrCd,strSaleDt,strPosNo,strTranNo);
</script>
<!--*************************************************************************-->
<!--* C. 가우스	이벤트 처리	끝																									   *-->
<!--*************************************************************************-->
...
<object	id="TR_MAIN5" classid=<%=Util.CLSID_TRANSACTION%>>
  <param name="KeyName"	value="Toinb_dataid5">
</object>
</comment><script>_ws_(_NSID_);</script>
<!--*************************************************************************-->
...
window.onresize = function(){
    ...
    if((parseInt(window.document.body.clientHeight)) <= top1) {
    	grd_height = top1;    	
    } else {
    ...
    if((parseInt(window.document.body.clientHeight)) <= top3) {
    	grd_height = top3;    	
    } else {
		  ...
		  <td width="220"><table width="100%" border="0" cellspacing="0" cellpadding="0"
			  ...
			  <tr>
				<td	width="100%"><comment id="_NSID_">
					<OBJECT
							id=GD_MASTER width=300	height=770 classid=<%=Util.CLSID_GRID%>>
						...
						<td width="400">
							<table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
								<tr>
									<td	width="100%">
										<comment id="_NSID_">
											<object	id="GD_MASTER2"	width="100%" height="280" classid="<%=Util.CLSID_GRID%>">
							...
							<table width="100%"	border="0" cellspacing="0" cellpadding="0" class="BD4A">
								<tr>
									<td	width="100%" >
										<comment id="_NSID_">
											<object	id="GD_MASTER5"	width="100%" height="285" classid="<%=Util.CLSID_GRID%>">
						...
						<td	class="PL05">
							<table width="100%"	border="0" cellspacing="0" cellpadding="0" class="BD4A">
								<tr>
									<td	width="100%" >
										<comment id="_NSID_">
											<object	id="GD_MASTER3"	width="100%" height="600" classid="<%=Util.CLSID_GRID%>">
							...
							<table width="100%"	border="0" cellspacing="0" cellpadding="0" class="BD4A"  style=display:none>
							  <tr>
								<td	width="100%" >
								<comment id="_NSID_">
