...
try {
	...
	if(CHART_GB.equals("IFRAME_SALE_AND_MANAGE_CHART")) {
		...
		for(int i = 0; i < list.size(); i++) {
			...
			chart1.getData().set(1, i, Long.parseLong((String)tMap.get("pre_rslt_amt")));
...
//	    chart1.renderControl();
	...
	//손익계획 월별추이 차트
	} else if(CHART_GB.equals("IFRAME_SALE_AND_MANAGE_PLAN_CHART")) {
		...
		for(int i = 0; i < 12; i++) {
			...
			chart1.getData().set(1, i, form.getLong(1,"PRE_RSLT_AMT_" + iMon));
		...
		//상위타이틀
		TitleDockable title = new TitleDockable(form.getParam("ORG_NAME") + " " + form.getParam("BIZ_CD_NM") + " 판관비계획 월별추이");
		title.setFont(new Font("Malgun Gothic",Font.BOLD,12));
		title.setAlignment(StringAlignment.CENTER);
		chart1.getTitles().add(title);
		...
		*/
	    ...
	    chart1.getDataGrid().setFont(new java.awt.Font("Malgun Gothic",java.awt.Font.PLAIN,11));
