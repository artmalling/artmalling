<!-- 
/*******************************************************************************
 * 시스템명 : 경영실적 > 기초자료> 경영계획2차배부처리
 * 작 성 일 : 2011.06.02
 * 작 성 자 : 이정식
 * 수 정 자 : 
 * 파 일 명 : meis0300.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 경영계획 본사 공통비  1차 배부내역 및 점 공통비용에 대해 2차 배부 처리한다.
 * 이    력 :
 *        2011.06.02 (이정식) 신규작성
 ******************************************************************************/
-->
...
function doInit(){
    ...
    registerUsingDataset("meis0300","DS_BIZ_PLAN" );
}
...
function setCombo(){
    ...
    getDivCombo(DS_DIV_CD, EM_PLAN_YM.Text, "2", "1", "N", lo_strCd, lvl); //배부기준콤보 데이터가져오기 ( gauce.js )
}
...
function gridCreate(){
	var hdrProperies = '<FC>id={currow}     width=35  align=center name="NO"</FC>'
                     ...
                     + '<C>id=SD_DIV_CD     width=80  align=left   name="*배부기준" edit={IF(EDIT_FLAG="1","true","false")} EditStyle=Lookup data="DS_DIV_CD:CODE:NAME"</C>'
...
function btn_Search() {
    ...
    DS_BIZ_PLAN.DataID   = "/mss/meis030.me?goTo="+goTo+parameters;
...
function btn_Save() {
    ...
    for(var i = 1; i <= DS_BIZ_PLAN.CountRow; i++){
        if(DS_BIZ_PLAN.NameValue( i, "CHECK_FLAG") == "T"){
            if(isNull(DS_BIZ_PLAN.NameValue( i, "SD_DIV_CD"))){
            	showMessage(EXCLAMATION, Ok,  "USER-1002", "배부기준코드");
                setFocusGrid(GD_BIZ_PLAN,DS_BIZ_PLAN, i, "SD_DIV_CD");
                return;
            }
            
            if(isNull(DS_BIZ_PLAN.NameValue( i, "SD_DIV_BIZ_CD"))){
                showMessage(EXCLAMATION, Ok,  "USER-1002", "배부항목");
                setFocusGrid(GD_BIZ_PLAN,DS_BIZ_PLAN, i, "SD_DIV_BIZ_CD");
                return;
            }
        }
    }
    ...
    TR_MAIN.Action   = "/mss/meis030.me?goTo=save";  
...
function getBizData(row, colid , popFlag){
    var strCd = DS_BIZ_PLAN.NameValue(row,"SD_DIV_BIZ_CD");
    ...
    if( rtnMap != null){
        DS_BIZ_PLAN.NameValue(row,"SD_DIV_BIZ_CD") = rtnMap[0];
        DS_BIZ_PLAN.NameValue(row,"BIZ_CD_NM") = rtnMap[1];
    }else{
        if(DS_BIZ_PLAN.NameValue(row, "BIZ_CD_NM") == "") DS_BIZ_PLAN.NameValue(row, "SD_DIV_BIZ_CD") = "";
    }
    
    if(DS_BIZ_PLAN.NameValue(row, "SD_DIV_BIZ_CD") != "" && DS_BIZ_PLAN.NameValue(row, "SD_DIV_CD") != ""){
    ...
    for(var i=1; i<=DS_BIZ_PLAN.CountRow; i++){ 
        if(DS_BIZ_PLAN.NameString(i, "EDIT_FLAG") == "1"){
            if(DS_BIZ_PLAN.NameValue(i, "LST_DIV_CD") != "" 
                ...
                if(!(DS_BIZ_PLAN.NameValue(i, "LST_DIV_CD") == DS_BIZ_PLAN.NameValue(i, "SD_DIV_CD") 
                    && DS_BIZ_PLAN.NameValue(i, "LST_BIZ_CD") == DS_BIZ_PLAN.NameValue(i, "SD_DIV_BIZ_CD"))){
                	if(DS_DIV_CD.NameValueRow("CODE", DS_BIZ_PLAN.NameValue(i, "LST_DIV_CD")) != 0){
                		DS_BIZ_PLAN.NameValue(i, "SD_DIV_CD")     = DS_BIZ_PLAN.NameValue(i, "LST_DIV_CD");
                        DS_BIZ_PLAN.NameValue(i, "SD_DIV_BIZ_CD") = DS_BIZ_PLAN.NameValue(i, "LST_BIZ_CD");
            ...
            <td class="PT03"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr valign="top">
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
                  <tr>
                    <td>
                      <comment id="_NSID_"><object id=GD_BIZ_PLAN width="100%" height=459 classid=<%=Util.CLSID_GRID%>>
