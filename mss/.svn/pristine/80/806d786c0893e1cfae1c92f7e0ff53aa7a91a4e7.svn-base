<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 > 임대관리 > CESBILLING > 계량기 관리
 * 작 성 일 : 2010.01.14
 * 작 성 자 : 박래형
 * 수 정 자 : 
 * 파 일 명 : MREN6020.jsp
...
function doInit(){
    ...
    initComboStyle(LC_GAUG_TYPE,DS_GAUG_TYPE,               "CODE^0^30,NAME^0^80", 1, READ);    // 계량기구분   
    initEmEdit(EM_INST_PLC,         "GEN^40",READ);                                             // 설치장소
    ...
    initEmEdit(EM_VALID_S_DT,       "YYYYMMDD", PK);                                            // 유효시작일자
    initEmEdit(EM_VALID_E_DT,       "YYYYMMDD", PK);                                            // 유효종료일자
    initEmEdit(EM_FILE_PATH,        "GEN^50",READ);                                             // 첨부파일 
    
    getFlc("DS_S_FCL_FLAG",             "M", "", "Y", "N");                                    // [조회용]시설구분(점코드)
...
function btn_Search() {
    if (DS_IO_MASTER.IsUpdated||DS_IO_DETAIL.IsUpdated) {//변경데이터 있을 시 확인
        ...
        if (ret == "1") {
            ...
            TR_MAIN.Action = "/mss/mren602.mr?goTo=" + goTo + parameters;
...
function btn_Save() {
    ...
    if (DS_IO_MASTER.IsUpdated||DS_IO_DETAIL.IsUpdated) {//변경 데이터가 있을때만 저장
      ...
      //변경또는 신규 내용을 저장하시겠습니까?
        ...
        var goTo = "save";
        TR_SAVE.Action = "/mss/mren602.mr?goTo=" + goTo;
...
function getDetail() {  
    ...
    var parameters = ""
        + "&strFlcFlag="    + encodeURIComponent(strFlcFlag)
        + "&strGaugID="     + encodeURIComponent(strGaugID);
    TR_SUB.Action = "/mss/mren602.mr?goTo=" + goTo + parameters;
...
if (row > 0) {
	...
	//계량기사용구분,공동안분기준
    LC_GAUG_USE_FLAG.Enable  = true;  // CES에만 적용[2010.04.12]
    LC_DIV_RULE_TYPE.Enable  = true;  // CES에만 적용[2010.04.12]  
    
    objectControl(true); //(CES일시)시설구분 = 설치장소일 경우만 활성화
    objectControlType1();
    //계량기레벨에 따른 계량기 정보항목 컨트롤
    if (DS_IO_MASTER.NameValue(row, "GAUG_LVL") == "3") {
    	//if (LC_S_FCL_FLAG.ValueOfIndex("HUSE_FLAG", LC_S_FCL_FLAG.Index) != "2") objectControl(false); // CES에만 적용[2010.04.12] 
        enableControl(IMG_DEL_ROW, false);
        enableControl(IMG_ADD_ROW, false);
    } else {
        enableControl(IMG_DEL_ROW, true);
...
if (LC_GAUG_TYPE.BindColVal == "10") {
	...
	if (LC_GAUG_KIND_CD.BindColVal == "18") {
		if (DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "GAUG_LVL") == "3") { //LV3일경우는 수정불가
		    EM_USE_EQUIP_QTY.Enable  = false;
		} else {
		    EM_USE_EQUIP_QTY.Enable  = true;
		}
	} else {
	    EM_USE_EQUIP_QTY.Enable  = false;1
	}
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tr>
								<td><comment id="_NSID_"><OBJECT id=GD_DETAIL
									width=100% height=193 tabindex=1 classid=<%=Util.CLSID_GRID%>>
