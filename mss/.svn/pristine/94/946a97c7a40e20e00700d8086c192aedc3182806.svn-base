<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 > 상품권관리 > 상품권 입/출고 > 지점 입고 내역조회
 * 작 성 일 : 2011.03.14
 * 작 성 자 : 신익수
 * 수 정 자 : 
 * 파 일 명 : mgif2060.jsp
...
function gridCreate(){
	 var hdrProperies =   '<FC>id={currow}         name="NO"          width=25   align=center</FC>'
						 + '<FC>id=REQ_STR         name="점코드"       width=30  SHOW=FALSE  </FC>'
						 + '<FC>id=REQ_STR_NM      name="점"          width=180  align=left sumtext="합계"</FC>'
						 + '<FC>id=REQ_DT         name="신청일자"     width=120   align=center   show=false</FC>'
						 + '<FC>id=IN_CONF_DT      name="입고일자" width=150   align=center mask="XXXX/XX/XX" </FC>'
						 + '<FC>id=REQ_SLIP_NO     name="순번"        width=150   align=center </FC>'
						 + '<FC>id=OUT_QTY_SUM     name="수량"     width=130   align=light sumtext=@sum </FC>'
						 + '<FC>id=OUT_AMT_SUM   name="입고금액"     width=150   align=light sumtext=@sum </FC>'
						 ;
	initGridStyle(GD_MASTER, "common", hdrProperies, false);
	
	var hdrProperies1 =   '<FC>id={currow}             name="NO"          width=25   align=center</FC>'
						+ '<FC>id=GIFT_TYPE_NAME    name="상품권종류"       width=110  align=left  sumtext="합계"</FC>'
						+ '<FC>id=GIFT_AMT_NAME     name="금종"          width=120  align=left </FC>'
						...
						+ '<FC>id=OUT_QTY                   name="수량"     width=70   align=light sumtext=@sum </FC>'
						+ '<FC>id=OUT_AMT               name="입고금액"     width=80   align=light sumtext=@sum </FC>'
						;
 ...
 * 3. 함수
 *************************************************************************/
function getOutReqDtl(strReqStr,strReqYmd,strReqSlipNo){
   
    var goTo       = "getOutReqDtl" ;    
    var action     = "O";     
    var parameters = "&strReqStr="   + encodeURIComponent(strReqStr)
                   + "&strReqYmd="   + encodeURIComponent(strReqYmd)
                   + "&strReqSlipNo="+ encodeURIComponent(strReqSlipNo);
    
    TR_MAIN1.Action="/mss/mgif206.mg?goTo="+goTo+parameters;  
...
if(row > 0 && old_Row != row ){
    ...
    var strReqYmd = DS_O_MASTER.NameValue(row, "IN_CONF_DT");
    var strReqSlipNo = DS_O_MASTER.NameValue(row, "REQ_SLIP_NO");
    
    //지점입고 내역 디테일 조회
    getOutReqDtl(strReqStr,strReqYmd,strReqSlipNo);
}
old_Row = row;
</script>
<!--------------------- 6. 컴포넌트 이벤트 처리  ------------------------------>
...
<!--  그리드의 구분dot & 여백  -->
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
      <tr>
      <td>
          <comment id="_NSID_"><OBJECT id=GD_DETAIL width=100% height=270 classid=<%=Util.CLSID_GRID%>>
