<!-- 
/*******************************************************************************
 * 시스템명 : 상품권관리 > 상품권 마스터 관리> 상품권 마스터 관리
 * 작 성 일 : 2011.01.14
 * 작 성 자 : 
 * 수 정 자 : 
 * 파 일 명 : mgif104.jsp
 ...
 */
 ...
 var top2 = 400;		//해당화면의 동적그리드top위치
function doInit(){

	//Master 그리드 세로크기자동조정  2013-07-17
	 var obj   = document.getElementById("GD_STRMST"); 
	 obj.style.height  = (parseInt(window.document.body.clientHeight)-top) + "px";
	 
	 var obj   = document.getElementById("GD_VENFEEINFO"); 
	obj.style.height  = (parseInt(window.document.body.clientHeight)-top2) + "px";

    // Input Data Set Header 초기화

    // Output Data Set Header 초기화
    DS_IO_STRMST.setDataHeader('<gauce:dataset name="H_STRMST"/>');
    ...
    initEmEdit(EM_RATE, "NUMBER^3^2", READ);                  //수수료율
  //  initEmEdit(EM_APP_DT, "YYYYMMDD", NORMAL);                //적용일자
  //  initEmEdit(EM_BRCH_REC_FEE_RATE, "NUMBER^3^2", NORMAL);   //상품권 지급 수수료율
    
    
    //콤보 초기화
    initComboStyle(LC_S_STR,DS_O_S_STR, "CODE^0^30,NAME^0^80", 1, NORMAL);                      //점(조회)
    ...
    registerUsingDataset("mgif104", "DS_IO_STRMST,DS_IO_VENFEEINF");  
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}    name="NO"    edit=none     width=25   align=center</FC>'
                     + '<FC>id=STR_CD      name="점" edit={if(SysStatus == "I", "true", "false")} width=70 editstyle=Lookup Data="DS_I_STR:CODE:NAME"  align=left</FC>'
                     + '<FC>id=VEN_CD    name="혐력사;코드" edit={if(SysStatus == "I", "true", "false")}  EditStyle=Popup    width=60  align=center</FC>'
                     + '<FC>id=VEN_NAME    name="협력사명"   edit=none  width=100   align=left</FC>'
                     + '<FC>id=GIFT_VEN_FLAG     name="상품권;협력사구분" edit=none editstyle=Lookup Data="DS_O_GIFT_VEN_FLAG:CODE:NAME" width=70 align=left</FC>';
                      
    initGridStyle(GD_STRMST, "common", hdrProperies, true);
    ...
    var hdrProperies1 = '<FC>id={currow}    name="NO"   edit=none      width=50   align=center</FC>'
        ...
        + '<FC>id=BRCH_REC_FEE_RATE edit=none   name="가맹점 수수료율"  edit=RealNumeric  edit={if(APP_DT <= TODAY, "false", "true")}  width=150  align=right</FC>';
...
function btn_Search() {
    ...
    var strBuySaleFlag      = "%";//LC_S_BUY_SALE_FLAG.BindColVal;
    ...
    btnSearchClick = true;
    TR_MAIN.Action="/mss/mgif104.mg?goTo="+goTo+parameters;  
...
function btn_Save() {
    ...
    alert(DS_IO_STRMST.NameValue(DS_IO_STRMST.RowPosition, "GIFT_VEN_FLAG"));
    ...
    btnSaveClick = true;
    TR_MAIN.Action="/mss/mgif104.mg?goTo=save";
 ...
 *************************************************************************/
 ...
 function setGiftVenFlag() {
     ...
     DS_O_GIFT_VEN_FLAG.NameValue(DS_O_GIFT_VEN_FLAG.CountRow, "CODE") = "2";
     DS_O_GIFT_VEN_FLAG.NameValue(DS_O_GIFT_VEN_FLAG.CountRow, "NAME") = "가맹점";
     DS_O_GIFT_VEN_FLAG.AddRow();
     ...
     if(DS_IO_VENFEEINFO.CountRow > 0){
	     ...
	     for(var i=1; i<=DS_IO_VENFEEINFO.CountRow;i++){
	    	 if(DS_IO_VENFEEINFO.NameValue(i,"APP_DT") <= strToday){
	    		 strRate = DS_IO_VENFEEINFO.NameValue(i,"BRCH_REC_FEE_RATE");
	    	 }
	     }
     }
 ...
 function getStrInfo() {
     ...
     var parameters = "&strStrCd="+encodeURIComponent(strStrCd)
                    + "&strVenCd="+encodeURIComponent(strVenCd);
     TR_MAIN.Action="/mss/mgif104.mg?goTo="+goTo+parameters;  
     TR_MAIN.KeyValue="SERVLET("+action+":DS_IO_STRINFO=DS_IO_STRINFO)"; //조회는 O
     TR_MAIN.Post();
    
     if(DS_IO_STRINFO.CountRow == 1){
    	 for(var i=1;i<=DS_IO_STRMST.CountColumn;i++){
    		 var strColName = DS_IO_STRMST.ColumnID(i);
    		 DS_IO_STRMST.NameValue(DS_IO_STRMST.RowPosition, strColName) = DS_IO_STRINFO.NameValue(1, strColName); 
    	 }
    	 addRow();
     }
 }
 ...
 function getVenCd(){
	 ...
	 getMssEvtVenPop( EM_S_VEN_CD, EM_S_VEN_NM, 'S', '4', '', LC_S_STR.BindColVal, '')
   }
</script>
...
if((DS_IO_STRMST.IsUpdated || DS_IO_VENFEEINFO.IsUpdated) && !btnSaveClick){
     if(showMessage(QUESTION , YESNO, "USER-1049") != 1 ){
         return false;
         }
     if(DS_IO_STRMST.SysStatus(row) == 1){
    	 DS_IO_STRMST.DeleteRow(row);
     }
...
<script language=JavaScript for=DS_IO_STRMST event=OnColumnChanged(row,colid)>
if(colid == "STR_CD"){// 점을 변경하는 경우 이후 내용 클리어
   if(strStrCd != "" && strStrCd != this.NameValue(row, colid)){
	   if(showMessage(QUESTION , YESNO, "USER-1000", "점코드를 변경하시면 협력사 정보가 초기화 됩니다. 초기화 하시겠습니까?") != 1 ){
		   this.NameValue(row, colid) = strStrCd;	         
		   return false;
	    }else{
	    	 for(var i=2;i<=DS_IO_STRMST.CountColumn;i++){
	             var strColName = DS_IO_STRMST.ColumnID(i);
	             DS_IO_STRMST.NameValue(DS_IO_STRMST.RowPosition, strColName) = ""; 
	         }
	    	 strStrCd = "";
	    	 DS_IO_VENFEEINFO.ClearData();
	    } 
...
<script language=JavaScript for=DS_IO_VENFEEINFO event=CanRowPosChange(row)>
if(btnSearchClick) return;
return true;
</script>
...
if(colid == "BRCH_REC_FEE_RATE" && DS_IO_VENFEEINFO.NameValue(row, "BRCH_REC_FEE_RATE") != ""){
...
<script language=JavaScript for=GD_STRMST    event=OnPopup(row,colid,data)>
    if(colid == "VEN_CD"){
      //  var rtnMap = venToGridPop(DS_IO_STRMST.NameValue(row, "VEN_CD"),"", "","","","","","95");
       
      if(DS_IO_STRMST.NameValue(row, "STR_CD") == ""){
    	  showMessage(EXCLAMATION, OK, "USER-1002", "점코드");
    	  setFocusGrid(GD_STRMST, DS_IO_STRMST,row,"STR_CD");
          return; 
      }
      var strCode = DS_IO_STRMST.NameValue(row, colid);
      DS_IO_STRMST.NameValue(row, "VEN_CD") = "";
      DS_IO_STRMST.NameValue(row, "VEN_NAME") = "";
      var rtnMap =  getMssEvtVenPop( strCode, '', 'S', '4', '', DS_IO_STRMST.NameValue(row, "STR_CD"), '');
      if(rtnMap != null){
    	  strStrCd = DS_IO_STRMST.NameValue(row, "STR_CD");
          DS_IO_STRMST.NameValue(row, "VEN_CD") = rtnMap.CODE;
          DS_IO_STRMST.NameValue(row, "VEN_NAME") = rtnMap.NAME;
          // 협력사 정보 조회
          getStrInfo();
      }
    }
...
<comment id="_NSID_"><object id="DS_IO_STRINFO"  classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
        ...
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
         ...
         <!--  <th width="80">매출매입구분</th>
            <td width="130"><comment id="_NSID_">
                        <object id=LC_S_BUY_SALE_FLAG classid=<%= Util.CLSID_LUXECOMBO %> height=100 width=130 align="absmiddle" tabindex=1>
                        </object>
                    </comment><script>_ws_(_NSID_);</script></td> -->   
            <th width="80">가맹점</th>
            <td >
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr valign="top">
        <td width="230">
            ...
            <tr>
            <td>
            <comment id="_NSID_"><OBJECT id=GD_STRMST width=370 height=800 classid=<%=Util.CLSID_GRID%>>
        ...
        <td class="PL05">
          ...
          <tr height=18>
            <td  colspan=2>
               <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="sub_title"><img src="/<%=dir%>/imgs/comm/ring_blue.gif" width="11" height="12" align="absmiddle" class="PR03"/>가맹점 마스터 정보</td>
              </tr>
            </table>
           </td>
          </tr>
                    ...
                    <tr>
	                  <td>
	                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="BD4A">
	                      <tr>
	                        <td> 
	                        <comment id="_NSID_"><OBJECT id=GD_VENFEEINFO width=100% height=500 classid=<%=Util.CLSID_GRID%>>
