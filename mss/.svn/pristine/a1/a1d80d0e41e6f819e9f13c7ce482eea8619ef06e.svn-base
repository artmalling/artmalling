...
public class MRen403DAO extends AbstractDAO {
	/**
			...
			while (mi1.next()) {
				...
				if(mi1.IS_DELETE()){// 삭제
					updateTaxInfo(form, userId, mi1.getString("STR_CD"), 
												...
												mi1.getString("TAX_ISSUE_FLAG"),
												mi1.getString("TAX_FLAG_CD")
												);
	...
	public int updateTaxInfo(ActionForm form, String userId, String strStrCd 
		...
		try {
			...
			sql.setString(i++, strTaxIssueFlag);		// TAX_ISSUE_FLAG
			sql.setString(i++, strTaxIssueFlag);		// TAX_ISSUE_FLAG
			sql.setString(i++, strTaxIssueFlag);		// TAX_ISSUE_FLAG
			sql.setString(i++, strTaxFlag);				// TAX_FLAG
			sql.setString(i++, strTaxIssueFlag);		// TAX_ISSUE_FLAG
			sql.setString(i++, strTaxFlag);				// TAX_FLAG
