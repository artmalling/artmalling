...
function doInit(){
    ...
    DS_BIZ_TREE.setDataHeader('<gauce:dataset name="H_SEL_BIZ_PLAN_TREE"/>');
    DS_BIZ_DTL.setDataHeader('<gauce:dataset name="H_SEL_BIZ_PLAN_DTL"/>');
    DS_TEMP.setDataHeader('CODE:STRING(1)');
    //EMEDIT 설정
    setEmEdit();
    ...
    registerUsingDataset("meis0210","DS_BIZ_TREE,DS_BIZ_DTL" );
}
...
function setEmEdit(){
    ...
    initEmEdit(EM_PLAN_YM, "THISMN", PK); //경영계획년월    
...
function btn_Search() {
    ...
    lo_PlanYm      = EM_PLAN_YM.text;
    var goTo       = "searchBizPlanTree" ;    
    var action     = "O";     
    var parameters = "&strPlanYm=" + EM_PLAN_YM.text; //경영계획년월
    
    DS_BIZ_TREE.DataID   = "/mss/meis021.me?goTo="+goTo+parameters;
...
function btn_Delete() {
    ...
    TR_MAIN.Action   = "/mss/meis021.me?goTo=delete";
...
function btn_Save() {
	var copyFlag = 0;
	if(chk_Copy.checked) copyFlag = 1;
	...
	if(!copyFlag) {
		...
		DS_TEMP.ClearData();
		DS_TEMP.AddRow();
		DS_TEMP.NameValue(DS_TEMP.CountRow, "CODE")   = 1;
	}
    ...
    var parameters   = "&strBizCd="  + encodeURIComponent(saveBizCd) //실적항목코드
                     + "&strPlanYm=" + encodeURIComponent(lo_PlanYm)
                     + "&strCpFlag=" + encodeURIComponent(copyFlag)
                     ;
    
    TR_MAIN.Action   = "/mss/meis021.me?goTo=save"+parameters;  
    TR_MAIN.KeyValue = "SERVLET(I:DS_BIZ_TREE=DS_BIZ_TREE,I:DS_BIZ_DTL=DS_BIZ_DTL,I:DS_TEMP=DS_TEMP)"; 
    TR_MAIN.Post();
...
function btn_addRow(){
    ...
    DS_BIZ_DTL.AddRow();
    DS_BIZ_DTL.NameValue(DS_BIZ_DTL.CountRow, "BIZ_PLAN_YM") = lo_PlanYm;
...
function addTreeRow(row) {
     ...
     var planYm = DS_BIZ_TREE.NameValue(row-1, "BIZ_PLAN_YM");
     ...
     DS_BIZ_TREE.NameValue(row, "BIZ_PLAN_YM")  = planYm;
...
function copyMstData(){
    ...
    var returnVal = window.showModalDialog( "/mss/meis021.me?goTo=list1&strPlanYm=" + EM_PLAN_YM.text
                                          , ""
...
function copyPreData(){
    ...
    var returnVal = window.showModalDialog( "/mss/meis021.me?goTo=list2&strPlanYm=" + EM_PLAN_YM.text
                                          , ""
                                          , "dialogWidth:870px;dialogHeight:450px;scroll:no;" 
                                          + "resizable:no;help:no;unadorned:yes;center:yes;status:no");
...
function getExcelPopup(){
    ...
    var returnVal = window.showModalDialog( "/mss/meis021.me?goTo=list3&strPlanYm=" + EM_PLAN_YM.text
                                          , ""
...
<!-- ====================Output용================== -->
    ...
    <!--[콤보]사용여부 -->
    <object id="DS_TEMP"      classid=<%= Util.CLSID_DATASET %>></object>
<!-- ====================Input용================== -->
            ...
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td class="right PB02" valign="bottom">
                  <input type="checkbox" id=chk_Copy align="absmiddle" tabindex=1 >년복사
                  <img src="/<%=dir%>/imgs/btn/copy_list.gif" onclick="javascript:copyMstData();" />
                  <img src="/<%=dir%>/imgs/btn/copy_lastplan.gif" onclick="javascript:copyPreData();" />
