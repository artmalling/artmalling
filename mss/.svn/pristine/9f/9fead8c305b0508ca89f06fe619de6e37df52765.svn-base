...
function doInit(){
    ...
    DS_IO_MASTER.setDataHeader('<gauce:dataset name="H_SEL_MR_POWERPRCH"/>');
    ...
    //콤보 초기화
    initComboStyle(LC_S_PWR_TYPE ,DS_LC_S_PWR_TYPE , "CODE^0^30,NAME^0^80", 1, PK); // [조회용]전압구분     
    //시스템 코드 공통코드에서 가지고 오기
    getEtcCode("DS_LC_S_PWR_TYPE",      "D", "M047", "N", "N", LC_S_PWR_TYPE );     // [조회용]전압구분     
    LC_S_PWR_TYPE.Focus();
    DS_LC_S_PWR_TYPE.Filter(); // 전압구분필터링
}
...
function gridCreate(gdGnb){
	...
	if (gdGnb == "MST") {
	    var hdrProperies = ''
	        ...
	        + '                                         WIDTH=110  ALIGN=LEFT     EDITSTYLE=LOOKUP   DATA="DS_LC_S_PWR_TYPE:CODE:NAME"'
	        + '                                         BGCOLOR={IF(DUPCHK="Y","RED","WHITE")}</FC>'
	        + '<FC>ID=SEQ          NAME="SEQ"'
	        + '                                         WIDTH=130  SHOW=FALSE</FC>'
	        + '<G>                 NAME="사용량(KWh)"'
	        + '<FC>ID=USE_S_QTY    NAME="*FROM"'
	        + '                                         WIDTH=110  ALIGN=RIGHT    EDIT=NUMERIC    BGCOLOR={IF(DUPCHK="Y","RED","WHITE")}</FC>'
	        + '<FC>ID=USE_E_QTY    NAME="*TO"'
	        + '                                         WIDTH=130  ALIGN=RIGHT    EDIT=NUMERIC    BGCOLOR={IF(DUPCHK="Y","RED","WHITE")}</FC></G>'
	        + '<G>                 NAME="정상(원/KWh)"'
	        + '<FC>ID=BAS_PRC      NAME="*단위기본요금"'
	        + '                                         WIDTH=130  ALIGN=RIGHT    EDIT=NUMERIC    BGCOLOR={IF(DUPCHK="Y","RED","WHITE")}</FC>'
	        + '<FC>ID=UNIT_PRC     NAME="*전력량요금"'
	        + '                                         WIDTH=130  ALIGN=RIGHT    EDIT=REALNUMERIC BGCOLOR={IF(DUPCHK="Y","RED","WHITE")}</FC></G>'
	        + '<FC>ID=REMARK       NAME="비고"'
	        + '                                         WIDTH=130  ALIGN=LEFT     LANGUAGE="1"</FC>'
...
function btn_Search() {
    if (DS_IO_MASTER.IsUpdated) {//변경데이터 있을 시 확인
        ...
        if (ret == "1") {
            var strPwrType  = LC_S_PWR_TYPE .BindColVal;    // [조회용]전압구분
            var goTo = "getMaster";
            var parameters = ""
                + "&strPwrType="    + encodeURIComponent(strPwrType);
            TR_MAIN.Action = "/mss/mren104.mr?goTo=" + goTo + parameters;
...
function btn_Save() {
    if (DS_IO_MASTER.IsUpdated) {//변경 데이터가 있을때만 저장
	    ...
	    //저장
        var goTo = "save";
        TR_SAVE.Action = "/mss/mren104.mr?goTo=" + goTo;
...
function setUseQty(Row) {
   	var maxToVal = 0;
   	var strChkKeyVal  = DS_IO_MASTER.NameValue(Row, "PWR_TYPE");
    for (i=1; i<=DS_IO_MASTER.CountRow; i++) {
    	if (i != Row) {
	    	var tmpChkKeyVal =  DS_IO_MASTER.NameValue(i, "PWR_TYPE");
...
function checkValidate() {
    ...
    for (var n=1; n<=DS_IO_MASTER.CountRow; n++) {
        if (DS_IO_MASTER.RowStatus(n) != 0 ) {
            ...
            if (DS_IO_MASTER.NameValue(n,"USE_E_QTY") == 0) {
                ...
                showMessage(INFORMATION, OK, "USER-1002", "사용량(KWh)-TO");
                return false;
            }
            ...
            // 업종구분
            if (DS_IO_MASTER.NameValue(n,"USE_E_QTY") == "") {
                DS_IO_MASTER.RowPosition = n;
                GD_MASTER.SetColumn("USE_E_QTY");
                showMessage(INFORMATION, OK, "USER-1002", "사용량(KWh)-TO");
                return false;
            ...
            // 단위기본요금
            if (DS_IO_MASTER.NameValue(n,"BAS_PRC") == "") {
                DS_IO_MASTER.RowPosition = n;
                GD_MASTER.SetColumn("BAS_PRC");
                showMessage(INFORMATION, OK, "USER-1002", "단위기본요금");
                return false;
            }
            
            // 단가
            if (DS_IO_MASTER.NameValue(n,"UNIT_PRC") == "") {
                DS_IO_MASTER.RowPosition = n;
			...
			// 비고
			var tmpLenA = checkLenByte(DS_IO_MASTER, n, "REMARK")
			if (tmpLenA > 50 ) {
                DS_IO_MASTER.RowPosition = n;
                GD_MASTER.SetColumn("REMARK");
			    showMessage(STOPSIGN, OK, "USER-1000", "50Byte 이상 초과 입력할수 없습니다.");
			    return false;
			} 
        }
    }
	...
	for (var i=1; i<=DS_IO_MASTER.CountRow; i++) {
        ...
        for (var k=i+1; k<=DS_IO_MASTER.CountRow; k++) {
            var targetChkKeyVal = DS_IO_MASTER.NameValue(k, "PWR_TYPE");
	...
	if (DS_O_DUPKEYVALUE.CountRow < 1 ) {
	    ...
	    for (var i=1; i<=DS_IO_MASTER.CountRow; i++) {
	        if (DS_IO_MASTER.RowStatus(i) == 1 || DS_IO_MASTER.RowStatus(i) == 3) { // 신규일시
	            var tmpKey = DS_IO_MASTER.NameValue(i,"PWR_TYPE")
...
//단가의 음수제외
if (colid == "UNIT_PRC") {
	if (eval(DS_IO_MASTER.NameValue(row, colid)) < 0 ) 
		DS_IO_MASTER.NameValue(row, colid) = Math.abs(DS_IO_MASTER.NameValue(row, colid));
}
</script>
...
<script language=JavaScript for=DS_LC_S_PWR_TYPE event=OnFilter(row)>
// 요금제구분
if (DS_LC_S_PWR_TYPE.NameValue(row,"CODE") == "1"||DS_LC_S_PWR_TYPE.NameValue(row,"CODE") == "5") {// 저압, 고압
    return true;
} else { 
    return false;
}
...
<!-- =============== ONLOAD용 -->
<comment id="_NSID_">
<object id="DS_LC_S_PWR_TYPE"    classid=<%=Util.CLSID_DATASET%>>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					class="s_table">
					<tr>
						<th width="80" class="point">전압구분</th>
						<td ><comment id="_NSID_"> <object
							id=LC_S_PWR_TYPE  classid=<%=Util.CLSID_LUXECOMBO%>
