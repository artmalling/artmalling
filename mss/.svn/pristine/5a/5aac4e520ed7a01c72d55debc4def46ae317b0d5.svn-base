...
function doInit(){
  ...
  //종료시 데이터 변경 체크 (common.js)
    registerUsingDataset("mgif108", "DS_IO_MASTER,DS_IO_DETAIL");  
}
...
function btn_Search() {
    ...
    TR_MAIN.Action="/mss/mgif108.mg?goTo="+goTo+parameters;  
...
function btn_Save() {
    ...
    TR_MAIN.Action="/mss/mgif108.mg?goTo=save"; 
 ...
 *************************************************************************/
 ...
 function getDetail () {
    ...
    TR_DETAIL.Action="/mss/mgif108.mg?goTo="+goTo+parameters;  
 ...
 if(colid == "IN_GIFT_S_NO" || colid == "IN_GIFT_E_NO"){
	 if(DS_IO_DETAIL.NameValue(row,"IN_GIFT_S_NO") != "" && DS_IO_DETAIL.NameValue(row,"IN_GIFT_E_NO") != ""){
	     // 입고 가능 수량  조회
	      ...
	      TR_DETAIL.Action="/mss/mgif108.mg?goTo=getGiftCnt"+parameters;  
...
if(colid == "IN_GIFT_S_NO" || colid == "IN_GIFT_E_NO"){
    ...
    if ( strGiftCardNo.length != 13 && strGiftCardNo.length != 16 && strGiftCardNo.length != 18) {
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td>
        ...
        <tr>
        <td>
            <comment id="_NSID_"><OBJECT id=GD_DETAIL width=100% height=300 classid=<%=Util.CLSID_GRID%>>
