...
function doInit(){
    ...
    registerUsingDataset("meis0211","DS_BIZ_MST" );
...
function btn_Search() {
    ...
    DS_BIZ_MST.DataID = "/mss/meis021.me?goTo=searchMst&strPlanYm=<%=strPlanYm%>"; 
    DS_BIZ_MST.Reset();
}
...
<script language=JavaScript for=DS_BIZ_MST event=OnRowPosChanged(row)>
     if (this.CountRow >0) {
    	 ...
    	 else showMessage(EXCLAMATION, OK, "USER-1000", "항목마스터 데이터가 없습니다." );
     }
</script>
<!--------------------- 6. 컴포넌트 이벤트 처리  ------------------------------->
<!--헤더 체크시 일괄체크 처리-->
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="" class="title">
              ...
              <SPAN id="title1" class="PL03">경영계획지침관리(항목마스터) </SPAN>
            </td>
            <td class="right PB02">
