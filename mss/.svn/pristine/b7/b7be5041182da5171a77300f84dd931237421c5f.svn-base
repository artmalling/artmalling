 ...
 * @caused by
 */
public class MEis044DAO extends AbstractDAO {
	 ...
	 * @throws Exception
	 */
	public List<Map> searchSalesPlan(ActionForm form) throws Exception {
		List<Map> ret        = null;
		SqlWrapper sql  = null;
		Service svc     = null;
		String strLoc   = null;
		...
		try {
			...
			if(strOrgLevel.equals("1")) {
				strLoc = "SEL_SALES_PLAN_STR";
			//팀
			} else if(strOrgLevel.equals("3")) {
				strLoc = "SEL_SALES_PLAN_TEAM";
			//PC
			} else if(strOrgLevel.equals("4")) {
				strLoc = "SEL_SALES_PLAN_PC";
			}
