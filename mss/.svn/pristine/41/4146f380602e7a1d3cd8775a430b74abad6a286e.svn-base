...
function doInit() {
    ...
    initEmEdit(EM_S_VEN_NM,   "GEN^40",READ);                               9                // [조회용]협력업사명   
   // initComboStyle(LC_STR_CD,DS_STR_CD,                 "CODE^0^30,NAME^0^100", 1, PK);     // 시설구분
    ...
    initEmEdit(EM_COST_AMT,                             "NUMBER^7^3", PK);                       // 단가
    ...
    registerUsingDataset("mren308","DS_IO_MASTER" );
    /*EXCEL파일*/
...
function gridCreate() {
	var hdrProperies = '<FC>ID={CURROW}     NAME="NO"           WIDTH=30   ALIGN=CENTER </FC>'
        + '<FC>ID=VEN_CD       NAME="협력사코드"    WIDTH=75   ALIGN=CENTER EditStyle=Popup edit=none</FC>'
        + '<FC>ID=VEN_NM       NAME="협력사명"      WIDTH=170  ALIGN=LEFT edit=none</FC>'
        + '<FC>ID=MNTN_ITEM_CD NAME="관리비항목"    WIDTH=80  ALIGN=LEFT edit=none  EditStyle=Lookup    Data="DS_MR_MNTNITEM:CODE:NAME"</FC>'
        + '<FC>ID=USE_QTY      NAME="사용량"        WIDTH=90   ALIGN=RIGHT EDIT=REALNUMERIC edit=true</FC>'
        + '<FC>ID=COST_AMT     NAME="단가"          WIDTH=90   ALIGN=RIGHT EDIT=REALNUMERIC edit=true </FC>'
        + '<FC>ID=SUM_QTY      NAME="누계량"        WIDTH=90   ALIGN=RIGHT  edit=none</FC>' 
        + '<FC>ID=USE_AMT      NAME="총금액"        WIDTH=90   ALIGN=RIGHT  edit=none</FC>' 
        + '<FC>ID=VAT_AMT      NAME="VAT"           WIDTH=70   ALIGN=RIGHT  edit=none</FC>'        
        + '<FC>ID=CAL_YM       NAME="정산년월"      WIDTH=170  ALIGN=LEFT edit=none  show="false"</FC>'
    ...
    if (DS_IO_MASTER.IsUpdated) {//변경데이터 있을 시 확인
        ...
        if (ret == "1") {
         ...
         //   LC_STR_CD.BindColVal = LC_S_STR_CD.BindColVal;
            ...
            var strCalYmBf  = setDateAdd("M", -1, strCalYM + "01").substring(0, 6);  //전월
            ...
            var parameters = "&strStrCd="      + encodeURIComponent(strStrCd)
                		   ...
                		   + "&strCalYmBf="    + encodeURIComponent(strCalYmBf)
                		   + "&strMntnitem="   + encodeURIComponent(strMntnitem)
                		   + "&strVenCd="      + encodeURIComponent(strVenCd);
            TR_MAIN.Action = "/mss/mren308.mr?goTo=" + goTo + parameters;
            TR_MAIN.KeyValue = "SERVLET(O:DS_IO_MASTER=DS_IO_MASTER)";
            TR_MAIN.Post();
            
            
            if(DS_IO_MASTER.CountRow > 0){
    			EM_COST_AMT.Text = DS_IO_MASTER.NameValue(1, "COST_AMT"); 
    		}else{
    			EM_COST_AMT.Text = "";
    		}
		...
		var strStrCd    = LC_S_STR_CD.BindColVal;       // 시설구분
		var strCalYM    = EM_S_CAL_YM.Text;             // 부과년월
		var strCalYmBf  = setDateAdd("M", -1, strCalYM + "01").substring(0, 6);  // [조회용]전월
		var strMntnitem = LC_S_MR_MNTNITEM.BindColVal;  // 관리비항목
		var strVenCd    = EM_S_VEN_CD.Text;             // 협력사코드
...
function btn_New() {
	...
	if (EM_COST_AMT.Text == 0 || EM_COST_AMT.Text == "") {
	...
	for (var i=1; i<=DS_O_VEN_CD.CountRow; i++) {
		...
		DS_IO_MASTER.NameValue(i, "SUM_QTY") = DS_O_VEN_CD.NameValue(i, "SUM_QTY");
...
function btn_Delete() {
	...
	if (DS_IO_MASTER.RowStatus(DS_IO_MASTER.RowPosition) == 1) { // 신규시에만 삭제
	    ...
	    TR_MAIN.Action = "/mss/mren308.mr?goTo=" + goTo;
...
function btn_Save() {
    if (DS_IO_MASTER.IsUpdated) {//변경 데이터가 있을때만 저장
        ...
        //var strCostAmt  = EM_COST_AMT.Text;             // 사용단가
        
        
        var parameters = "&strStrCd="      + encodeURIComponent(strStrCd)
                       + "&strCalYm="      + encodeURIComponent(strCalYm);
                       //+ "&strCostAmt="    + strCostAmt;
        
        
        var goTo = "save";
        
        TR_SAVE.Action = "/mss/mren308.mr?goTo=" + goTo + parameters;
...
function checkValidateSave() {
    ...
    for (var i=1; i<=DS_IO_MASTER.CountRow; i++) {        
        ...
        // 단가        
        if (DS_IO_MASTER.NameValue(i,"COST_AMT") == "") {
            showMessage(STOPSIGN, OK, "USER-1000", "단가를 입력하세요.");
            DS_IO_MASTER.RowPosition = i;
            GD_MASTER.SetColumn("COST_AMT");
            return false;
        } 
        // 금액
        /*
        if (DS_IO_MASTER.NameValue(i,"USE_QTY") == "") {
            showMessage(STOPSIGN, OK, "USER-1000", "사용량을 입력하세요.");
            DS_IO_MASTER.RowPosition = i;
            GD_MASTER.SetColumn("USE_QTY");
            return false;
        } */
...
<script language=JavaScript for=GD_MASTER event=OnExit(row,colid,oldData)>
    ...
    case "USE_QTY" :
        	if( changeFlag ){
        		eval(this.DataID).NameValue(row,"USE_AMT") = getRoundDec("1" ,eval(this.DataID).NameValue(row,"USE_QTY") * eval(this.DataID).NameValue(row,"COST_AMT"));
        		eval(this.DataID).NameValue(row,"VAT_AMT") = eval(this.DataID).NameValue(row,"USE_QTY") * eval(this.DataID).NameValue(row,"COST_AMT")
        		- getRoundDec("1" ,eval(this.DataID).NameValue(row,"USE_QTY") * eval(this.DataID).NameValue(row,"COST_AMT") / 1.1);
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>						
						...
						<th width="80" class="point">단가</th>
						<td colspan="3"><comment id="_NSID_"><object
							id=EM_COST_AMT style="vertical-align: middle;"
							classid=<%=Util.CLSID_EMEDIT%> width="140"></object></comment><script>_ws_(_NSID_);</script>
					</tr>
				</table>
				</td>
			</tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				<td><comment id="_NSID_"><object id=GD_MASTER
					width="100%" height=446 classid=<%=Util.CLSID_GRID%>>
