...
public class MRen603DAO extends AbstractDAO {
	...
	public List getDetail(ActionForm form) throws Exception {
		...
		try {
			...
			svc = (Service) form.getService();
			sql = new SqlWrapper();
			String strHuseFlag = String2.nvl(form.getParam("strHuseFlag")); // 시설구분
			String strGaugID = String2.nvl(form.getParam("strGaugID")); // 계량기ID
			int i = 0;

			if (strHuseFlag.equals("1")) { // 비주거
				sql.put(svc.getQuery("SEL_MR_GAUGMST_DTL_C"));
			} else {
				sql.put(svc.getQuery("SEL_MR_GAUGMST_DTL_H"));
			}
			sql.setString(++i, strGaugID);
			list = select2List(sql);
		} catch (Exception e) {
