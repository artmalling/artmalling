<!-- 
/*******************************************************************************
 * 시스템명 : 경영실적 > 기준정보> 경영실적항목관리
 * 작 성 일 : 2011.05.02
 * 작 성 자 : 이정식
 * 수 정 자 : 
 * 파 일 명 : meis0010.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 경영실적항목을 등록하고 회계계정과목/예산항목을 매핑한다.
 * 이    력 :
 *        2011.05.02 (이정식) 신규작성
 ******************************************************************************/
-->
...
function doInit(){
    ...
    DS_BIZ_TREE.setDataHeader('<gauce:dataset name="H_SEL_BIZ_TREE"/>');
    DS_BIZ_DTL.setDataHeader('<gauce:dataset name="H_SEL_BIZ_DTL"/>');
    //EMEDIT 설정
    setEmEdit();
    ...
    registerUsingDataset("meis0010","DS_BIZ_TREE,DS_BIZ_DTL" );
}
...
function gridCreate(){
	var hdrProperies = '<C>id={currow}  width=35  align=center name="NO"</C>'
                     ...
                     + '<C>id=ACC_CD    width=110 align=center name="*계정/예산항목" edit="AlphaNum"</C>'
                     + '<C>id=ACC_CD_NM width=200 align=left   name="*항목명" edit="Any" </C>'
                     ;
...
function btn_Search() {
	...
	var goTo       = "searchBizTree" ;    
    var action     = "O";     
    var parameters = "&strBizLCd=" + LC_BIZL_CD.BindColVal //대분류코드
                   + "&strBizMCd=" + LC_BIZM_CD.BindColVal //중분류코드
                   + "&strBizSCd=" + LC_BIZS_CD.BindColVal //소분류코드
                   ;
    
    DS_BIZ_TREE.DataID   = "/mss/meis001.me?goTo="+goTo+parameters;
...
function btn_Delete() {
    ...
    TR_MAIN.Action   = "/mss/meis001.me?goTo=delete"+parameters;
    ...
    if( TR_MAIN.ErrorCode == 0){
        var sLcd = LC_BIZL_CD.BindColVal;
        var sMcd = LC_BIZM_CD.BindColVal;
        var sScd = LC_BIZS_CD.BindColVal;
        btn_Search();
        var row = DS_BIZ_TREE.NameValueRow("BIZ_CD",saveBizCd);
        row = row<1?1:row;
        DS_BIZ_TREE.RowPosition = row;

        getBizCdCombo(DS_BIZL_CD, "1", "Y");
        setTimeout("setComboData(LC_BIZL_CD,'"+sLcd+"');",50);
        setTimeout("setComboData(LC_BIZM_CD,'"+sMcd+"');",70);
        setTimeout("setComboData(LC_BIZS_CD,'"+sScd+"');",90);
 ...
 */
function btn_Save() {
	if( !DS_BIZ_TREE.IsUpdated && !DS_BIZ_DTL.IsUpdated){
        //저장할 내용이 없습니다.
    ...
    TR_MAIN.Action   = "/mss/meis001.me?goTo=save"+parameters;  
...
function addTreeRow(row) {
     ...
     //var bizCd  = DS_BIZ_TREE.NameValue(row-1, "BIZ_CD"); 
     ...
     DS_SEQ.ClearData();
     var goTo        = "getSeq" ;
     var parameters  = "&strLvl=" + bizLvl;
     DS_SEQ.DataID   = "/mss/meis001.me?goTo="+goTo+parameters;
     DS_SEQ.SyncLoad = "true";
     DS_SEQ.Reset();     
     if(DS_SEQ.CountRow == 1) DS_BIZ_TREE.NameValue(row, "PRT_SEQ") = DS_SEQ.NameValue(1, "PRT_SEQ");
     //
...
function inputCmpMode( mode ){
    ...
    enableControl(EM_BIZ_NM,  upd);                 //실적항목명 
    enableControl(LC_BIZ_LVL, read);                //항목레벨
    enableControl(EM_L_CD,    bizLvl==1?ins:false); //대분류코드 
    enableControl(EM_L_NM,    read);                //대분류명 
    enableControl(EM_M_CD,    bizLvl==2?ins:false); //중분류코드
    enableControl(EM_M_NM,    read);                //중분류명
    enableControl(EM_S_CD,    bizLvl==3?ins:false); //소분류코드 
...
function searchBizDtl(){
    ...
    DS_BIZ_DTL.DataID = "/mss/meis001.me?goTo="+goTo+parameters;
...
function getExcelPopup(){
	var returnVal = window.showModalDialog( "/mss/meis001.me?goTo=popup"
			                              , ""
...
<script language=JavaScript for=EM_L_CD event=onKillFocus()>
    ...
    EM_BIZ_CD.text = EM_L_CD.text + "0000";
</script>
...
<script language=JavaScript for=EM_M_CD event=onKillFocus()>
    ...
    EM_BIZ_CD.text = EM_L_CD.text + EM_M_CD.text + "00";
</script>
...
<!-- ====================Output용================== -->
    ...
    <!--출력순서값 -->
    <object id="DS_SEQ"       classid=<%= Util.CLSID_DATASET %>></object>
<!-- ====================Input용================== -->
            ...
            <td width="100%" class="PT03">
              <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD2A">
                <tr>
                  <td>
                    <comment id="_NSID_">
                      <object id=TV_MAIN classid=<%=Util.CLSID_TREEVIEW%> height=449 width=100% >
          ...
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td class="right PB02" valign="bottom"><img src="/<%=dir%>/imgs/btn/excel_s.gif" onclick="getExcelPopup();" /></td>
              </tr>
            </table></td>
          </tr>
          <tr>
              ...
              <tr>
                <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
                  <tr>
                    ...
                    <td width="140">
                      <comment id="_NSID_">
                        <object id=EM_BIZ_CD classid=<%=Util.CLSID_EMEDIT%> width=140 tabindex=1 align="absmiddle">
                        </object>
                      </comment>
            ...
            <td class="PT03"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr valign="top">
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
                  <tr>
                    <td>
                      <comment id="_NSID_"><object id=GD_BIZ_DTL width="100%" height=322 classid=<%=Util.CLSID_GRID%>>
