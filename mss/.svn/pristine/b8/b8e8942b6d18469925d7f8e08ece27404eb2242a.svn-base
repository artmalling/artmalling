...
public class MEdi102DAO extends AbstractDAO {
	...
	public List getComboBuyer(ActionForm form) throws Exception {
		...
		String noteid = String2.nvl(form.getParam("noteid"));			//사원번호
		...
		if(noteid != "undefined" && noteid != null && !noteid.equals("") ){
			System.out.println("22222222222222222222222222222222");
			
			query = svc.getQuery("SEL_COMBOBUYER3");
			
			sql.put(query);
			
			sql.setString(++i, noteid);
			...
			if(list.size() == 0){
				query = svc.getQuery("SEL_COMBOBUYER2");
				sql.put(query);
				list = select2List(sql);
				sql.close();
			}
