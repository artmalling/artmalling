...
function btn_Search() {
    ...
    if (isNull(EM_COND_DT.text)){
        showMessage(EXCLAMATION, OK, "USER-1003", "조회일자"); //(조회일자)은/는 반드시 입력해야 합니다.
        EM_COND_DT.focus();
        return;
    }
      
    var parameters = "&strStrCd="  + LC_STR_CD.BindColVal //점코드
                   + "&strSaleYmd=" + EM_COND_DT.text      //조회일자
                   ;
    
    IFrame.location.href="/mss/jsp/meis/meis2001.jsp?"+parameters;
}
...
function btn_Print() {
    ...
    var parameters = "&strStrCd="  + LC_STR_CD.BindColVal //점코드
                   + "&strSaleYmd=" + EM_COND_DT.text      //조회일자
                   ;
    window.open("/mss/jsp/meis/meis2001.jsp?"+parameters,"OZPRINT", 1000, 700);	 
}
...
<!--------------------- 6. 컴포넌트 이벤트 처리  ------------------------------->
<!-- 조회일자 onKillFocus() -->
<script language=JavaScript for=EM_COND_DT event=onKillFocus()>
    if(!checkDateTypeYMD(EM_COND_DT)){
    	EM_COND_DT.Text = "<%=toDay%>";
    }
</script>    
<!--*************************************************************************-->
          ...
          <tr>
            ...
            <td>
              ...
              <img src="/<%=dir%>/imgs/btn/date.gif" class="PR03"
                onclick="javascript:openCal('G',EM_COND_DT)" align="absmiddle" />
            </td>
          </tr>
        </table></td>
      </tr>
