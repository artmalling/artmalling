...
function btn_Search() {
	...
	TR_MAIN.Action="/mss/mcae106.mc?goTo="+goTo+parameters;	 
...
function btn_Save()	{
	 ...
	 //	사은행사 마스터	정보 필수입력사항
	  ...
	  DS_IO_STREVT_COPY.ImportData(strData);
	   ...
	   TR_MAIN.Action="/mss/mcae106.mc?goTo=save&strAllBrand="+encodeURIComponent(CHK_ALLBRAND.checked); 
...
<script language=JavaScript for=LC_EVENT_TYPE event=OnSelChange()>
    if(  this.BindColVal == '03' && this.Enable ){ // 보너스카드 고객 행사
        ...
        enableControl(LC_EVENT_GIFT_CYC, true);  // 강연식_20150728 -보너스카드 고객 행사 시 지급주기 변경가능
        setComboData(LC_EVENT_GIFT_CYC,"");     //  강연식_20150728 -보너스카드 고객 행사 시 지급주기 변경가능
        enableControl(LC_RECP_ADD_YN, false);
        setComboData(LC_RECP_ADD_YN,"Y");  
        enableControl(LC_RECP_TODAY_YN, false);
        setComboData(LC_RECP_TODAY_YN,"Y");  
...
//        setComboData(LC_EVENT_GIFT_CYC,"P");  
        enableControl(LC_EVENT_GIFT_CYC, true);  // 강연식_20140227 -제휴카드 행사 시 지급주기 변경가능
        setComboData(LC_EVENT_GIFT_CYC,"");     //  강연식_20140227 -제휴카드 행사 시 지급주기 변경가능
        enableControl(LC_RECP_ADD_YN, false);
        setComboData(LC_RECP_ADD_YN,"Y");  
        enableControl(LC_RECP_TODAY_YN, false);
        setComboData(LC_RECP_TODAY_YN,"Y");  
    	...
    	enableControl(LC_CARD_ONLY_YN, false);
        setComboData(LC_CARD_ONLY_YN,"N");  
        enableControl(LC_EVENT_GIFT_CYC, true);
        setComboData(LC_EVENT_GIFT_CYC,"");  
