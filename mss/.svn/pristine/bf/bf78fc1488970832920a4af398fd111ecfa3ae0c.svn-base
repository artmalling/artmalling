<!-- 
/*******************************************************************************
 * 시스템명 : 경영지원 > 임대관리 > CESBILLING > 계량기 매핑 관리
 * 작 성 일 : 2010.06.03
 * 작 성 자 : 김유완
 * 수 정 자 : 
 * 파 일 명 : MREN6030.jsp
...
var g_popFlag  = "H";   // 팝업구분 C:계약ID, H:주거ID
var g_gridName = "동호"; // 그리드명(시설구분 주거:협력사명, 비주거:동호)
/*************************************************************************
 * 1. 초기설정
 *************************************************************************/
/**
...
function doInit(){
    ...
    getFlc("DS_S_FCL_FLAG",             "M", "", "Y", "N");                                     // [조회용]시설구분:주거,비주거
...
function btn_Search() {
    if (DS_IO_MASTER.IsUpdated||DS_IO_DETAIL.IsUpdated) {//변경데이터 있을 시 확인
        ...
        if (ret == "1") {
            ...
            TR_MAIN.Action = "/mss/mren603.mr?goTo=" + goTo + parameters;
            TR_MAIN.KeyValue = "SERVLET(O:DS_IO_MASTER=DS_IO_MASTER)";
            getGlobalValSet(); //변수 SET
            TR_MAIN.Post();
            // 조회결과 Return
...
function btn_Save() {
    ...
    if (DS_IO_DETAIL.IsUpdated) {//변경 데이터가 있을때만 저장
        ...
        //저장
        var goTo = "save";
        TR_SAVE.Action = "/mss/mren603.mr?goTo=" + goTo;
 ...
 * return값 :
 */
function getGlobalValSet() {
	// 값변경
	 if(LC_S_FCL_FLAG.ValueOfIndex("HUSE_FLAG", LC_S_FCL_FLAG.index)== "2") { 
	     g_gridName =   "동호"; 
	     g_popFlag  =   "H";
	 } else {
	     g_gridName =   "협력사명"; 
...
function getDetail() {  
    ...
    var strHuseFlag = "1";
     if (g_popFlag=="C") strHuseFlag = "1"; else strHuseFlag = "2";
    var goTo = "getDetail";
    var parameters = ""
        + "&strHuseFlag ="     + encodeURIComponent(strHuseFlag)
        + "&strGaugID   ="     + encodeURIComponent(strGaugID);
    TR_SUB.Action = "/mss/mren603.mr?goTo=" + goTo + parameters;
...
//계약ID 팝업호출
    ...
    var rt = getCntrPop( strCntrId, strFlc, "", strMstGaugTy, strMstGaugKi, "S", "N", g_popFlag);
    ...
    if (validateChk) {
        ...
        DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "VEN_CD") = rt.VEN_CD;         //시설구분이 비주거:협력사코드, 주거:빈값
        DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "VEN_NM") = rt.VEN_NM;         //시설구분이 비주거:협력사명, 주거:동호
        ...
        DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "VEN_CD") = "";    //시설구분이 비주거:협력사코드, 주거:빈값
        DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "VEN_NM") = "";    //시설구분이 비주거:협력사명, 주거:동호
...
if (colid == "CNTR_ID") {
    ...
    if (strCntrId.length > 0) {
    	...
    	getCntrNonPop( "DS_O_RESULT", strCntrId, strFlc, "", strMstGaugTy, strMstGaugKi, "G", "N", "Y", g_popFlag);
        ...
        if (DS_O_RESULT.CountRow != 1 ) {
           ...
           if (validateChk) {
               ...
               DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "VEN_CD")   = DS_O_RESULT.NameValue(1, "VEN_CD");       //시설구분이 비주거:협력사코드, 주거:빈값
               DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "VEN_NM")   = DS_O_RESULT.NameValue(1, "VEN_NM");       //시설구분이 비주거:협력사명, 주거:동호
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0"
            ...
            <tr valign="top">
                <td><comment id="_NSID_"><OBJECT id=GD_MASTER
                    width=100% height=205 tabindex=1 classid=<%=Util.CLSID_GRID%>>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr valign="top">
				<td><comment id="_NSID_"><OBJECT id=GD_DETAIL
					width=100% height=231 tabindex=1 classid=<%=Util.CLSID_GRID%>>
