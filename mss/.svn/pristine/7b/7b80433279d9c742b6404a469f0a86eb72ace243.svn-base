...
public class MEis081Action extends DispatchAction {
	/*
	 * Java Pattern 에서 지원하는 logger를 사용할 수 있도록 객체를 선언
	 */
	private Logger logger = Logger.getLogger(MEis081Action.class);

	/** 
	 * <p>주간 TREND 현황  화면로딩</p>
	 */
	...
	public ActionForward chart(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			dao         = new MEis081DAO();
			strRtn1     = dao.searchWeeklySaleTrend(form, strUserId);
			strRtn2     = dao.searchWeeklyCustomInfo(form, strUserId);
			strErrorMsg = "";
		} catch (Exception e) {
			...
			request.setAttribute("xmlData1", strRtn1);
			request.setAttribute("xmlData2", strRtn2);
		}
	 ...
	 * <p>주간 매출정보조회</p>
	 */
	public ActionForward searchWeeklyTrend(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		try {
			...
			list    = dao.searchWeeklyTrend(form, strUserId);
			helper.setDataSetHeader(dSet, "H_SEL_TREND");
