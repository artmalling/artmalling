...
public class CCom222Action extends DispatchAction {
	...
	public ActionForward searchOnPop(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			helper.setDataSetHeader(dSet[1], "H_PBNPMK_SRT");
	 ...
	 * 
	 */
	public ActionForward searchCombo(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		List list = null;
		GauceHelper2 helper = null;
		...
		try {
			...
			mi = helper.getMutiInput(dSet[0]);
			helper.setDataSetHeader(dSet[1], "H_PBNPMK_SRT_COMBO");

			dao = new CCom222DAO();
			list = dao.searchCombo(form, mi);
