...
public class CCom202DAO extends AbstractDAO {
	/**
	 * 조회
	 * @param svc
	 * @return List
	...
	public List getDate(ActionForm form, MultiInput mi)
		...
		try {
			...
			sql.setString(++i, mi.getString("GTYPE"));
			sql.setString(++i, mi.getString("ISSUE"));
			...
			if (!addCondition.equals("")) {
				...
				for (int idx = 0; idx < addConds.length; idx++) {
					switch(idx){ 
				        case 0: //추가
				        	//sql.put(svc.getQuery("SEL_CODE_W_ISSUE_TYPE"));
