...
function doInit(){
    ...
    // EMedit에 초기화
	initEmEdit(EM_BRCH_NAME, "GEN^40",       NORMAL);	   //가맹점명
	initEmEdit(EM_COMP_NO,	 "#00-00-00000", NORMAL);	   //사업자번호
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}				name="NO"				width=30	align=center</FC>'
					 + '<FC>id=BRCH_ID				name="가맹점코드"		width=80	align=center</FC>'
					 + '<FC>id=BRCH_NAME			name="가맹점명"			width=100	align=left</FC>'
					 + '<FC>id=STR_CD               name="소속점"            width=90    align=left  show=false</FC>'
                     + '<FC>id=STR_NAME				name="소속점명"			width=90	align=left</FC>'
                     + '<FC>id=COMP_NO				name="사업자번호"		width=95	align=center  mask="XXX-XX-XXXXX"</FC>'
                     + '<FC>id=REP_BRAND_NM			name="브랜드명"			width="120" </FC>';
...
function showMaster(){
	...
	if(trim(EM_BRCH_NAME.text) == ""){
		//showMessage(STOPSIGN, OK, "USER-1047",  "1");
		//return;
	}

    var strbrchName = EM_BRCH_NAME.text;
	var strcompNo	= EM_COMP_NO.text;

    var goTo       = "searchMaster" ;    
    var action     = "O";     
    var parameters = "&strbrchName="+encodeURIComponent(strbrchName)+
					 "&strcompNo="+encodeURIComponent(strcompNo);
    TR_MAIN.Action="/<%=dir%>/ccom400.cc?goTo="+goTo+parameters;  
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="396" class="title"><img
							...
							align="absmiddle" class="popR05 PL03" /> 가맹점 조회</td>
						<td>
					...
					<tr>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							...
							<tr>
								...
								<td><comment id="_NSID_"> <object id=EM_BRCH_NAME
									classid=<%=Util.CLSID_EMEDIT%>
									onkeyup="javascript:checkByteStr(EM_BRCH_NAME, 40, '', '', '');"
									width=150 tabindex=1 align="absmiddle"></object> </comment><script> _ws_(_NSID_);</script></td>
							</tr>
							<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td><!-- 마스터 --> <comment id="_NSID_"> <OBJECT
							id=GD_MASTER width="540" height=310 classid=<%=Util.CLSID_GRID%>
							tabindex=1>
