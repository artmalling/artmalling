    ...
    /**
        ...
        */
        ...
        {   
            ...
            // EMedit 초기화
            initEmEdit(EM_ORG_CD, "CODE^10^#", NORMAL);
            initEmEdit(EM_ORG_NM, "GEN^40", NORMAL);

            //Combo 초기화
            initComboStyle(LC_STR_CD,DS_STR_CD, "CODE^0^20,NAME^0^80", 1, NORMAL);  //점
            initComboStyle(LC_ORG_LVL,DS_ORG_LVL, "CODE^0^20,NAME^0^80", 1, NORMAL);  //조직레벨
            
            //시스템 코드 공통코드에서 가지고 오기( popup.js )
            getEtcCode("DS_ORG_LVL", "D", "P048", "Y");
            
            //점콤보 가져오기 ( popup_dps.js )
            getStore("DS_STR_CD", "N", "", "Y");
            
            //코너제외
            DS_ORG_LVL.DeleteRow(DS_ORG_LVL.NameValueRow("CODE","5")); 

            //콤보데이터 기본값 설정( gauce.js )
            setComboData(LC_STR_CD, getStrCd());
            setComboData(LC_ORG_LVL, getOrgLvl());
            
            // Input Data Set Header 초기화 
            DS_I_COND.setDataHeader  ('ORG_CD:STRING(10),ORG_NM:STRING(40),AUTH_GB:STRING(1),ADD_CONDITION:STRING(200)');
            DS_I_COND.ClearData();
            DS_I_COND.Addrow();
        ...
        function gridCreate1(){
            var hdrProperies = '<FC>ID="STR_CD"     name="점코드"     width=80,   edit=none,   align=left, show=false</FC>'
                             + '<FC>ID="DEPT_CD"    name="부서코드"   width=80,   edit=none,   align=left, show=false </FC>'
                             + '<FC>ID="TEAM_CD"    name="팀코드"     width=80,   edit=none,   align=left, show=false  </FC>'
                             + '<FC>ID="PC_CD"      name="PC코드"     width=80,   edit=none,   align=left, show=false </FC>'
                             + '<FC>ID="ORG_CD"     name="조직코드"   width=80,   edit=none,   align=center </FC>'
                             + '<FC>ID="ORG_NAME"   name="조직명"     width=205,  edit=none,   align=left,  </FC>';
            if( multiGb == "M"){
                hdrProperies = '<FC>ID="SEL"        name="선택"       width=30,   align=center, EditStyle="CheckBox"</FC>'
                	         + '<FC>ID="STR_CD"     name="점코드"     width=80,   edit=none,   align=left, show=false</FC>'
                             + '<FC>ID="DEPT_CD"    name="부서코드"   width=80,   edit=none,   align=left, show=false </FC>'
                             + '<FC>ID="TEAM_CD"    name="팀코드"     width=80,   edit=none,   align=left, show=false  </FC>'
                             + '<FC>ID="PC_CD"      name="PC코드"     width=80,   edit=none,   align=left, show=false </FC>'
                             + '<FC>ID="ORG_CD"     name="조직코드"   width=80,   edit=none,   align=center </FC>'
                             + '<FC>ID="ORG_NAME"   name="조직명"     width=175,  edit=none,   align=left,  </FC>';
            }                 
 ...
 *************************************************************************/ 
  ...
  */
  ...
  { 
      ...
      TR_MAIN.Action="/pot/ccom008.cc?goTo=searchOnPop";
...
function getStrCd(){
    ...
    if( addConditions.length < 6)
        return '%';
    
    return addConditions[5];
}
...
function getOrgLvl(){
    if( addCondition == '')
        return '%';
    var addConditions = addCondition.split('#G#');
    if( addConditions.length < 4)
        return '%';
    
    return addConditions[3];
}
...
function getNewAddCondition(){
    var orgLvl = LC_ORG_LVL.BindColVal;
    var strCd = LC_STR_CD.BindColVal;
	        
    var addConditions = addCondition.split('#G#');
	    
    addConditions[3] = orgLvl;
    addConditions[5] = strCd;
...
<comment id="_NSID_"><object id=DS_ORG_LVL classid=<%=Util.CLSID_DATASET%>></object></comment><script>_ws_(_NSID_);</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td class="popT01 PB03"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
              <tr>
                <th width="50">점</th>
                <td width="75">
                <comment id="_NSID_">
                    <object id=LC_STR_CD classid=<%=Util.CLSID_LUXECOMBO%> height=20 width=75 >
                    </object>
                ...
                <td>
                <comment id="_NSID_">
                    <object id=LC_ORG_LVL classid=<%=Util.CLSID_LUXECOMBO%> height=20 width=100 >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
              </tr>
              <tr>
                ...
                <td colspan = "3">
                <comment id="_NSID_">
                    <object id=EM_ORG_CD classid=<%=Util.CLSID_EMEDIT%> height=20 width=150 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
              </tr>
              <tr>
                ...
                <td colspan = "3">
                <comment id="_NSID_">
                    <object id=EM_ORG_NM classid=<%=Util.CLSID_EMEDIT%> height=20 width=245 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
              </tr>
      ...
      <tr>
        ...
        <td>
            <!-- 마스터 -->
            <comment id="_NSID_"><object id=GD_SEARCH height="260px" width="100%" classid=<%=Util.CLSID_GRID%>>
