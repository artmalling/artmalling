...
<!--*************************************************************************-->
<%
	String flag = (request.getParameter("flag") != null && !"".equals(request.getParameter("flag").trim())) ? request.getParameter("flag") : "";
...
function doInit(){
    ...
    //EMedit에 초기화
    initEmEdit(EM_CUST_NAME_S,   "GEN^40", NORMAL);                  //법인명
    initEmEdit(EM_OFFI_PH1_S,    "000",    NORMAL);                  //휴대폰번호1
    initEmEdit(EM_OFFI_PH2_S,    "0000",   NORMAL);                  //휴대폰번호2
    initEmEdit(EM_OFFI_PH3_S,    "0000",   NORMAL);                  //휴대폰번호3
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}        name="NO"                width=30   align=center</FC>'
                     + '<FC>id=CUST_ID         name="법인코드"          width=80    align=center</FC>'
                     + '<FC>id=CUST_NAME       name="법인명"            width=100   align=left</FC>'
                     + '<FC>id=OFFI_PH1        name="담당자전화"        width=96    align=center</FC>'
                     + '<FC>id=OFFI_PH2        name="담당자전화"        width=0     align=center</FC>'
                     + '<FC>id=OFFI_PH3        name="담당자전화"        width=0     align=center</FC>'
                     + '<FC>id=HOME_PH1        name="대표전화"          width=96    align=center</FC>'
                     + '<FC>id=HOME_PH2        name="대표전화"          width=0     align=center</FC>'
                     + '<FC>id=HOME_PH3        name="대표전화"          width=0     align=center</FC>'
                     + '<FC>id=EMAIL1          name="이메일주소"        width=130   align=left</FC>'
                     + '<FC>id=EMAIL2          name="이메일주소"        width=0 </FC>'
                     + '<FC>id=HOME_ADDR       name="사업장주소"        width=200   align=left</FC>';
...
function btn_Search() {
	...
	if(trim(EM_CUST_NAME_S.Text).length == 0 && trim(EM_OFFI_PH1_S.Text).length == 0 
	  && trim(EM_OFFI_PH2_S.Text).length == 0 && trim(EM_OFFI_PH2_S.Text).length == 0
	  && trim(EM_OFFI_PH3_S.Text).length == 0 && trim(EM_HOME_PH1_S.Text).length == 0
	...
	if(trim(EM_CUST_NAME_S.text).length != 0){
        if(trim(EM_CUST_NAME_S.text).length < 2){
            showMessage(EXCLAMATION, OK, "USER-1047",  msg+"은 2");
            EM_CUST_NAME_S.Focus();
            return;
        }
    }
   　  if(trim(EM_OFFI_PH1_S.Text) != "" && (trim(EM_OFFI_PH2_S.Text) == "" || trim(EM_OFFI_PH3_S.Text) == "")
	 || trim(EM_OFFI_PH2_S.Text) != "" && (trim(EM_OFFI_PH1_S.Text) == "" || trim(EM_OFFI_PH3_S.Text) == "")
	 || trim(EM_OFFI_PH3_S.Text) != "" && (trim(EM_OFFI_PH1_S.Text) == "" || trim(EM_OFFI_PH2_S.Text) == "")){
	    showMessage(EXCLAMATION, OK, "USER-1000",  "담당자전화번호를  정확히 입력하세요.");
	    if(trim(EM_OFFI_PH1_S.Text) == ""){
	        EM_OFFI_PH1_S.Focus();
	    }else if(trim(EM_OFFI_PH2_S.Text) == ""){
	        EM_OFFI_PH2_S.Focus();
	    }else if(trim(EM_OFFI_PH3_S.Text) == ""){
	        EM_OFFI_PH3_S.Focus();
	    }
	    return false;
	}
    if(trim(EM_HOME_PH1_S.Text) != "" && (trim(EM_HOME_PH2_S.Text) == "" || trim(EM_HOME_PH3_S.Text) == "")
     ...
     || trim(EM_HOME_PH3_S.Text) != "" && (trim(EM_HOME_PH1_S.Text) == "" || trim(EM_HOME_PH2_S.Text) == "")){
        showMessage(EXCLAMATION, OK, "USER-1000",  "대표전화번호를  정확히 입력하세요.");
...
function showMaster(){
	...
	DS_I_CONDITION.setDataHeader( 'SERVICE_ID:STRING(50),COMP_PERS_FLAG:STRING(1),CUST_NAME:STRING(40),OFFI_PH1:STRING(40),OFFI_PH2:STRING(4),OFFI_PH3:STRING(4),HOME_PH1:STRING(40),HOME_PH2:STRING(4),HOME_PH3:STRING(4),EMAIL1:STRING(40),EMAIL2:STRING(40),SCR_ID:STRING(7)');
	
    var strCompPersFlag = '<%=flag%>';
    var strCustName = EM_CUST_NAME_S.text;
    var strMphone1  = EM_OFFI_PH1_S.text;
    var strMphone2  = EM_OFFI_PH2_S.text;
    var strMphone3  = EM_OFFI_PH3_S.text;
    ...
    DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "OFFI_PH1")   = strMphone1;
    DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "OFFI_PH2")   = strMphone2;
    DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "OFFI_PH3")   = strMphone3;
    ...
    TR_MAIN.Action   = "/<%=dir%>/ccom420.cc?goTo="+goTo; 
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="396" class="title"><img
							...
							align="absmiddle" class="popR05 PL03" /> <span id="title"
							style="font-size: 14px; color: #000000; font-weight: bold;">법인명
						</span>조회</td>
						<td>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							class="s_table">
							<tr>
								<th width="100"><span id="titleCust" style="Color: 146ab9">법인명</span></th>
								<td><comment id="_NSID_"> <object
									id=EM_CUST_NAME_S classid=<%=Util.CLSID_EMEDIT%> width=132
									tabindex=1 align="absmiddle" onkeyup="javascript:checkByteStr(EM_CUST_NAME_S, 40, '');" onkeydown="if(event.keyCode==13){fn_EnterSrch()}"></object>
								</comment> <script> _ws_(_NSID_);</script>&nbsp;(2자 이상 입력하세요.)</td>
							</tr>
							<tr>
								<th width="100">담당자전화</th>
								<td><comment id="_NSID_"> <object
									id=EM_OFFI_PH1_S classid=<%=Util.CLSID_EMEDIT%> width=30
									tabindex=1 align="absmiddle"></object> </comment> <script> _ws_(_NSID_);</script>
								- <comment id="_NSID_"> <object id=EM_OFFI_PH2_S
									classid=<%=Util.CLSID_EMEDIT%> width=40 tabindex=1
									align="absmiddle"></object> </comment> <script> _ws_(_NSID_);</script> -
								<comment id="_NSID_"> <object id=EM_OFFI_PH3_S
									classid=<%=Util.CLSID_EMEDIT%> width=40 tabindex=1
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td><!-- 마스터 --> <comment id="_NSID_"> <object
							id=GR_MASTER width="540" height=250 classid=<%=Util.CLSID_GRID%> tabindex=1>
