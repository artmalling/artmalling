...
{   
    ...
    if(addConditions[7] == "1"){
    	LC_STR_CD.Enable = false;
    	LC_ORG_LVL.Enable = false;
    }
...
function gridCreate1(){
    var hdrProperies = '<FC>ID="STR_CD"     name="점코드"     width=80,   edit=none,  align=left, show=false</FC>'
                     + '<FC>ID="DPET_CD"    name="부서코드"   width=80,  edit=none,   align=left, show=false </FC>'
                     ...
                     + '<FC>ID="CORNER_CD"  name="코너코드"   width=80,  edit=none,   align=left, show=false </FC>'
...
{ 
    ...
    TR_MAIN.Action="/pot/ccom007.cc?goTo=searchOnPop";
...
function getStrCd(){
    ...
    return addConditions[5]==''?'%':addConditions[5];
}
/**
 * getOrgLvl()
 * 작 성 자 : FKL
...
function getOrgLvl(){
	...
	return addConditions[3]==''?'%':addConditions[3];
}
