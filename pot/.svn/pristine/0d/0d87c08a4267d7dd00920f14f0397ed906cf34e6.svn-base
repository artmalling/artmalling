...
{     	
    ...
    // EMedit 초기화
    initEmEdit(EM_POS_NO, "CODE^4^0", NORMAL);
    initEmEdit(EM_POS_NAME, "GEN^20", NORMAL);

    // Input Data Set Header 초기화 
	var strDataHd  = "";
	strDataHd  = strDataHd  = "STR_CD:STRING(2),POS_NO:STRING(40),PID:STRING(7),SQL_GB:STRING(100)";
    ...
    EM_POS_NO.Focus();
    // 자동조회
	var strPosNo = gArrParams.POS_NO != "undefined" ? gArrParams.POS_NO : "";
    if(strPosNo.length > 0){
    	EM_POS_NO.Text = strPosNo;
    	btn_Search();
    }
} 
...
function gridCreate1(){
	...
	var hdrProperies = '<FC>id={currow}     name="NO"       width=40    align=center  edit=none </FC>'
                     ...
                     + '<FC>ID="POS_NO"     name="POS번호"  width=60,   edit=none     align=center  </FC>'
                     + '<FC>ID="POS_NAME"   name="POS명"    width=80,   edit=none     align=left    </FC>'
                     + '<FC>id=HALL_CD      name="관"       width=60    edit=none     align=left    EditStyle=Lookup Data="DS_HALL_CD:CODE:NAME" </FC>'
                     + '<FC>id=FLOR_CD      name="층"       width=80    edit=none     align=left    EditStyle=Lookup Data="DS_FLOR_CD:CODE:NAME" </FC>'
    ;
...
function btn_Search(){ 
	...
	DS_I_COND.NameValue(nRow, "POS_NO") = EM_POS_NO.Text;
...
function btn_Conf(){
    ...
    if(gStrMulti != "M"){	// 멀티 일 경우
    	...
    	arrList["POS_NO"]    = DS_O_RESULT.NameValue(nRow, "POS_NO");
    	arrList["SHOP_NAME"] = DS_O_RESULT.NameValue(nRow, "SHOP_NAME");
    	arrList["POS_NAME"]  = DS_O_RESULT.NameValue(nRow, "POS_NAME");
    	arrList["POS_FLAG"]  = DS_O_RESULT.NameValue(nRow, "POS_FLAG");
    	arrList["FLOR_CD"]   = DS_O_RESULT.NameValue(nRow, "FLOR_CD");
    	arrList["HALL_CD"]   = DS_O_RESULT.NameValue(nRow, "HALL_CD");
  ...
  <tr>
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="" class="title">
              ...
              <span id="title1" class="PL03">POS번호</span>
            </td>
        ...
        <td class="popT01 PB03"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="o_table">
          <tr>
            <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
              <tr>
                <th width="60">POS번호</th>
                <td width="66">
                <comment id="_NSID_">
                    <object id=EM_POS_NO classid=<%=Util.CLSID_EMEDIT%> height=20 width=65 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
              ...
              <tr>
                ...
                <td>
                <comment id="_NSID_">
                    <object id=EM_POS_NAME classid=<%=Util.CLSID_EMEDIT%> height=20 width=245 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
              </tr>
      ...
      <tr>
        ...
        <td>
            <!-- 마스터 -->
            <comment id="_NSID_"><object id=GD_SEARCH height="375px" width="100%" classid=<%=Util.CLSID_GRID%>>
