...
public class NamoMime
{
	...
	*/
	...
	protected void splitSinglePart(String encodedString) throws IOException
	{
		...
		while(((line = br.readLine()) != null) && (nowState != 5))							// 한줄씩 읽어 들인다
		{
			line = line.trim();
			...
			else
			{
				...
				{
					//System.out.println(nowState);//wook
					...
					{
						System.out.println(compare.indexOf("name"));//wook
	...
	public void saveFile() throws MimeDecodeException
	{
		...
		for(i = 1; i < decodePart.size(); i++)
		{
			...
			try
			{
				...
				outFile = new File(getWritableFileName(savePath + "/" + part.getName()+ System.getProperty("file.separator")));	// 파일을 새로 생성한다
				
				//outFile = new File(getWritableFileName(savePath + "/" + part.getName()));									// 파일을 새로 생성한다
				System.out.println(savePath+ "/" + part.getName());
