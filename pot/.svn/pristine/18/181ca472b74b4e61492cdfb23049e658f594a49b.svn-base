 ...
 * 1. 초기설정
 *************************************************************************/
var bizCd  = dialogArguments[0];
var planYm = dialogArguments[1];
var type   = dialogArguments[2];
/**
 * doInit()
 * 작 성 자 : 이정식
 * 작 성 일 : 2010-05-11
...
function setEmEdit(){
    ...
    initEmEdit(EM_SEL_NAME, "GEN^60", NORMAL);
    EM_SEL_CODE.Text = bizCd;
}
...
function gridCreate(){
	var hdrProperies = '<C>id={currow} width=30  align=center name="NO"</C>'
                     + '<C>id=CODE     width=90  align=center name="실적항목코드" </C>'
                     + '<C>id=NAME     width=170 align=left   name="실적항목명" </C>'
                     ;
...
function btn_Search(){
	...
	//3. 조회시작
	var goTo       = "getBizPlanList";
	var action     = "O";
	var parameters = "&strCode="   + encodeURIComponent(EM_SEL_CODE.Text) //코드
	               + "&strName="   + encodeURIComponent(EM_SEL_NAME.Text) //명
	               + "&strPlanYm=" + encodeURIComponent(planYm) //계획년월
	               + "&strType="   + encodeURIComponent(type)
	               ;
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td width="" class="title">
                  ...
                  <SPAN id="title1" class="PL03">경영실적항목</SPAN>
                </td>
                <td>
