<!-- 
/*******************************************************************************
 * 시스템명 : POS 영수증 메세지 조회  팝업
 * 작 성 일 : 2010.02.17
 * 작 성 자 : 이재득
 * 수 정 자 : 
 * 파 일 명 : ccom0200.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : POS 영수증 메세지 조회  팝업
 * 이    력 :
 ******************************************************************************/
--> 
 ...
 *************************************************************************/
 ...
 var msgId          = dialogArguments[1];
 var msgFlag        = dialogArguments[2];
...
{     	
    ...
    // EMedit 초기화
    initEmEdit(EM_MSG_ID,   "CODE^4^0", NORMAL);
    initEmEdit(EM_MSG_EXPL, "GEN^40", NORMAL);

    // Input Data Set Header 초기화 
    DS_I_COND.setDataHeader  ('MSG_EXPL:STRING(40),MSG_ID:STRING(4),ADD_CONDITION:STRING(200)');
    DS_I_COND.ClearData();
    DS_I_COND.Addrow();
    
    DS_I_COND.NameValue(DS_I_COND.RowPosition, "MSG_EXPL") = msgFlag; 
    DS_I_COND.NameValue(DS_I_COND.RowPosition, "MSG_ID") = msgId;    
    DS_I_COND.NameValue(DS_I_COND.RowPosition, "ADD_CONDITION") = addCondition;

    EM_MSG_ID.Focus();
    if(msgId.length > 0 || msgFlag.length > 0   ){
        btn_Search();
    }
} 

function gridCreate1(){
    var hdrProperies = '<FC>ID="MSG_FLAG"     name="메세지구분"       width=100,   edit=none,   align=left, SHOW=false</FC>'
                     + '<FC>ID="MSG_ID"       name="메세지ID"         width=100,   edit=none,   align=left</FC>'
                     + '<FC>ID="MSG_EXPL"     name="메세지명"         width=180,   edit=none,   align=left, </FC>';
    if( multiGb == "M"){
        hdrProperies = '<FC>ID="SEL"          name="선택"             width=30,   align=center, EditStyle="CheckBox"</FC>'
                     + '<FC>ID="MSG_FLAG"     name="메세지구분"       width=100,   edit=none,   align=left, SHOW=false</FC>'
                     + '<FC>ID="MSG_ID"       name="메세지ID"         width=100,   edit=none,   align=left</FC>'
                     + '<FC>ID="MSG_EXPL"     name="메세지명"         width=180,   edit=none,   align=left, </FC>';
    }                 
 ...
 *************************************************************************/ 
  ...
  */
  ...
  { 
      ...
      TR_MAIN.Action="/pot/ccom020.cc?goTo=searchOnPop";
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="" class="title">
              ...
              <span id="title1" class="PL03">POS 영수증 메세지</span>
            </td>
        ...
        <td class="popT01 PB03"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="o_table">
          <tr>
            <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
              <tr>
                <th width="80">메세지ID</th>
                <td >
                <comment id="_NSID_">
                    <object id=EM_MSG_ID classid=<%=Util.CLSID_EMEDIT%> height=20 width=110 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
              ...
              <tr>
                ...
                <td>
                <comment id="_NSID_">
                    <object id=EM_MSG_EXPL classid=<%=Util.CLSID_EMEDIT%> height=20 width=245 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
              </tr>
...
<object id=BO_SEARCH classid=<%= Util.CLSID_BIND %>>
        ...
        <param name=BindInfo        value='
            <c>col=MSG_ID                ctrl=EM_MSG_ID               param=Text</c>
            <c>col=MSG_EXPL              ctrl=EM_MSG_EXPL             param=Text</c>
        '>
</object>
</comment><script>_ws_(_NSID_);</script>
</body>
