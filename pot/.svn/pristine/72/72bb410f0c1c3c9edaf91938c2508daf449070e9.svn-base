...
public class TCom301DAO extends AbstractDAO { 
        ...
        try { 
			...
			*/
			...
			if( !strSystemGbn.equals("%")){
				...
				strQuery += svc.getQuery("SEL_LOG_WHERE_SUB_SYSTEM") + "\n";
			}
			
			if( !strLcode.equals("%")){
				sql.setString(i++, strLcode);
				strQuery += svc.getQuery("SEL_LOG_WHERE_LCODE") + "\n";
			}

			if( !strMcode.equals("%")){
				sql.setString(i++, strMcode);
				strQuery += svc.getQuery("SEL_LOG_WHERE_MCODE") + "\n";
			}

			if( !strScode.equals("%")){
				sql.setString(i++, strScode);
				strQuery += svc.getQuery("SEL_LOG_WHERE_SCODE") + "\n";
			}
 
			if( !strUserCd.equals("")){
				sql.setString(i++, strUserCd);
				sql.setString(i++, strUserCd);
				strQuery += svc.getQuery("SEL_LOG_WHERE_USER_CD") + "\n";
			}
			
			if( !strSdate.equals("")&&!strEdate.equals("")){ 
				sql.setString(i++, strSdate);
				sql.setString(i++, strEdate);
				strQuery += svc.getQuery("SEL_LOG_WHERE_DOWN_DATE") + "\n";
			}
