 ...
 *************************************************************************/
 ...
 var arggaugGbn     = dialogArguments[3];
 var argMgbn        = dialogArguments[4];
 var argUseYn       = dialogArguments[5];
 var argGaugKind    = dialogArguments[6];
 var addCondition   = dialogArguments[7];
...
{       
    ...
    initComboStyle(LC_SEL_GAUG_TYPE,DS_SEL_GAUG_TYPE,   "CODE^0^30,NAME^0^80", 1, NORMAL);  // [조회용]계량기구분(조회)

    getEtcCode("DS_SEL_GAUG_TYPE",      "D", "M039", "Y", "N", LC_SEL_GAUG_TYPE);       // [조회용]계량기구분
    getEtcCode("DS_GAUG_LVL",           "D", "M086", "N");                              // 계량기레벨
    
    EM_SEL_CODE.Text = argCd;    
    LC_SEL_GAUG_TYPE.BindColVal = arggaugGbn;
    if (argUseYn == "T") LC_SEL_GAUG_TYPE.Enable = false;
    EM_SEL_CODE.Focus();
...
function gridCreate(mGbn){
    ...
    if (mGbn == "M") {
        hdrProperies = ''
            ...
            + '                                                 WIDTH=100PX     EDIT=NONE   ALIGN=LEFT</FC>'
            + '<FC>ID=GAUG_LVL      NAME="계량기레벨"'         
            + '                                                 WIDTH=100PX     EDIT=NONE   ALIGN=LEFT      EDITSTYLE=LOOKUP   DATA="DS_GAUG_LVL:CODE:NAME"</FC>'
            + '<FC>ID=VALID_S_DT    NAME="유효시작일자"'         
            + '                                                 WIDTH=100PX     EDIT=NONE   ALIGN=CENTER    MASK="XXXX/XX/XX" </FC>'
 ...
 *************************************************************************/ 
  ...
  */
  ...
  { 
        ...
        DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "GAUGTYPE")= LC_SEL_GAUG_TYPE.BindColVal; 
        ...
        DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "GAUGKIND") = argGaugKind; 
  ...
  */  
  function doDoubleClick(row)
  {
      ...
      if (DS_O_RESULT.NameValue(row, "USE_YN") == "N" ) {
          showMessage(INFORMATION, OK, "USER-1000", "선택한 계량기 ID는 이미 타 상위ID에 등록되어 있습니다.");
          return; 
      }
...
if (argMgbn == "M") { 
	if (bCheck == '1'){ // 전체체크
        for(var i=1; i<=DS_O_RESULT.CountRow; i++){
        	if (DS_O_RESULT.NameValue(i, "USE_YN") == "Y" ) { //사용가능계량기 ID만 처리
...
<comment id="_NSID_"><object id=DS_SEL_GAUG_TYPE    classid=<%=Util.CLSID_DATASET%>></object></comment><script>_ws_(_NSID_);</script>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0"
                    ...
                    <tr>
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0"
                            class="s_table">
                            <tr>
                                <th width="60">계량기ID</th>
                                <td width="65"><comment id="_NSID_"> <object id=EM_SEL_CODE
		                        ...
		                        <th width="60">계량기구분</th>
		                        <td ><comment id="_NSID_"> <object
		                            id=LC_SEL_GAUG_TYPE classid=<%= Util.CLSID_LUXECOMBO %> width="100%"
		                            height=20 align="absmiddle"> </object> </comment><script>_ws_(_NSID_);</script>
		                        </td>
                            </tr>
                        </table>
                        </td>
