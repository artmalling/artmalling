...
public class CCom010DAO extends AbstractDAO {
    /*
    ...
    public List searchOnPop(ActionForm form, MultiInput mi, boolean codeLikeGb) throws Exception {
      ...
      try {
        ...
        if( !mi.getString("PUMMOK_CD").equals("") ){
        	if(codeLikeGb)
        		sql.put(svc.getQuery("SEL_PBNPMK_WHERE_LIKE_PUMMOK_CD"));
        	else
        ...
        if( !mi.getString("PUMMOK_NM").equals("") ){
            sql.put(svc.getQuery("SEL_PBNPMK_WHERE_PUMMOK_NM"));
				...
				switch(idx){
				    case 0: //신선제품여부
				        if( !addConds[idx].equals("") && !addConds[idx].equals("%") ){
				            sql.put(svc.getQuery("SEL_PBNPMK_WHERE_FRESH_YN"));
				            sql.setString(++i, addConds[idx] );
				        }
				    	break;
				    case 1:  //사용여부
				        if( !addConds[idx].equals("") && !addConds[idx].equals("%") ){
				            sql.put(svc.getQuery("SEL_PBNPMK_WHERE_USE_YN"));
				            sql.setString(++i, addConds[idx]);
				            sql.setString(++i, addConds[idx]);
				        }
				    	break;
    ...
    public List searchCombo(ActionForm form, MultiInput mi) throws Exception {
      ...
      try {
        ...
        sql.put(svc.getQuery("SEL_PBNPMK_COMBO"));
        sql.setString(++i, mi.getString("PUMBUN_CD"));
        if( !mi.getString("USE_YN").equals("")){
            sql.put(svc.getQuery("SEL_PBNPMK_COMBO_WHERE_USE_YN"));
            sql.setString(++i, mi.getString("USE_YN"));        	
        }
        sql.put(svc.getQuery("SEL_PBNPMK_COMBO_ORDER"));        
        list = select2List(sql);
