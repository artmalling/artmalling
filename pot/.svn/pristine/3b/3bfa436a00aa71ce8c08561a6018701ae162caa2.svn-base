...
function doInit()
{
    ...
    DS_O_NOTI_USER_LIST.setDataHeader('<gauce:dataset name="H_NOTI_USER_LIST"/>');
...
function gridCreate2(){
    ...
    var hdrProperies =  '<FC>id={currow}		width=30	align=center	name="NO"													</FC>'
       				  + '<FC>id=USER_ID			width=60					name="사번"			edit=none								</FC>'
                      + '<FC>ID=USER_NAME		width=70	align=center	name="성명"			edit=none								</FC>' 
                      + '<FC>ID=FIR_READ_DATE	width=130	align=center	name="조회일시"    	edit=none	Mask="XXXX/XX/XX XX:XX:XX"	</FC>';

    initGridStyle(GD_NOTI_USER_LIST, "common", hdrProperies, true );  
}
...
function btn_Search() { 
	...
	; 
     ...
     TR_MAIN.Action="/pot/tcom204.tc?goTo="+goTo+parameters;  
 ...
 * return값 : void
 */
function fn_NotiUserSearch(strNotiID) {
	var goTo		= "selectNotiUserList" ;
	var action		= "O";

	var parameters  = "&strNotiID=" +encodeURIComponent(strNotiID);

	TR_MAIN2.Action="/pot/tcom204.tc?goTo="+goTo+parameters;
	TR_MAIN2.KeyValue="SERVLET("+action+":DS_O_NOTI_USER_LIST=DS_O_NOTI_USER_LIST)"; //조회는 O
	TR_MAIN2.Post();

	// 조회결과 Return
	setPorcCount("SELECT", DS_O_NOTI_USER_LIST.CountRow);
}
</script>
</head>
...
<!--------------------- 3. DataSet Success 메세지 처리  ----------------------->
<script language=JavaScript for=DS_O_NOTI_LIST event=OnLoadCompleted(rowcnt)>

	//alert(rowcnt);
	//alert(DS_O_NOTI_LIST.NameValue(rowcnt , "NOTI_ID"));
	//사용자 조회
	fn_NotiUserSearch(DS_O_NOTI_LIST.NameValue(rowcnt , "NOTI_ID"));
</script>
<!--------------------- 4. DataSet Fail 메세지 처리  -------------------------->
<!--------------------- 5. DataSet 이벤트 처리  ------------------------------->   
<!--------------------- 6. 컴포넌트 이벤트 처리  ------------------------------> 
...
<script language=JavaScript for=GD_NOTI_LIST event=OnClick(row,colid)>
    ...
    //사용자 조회
    fn_NotiUserSearch(DS_O_NOTI_LIST.NameValue(row , "NOTI_ID"));
</script>  
...
<script language=JavaScript for=GD_NOTI_LIST event=OnDblClick(row,colid)>
	...
	var returnVal =  window.showModalDialog("/pot/tcom204.tc?goTo=writePop"
	                , arrArg
	                , "dialogWidth:900px;dialogHeight:755px;scroll:no;" +
...
<comment id="_NSID_"><object id="DS_O_NOTI_USER_LIST" classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
    ...
    <tr>
    	<td>
    		<table cellpadding="0" border="0" cellspacing="0" class="BD4A" width="100%">
    			<tr>
					<td class="PT01 PB03" valign="top" width="70%">
						<table cellpadding="0" border="0" cellspacing="0" class="BD4A"  width="100%">
							<tr>
								<td>
									<comment id="_NSID_">
										<OBJECT id=GD_NOTI_LIST width="100%" height="453" classid=<%=Util.CLSID_GRID%>>
					...
					<td width="5"></td>
					<td class="PT01 PB03" valign="top" width="29%">
						<table cellpadding="0" border="0" cellspacing="0" class="BD4A"  width="100%">
							<tr>
								<td>
									<comment id="_NSID_">
										<OBJECT id=GD_NOTI_USER_LIST width="100%" height="453" classid=<%=Util.CLSID_GRID%>>
			                				<param name="DataID" value="DS_O_NOTI_USER_LIST">
			            				</OBJECT>
			            			</comment>
			            			<script>_ws_(_NSID_);</script> 
								</td>
