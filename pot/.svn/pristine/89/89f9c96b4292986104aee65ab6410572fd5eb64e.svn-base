 ...
 *************************************************************************/
 ...
 var fnbCornerCd      = dialogArguments[1];
 var fnbCornerNm      = dialogArguments[2];
...
{   
    ...
    var fnbFlag = getFnbShopFlag();
    if(fnbFlag !=''){
        if(fnbFlag=="1"){
            title1.innerText            = "메뉴분류";
            TH_MENU_FLAG_CD.innerText   = "메뉴분류코드";
            TH_MENU_FLAG_NAME.innerText = "메뉴분류명";
            document.title              = "메뉴분류 -POPUP";
            GD_SEARCH.ColumnProp("MENU_FLAG_CD","name")   = "메뉴분류코드";
            GD_SEARCH.ColumnProp("MENU_FLAG_NAME","name") = "메뉴분류명";
        }else{
            title1.innerText            = "F&B코너";
            TH_MENU_FLAG_CD.innerText   = "코너코드";
            TH_MENU_FLAG_NAME.innerText = "코너명";
            document.title              = "F&B코너 -POPUP";
            GD_SEARCH.ColumnProp("MENU_FLAG_CD","name")   = "코너코드";
            GD_SEARCH.ColumnProp("MENU_FLAG_NAME","name") = "코너명";
            
        }
    }
            
    // EMedit 초기화
    initEmEdit(EM_MENU_FLAG_CD, "CODE^2", NORMAL);
    initEmEdit(EM_MENU_FLAG_NAME, "GEN^20", NORMAL);

    // Input Data Set Header 초기화 
    DS_I_COND.setDataHeader  ('MENU_FLAG_CD:STRING(2),MENU_FLAG_NAME:STRING(20),ADD_CONDITION:STRING(200)');
    DS_I_COND.ClearData();
    DS_I_COND.Addrow();

    DS_I_COND.NameValue(DS_I_COND.RowPosition, "MENU_FLAG_CD") = fnbCornerCd; 
    DS_I_COND.NameValue(DS_I_COND.RowPosition, "MENU_FLAG_NAME") = fnbCornerNm; 
    DS_I_COND.NameValue(DS_I_COND.RowPosition, "ADD_CONDITION") = addCondition;

    EM_MENU_FLAG_CD.Focus();
    if(fnbCornerCd.length > 0 || fnbCornerNm.length > 0   ){
        btn_Search();
    }
} 

function gridCreate1(){
    var hdrProperies = '<FC>ID="MENU_FLAG_CD"     name="코너코드"       width=100,   edit=none,   align=center</FC>'
                     + '<FC>ID="MENU_FLAG_NAME"   name="코너명"         width=180,  edit=none,   align=left  </FC>';
    if( multiGb == "M"){
        hdrProperies = '<FC>ID="SEL"              name="선택"     width=30,   align=center, EditStyle="CheckBox"</FC>'
                     + '<FC>ID="MENU_FLAG_CD"     name="코너코드"       width=100,   edit=none,   align=left, SORT=TRUE</FC>'
                     + '<FC>ID="MENU_FLAG_NAME"   name="코너명"         width=180,  edit=none,   align=left  </FC>';
    }                 
 ...
 *************************************************************************/ 
  ...
  */
  ...
  { 
      ...
      TR_MAIN.Action="/pot/ccom018.cc?goTo=searchOnPop";
...
*************************************************************************/
    ...
    */  
     ...
     * return값 : void
     **/  
    function getFnbShopFlag(){
        var tmp = addCondition.split("#G#");
        if(tmp.length > 3){
            return tmp[3];
        }
        return '';
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="" class="title">
              ...
              <span id="title1" class="PL03">F&B코너 </span>
            </td>
        ...
        <td class="popT01 PB03"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="o_table">
          <tr>
            <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
              <tr>
                <th width="80" id=TH_MENU_FLAG_CD >코너코드</th>
                <td >
                <comment id="_NSID_">
                    <object id=EM_MENU_FLAG_CD classid=<%=Util.CLSID_EMEDIT%> height=20 width=65 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
              </tr>
              <tr>
                </td>
                <th id=TH_MENU_FLAG_NAME >코너명</th>
                <td>
                <comment id="_NSID_">
                    <object id=EM_MENU_FLAG_NAME classid=<%=Util.CLSID_EMEDIT%> height=20 width=225 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
              </tr>
...
<object id=BO_SEARCH classid=<%= Util.CLSID_BIND %>>
        ...
        <param name=BindInfo        value='
            <c>col=MENU_FLAG_CD                ctrl=EM_MENU_FLAG_CD               param=Text</c>
            <c>col=MENU_FLAG_NAME              ctrl=EM_MENU_FLAG_NAME             param=Text</c>
        '>
</object>
</comment><script>_ws_(_NSID_);</script>
</body>
