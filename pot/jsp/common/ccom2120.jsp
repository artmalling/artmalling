 ...
 *************************************************************************/
 ...
 var argNm2         = dialogArguments[3];
 var argStrCd       = dialogArguments[4];
 var argMgbn        = dialogArguments[5];
 var argSgbn        = dialogArguments[6];
...
{       
    ...
    // EMedit 초기화
    initEmEdit(EM_SEL_CODE,    "CODE^8^#",  NORMAL);
    initEmEdit(EM_SEL_NAME,    "GEN^40",    NORMAL);
    initEmEdit(EM_SEL_NAME2,   "GEN^40",    NORMAL);

    EM_SEL_CODE.Text = argCd;    
    EM_SEL_NAME.Text = argNm; 
    EM_SEL_NAME2.Text = argNm2; 
...
function gridCreate(mGbn){
    ...
    if (mGbn == "M") {
        hdrProperies = ''
            ...
            + '<FC>ID=HUSE_ID       NAME="주거ID"'         
            + '                                         WIDTH=100  EDIT=NONE    ALIGN=CENTER</FC>'
            + '<FC>ID=DONG          NAME="동"'        
            + '                                         WIDTH=70   EDIT=NONE    ALIGN=LEFT</FC>'
            + '<FC>ID=HO            NAME="호"'        
            + '                                         WIDTH=70   EDIT=NONE    ALIGN=LEFT</FC>'
            + '<FC>ID=CNTR_AREA     NAME="면적"'        
        ...
        hdrProperies = ''
            ...
            + '                                         WIDTH=70   EDIT=NONE    ALIGN=LEFT</FC>'
            + '<FC>ID=CNTR_AREA     NAME="면적"'        
            + '                                         WIDTH=110  EDIT=NONE    ALIGN=RIGHT</FC>';
    }
 ...
 *************************************************************************/ 
  ...
  */
  ...
  { 
        ...
        DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "NAME2")= EM_SEL_NAME2.Text; 
        DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "STRCD")= argStrCd; 
        DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "MGBN") = argMgbn; 
        DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "SGBN") = argSgbn; 
 ...
 *************************************************************************/
  ...
  */  
  function doDoubleClick(row)
  {
      ...
      if (row > 0) 
      {   
    	  if (DS_O_RESULT.NameValue(row, "USE_YN") != "Y") {
    		  showMessage(INFORMATION, OK, "USER-1000", "선택한 주거ID는 해당 시설에 이미 등록되어 있습니다. ");
    		  return false; 
    	  }
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0"
                            ...
                            <tr>
                                ...
                                <td width="65"><comment id="_NSID_"> <object id=EM_SEL_CODE
                                    classid=<%=Util.CLSID_EMEDIT%> height=20 width=60
                                    onKeyDown="if(event.keyCode == 13) btn_Search();"> </object> </comment><script>_ws_(_NSID_);</script>
                                </td>
                                <th width="45">동</th>
                                <td width="75"><comment id="_NSID_"> <object id=EM_SEL_NAME
                                    classid=<%=Util.CLSID_EMEDIT%> height=20 width=70
                                    onKeyDown="if(event.keyCode == 13) btn_Search();"> </object> </comment><script>_ws_(_NSID_);</script>
                                </td>
                                <th width="45">호</th>
                                <td><comment id="_NSID_"> <object id=EM_SEL_NAME2
                                    classid=<%=Util.CLSID_EMEDIT%> height=20 width=70
                                    onKeyDown="if(event.keyCode == 13) btn_Search();"> </object> </comment><script>_ws_(_NSID_);</script>
                                </td>
                            </tr>
