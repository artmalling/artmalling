 ...
 *************************************************************************/
 ...
 var buyerCd        = dialogArguments[1];
 var buyerNm        = dialogArguments[2];
...
{     	
    ...
    // EMedit 초기화
    initEmEdit(EM_BUYER_CD, "CODE^6^#", NORMAL);
    initEmEdit(EM_BUYER_NAME, "GEN^20", NORMAL);

    // Input Data Set Header 초기화 
    DS_I_COND.setDataHeader  ('BUYER_CD:STRING(6),BUYER_NAME:STRING(20),AUTH_GB:STRING(1),ADD_CONDITION:STRING(200)');
    DS_I_COND.ClearData();
    DS_I_COND.Addrow();

    DS_I_COND.NameValue(DS_I_COND.RowPosition, "BUYER_CD") = buyerCd; 
    DS_I_COND.NameValue(DS_I_COND.RowPosition, "BUYER_NAME") = buyerNm;
    ...
    EM_BUYER_CD.Focus();
    if(buyerCd.length > 0 || buyerNm.length > 0   ){
        btn_Search();
    }
} 
...
function gridCreate1(){
    ...
    var hdrProperies = '<FC>ID="BUYER_CD"     name="바이어(SM)코드"       width=120,   edit=none,   align=center</FC>'
                     + '<FC>ID="BUYER_NAME"   name="바이어(SM)명"         width=160,   edit=none,   align=left  show=false </FC>'
                     + '<FC>ID="PUMBUN_CD"    name="정부구분"         width=80,    edit=none,   align=left show=false </FC>'
                     + '<FC>ID="BUYER_CHAR_NAME"  name="바이어(SM)명"     width=150,   edit=none,   align=left </FC>';
                     
    if( multiGb == "M"){
    	hdrProperies = '<FC>ID="SEL"          name="선택"              width=30,   align=center, EditStyle="CheckBox"</FC>'
    		         + '<FC>ID="BUYER_CD"     name="바이어(SM)코드"     width=120,   edit=none,   align=center</FC>'
                     + '<FC>ID="BUYER_NAME"   name="바이어(SM)명"       width=150,   edit=none,   align=left </FC>'
                     + '<FC>ID="PUMBUN_CD"    name="정부구분"           width=80,    edit=none,   align=left show=false </FC>'
                     ;
    }
...
{ 
    ...
    TR_MAIN.Action="/pot/ccom026.cc?goTo=searchOnPop";
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="" class="title">
              ...
              <span id="title1" class="PL03">바이어(SM)코드 </span>
            </td>
        ...
        <td class="popT01 PB03"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="o_table">
          <tr>
            <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
              <tr>
                <th width="90">바이어(SM)코드</th>
                <td >
                <comment id="_NSID_">
                    <object id=EM_BUYER_CD classid=<%=Util.CLSID_EMEDIT%> height=20 width=150 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
              </tr>
              <tr>
                <th width="90">바이어(SM)명</th>
                <td>
                <comment id="_NSID_">
                    <object id=EM_BUYER_NAME classid=<%=Util.CLSID_EMEDIT%> height=20 width=210 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
              </tr>
...
<object id=BO_SEARCH classid=<%= Util.CLSID_BIND %>>
        ...
        <param name=BindInfo        value='
            ...
            <c>col=BUYER_NAME              ctrl=EM_BUYER_NAME             param=Text</c>
        '>
</object>
</comment><script>_ws_(_NSID_);</script>
</body>
