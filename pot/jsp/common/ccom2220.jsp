<!-- 
/*******************************************************************************
 * 시스템명 : 브랜드별 품목 단축코드 조회  팝업
 * 작 성 일 : 2012.05.10
 * 작 성 자 : DHL
 * 수 정 자 : 
 * 파 일 명 : ccom2220.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 브랜드별 품목 단축코드 조회 팝업
 * 이    력 :
 ******************************************************************************/
--> 
...
<html>
<head>
<title>브랜드별 품목 단축코드-POPUP</title>
...
{     	
    ...
    initEmEdit(EM_PMK_SRT_CD, "CODE^4^0", NORMAL);
    initEmEdit(EM_PUMMOK_NM, "GEN^40", NORMAL);

    // Input Data Set Header 초기화 
    DS_I_COND.setDataHeader  ('PUMBUN_CD:STRING(6),PUMMOK_SRT_CD:STRING(4),PUMMOK_CD:STRING(8),PUMMOK_NM:STRING(40),ADD_CONDITION:STRING(200)');
    //DS_I_COND.setDataHeader  ('PUMBUN_CD:STRING(6),PUMMOK_SRT_CD:STRING(4),PUMMOK_NM:STRING(40),ADD_CONDITION:STRING(200)');
    DS_I_COND.ClearData();
    DS_I_COND.Addrow();

    DS_I_COND.NameValue(DS_I_COND.RowPosition, "PUMBUN_CD") = pumbunCd; 
    DS_I_COND.NameValue(DS_I_COND.RowPosition, "PUMMOK_SRT_CD") = pmkSrtCd;
    ...
    if(pumbunCd.length > 0 || pmkSrtCd.length > 0 || pummokNm.length > 0){
        btn_Search();
    }
} 

function gridCreate1(){
    var hdrProperies = '<FC>ID="PUMMOK_SRT_CD"   name="단축코드"    width=50,   edit=none,   align=center</FC>'
    ...
    if( multiGb == "M"){
        hdrProperies = '<FC>ID="SEL"              name="선택"     width=30,    align=center, EditStyle="CheckBox"</FC>'
        	         + '<FC>ID="PUMMOK_SRT_CD"    name="단축코드"   width=50,   edit=none,   align=center</FC>'
        	         + '<FC>ID="PUMMOK_CD"        name="품목코드"   width=80,   edit=none,   align=center</FC>'
        	         + '<FC>ID="PUMMOK_NAME"      name="품목명"    width=180,  edit=none,   align=left  </FC>'
                     + '<FC>ID="RECP_NAME"        name="영수증명"   width=50,   edit=none,   align=left,  show=false  </FC>'
                     + '<FC>ID="PUMMOK_LEVEL"     name="품목레벨"   width=50,   edit=none,   align=left,  show=false </FC>'
 ...
 *************************************************************************/ 
  ...
  */
  ...
  { 
      ...
      TR_MAIN.Action="/pot/ccom222.cc?goTo=searchOnPop";
        ...
        if (row > 0) 
        {
            ...
            if( multiGb == "M"){
                ...
                for(var j=1;j<=DS_O_RESULT.CountColumn;j++)
                {
                    ...
                    //alert("DS_O_RESULT.ColumnID(j) : " + DS_O_RESULT.ColumnID(j));
                }
                objJson += "}]"; 
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="" class="title">
              ...
              <span id="title1" class="PL03">브랜드별 품목 단축코드</span>
            </td>
          ...
          <tr>
            <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
              ...
              <tr>
                ...
                <td width="65">
                <comment id="_NSID_">
                    <object id=EM_PMK_SRT_CD classid=<%=Util.CLSID_EMEDIT%> height=20 width=65 onKeyDown="if(event.keyCode == 13) btn_Search();" >
                    </object>
                </comment><script>_ws_(_NSID_);</script>
                </td>
                <th width="50">품목명</th>
...
<object id=BO_SEARCH classid=<%= Util.CLSID_BIND %>>
        ...
        <param name=BindInfo        value='
            ...
            <c>col=PUMMOK_SRT_CD          ctrl=EM_PMK_SRT_CD            param=Text</c>
