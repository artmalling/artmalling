...
public class POrd107DAO extends AbstractDAO {
    	
	/**
	 * 의류단품 대출입발주 리스트 내역 조회
	 * 
	 * @param form
	 * @return
	 * @throws Exception
	...
	public int save(ActionForm form, MultiInput mi[], String userId, String org_flag)
		...
		String strStrCd      = "";	    // 대출입매장
		String strSlipNo1    = "";	    // 대출입전표번호
		String strPairStrCd  = "";	    // 대출입상대매장
		String strPairSlipNo = "";	    // 대출입상대전표번호   
		
		String strCSlipNo    = "";		// 대출전표번호
		String strDSlipNo    = "";		// 대입전표번호
		...
		try {
			...
			while (mi1.next()) {
				...
				if (mi1.IS_INSERT()) { // 저장
					...
					if("C".equals(mi1.getString("SLIP_FLAG")))
						sql.setString(i++, "1");							/* 출입구분           : 1(대출) */
					else
						sql.setString(i++, "2");							/* 출입구분           : 2(대입) */   
					...
					// SMS전송
					if("C".equals(mi1.getString("SLIP_FLAG")) && res > 0)
				...
				if("C".equals(mi1.getString("SLIP_FLAG"))){				// 대출일 경우
				...
				}else if("D".equals(mi1.getString("SLIP_FLAG"))){		// 대입일 경우
				...
				// 4. 상세 데이터를 저장한다.
				saveDetail1(form, mi1, mi2, strCSlipNo, userId);
			...
			System.out.println("strStrCd::"+map.get("strStrCd").toString());
			System.out.println("strSlipNo1::"+map.get("strSlipNo1").toString());
			System.out.println("strPairStrCd::"+map.get("strPairStrCd").toString());
			System.out.println("strPairSlipNo::"+map.get("strPairSlipNo").toString());
	 ...
	 * @throws Exception
	 */
	public int saveDetail1(ActionForm form, MultiInput mi1, MultiInput mi2, String strCSlipNo, String userId) 
						throws Exception {
	...
	public int saveDetail2(ActionForm form, MultiInput mi1, MultiInput mi3, String strDSlipNo, String userId) 
		...
		try {
			System.out.println("strCSlipNo!!::"+strDSlipNo);
			if( getTrConnection() == null){
				connect("pot");
				begin();
			}
			...
			if(!"".equals(strDSlipNo)){
				...
				while (mi3.next()) {
					...
					if (mi3.IS_INSERT()) { // 저장
							...
							sql.setString(i++, mi3.getString("STYLE_CD"));
							sql.setString(i++, mi3.getString("COLOR_CD"));
							sql.setString(i++, mi3.getString("SIZE_CD"));  	
	...
	public int savePairUpdate(ActionForm form, HashMap map) 
		...
		Service svc = null;
		int i;
		String strStrCd		    = "";		// 대출매장
		String strSlipNo1		= "";		// 대입매장
		String strPairStrCd     = "";	    // 대입매장
		String strPairSlipNo    = "";	    // 대입전표번호
	...
	public int delete(ActionForm form, String userId)
		...
		try {
			...
			sql.setString(1, form.getParam("strStrCd"));
			sql.setString(2, form.getParam("strPslipNo"));
			res = update(sql);
