...
function doInit(){
    ...
    registerUsingDataset("psal451","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}					name="NO"				width=30	align=center</FC>'
                     ...
                     + '<FC>id=TEAM_CD					name="파트코드"			width=50    align=center	suppress=2			SubSumText=""	SubBgColor=#99FFCC</FC>'
                     + '<FC>id=TEAM_ORG_NAME			name="파트명"				width=80    align=left		suppress=2			SubSumText=""	SubBgColor=#99FFCC</FC>'
                     + '<FC>id=PC_CD					name="PC코드"			width=50    align=center	suppress=1			SubSumText=""	SubBgColor=#99FFCC</FC>'
                     + '<FC>id=PC_ORG_NAME 				name="PC명"				width=80    align=left		suppress=1 			SubSumText=""	SubBgColor=#99FFCC</FC>'
                     + '<FC>id=PUMMOK_NM 				name="품목"				width=100	align=center						SubSumText=""	SubBgColor=#99FFCC</FC>'
         			 + '<FC>id=BRAND_NM 				name="브랜드"			width=100	align=center	SumText=합계 		SubSumText={decode(curlevel,1,"PC합계",2,"파트합계")}	SubBgColor=#99FFCC</FC>'
         			 + '<G>name="금년"'
                     + '<FC>id=NOR_TOT_SALE_AMT			name="정상"				width=90    align=right		mask="###,###"		sumtext={sum(NOR_TOT_SALE_AMT)}		subsumtext={subsum(NOR_TOT_SALE_AMT)}		SubBgColor=#99FFCC </FC>'
                     + '<FC>id=SALE_TOT_SALE_AMT		name="세일"				width=90    align=right		mask="###,###"		sumtext={sum(SALE_TOT_SALE_AMT)}	subsumtext={subsum(SALE_TOT_SALE_AMT)}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=EVENT_TOT_SALE_AMT		name="행사"				width=90    align=right		mask="###,###"		sumtext={sum(EVENT_TOT_SALE_AMT)}	subsumtext={subsum(EVENT_TOT_SALE_AMT)}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=TOT_TOT_SALE_AMT			name="소계"				width=90    align=right		mask="###,###"		sumtext={sum(TOT_TOT_SALE_AMT)}		subsumtext={subsum(TOT_TOT_SALE_AMT)}		SubBgColor=#99FFCC</FC>'
         			 + '</G>'
         			 
         			 + '<G>name="전년"'
                     + '<FC>id=NOR_TOT_SALE_AMT2		name="정상"				width=90    align=right		mask="###,###"		sumtext={sum(NOR_TOT_SALE_AMT2)}		subsumtext={subsum(NOR_TOT_SALE_AMT2)}		SubBgColor=#99FFCC </FC>'
                     + '<FC>id=SALE_TOT_SALE_AMT2		name="세일"				width=90    align=right		mask="###,###"		sumtext={sum(SALE_TOT_SALE_AMT2)}		subsumtext={subsum(SALE_TOT_SALE_AMT2)}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=EVENT_TOT_SALE_AMT2		name="행사"				width=90    align=right		mask="###,###"		sumtext={sum(EVENT_TOT_SALE_AMT2)}		subsumtext={subsum(EVENT_TOT_SALE_AMT2)}	SubBgColor=#99FFCC</FC>'
                     + '<FC>id=TOT_TOT_SALE_AMT2		name="소계"				width=90    align=right		mask="###,###"		sumtext={sum(TOT_TOT_SALE_AMT2)}		subsumtext={subsum(TOT_TOT_SALE_AMT2)}		SubBgColor=#99FFCC</FC>'
         			 + '</G>'
                     
         			 + '<G>name="신장율"'
                     + '<FC>id=INC_NOR_SALE_RATE		name="정상"				width=60    align=right     mask="#,###.0"		sumtext={if(sum(NOR_TOT_SALE_AMT2)   = 0, 0.0, round(sum(NOR_TOT_SALE_AMT   - NOR_TOT_SALE_AMT2  ) / sum(NOR_TOT_SALE_AMT2)   * 100, 1))}		subsumtext={if(subsum(NOR_TOT_SALE_AMT2)   = 0, 0.0, round(subsum(NOR_TOT_SALE_AMT   - NOR_TOT_SALE_AMT2)   / subsum(NOR_TOT_SALE_AMT2)   * 100,1))}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=INC_SALE_SALE_RATE		name="세일"				width=60    align=right     mask="#,###.0"		sumtext={if(sum(SALE_TOT_SALE_AMT2)  = 0, 0.0, round(sum(SALE_TOT_SALE_AMT  - SALE_TOT_SALE_AMT2 ) / sum(SALE_TOT_SALE_AMT2)  * 100, 1))}		subsumtext={if(subsum(SALE_TOT_SALE_AMT2)  = 0, 0.0, round(subsum(SALE_TOT_SALE_AMT  - SALE_TOT_SALE_AMT2)  / subsum(SALE_TOT_SALE_AMT2)  * 100,1))}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=INC_EVENT_SALE_RATE		name="행사"				width=60    align=right     mask="#,###.0"		sumtext={if(sum(EVENT_TOT_SALE_AMT2) = 0, 0.0, round(sum(EVENT_TOT_SALE_AMT - EVENT_TOT_SALE_AMT2) / sum(EVENT_TOT_SALE_AMT2) * 100, 1))}		subsumtext={if(subsum(EVENT_TOT_SALE_AMT2) = 0, 0.0, round(subsum(EVENT_TOT_SALE_AMT - EVENT_TOT_SALE_AMT2) / subsum(EVENT_TOT_SALE_AMT2) * 100,1))}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=INC_TOT_SALE_RATE		name="소계"				width=60    align=right     mask="#,###.0"		sumtext={if(sum(TOT_TOT_SALE_AMT2)   = 0, 0.0, round(sum(TOT_TOT_SALE_AMT2  - TOT_TOT_SALE_AMT2  ) / sum(TOT_TOT_SALE_AMT2)   * 100, 1))}		subsumtext={if(subsum(TOT_TOT_SALE_AMT2)   = 0, 0.0, round(subsum(TOT_TOT_SALE_AMT   - TOT_TOT_SALE_AMT2  ) / subsum(TOT_TOT_SALE_AMT2)   * 100,1))}		SubBgColor=#99FFCC</FC>'
         			 + '</G>';
    ...
    DS_O_MASTER.SubsumExpr	= "2:TEAM_CD,1:PC_CD";	//파트, PC별 소계
}
...
function btn_Search() {
    ...
    var parameters = "&strStrCd="           +encodeURIComponent(strStrCd)
				   ...
				   ;
    TR_MAIN.Action="/dps/psal451.ps?goTo="+goTo+parameters;  
...
function btn_Excel() {
	...
	var strStrCd        = LC_STR_CD.Text;      //점(BindColumn)
	var strDeptCd       = LC_DEPT_CD.Text;     //팀(BindColumn)
	var strTeamCd       = LC_TEAM_CD.Text;     //파트(BindColumn)
	var strPcCd         = LC_PC_CD.Text;       //PC(BindColumn)
	var strSaleDtS      = EM_SALE_DT_S.text;         //시작일자
	var strSaleDtE      = EM_SALE_DT_E.text;         //종료일자 
