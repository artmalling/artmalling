...
function doInit(){
    ...
    registerUsingDataset("pstk204","DS_IO_MASTER" );
}
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}        name="NO"              width=30     align=center    edit=none  sumtext=""</FC>'    	             
                     ...
                     + '<FC>id=SKU_CD          name="*단품코드"       width=120    align=center  EditStyle=Popup sumtext=""  edit={IF(CLOSE_DT="", "true", "false")}  BgColor={if(ERROR_CHECK = "","white","orange") }</FC>'
                     ...
                     + '<FC>id=BUY_SALE_PRC    name="단가"             width=80    align=right  edit=none sumtext=""  BgColor={if(ERROR_CHECK = "","white","orange") }</FC>'
                     // MARIO OUTLET
                     //+ '<FG>                   name="장부재고" '
...
function btn_Delete() {	
    ...
    TR_MAIN.Action="/dps/pstk204.pt?goTo=delete";
...
function btn_Save() {	 
    ...
    TR_MAIN.Action="/dps/pstk204.pt?goTo=save"+parameters;
    TR_MAIN.KeyValue="SERVLET(I:DS_IO_MASTER=DS_IO_MASTER)"; //조회는 O
    TR_MAIN.Post();
  ...
  */
  function getExcelUpLoadPop(){
   ...
   // 필수 입력 내용 체크
      ...
      var returnVal = window.showModalDialog("/dps/pstk204.pt?goTo=popUpList",
                                             arrArg,
