 ...
 */
public class PStk403DAO extends AbstractDAO {
    ...
    public List searchMaster(ActionForm form) throws Exception {
		...
		if( !strSlipFlag.equals("%")){
			...
			strQuery += svc.getQuery("SEL_OUTSTOREHD_WHERE_SLIP_FLAG") + "\n";
		}
	...
	public int save(ActionForm form, MultiInput mis[], String strID)
		...
		try {
			...
			while (mi.next()) {
				...
				if (mi.IS_INSERT()) {
					//신규 전표일련번호
					sql.put(svc.getQuery("SEL_OUTSTOREHD_NEXT_SLIP_SEQ_NO"));
					...
					sql.put(svc.getQuery("INS_OUTSTOREHD"));
					...
					sql.setString(i++, mi.getString("OUT_TOT_QTY"));					
					sql.setString(i++, mi.getString("OUT_TOT_AMT"));			
			...
			while (mi.next()) {
				...
				if (mi.IS_INSERT()) {
					...
					//중복되는 단품 조회
					sql.put(svc.getQuery("SEL_OUTSTOREDT_SKU_CD_CNT"));
					sql.setString(1, mi.getString("STR_CD"));	
					sql.setString(2, newSlipNo);	
					sql.setString(3, newSlipSeqNo);	
					...
					sql.put(svc.getQuery("INS_OUTSTOREDT"));
					...
					sql.setString(i++, mi.getString("OUT_PRC"));					
				...
				//수정 입력시
					...
					sql.put(svc.getQuery("UPD_OUTSTOREDT"));
				...
				// 삭제시	
					...
					sql.put(svc.getQuery("DEL_OUTSTOREDT"));					
					sql.setString(i++, mi.getString("STR_CD"));	
					sql.setString(i++, mi.getString("SLIP_NO"));	
					sql.setString(i++, mi.getString("SLIP_SEQ_NO"));	
					sql.setString(i++, mi.getString("SEQ_NO"));
		...
		try {
			...
			while (mi.next()) {
				...
				if (mi.IS_DELETE()){
					...
					sql.put(svc.getQuery("DEL_OUTSTOREHD_TO_OUTSTOREDT"));					
					sql.setString(i++, mi.getString("STR_CD"));	
					sql.setString(i++, mi.getString("SLIP_NO"));	
					sql.setString(i++, mi.getString("SLIP_SEQ_NO"));	
					// 저장품출고상세 삭제한다.
					...
					sql.put(svc.getQuery("DEL_OUTSTOREHD"));					
					sql.setString(i++, mi.getString("STR_CD"));	
					sql.setString(i++, mi.getString("SLIP_NO"));	
					sql.setString(i++, mi.getString("SLIP_SEQ_NO"));	
					// 저장품출고헤더 삭제한다.
