 ...
 * 개    요 : 규격단품으로 관리하는 상품의 매가인상하 발주 등록한다.
 * 이    력 :
 * 
 ******************************************************************************/
-->
...
//    LC_INC_RSN_CD.Index    = 0;
    ...
    registerUsingDataset("pord111","DS_O_LIST,DS_IO_MASTER,DS_IO_DETAIL");
 } 
...
function btn_Delete() {
    ...
    if(showMessage(Question, YESNO, "USER-1023") == 1){
        ...
        TR_MAIN.Action="/dps/pord111.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){
        ...
        TR_SAVE.Action="/dps/pord111.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
...
function getList(){
   ...
   TR_S_MAIN.Action  = "/dps/pord111.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord111.po?goTo="+goTo+parameters;  
...
function getSkuInfo(){  
   ...
   var parameters =  "&strStrCd="		+encodeURIComponent(strStrCd)    
                   ...
                   + "&strSkuCd="		+encodeURIComponent(vstrSkuCd);
   TR_S_MAIN.Action   = "/dps/pord111.po?goTo="+goTo+parameters;  
