...
function doInit(){
    ...
    registerUsingDataset("psal512","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"           width=30    align=center    </FC>'
                     ...
                     + '<FC>id=SALE_DT                 name="일자"          width=80    align=center       suppress=1  mask="XXXX/XX/XX"</FC>'
                     + '<FC>id=POS_NO                  name="POS번호"       width=80    align=center      suppress=2 </FC>'
                     + '<FC>id=TRAN_NO                 name="거래번호"       width=50    align=center      suppress=3   </FC>'
                     + '<G>                            name="판매사원"'
                     + '<FC>id=SALE_USER_ID            name="사번"         width=80    align=left      suppress=4     sumtext="합계"</FC>'
                     + '<FC>id=EMP_NAME                name="명"           width=80    align=center      suppress=5    </FC>'
                     + '</G>'
                     + '<FC>id=CANCEL_FLAG             name="취소구분"      width=80    align=left   suppress=4   </FC>'
                     + '<FC>id=R_CANCEL                name="취소사유"      width=80    align=left   suppress=5   </FC>'
                     + '<FC>id=SALE_TOT_AMT            name="총매출"        width=90    align=right    mask="###,###"      sumtext={subsum(SALE_TOT_AMT)}</FC>'
                     + '<FC>id=REDU_TOT_AMT            name="할인"          width=75    align=right    mask="###,###"    sumtext={subsum(REDU_TOT_AMT)}</FC>'
                     + '<FC>id=NORM_SALE_AMT           name="매출"          width=90    align=right    mask="###,###"    sumtext={subsum(NORM_SALE_AMT)}</FC>'
                     + '<FC>id=DC_TOT_AMT              name="에누리"        width=70    align=right    mask="###,###"    sumtext={subsum(DC_TOT_AMT)}</FC>'
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}               name="NO"           width=30    align=center      </FC>'
                     + '<FC>id=SEQ_NO                 name="SEQ"          width=30    align=center       suppress=1</FC>'
                     + '<G>                           name="브랜드"'
                     + '<C>id=PUMBUN_CD               name="코드"          width=60    align=center        suppress=2   </C>'
                     + '<C>id=PUMBUN_NAME             name="명"            width=90    align=left           suppress=2   </C>'
                     + '</G>'
                     + '<G>                           name="품목"'
                     + '<C>id=PUMMOK_CD               name="코드"          width=80    align=center        suppress=3   </C>'
                     + '<C>id=PUMMOK_NAME             name="명"            width=90    align=left           suppress=3   </C>'
                     + '</G>'
                     + '<G>                           name="단품"          '
                     + '<C>id=SKU_CD                  name="코드"          width=100    align=center        suppress=4   </C>'
                     + '<C>id=SKU_NAME                name="명"            width=90    align=left           suppress=4   </C>'
                     + '</G>'
                     + '<C>id=SALE_QTY                name="매출수량"      width=70    align=right    mask="###,###"       sumtext=@sum </C>'
                     + '<C>id=SALE_PRC                name="총매출"        width=90    align=right    mask="###,###"       sumtext=@sum </C>'
                     + '<C>id=REDU_PRC                name="할인"          width=70    align=right    mask="###,###"      sumtext=@sum </C>'
                     + '<C>id=NORM_SALE_AMT           name="매출"          width=90    align=right    mask="###,###"      sumtext=@sum </C>'
                     + '<C>id=DC_PRC                  name="에누리"        width=70    align=right    mask="###,###"       sumtext=@sum </C>'
                     + '<C>id=NET_SALE_AMT            name="순매출"        width=90    align=right     mask="###,###"      sumtext=@sum </C>'
                     + '<C>id=SKU_FLAG                name="단품구분"      width=80    align=center    show=false</C>'
                     ;        
    ...
    GD_DETAIL.ColumnProp("SKU_NAME", "sumtext")        = "합계";
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal512.ps?goTo="+goTo+parameters;  
 ...
 */
 function searchDetail(){
     ...
     var strSaleDt       = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "SALE_DT");         //일자
     var strPosNo        = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "POS_NO");          //POS번호
     ...
     TR_DETAIL.Action="/dps/psal512.ps?goTo="+goTo+parameters;  
     TR_DETAIL.KeyValue="SERVLET("+action+":DS_O_DETAIL=DS_O_DETAIL)"; //조회는 O
     TR_DETAIL.Post();
     
     if(DS_O_DETAIL.NameValue(DS_O_DETAIL.RowPosition, "SKU_FLAG") == "1"){
    	 GD_DETAIL.ColumnProp("SKU_NAME", "Show")           = true;
         GD_DETAIL.ColumnProp("SKU_CD", "Show")             = true;
     }
     else{
    	 GD_DETAIL.ColumnProp("SKU_NAME", "Show")           = false;
         GD_DETAIL.ColumnProp("SKU_CD", "Show")             = false;
     }
     // 조회결과 Return
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=300 classid=<%=Util.CLSID_GRID%> >
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_DETAIL width=100% height=150 classid=<%=Util.CLSID_GRID%> >
