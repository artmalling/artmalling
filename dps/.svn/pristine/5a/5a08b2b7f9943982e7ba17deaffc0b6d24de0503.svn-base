...
public class PSal208DAO extends AbstractDAO {
	...
	public List searchMaster_new(ActionForm form, String OrgFlag, String userId) throws Exception {
		...
		connect("pot");
		sql.put(svc.getQuery("SEL_PS_ACTPUMBUN_NEW"));
		...
		try {
			...
			while (mi.next()) {//DateSet의 현제 Row의 상태를 체크한다 IS_INSERT, IS_UPDATE, IS_DELETE
				...
				if (mi.IS_UPDATE()) {
					...
					sql.setString(i++, mi.getString("ACT_AMT"));
					
					sql.setString(i++, strID);
					
					sql.setString(i++, mi.getString("STR_CD"));
					
					sql.setString(i++, mi.getString("EXE_YM"));
