...
public class PSal915DAO extends AbstractDAO {
	/**
	 * <p>
	 * 청구대상 데이터 조회
	 * </p>
	...
	public List searchMaster(ActionForm form) throws Exception {
		...
		String strDueDtYn  = String2.nvl(form.getParam("strDueDtYn"));
		...
		if(!strBcompCd.equals("%")){
			query += svc.getQuery("SEL_MASTER_WHERE_BCOMP_CD") + "\n";
			sql.setString(i++, strBcompCd);
		}
		
		if(!strJbrchId.equals("")){
			query += svc.getQuery("SEL_MASTER_WHERE_JBRCH_ID") + "\n";
			sql.setString(i++, strJbrchId);
		}
		
		if(strDueDtYn.equals("Y")){
			query += svc.getQuery("SEL_MASTER_WHERE_DUE_DT") + "\n";
			//sql.setString(i++, strDueDtYn);
		}
		
		query += svc.getQuery("SEL_MASTER_GROUP_BY") + "\n";
		query += svc.getQuery("SEL_MASTER_ORDER_BY");
		...
		try {
			...
			while (mi.next()) {
				...
				if ( mi.IS_DELETE()){
					...
					sql.put(svc.getQuery("DEL_PD_BUYREQPREPHOLD"));
					sql.setString(++i, mi.getString("PROC_DT"));
					sql.setString(++i, mi.getString("STR_CD"));
					sql.setString(++i, mi.getString("BCOMP_CD"));
					sql.setString(++i, mi.getString("JBRCH_ID"));				
