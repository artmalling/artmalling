...
//    initEmEdit(EM_GAP_RATE,        "NUMBER^3^2" , READ);           // 차익율
   ...
   // initEmEdit(EM_I_SLIP_NO5,            "GEN^11",      READ);          //전표번호   
    ...
    LC_S_S_PROC_STAT.Index = 0; //전표미확정:초기화
    CHK_1.checked            = true;
    LC_S_STR.Focus();   
    
    registerUsingDataset("pord210","DS_O_LIST" ); 
 } 
 ...
 function gridCreate7(){
     var hdrProperies = '<FC>id={currow}              name="NO"        width=30     align=center  Edit=none sumtext=""    </FC>'
                      ...
                      + '<FC>id=INVOICE_AMT           name="외화금액"   width=100    align=right  Edit=none  sumtext=@sum </FC>'
                      + '<FC>id=INVOICE_KOR_AMT       name="원화금액"   width=100    align=right  Edit=none  sumtext=@sum </FC>'
                      + '</FG>'
                      + '<FG> name="매가 (부가세 포함)"'
...
function btn_Conf() {
    ...
    //승인취소하려면
    if(DS_O_LIST.Namevalue(DS_O_LIST.RowPosition ,"SLIP_PROC_STAT") == "05"){ //매입팀장 확정상태
        //승인 취소 하시겠습니까?
        if( showMessage(STOPSIGN, YESNO, "USER-1198") != 1 )
            return;
        else{       
            if( showMessage(STOPSIGN, YESNO, "USER-1212", "취소") == 1 ){        //정말 취소 하시겠습니까.
                strSlipProcStat = "05" //승인상태의 플래그로 승인을 취소하기 위함.
                sendFlag = "2";        //승인취소
                //alert("승인취소한다"); 
            }else{
                return;
...
//          var strSlipProcStat = DS_O_LIST.NameValue(i + 1,"SLIP_PROC_STAT");
         ...
         var params = "&strSlipProcStat="+encodeURIComponent(strSlipProcStat)
            ...
            TR_MAIN.Action="/dps/pord210.po?goTo=conf"+params;
            TR_MAIN.Post();     
            k = k+1;
         }       
    }    
    ...
    if(strSlipProcStat=="05"){//확정상태였던 전표의 취소된 건수
...
function getList(){
  ...
  // var strSlip_flag     = RD_S_SLIP_FLAG.CodeValue;              // 전표구분 
   ...
   TR_L_MAIN.Action  = "/dps/pord210.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord210.po?goTo="+goTo+parameters;  
 ...
 function getMaster5(){    
  ...
  /* var strInvoiceYm       = DS_LIST.NameValue(DS_LIST.RowPosition, "INVOICE_YM");      // INVOICE_YM
   ...
   var parameters =  "&strStrCd="+encodeURIComponent(strStrCd)   
                   + "&strSlipNo="+encodeURIComponent(strSlipNo)
                   ;  
   TR_S_MAIN.Action="/dps/pord210.po?goTo="+goTo+parameters;  
                               ...
                               <td>
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                                    <tr>
                                        <td>
                                          <comment id="_NSID_"> 
                                              <OBJECT id=GR_DETAIL4 width=100% height=223  classid=<%=Util.CLSID_GRID%>>
