...
public class POrd304DAO extends AbstractDAO {
	/**
	 * 품번에 단품종류를 조회한다.
	 * 
	 * @param form
	...
	public List getMaster(ActionForm form, String userId) throws Exception {
		...
		if(strPrtReqSeqNo.equals("") || strPrtReqSeqNo.equals(null)){
			...
			sql.setString(i++, strStrCd);           
			strQuery = svc.getQuery("SEL_MASTER_ODD_PRT_REQ_NO") + "\n";
	 ...
	 * @throws Exception
	 */
	public List getPbnSku(ActionForm form) throws Exception {
...
//				sql.setString(10, mi1.getString("PRT_REQ_DT"));
		...
		while (mi2.next()) {		//단품 데이터
			...
			if (mi2.IS_INSERT()) { // 저장
				...
				String strPrtReqSeqNo2= String2.nvl(form.getParam("strPrtReqSeqNo"));
...
//				System.out.println("############"+mapSeqNo.get("PRT_DTL_SEQ_NO").toString());
				...
				sql.put(svc.getQuery("INS_PBNSKU")); 
				...
				sql.setString(7, mi2.getString("SKU_CD"));
				sql.setString(8, mi2.getString("STYLE_CD"));
				sql.setString(9, mi2.getString("COLOR_CD"));
				sql.setString(10, mi2.getString("SIZE_CD"));
				sql.setString(11, mi2.getString("REQ_CNT"));
				sql.setString(12, mi2.getString("SALE_PRC"));
				sql.setString(13, userId);
				sql.setString(14, userId);
				
			}else if(mi2.IS_UPDATE()){// 수정
				
				sql.put(svc.getQuery("UPD_PBNSKU")); 

				sql.setString(1, mi2.getString("PUMMOK_SRT_CD")); 
				sql.setString(2, mi2.getString("PUMMOK_CD")); 
				sql.setString(3, mi2.getString("SKU_CD"));
				sql.setString(4, mi2.getString("STYLE_CD"));
				sql.setString(5, mi2.getString("COLOR_CD"));
				sql.setString(6, mi2.getString("SIZE_CD"));
				sql.setString(7, mi2.getString("REQ_CNT"));
				sql.setString(8, mi2.getString("SALE_PRC"));
				sql.setString(9, userId);				
				sql.setString(10, mi2.getString("STR_CD"));
				sql.setString(11, mi2.getString("PRT_REQ_SEQ_NO"));
				sql.setString(12, mi2.getString("PRT_DTL_SEQ_NO"));
