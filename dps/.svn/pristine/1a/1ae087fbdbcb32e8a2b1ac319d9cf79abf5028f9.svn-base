<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 대금지불 > 매출세금계산서 수기 접수 처리
 * 작 성 일 : 2010.04.14
 * 작 성 자 : 김경은
 * 수 정 자 : 
 * 파 일 명 : ppay1140.jsp
 ...
 */
 ...
 function doInit(){
     ...
     document.getElementById("detail1").style.display ="";       //1
     document.getElementById("detail2").style.display ="none";   //2
 ...
 function gridCreate3(){
     var hdrProperies = '<FC>id={currow}          name="NO"           width=30    Edit=none sumtext="합계" align=center</FC>'
                      + '<FC>id=INPUT_DT          name="입력일"        width=80    Edit=none Mask="XXXX/XX/XX" align=center</FC>'
                      + '<FC>id=SEQ_NO            name="순번"          width=60    EditStyle=Lookup Data="DS_O_SLIP_FLAG:CODE:NAME"  align=center</FC>'
                      + '<FC>id=REASON_CD         name="공제코드"      width=50   Edit=none align=center</FC>'
                      + '<FC>id=REASON            name="공제명"        width=100    Edit=none align=left</FC>'
                      + '<FC>id=VAT_YN            name="VAT여부"       width=80    Edit=none align=left</FC>'
                      + '<FC>id=SUP_AMT           name="공급가"        width=80    Edit=none sumtext=@sum  align=right</FC>'
                      + '<FC>id=VAT_AMT           name="부가세"        width=80    Edit=none sumtext=@sum  align=right</FC>'
                      + '<FC>id=INPUT_AMT         name="입력금액"      width=80    Edit=none sumtext=@sum  align=right</FC>'
 ...
 function gridCreate4(){
     var hdrProperies = '<FC>id={currow}             name="NO"              width=30     align=center</FC>'
                      ...
                      + '<FC>id=SALE_TOT_AMT         name="총매출"           width=100    align=right  sumtext=@sum  </FC>'
                      + '<FC>id=REDU_AMT             name="할인"             width=100    align=right  sumtext=@sum  </FC>'
                      + '<FC>id=SALE_AMT             name="매출"             width=100    align=right  sumtext=@sum  </FC>'
                      + '<FC>id=GAP_SALE_COMMIS_AMT  name="이익액(매출수수료)" width=100    align=right  sumtext=@sum  </FC>';
                      
     initGridStyle(GR_DETAIL3, "common", hdrProperies, false);     

     GR_DETAIL3.ViewSummary = "1";   
     GR_DETAIL3.ColumnProp("PUMBUN_CD", "sumtext")           = "합계";  
 }
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){    // validation 체크
        TR_MAIN.Action="/dps/ppay114.pp?goTo=save&strFlag=save"; 
 ...
 */
 function getList(){
    ...
    TR_MAIN.Action  = "/dps/ppay114.pp?goTo="+goTo+parameters;  
 ...
 */
 function getDetail(){
    ...
    var strIssueFlag  = DS_O_LIST.NameValue(DS_O_LIST.RowPosition, "TAX_ISSUE_FLAG");       // 세금계산서구분
    ...
    TR_DETAIL.Action="/dps/ppay114.pp?goTo="+goTo+parameters;  

    if(strIssueFlag.substr(1,1) == "2"){     // 공제일경우
    	TR_DETAIL.KeyValue="SERVLET("+action+":DS_IO_MASTER=DS_IO_MASTER,"+action+":DS_IO_DETAIL=DS_IO_DETAIL)"; //조회는 O
    }else{
    	TR_DETAIL.KeyValue="SERVLET("+action+":DS_IO_MASTER=DS_IO_MASTER,"+action+":DS_IO_DETAIL=DS_IO_DETAIL2)"; //조회는 O
    }    
 ...
 function setGrid(flag) {
     ...
     if(flag){
         ...
         document.getElementById("detail1").style.display ="none";   //1
         document.getElementById("detail2").style.display ="";       //2
     }
  }
...
<script language=JavaScript for=DS_O_LIST event=onRowPosChanged(row)>
    ...
    if(row != 0){
        ...
        */
        var strTaxIssueFlag = this.NameValue(row, "TAX_ISSUE_FLAG");
        if(strTaxIssueFlag.substr(1,1) == "2"){     // 공제일경우
            gridFlag = false;
        }else{
            gridFlag = true;
        }    
...
<!-- TEST-->
<script language=JavaScript for=LC_S_ETAX_STAT event=OnSelChange()>

	if(LC_S_ETAX_STAT.Index == 0){
	    document.getElementById("detail1").style.display ="";       //1
	    document.getElementById("detail2").style.display ="none";   //2
	}else{
	    document.getElementById("detail1").style.display ="none";   //1
                            ...
                            <tr valign="top">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                                    <tr>
                                        <td>
                                          <comment id="_NSID_"> 
                                              <OBJECT id=GR_DETAIL3 width=100% height=107    classid=<%=Util.CLSID_GRID%>>
