...
function doInit(){
    ...
    DS_O_MASTER1.setDataHeader('<gauce:dataset name="H_MASTER"/>');
  	//그리드 초기화
    gridCreate1(); //마스터    
    // EMedit에 초기화
    ...
    registerUsingDataset("psal471","DS_O_MASTER" );
    //getCalandal();      //달력 
    
    //EM_EXE_YM_S.text = "201302";
    //EM_EXE_YM_E.text = "201302";
}
//align={IF(Price>10000,"left","right")}#B7F0B1
function gridCreate1(){
    var hdrProperies    = '<FC>id=GUBUN		       name="구분"    		    width=120   align=center bgcolor={decode(GUBUN,"일자",decode(GUBUN,"일자","#B7F0B1","#FAED7D")"#FAED7D","#B7F0B1")}</FC>'
				    	+ '<FC>id=SUN		       name="SUN(일)"        	width=110   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)}</FC>'
				    	+ '<FC>id=MON		       name="MON(월)"        	width=110   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)}</FC>'
				    	+ '<FC>id=TUE		       name="TUE(화)"        	width=110   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)}</FC>'
				    	+ '<FC>id=WED		       name="WED(수)"        	width=110   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)}</FC>'
				    	+ '<FC>id=THU		       name="THU(목)"        	width=110   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)}</FC>'
				    	+ '<FC>id=FRI		       name="FRI(금)"        	width=110   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)}</FC>'
				    	+ '<FC>id=SAT		       name="SAT(토)"        	width=110   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)}</FC>'
				    	+ '<FC>id=WEEK_TOT	       name="주간/전체합계"    	width=120   align=right  bgcolor={decode(GUBUN,"일자","#FAED7D",white)} value={decode(GUBUN,"일자","","달성율","","신장율","","대비달성율","",SUN+MON+TUE+WED+THU+FRI+SAT)} </FC>';
 ...
 */
     ...
     if(LC_DEPT_CD.Text == "전체") {
     	parameters = parameters + " 팀=" + LC_DEPT_CD.Text;
     }
     if(LC_TEAM_CD.Text == "전체") {
    	 parameters = parameters + " 파트=" + LC_TEAM_CD.Text;
      }
     if(LC_PC_CD.Text == "전체") {
    	 parameters = parameters + " PC=" + LC_PC_CD.Text;
      }
     
    	 parameters = parameters + " 매출년월=" + EM_EXE_YM_S.Text;
      
    
    	 parameters = parameters + " 대비년월=" + EM_EXE_YM_E.Text;
...
function callDetailPop(){
    ...
    TR_MAIN.Action="/dps/psal471.ps?goTo="+goTo+parameters;  
	...
	function getCalandal(){
        ...
        alert(firstday);		// ?
        alert(firstdayweek);	// 5
        alert(strWeekNum);		// 6
        alert(lastDay2);		// ?
        alert(lastDay2week);	// 4
        alert(strlastDay2);		// 5
        */
        
        
        week_cnt = DS_O_MASTER.NameValue(1, "WEEK_CNT");
        
        firstdayweek = firstdayweek + 2; // 달의 첫번째 시작컬럼 일 : 2   토 : 8
        
        
        
        var cur_row = 1;
        
        for(i = 1; i <= week_cnt; i++) {
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "일자";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "매출";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "계획";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "달성율";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "객수";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "객단가";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "이익";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "신장율";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "대비매출";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "대비목표";
        	gubun_cnt++;
        	DS_O_MASTER1.Addrow();
        	DS_O_MASTER1.NameValue(gubun_cnt, "GUBUN") = "대비달성율";
        	gubun_cnt++;
            ...
            for(k = 2; k <= 8; k++) {							//일자
            	...
            	{
            		if(daycnt <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row, column_Id) = daycnt+".                     ";
		            	//alert(daycnt);
		            	daycnt++;
            		}
            ...
            for(k = 2; k <= 8; k++) {							//매출
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt1 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+1, column_Id) = DS_O_MASTER.NameValue(1, "DAY"+daycnt1);
		            	//alert(DS_O_MASTER.NameValue(1, "DAY"+daycnt1));
		            	daycnt1++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//계획
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt2 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+2, column_Id) = DS_O_MASTER.NameValue(1, "ACT_DAY"+daycnt2);
		            	daycnt2++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//달성율
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt3 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+3, column_Id) = DS_O_MASTER.NameValue(1, "ACHIEVERATE"+daycnt3);
		            	daycnt3++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//객수
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt4 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+4, column_Id) = DS_O_MASTER.NameValue(1, "DAYCUSTCNT"+daycnt4);
		            	daycnt4++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//객단가
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt5 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+5, column_Id) = DS_O_MASTER.NameValue(1, "DAYCUSTDANGA"+daycnt5);
		            	daycnt5++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//이익
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt6 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+6, column_Id) = DS_O_MASTER.NameValue(1, "DAYPROFAMT"+daycnt6);
		            	daycnt6++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//신장율
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt7 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+7, column_Id) = DS_O_MASTER.NameValue(1, "IRATE"+daycnt7);
		            	daycnt7++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//대비매출
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt8 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+8, column_Id) = DS_O_MASTER.NameValue(1, "DAYCMPR"+daycnt8);
		            	daycnt8++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//대비목표
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt9 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+9, column_Id) = DS_O_MASTER.NameValue(1, "ACT_DAYCMPR"+daycnt9);
		            	daycnt9++;
            		}
			...
			for(k = 2; k <= 8; k++) {							//대비달성율
            	...
            	if(k >= firstdayweek)
            	{
            		if(daycnt10 <= lastday) {
		            	column_Id = DS_O_MASTER1.columnID(k);
		            	DS_O_MASTER1.NameValue(cur_row+10, column_Id) = DS_O_MASTER.NameValue(1, "CMPR_ACHIEVERATE"+daycnt10);
		            	daycnt10++;
            		}
        ...
        //DS_O_MASTER.NameValue(1, "DAY"+j);
        
        //DS_O_MASTER1.NameValue(1, "MON") = 
...
<script language="javascript">
    ...
    var varPreMon = (now.getYear().toString()-1)+ mon;
</script>
<!-- 조직 구분  변경시  -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
   ...
   -->
               ...
               class="BD4A">
               <tr>
                  <td><comment id="_NSID_"> <object id=GD_MASTER width="100%" height="100%" classid=<%=Util.CLSID_GRID%> tabindex=1>
