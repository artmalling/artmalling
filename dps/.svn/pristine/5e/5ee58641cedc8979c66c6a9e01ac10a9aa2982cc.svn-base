...
public class POrd114DAO extends AbstractDAO {
    	
	/**
	 * 규격단품 통합 매입발주 리스트 내역 조회
	 *       
	 * @param form
	 * @return
	 * @throws Exception
	...
	public List getSkuInfo(ActionForm form) throws Exception {
		...
		String strStrCd     = String2.nvl(form.getParam("strStrCd"));
		String strPStrCd    = String2.nvl(form.getParam("strPStrCd"));
...
System.out.println("strBuyerYN::"+strBuyerYN+":org_flag:"+org_flag);				
				...
				if (mi1.IS_INSERT()) { // 저장
					...
					sql.setString(i++, "A");       						    /* 전표구분           : A(매입) */
					sql.setString(i++, "0"); 								/* 발주주체구분 : 0(백화점발주) */ 
					sql.setString(i++, "2");   								/* 발주구분           : 2(통합발주) */   
					...
					sql.setString(i++, "N");   
					sql.setString(i++, mi1.getString("PAIR_STR_CD"));        /* 점입점 */
					sql.setString(i++, mi1.getString("BIZ_TYPE"));     
