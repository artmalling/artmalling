...
public class PCod521DAO extends AbstractDAO {
		...
		try {
			...
			while (mi.next()) {
				...
				if ( mi.IS_INSERT()) {
					...
					// 브랜드상품코드(MODEL_NO) 중복체크 시작 20170131 P.R.H
					sql.put(svc.getQuery("SEL_MODEL_CD_CNT"));						
					sql.setString(++i, mi.getString("MODEL_NO"));					
					Map map = selectMap( sql );
					strCnt = String2.nvl((String)map.get("CNT"));
					i = 0;
					sql.close();
...
//					System.out.println("strCnt = " + strCnt);
					...
					if( !strCnt.equals("0")) {
						throw new Exception("[USER]"+"브랜드상품코드(" + mi.getString("MODEL_NO") + ")가 존재합니다.");
					}
					// 브랜드상품코드(MODEL_NO) 중복체크 종료 20170131 P.R.H
