 ...
 */
 ...
 function doInit(){
     ...
     initEmEdit(EM_E_EVALU_YM         ,"THISMN"        ,PK);             // 평가년월
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}          name="NO"            width=30    SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} align=center </FC>'
                      ...
                      + '<G> name="이익율"'                                       
                      + '<C>id=SALE_PROF_RATE         name="율"        width=70    SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} align=right  Edit=none SubSumText={SubAvg(SALE_PROF_RATE)} </C>'                     
                      + '<C>id=PROF_RATE_SCORE   name="점수"      width=50    SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} align=right  Edit=none subsumtext=""</C>'                        
                      + '</G> ' 
 ...
 */
 function getMaster(){
    ...
    var strSEvaluYm      = EM_S_EVALU_YM.Text;                       // 평가년월(시작)
    var strEEvaluYm      = EM_E_EVALU_YM.Text;                       // 평가년월(종료)
    ...
    var parameters = "&strStrCd="+encodeURIComponent(strStrCd)     
                   ...
                   + "&strSEvaluYm="+encodeURIComponent(strSEvaluYm)
                   + "&strEEvaluYm="+encodeURIComponent(strEEvaluYm); 
    
    TR_S_MAIN.Action  = "/dps/psal707.ps?goTo="+goTo+parameters;  
 ...
 function checkValidation(Gubun) {
      ...
      if(Gubun == "Search"){   
         if(EM_S_EVALU_YM.Text.length == 0){
              showMessage(INFORMATION, OK, "USER-1002", "조회시작년월");
              EM_S_EVALU_YM.Focus();
              return false;
          }
         if(EM_E_EVALU_YM.Text.length == 0){
             showMessage(INFORMATION, OK, "USER-1002", "조회종료년월");
             EM_E_EVALU_YM.Focus();
             return false;
         }
         if(EM_S_EVALU_YM.Text > EM_E_EVALU_YM.Text){                              // 조회년월 정합성
...
<!-- 평가년월 KillFocus -->
<script language=JavaScript for=EM_E_EVALU_YM event=onKillFocus()>
    //변경된 내역이 없으면 무시
    if(!this.Modified)
        return;

    checkDateTypeYM(EM_E_EVALU_YM);
</script>
         ...
         <tr>
            <td>
                ...
                <td colspan="7">
                      ...
                      <comment id="_NSID_">
                          <object id=EM_E_EVALU_YM classid=<%=Util.CLSID_EMEDIT%>  width=85 tabindex=1 align="absmiddle">
                          </object>
                      </comment>
                      <script> _ws_(_NSID_);</script>
                      <img src="/<%=dir%>/imgs/btn/date.gif" onclick="javascript:openCal('M',EM_E_EVALU_YM)"  align="absmiddle"/>
                </td>
              </tr>
            </table>
            </td>
