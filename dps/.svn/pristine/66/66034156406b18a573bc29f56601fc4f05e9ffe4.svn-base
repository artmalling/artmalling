...
public class PCod906DAO extends AbstractDAO {
	...
	public int save(ActionForm form, MultiInput mi, String strID)
		...
		try {
			...
			while (mi.next()) {
				...
				if ( mi.IS_INSERT()) {
					flag = "I";
					sql.put(svc.getQuery("SEL_FNBMENUKIND_USE_YN"));							
					...
					if( !mstUseYn.equals("Y")) {
						throw new Exception("[USER]"+"[ "+ mi.getString("MENU_FLAG_CD")+"] 는 사용되지 않는 메뉴분류(코너)코드 입니다.");
					}
					sql.close();
					
					sql.put(svc.getQuery("SEL_FNBMENUMST_SKU_CD_CNT"));							
					sql.setString(1,  mi.getString("STR_CD"));										
					sql.setString(2,  mi.getString("FNB_SHOP_CD"));										
					sql.setString(3,  mi.getString("MENU_FLAG_CD"));									
					...
					if( !cnt.equals("0")) {
						throw new Exception("[USER]"+"[ "+ mi.getString("SKU_CD")+"] 이미 등록된 단품코드 입니다.");
					}
					sql.close();
					i = 0;
					sql.put(svc.getQuery("INS_FNBMENUMST"));
					...
					i = 0;						
					sql.put(svc.getQuery("UPD_FNBMENUMST"));
					sql.setString( ++i, mi.getString("RECP_NAME"       ));
					sql.setString( ++i, mi.getString("SORT_NO"         ));
					sql.setString( ++i, mi.getString("USE_YN"          ));
					sql.setString( ++i, strID);
					...
					flag = "D";
					i = 0;							
					sql.put(svc.getQuery("DEL_FNBMENUMST"));		
					sql.setString( ++i, mi.getString("STR_CD"          ));
					sql.setString( ++i, mi.getString("FNB_SHOP_CD"     ));
					sql.setString( ++i, mi.getString("MENU_FLAG_CD"    ));
					sql.setString( ++i, mi.getString("SKU_CD"          ));
	...
	public int sendfnbmenuemg(ActionForm form, String userId)
			throws Exception {
		...
		try {
			...
			int i = 1;            
			psql.put("DPS.PR_PCFNBMENUPOS_EMG_IFS", 5);
