<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 신용카드대금청구 > 청구관리 > 청구제외점검 등록
 * 작 성 일 : 2010.05.26
 * 작 성 자 : 장형욱
 * 수 정 자 : 
 * 파 일 명 : psal9130.jsp
...
function doInit(){
    ...
    registerUsingDataset("psal913","DS_IO_DETAIL");
}
...
function btn_Search() {  
    ...
    TR_MAIN.Action  = "/dps/psal913.ps?goTo="+goTo+parameters;  
...
function doClick(row){
    ...
    TR_DETAIL.Action  = "/dps/psal913.ps?goTo="+goTo+parameters;  
					...
					<tr>
						...
						<td colspan="5"><comment id="_NSID_"> <object
							...
							<param name=Format value="0^전체,A^A:전일취소,B^B:당일취소(원거래포함),C^C:수기입력,D^D:승인번호중복,
							                          ...
							                          I^I:카드빈매입사차이,J^J:당일수기취소(원거래포함)및원거래무,K^K:전일수기취소">
							<param name="AutoMargin" value="true">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				<td><comment id="_NSID_"> <object id=GD_MASTER
					width="100%" height=302 classid=<%=Util.CLSID_GRID%> tabindex=1>
