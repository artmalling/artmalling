 ...
 */
 ...
 function gridCreate1(){
	 var hdrProperies = '<FC>id={currow}          name="NO"            width=30    SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} align=center </FC>'
			          + '<FC>id=SEL               name="선택"          width=50    SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} align=center HeadCheckShow=true EditStyle=CheckBox</FC>'                 
			          ...
			          + '<C>id=TOT_SAMT          name="합계"      width=120    SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} align=right  Edit=none SubSumText={SubAvg(TOT_SAMT)} </C>'
			          + '</G> '    
			          ...
			          + '<G> name="평가매출"'                                       
			          + '<C>id=TOT_SAMT          name="매출"      width=120    SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} align=right  Edit=none SubSumText={SubAvg(TOT_SAMT)}</C>'                        
...
function btn_Conf() {
    ...
    //확정하시겠습니까?
    if( showMessage(QUESTION, YESNO, "USER-1024") == 1 ){    // validation 체크
        TR_MAIN.Action="/dps/psal705.ps?goTo=conf"+parameters; 
 ...
 */
 function getMaster(){
    ...
    TR_S_MAIN.Action  = "/dps/psal705.ps?goTo="+goTo+parameters;  
  ...
  */
   ...
   */
   function confCheck(strStrCd, strEvaluYm){
      ...
      TR_MAIN.Action  = "/dps/psal705.ps?goTo="+goTo+parameters;  
