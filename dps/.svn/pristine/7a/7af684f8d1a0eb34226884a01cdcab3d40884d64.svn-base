<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 발주관리 > 신선단품 대출입발주
 * 작 성 일 : 2010.03.08
 * 작 성 자 : 김 경 은
 * 수 정 자 : 
 * 파 일 명 : pord1060.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 신선단품으로 관리하는 상품의 대출입 발주 등록한다.
 * 이    력 :
 * 
 ******************************************************************************/
-->
 ...
 */
 ...
 function doInit(){
    ...
    registerUsingDataset("pord106","DS_IO_MASTER,DS_IO_DETAIL1,DS_IO_DETAIL2");   
 } 
 ...
 function gridCreate2(){
   var hdrProperies = '<FC>id={currow}            name="NO"          width=30    align=center  sumtext=""</FC>'                           
                      ...
                      + '<FC>id=NEW_COST_PRC      name="원가단가"    width=80    align=right   Edit=Numeric </FC>'                        
                      + '<FC>id=NEW_COST_AMT      name="원가금액"    width=110   align=right   Edit=none sumtext=@sum </FC>'                                                                   
                      + '<FC>id=NEW_SALE_PRC      name="매가단가"    width=80    align=right   Edit=Numeric </FC>'                        
 ...
 function gridCreate3(){
     var hdrProperies = '<FC>id={currow}          name="NO"          width=30    align=center  sumtext=""</FC>'                           
                      ...
                      + '<FC>id=SKU_NM            name="단품명"      width=100   align=left    Edit=none </FC>'                           
                      + '<FC>id=ORD_UNIT_NM       name="단위"        width=50    align=left    Edit=none </FC>'                                                     
                      + '<FC>id=ORD_QTY           name="*수량"       width=50    align=right   </FC>'               
                      + '<FC>id=RATE              name="목표이익율"    width=70    align=right Edit=none </FC>'
...
function btn_Delete() {
    ...
    if(showMessage(Question, YESNO, "USER-1023") == 1){  
        ...
        TR_MAIN.Action="/dps/pord106.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){
         ...
         TR_SAVE.Action="/dps/pord106.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
...
function getList(){
   ...
   TR_S_MAIN.Action  = "/dps/pord106.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord106.po?goTo="+goTo+parameters;  
...
function getPumbunInfo(gubun){
    ...
    var strVenCd        = "";                                                        // 협력사
    var strBuyerCd      = "";                                                        // 바이어
    var strPumbunType   = "";                                                        // 브랜드유형(0:정상)// MARIO OUTLET
    var strUseYn        = "Y";                                                       // 사용여부
...
function getSkuInfo(gubun){  
   ...
   if(gubun == "C"){
       ...
       DS_DETAIL.NameValue(DS_DETAIL.RowPosition, "RATE")          = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "GOAL_PROF_RATE");
   }
...
function getVenInfo(){
    var strStrCd      = LC_S_STR.BindColVal;                                       // 점
    var strUseYn      = "Y";                                                       // 사용여부
    var strPumBunType = "";                                                        // 브랜드유형 // MARIO OUTLET
    var strBizType    = "1";                                                       // 거래형태
  ...
  */
  function setGapCalc(row, colid){
	 ...
	 if(colid == "NEW_COST_PRC" || colid == "ORD_QTY" || colid == "NEW_SALE_PRC"){   
	      var strTaxFlag  = EM_TAX_FLAG.Text;                               // 대출면과세구분
	      var strPTaxFlag = EM_P_TAX_FLAG.Text;                             // 대입면과세구분
	      
	   	  var strBizType  = EM_BIZ_TYPE.Text;                               // 거래형태
	   	  var ord_qty     = DS_IO_DETAIL1.NameValue(row, "ORD_QTY");        // 발주수량
	   	  var cost_prc    = 0;                                              // 원가단가
	   	  var gapRate     = DS_IO_DETAIL1.NameValue(row, "NEW_GAP_RATE");   // 차익율    
	   	  var sale_prc    = 0;                                              // 매가단가
	   	  var cost_amt    = 0;                                              // 원가금액
	   	  var sale_amt    = 0;                                              // 매가금액
	   	  var totCostAmt  = 0;
	   	  var totSaleAmt  = 0;
	   	  var rate        = DS_IO_DETAIL1.NameValue(row, "RATE");             // 목표이익율
...
//          GR_DETAIL1.ColumnProp("NEW_GAP_RATE", "Edit") = "true";
          ...
          ////DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VAT_TAMT")  = DS_IO_DETAIL1.Sum(22,0,0);
          DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VAT_TAMT")  = getCalcPord("VAT_AMT", EM_TOT_COST_AMT.Text, "", "", strTaxFlag, roundFlag);
          DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "PVAT_TAMT") = getCalcPord("VAT_AMT", EM_TOT_COST_AMT.Text, "", "", strPTaxFlag, roundFlag);
	      ...
	      DS_IO_DETAIL2.NameValue(DS_IO_DETAIL2.RowPosition, "VAT_AMT")      = getCalcPord("VAT_AMT", EM_TOT_COST_AMT.Text, "", "", strPTaxFlag, roundFlag); // 대입부가세
  ...
  */
  function setGapCalc2(row, colid){ 
	  ...
	  if(colid == "ORD_QTY" || colid == "NEW_COST_AMT" || colid == "NEW_SALE_AMT"){
	    ...
	    var cost_amt = DS_IO_DETAIL2.NameValue(row, "NEW_COST_AMT");                  // 원가금액
	    var sale_amt = DS_IO_DETAIL2.NameValue(row, "NEW_SALE_AMT");                  // 매가금액
	    var cost_prc = getRoundDec(roundFlag, cost_amt/ord_qty);                      // 원가단가
	    var sale_prc = getRoundDec(roundFlag, sale_amt/ord_qty);                      // 매가단가
	
	    if(ord_qty > 0){
	        DS_IO_DETAIL2.NameValue(row, "NEW_COST_PRC") = cost_prc;                     // 원가단가
	        DS_IO_DETAIL2.NameValue(row, "NEW_SALE_PRC") = sale_prc;                     // 매가단가
	    }                                                                                                                         
	
	    DS_IO_DETAIL2.NameValue(row, "NEW_GAP_AMT")  = getCalcPord("GAP_AMT",  cost_amt, sale_amt, "", strTaxFlag, roundFlag);
	    DS_IO_DETAIL2.NameValue(row, "NEW_GAP_RATE") = getCalcPord("GAP_RATE", cost_amt, sale_amt, "", strTaxFlag, roundFlag);
	    totCostAmt   = DS_IO_DETAIL2.Sum(13,0,0);
        totSaleAmt   = DS_IO_DETAIL2.Sum(15,0,0);
        EM_P_GAP_TOT_AMT.Text  = getCalcPord("GAP_AMT",  totCostAmt, totSaleAmt, "", strTaxFlag, roundFlag);
        EM_P_GAP_RATE.Text     = getCalcPord("GAP_RATE", totCostAmt, totSaleAmt, "", strTaxFlag, roundFlag);
...
<script language=JavaScript for=DS_IO_DETAIL1
	event=OnColumnChanged(row,colid)>
    var orgValue    = "";
    var newValue    = "";
...
<script language="javascript" for=GR_DETAIL1
...
<script language="javascript" for=GR_DETAIL2
	event=CanColumnPosChange(Row,Colid)>
    switch (Colid) {
    case "SKU_CD":
...
<script>_ws_(_NSID_);</script>
<comment id="_NSID_">
<object id="DS_I_MASTER_TMP" classid=<%=Util.CLSID_DATASET%>></object>
</comment>
<script>_ws_(_NSID_);</script>
<comment id="_NSID_">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td colspan="3"><comment id="_NSID_"> <object
							...
							onclick="javascript:searchPumbunPop(true);" />
							<comment id="_NSID_"> <object
                            id=EM_S_PB_NM classid=<%=Util.CLSID_EMEDIT%> width=184 tabindex=1
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0"
							...
							<tr>
								...
								<td width="100"><comment id="_NSID_"> <object
									id=LC_STR classid=<%=Util.CLSID_LUXECOMBO%> width=95 tabindex=1
									align="absmiddle"> </object> </comment><script>_ws_(_NSID_);</script></td>
								<th width="70">전표번호</th>
								<td width="100"><comment id="_NSID_"> <object
									id=EM_SLIP_NO classid=<%=Util.CLSID_EMEDIT%> width=95
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								</td>
								<th width="75">전표상태</th>
								<td><comment id="_NSID_"> <object id=EM_PROC_STAT
							...
							<tr>
								...
								<td><comment id="_NSID_"> <object
									id=LC_AFT_ORD_FLAG classid=<%=Util.CLSID_LUXECOMBO%> width=95
									tabindex=1 align="absmiddle"> </object> </comment><script>_ws_(_NSID_);</script>
								</td>
								<th>상대전표</th>
								<td><comment id="_NSID_"> <object id=EM_P_SLIP_NO
									classid=<%=Util.CLSID_EMEDIT%> width=95 tabindex=1
									align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script></td>
								<th>상대전표상태</th>
								<td><comment id="_NSID_"> <object
									id=EM_S_PROC_STAT classid=<%=Util.CLSID_EMEDIT%> width=110
									tabindex=1 align="absmiddle"> </object> </comment><script>_ws_(_NSID_);</script>
								</td>
							</tr>
							...
							<tr>
								...
								<td colspan="3"><comment id="_NSID_"> <object id=EM_PUMBUN_CD
									...
									align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script> <img
									src="/<%=dir%>/imgs/btn/detail_search_s.gif" id="IMG_PUMBUN1"
									class="PR03" align="absmiddle"
									onclick="javascript:getPumbunInfo('C');" />
									<comment id="_NSID_"> 
									<object id=EM_PUMBUN_NM
                                    classid=<%=Util.CLSID_EMEDIT%> width=186 tabindex=1
                                    align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script></td>

								<th>대출과세구분</th>
								<td><comment id="_NSID_"> <object
									id=EM_TAX_FLAG_NM classid=<%=Util.CLSID_EMEDIT%> width=110
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								</td>
							</tr>
							...
							<tr>
								...
								<td colspan="3"><comment id="_NSID_"> <object
									id=EM_P_PUMBUN_CD classid=<%=Util.CLSID_EMEDIT%> width=75
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								<img src="/<%=dir%>/imgs/btn/detail_search_s.gif"
									id="IMG_PUMBUN2" class="PR03" align="absmiddle"
									onclick="javascript:getPumbunInfo('D');" />
									<comment id="_NSID_"> <object
                                    id=EM_P_PUMBUN_NM classid=<%=Util.CLSID_EMEDIT%> width=186
                                    tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script></td>
								<th>대입과세구분</th>
								<td><comment id="_NSID_"> <object
									id=EM_P_TAX_FLAG_NM classid=<%=Util.CLSID_EMEDIT%> width=110
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								</td>
							</tr>
							...
							<tr>
								...
								<td><comment id="_NSID_"> <object
									id=EM_PRC_APP_DT classid=<%=Util.CLSID_EMEDIT%> width=75
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								...
								<th>차익액/율</th>
								<td><comment id="_NSID_"> <object
									id=EM_GAP_TOT_AMT classid=<%=Util.CLSID_EMEDIT%> width=60
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
							...
							<tr>
								...
								<th>원가계</th>
								<td><comment id="_NSID_"> <object
									id=EM_TOT_COST_AMT classid=<%=Util.CLSID_EMEDIT%> width=95
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								</td>
								<th>매가계</th>
								<td><comment id="_NSID_"> <object
									id=EM_TOT_SALE_AMT classid=<%=Util.CLSID_EMEDIT%> width=110
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								</td>
							</tr>
							...
							<tr>
								...
								<td colspan="5"><comment id="_NSID_"> <object
									id=EM_REMARK classid=<%=Util.CLSID_EMEDIT%> width=497
									tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
								</td>
							</tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td class="right"><img src="/<%=dir%>/imgs/btn/add_row.gif"
									id="IMG_ADD1" width="52" height="18" hspace="2"
									onclick="javascript:addDetailRow('C');" /><img
									src="/<%=dir%>/imgs/btn/del_row.gif" id="IMG_DEL1" width="52"
									height="18" onclick="javascript:delDetailRow('C');" /></td>
							</tr>
							<tr valign="top">
								<td colspan=2>
								<table width="100%" border="0" cellspacing="0" cellpadding="0"
									...
									<tr>
										<td><comment id="_NSID_"> <OBJECT id=GR_DETAIL1
											width=100% height=125 classid=<%=Util.CLSID_GRID%>>
					...
					<tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td><b>[대입]</b></td>
								<td class="right"><img src="/<%=dir%>/imgs/btn/add_row.gif"
									id="IMG_ADD2" width="52" height="18" hspace="2"
									onclick="javascript:addDetailRow('D');" /><img
									src="/<%=dir%>/imgs/btn/del_row.gif" id="IMG_DEL2" width="52"
									height="18" onclick="javascript:delDetailRow('D');" /></td>
							</tr>
							<tr valign="top">
								<td colspan=2>
								<table width="100%" border="0" cellspacing="0" cellpadding="0"
									...
									<tr>
										<td><comment id="_NSID_"> <OBJECT id=GR_DETAIL2
											width=100% height=75 classid=<%=Util.CLSID_GRID%>>
...
<!-- 대출거래형태 --> <comment id="_NSID_"> <object id=EM_BIZ_TYPE
	classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1 align="absmiddle">
</object> </comment><script> _ws_(_NSID_);</script> <!-- 대출협력사코드 --> <comment id="_NSID_">
<object id=EM_VEN_CD classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1
	align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script> <!-- 대출과세구분 -->
<comment id="_NSID_"> <object id=EM_TAX_FLAG
	classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1 align="absmiddle">
</object> </comment><script> _ws_(_NSID_);</script> <!-- 대입바이어 --> <comment id="_NSID_">
<object id=EM_P_BUYER_CD classid=<%=Util.CLSID_EMEDIT%> width=30
	tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
<!-- 대입바이어명 --> <comment id="_NSID_"> <object id=EM_P_BUYER_NM
	classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1 align="absmiddle">
</object> </comment><script> _ws_(_NSID_);</script> <!-- 대입거래형태 --> <comment id="_NSID_">
<object id=EM_P_BIZ_TYPE classid=<%=Util.CLSID_EMEDIT%> width=30
	tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
<!-- 대입과세구분 --> <comment id="_NSID_"> <object id=EM_P_TAX_FLAG
	classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1 align="absmiddle">
</object> </comment><script> _ws_(_NSID_);</script> <!-- 대입협력사코드 --> <comment id="_NSID_">
<object id=EM_P_VEN_CD classid=<%=Util.CLSID_EMEDIT%> width=30
	tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
<!-- 대입수량합계 --> <comment id="_NSID_"> <object id=EM_P_TOT_QTY
	classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1 align="absmiddle">
</object> </comment><script> _ws_(_NSID_);</script> <!-- 대입원가계 --> <comment id="_NSID_">
<object id=EM_P_TOT_COST_AMT classid=<%=Util.CLSID_EMEDIT%> width=30
	tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
<comment id="_NSID_"> <!-- 대입매가계 --> <object
	id=EM_P_TOT_SALE_AMT classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1
	align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script> <!-- 대입명세건수 -->
<comment id="_NSID_"> <object id=EM_P_DTL_CNT
	classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1 align="absmiddle">
</object> </comment><script> _ws_(_NSID_);</script> <!-- 대입차익액합 --> <comment id="_NSID_">
<object id=EM_P_GAP_TOT_AMT classid=<%=Util.CLSID_EMEDIT%> width=30
	tabindex=1 align="absmiddle"> </object> </comment><script> _ws_(_NSID_);</script>
<!-- 대입신차익율 --> <comment id="_NSID_"> <object id=EM_P_GAP_RATE
	classid=<%=Util.CLSID_EMEDIT%> width=30 tabindex=1 align="absmiddle">
