...
// 	StartAdd();		// 처음 보이는 화면에 기본 타입을 보여주기 위해 설정
	...
	registerUsingDataset("psal308","DS_O_MASTER" );
	
}

function gridCreate1(){
	var	hdrProperies = '<FC>id={currow}				name="NO"			width=30	align=center	</FC>'
					 + '<FC>id=STR_CD				name="점코드"			width=90	align=center						show=false</FC>'
					 + '<FC>id=PC_CD				name="PC코드"		width=90	align=center						show=false</FC>'
					 ...
					 + '<G>							name="총매출" '
					 + '<C>id=SALE_AMT_TOT			name="매출"			width=100	align=right		mask="###,###"		sumtext={subsum(SALE_AMT_TOT)}</C>'
					 + '<C>id=CMPR_SALE_AMT_TOT		name="대비"			width=100	align=right		mask="###,###"		sumtext={subsum(CMPR_SALE_AMT_TOT)}</C>'
					 + '</G>'
					 + '<C>id=TREND_RATE			name="추세율"			width=100	align=right		mask="###.00"		sumtext={subsum(SALE_AMT_TOT)/subsum(CMPR_SALE_AMT_TOT)*100}</C>'
					 ...
					 + '<FC>id=CMPR_DT_S			name="대비시작일자"	width=80	align=center	mask="XXXX/XX/XX"	show=false</FC>'
					 + '<FC>id=CMPR_DT_E			name="대비종료일자"	width=80	align=center	mask="XXXX/XX/XX"	show=false</FC>'
					 ;
...
function btn_Search() {
	...
	TR_MAIN.Action="/dps/psal308.ps?goTo="+goTo+parameters;	 
...
function search_Detail() {
	...
	var	strSaleCmprDtS	= DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "CMPR_DT_S");	   //대비시작일자
	var	strSaleCmprDtE	= DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "CMPR_DT_E");	   //대비종료일자
	var	strOrgFlag		= EM_ORG_FLAG.CodeValue;
	...
	TR_DETAIL.Action="/dps/psal308.ps?goTo="+goTo+parameters;  
	...
	//viewChart("CHART_001","2",iframeWidth,iframeHeight,"매출월||대비월","SALE_AMT_TOT||CMPR_SALE_AMT_TOT","ORG_NAME",DS_O_MASTER,	GD_MASTER,"");
...
{	
	 ...
	 var url =	"/dps/jsp/psal/psal3081.jsp?"
			 + "&strStrCd="			  +strStrCd
			 + "&strDeptCd="		  +strDeptCd
			 + "&strTeamCd="		  +strTeamCd
			 + "&strPCCd="			  +strPCCd
			 ...
			 + "&strSaleCmprDtS="	  +strSaleCmprDtS
			 + "&strSaleCmprDtE="	  +strSaleCmprDtE
			 + "&strUserId="		  +strUserId
			 ;
	 
	 //document.getElementById('CHART_001').src=url;
...
function StartAdd(){
	...
	SetChartAxisStyle("Chart", "Left", 0, 2000000000, 400000000, "true", "true", "false");
	
	//타이틀
	TitleText("Chart", "Footer", "추세율현황(기간별)", "black", "14", 2, true);
...
function Start(){
	...
	Chart.Series(new_series).Title					= "대비";
...
<comment id="_NSID_"><object	id="DS_POS_FLOOR"	classid=<%=Util.CLSID_DATASET%>></object></comment><script>_ws_(_NSID_);</script>
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0"
									...
									<tr>
										...
										<td colspan="3"><comment id="_NSID_"> <object
											...
											<img src="/<%=dir%>/imgs/btn/date.gif"
											onclick="javascript:openCal('G',EM_SALE_CMPRDT_S)"
											align="absmiddle" /> ~ <comment id="_NSID_"> <object
											...
											<img src="/<%=dir%>/imgs/btn/date.gif"
											onclick="javascript:if(EM_SALE_CMPRDT_E.Enable) openCal('G',EM_SALE_CMPRDT_E);	"
											align="absmiddle" /></td>
									</tr>
								</table></td>
						</tr>
							...
							<td width="50%">
								<table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
									<tr>
										<td width="100%">
											<comment id="_NSID_"> 
												<object	id=GD_DETAIL width=100% height=750 classid=<%=Util.CLSID_GRID%> tabindex=1>
			...
			<table width="100%"	border="0" cellspacing="0" cellpadding="0">
			   <tr>
				  <td width="100%" height="270"	>
					  <iframe id="iframe_main" height="100%" width=	"100%"></iframe>
				  </td>
			   </tr>
			</table>
			</td>
...
<!-- 									<tr> -->
<!-- 										<td width="100%"> -->
<!-- 											<iframe width="100%" height=260 id=CHART_001 name=CHART_001 src=""	frameborder="0"	></iframe> -->
<%-- 											<comment id="_NSID_"> --%>
<%-- 												<object id=Chart width=800	height=260 classid=<%=Util.CLSID_SHIFTCHART%> tabindex=1> --%>
