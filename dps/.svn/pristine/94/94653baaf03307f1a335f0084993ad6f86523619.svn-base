...
//    initEmEdit(EM_GAP_RATE,        "NUMBER^3^2" , READ);           // 차익율
    ...
    registerUsingDataset("pord206","DS_O_LIST" );
 } 
...
function btn_Conf() {
    ...
    // 승인취소하려면
    if(DS_O_LIST.Namevalue(DS_O_LIST.RowPosition ,"SLIP_PROC_STAT") == "02"){ //영업팀장확정상태
        //승인 취소 하시겠습니까?
        if( showMessage(STOPSIGN, YESNO, "USER-1198") != 1 )
            return;
        else{       
            if( showMessage(STOPSIGN, YESNO, "USER-1212", "취소") == 1 ){        //정말 취소 하시겠습니까.
                strSlipProcStat = "02" //승인상태의 플래그로 승인을 취소하기 위함.
                sendFlag = "2";        //승인취소
                //alert("승인취소한다");   
            }else{
                return;
...
//          var strSlipProcStat = DS_O_LIST.NameValue(i + 1,"SLIP_PROC_STAT");
         ...
         var params = "&strSlipProcStat="+encodeURIComponent(strSlipProcStat)
            ...
            TR_MAIN.Action="/dps/pord206.po?goTo=conf"+params;
            TR_MAIN.Post();     
            k = k+1;
         }       
    }    
    ...
    if(strSlipProcStat=="02"){//확정상태였던 전표의 취소된 건수
...
function getList(){
  ...
  // var strSlip_flag     = RD_S_SLIP_FLAG.CodeValue;              // 전표구분 
   ...
   TR_L_MAIN.Action  = "/dps/pord206.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord206.po?goTo="+goTo+parameters;  
