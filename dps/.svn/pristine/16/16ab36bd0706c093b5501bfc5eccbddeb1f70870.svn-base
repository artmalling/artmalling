...
function doInit(){
    ...
    initEmEdit(EM_SALE_DT_S2,                      "YYYYMMDD",                PK);   //시작일자(대비기간)
    EM_SALE_DT_S2.alignment = 1;

    initEmEdit(EM_SALE_DT_E2,                      "YYYYMMDD",                PK);   //종료일자(대비기간)
    EM_SALE_DT_E2.alignment = 1;

    //현재년도 셋팅
    EM_SALE_DT_S.text =  varToMon+"01";
    EM_SALE_DT_E.text =  varToday;
    EM_SALE_DT_S2.text =  varToMon+"01";
    EM_SALE_DT_E2.text =  varToday;
    ...
    registerUsingDataset("psal452","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}					name="NO"				width=30	align=center</FC>'
                     ...
                     + '<FC>id=BRAND_NM 				name="브랜드"			width=100	align=center	SumText=합계 		SubSumText={decode(curlevel,1,"PC합계",2,"파트합계")}				SubBgColor=#99FFCC</FC>'
         			 + '<G>name="매출기간"'
                     + '<FC>id=TOT_SALE_AMT				name="총매출액"			width=90    align=right		mask="###,###"		sumtext={sum(TOT_SALE_AMT)}	subsumtext={subsum(TOT_SALE_AMT)}		SubBgColor=#99FFCC </FC>'
                     + '<FC>id=DC_AMT	    			name="에누리"			width=90    align=right		mask="###,###"		sumtext={sum(DC_AMT)}		subsumtext={subsum(DC_AMT)}		SubBgColor=#99FFCC </FC>'
                     + '<FC>id=NET_SALE_AMT				name="세제외순매출액"	width=90    align=right		mask="###,###"		sumtext={sum(NET_SALE_AMT)}	subsumtext={subsum(NET_SALE_AMT)}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=SALE_PROF_RATE			name="이익율(세제외순매출)"			width=140    align=right		mask="#,###.00"		sumtext={if(sum(NET_SALE_AMT) = 0 , 0.00 , round(sum(SALE_PROF_AMT)/sum(NET_SALE_AMT)*100,2))}		subsumtext={if(subsum(NET_SALE_AMT) = 0 , 0.00 , round(subsum(SALE_PROF_AMT)/subsum(NET_SALE_AMT)*100,2))}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=SALE_PROF_AMT			name="이익액"			width=90    align=right		mask="###,###"		sumtext={sum(SALE_PROF_AMT)}	subsumtext={subsum(SALE_PROF_AMT)}		SubBgColor=#99FFCC</FC>'
         			 + '</G>'
         			 
         			 + '<G>name="대비기간"'
                     + '<FC>id=TOT_SALE_AMT2			name="총매출액"			width=90    align=right     mask="###,###"		sumtext={sum(TOT_SALE_AMT2)}	subsumtext={subsum(TOT_SALE_AMT2)}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=DC_AMT2	    			name="에누리"			width=90    align=right		mask="###,###"		sumtext={sum(DC_AMT2)}			subsumtext={subsum(DC_AMT2)}		SubBgColor=#99FFCC </FC>'
                     + '<FC>id=NET_SALE_AMT2			name="세제외순매출액"	width=90    align=right     mask="###,###"		sumtext={sum(NET_SALE_AMT2)}	subsumtext={subsum(NET_SALE_AMT2)}		SubBgColor=#99FFCC</FC>'
                     + '<FC>id=SALE_PROF_RATE2			name="이익율(세제외순매출)"			width=140    align=right		mask="#,###.00"		sumtext={if(sum(NET_SALE_AMT2) = 0 , 0.00 , round(sum(SALE_PROF_AMT2)/sum(NET_SALE_AMT2)*100,2))}	subsumtext={if(subsum(NET_SALE_AMT2) = 0 , 0.00 , round(subsum(SALE_PROF_AMT2)/subsum(NET_SALE_AMT2)*100,2))}	SubBgColor=#99FFCC</FC>'
                     + '<FC>id=SALE_PROF_AMT2			name="이익액"			width=90    align=right     mask="###,###"		sumtext={sum(SALE_PROF_AMT2)}	subsumtext={subsum(SALE_PROF_AMT2)}		SubBgColor=#99FFCC</FC>'
         			 + '</G>'
                     
                     + '<FC>id=INC_RATE					name="신장율"			width=60    align=right     mask="#,###.00"		sumtext={if(sum(TOT_SALE_AMT2) = 0 , 0.00 , round(sum(TOT_SALE_AMT)/sum(TOT_SALE_AMT2)*100,2))}		subsumtext={if(subsum(TOT_SALE_AMT2) = 0 , 0.00 , round(subsum(TOT_SALE_AMT)/subsum(TOT_SALE_AMT2)*100,2))}		SubBgColor=#99FFCC</FC>'
                     ;
...
function btn_Search() {
    ...
    var strSaleDtS2     = EM_SALE_DT_S2.text;         //시작일자(대비기간)
    var strSaleDtE2     = EM_SALE_DT_E2.text;         //종료일자 (대비기간)
    ...
    var parameters = "&strStrCd="           +encodeURIComponent(strStrCd)
				   ...
				   ;
    TR_MAIN.Action="/dps/psal452.ps?goTo="+goTo+parameters;  
...
function chkValidation(gbn){
    ...
    case "search" :
        ...
        if (EM_SALE_DT_S2.text.replace(" ","").length == 8 
         && EM_SALE_DT_E2.text.replace(" ","").length == 8) {
	        
        	if(!isBetweenFromTo(EM_SALE_DT_S2.text, EM_SALE_DT_E2.text)){
	            showMessage(INFORMATION, OK, "USER-1015"); 
	            EM_SALE_DT_S2.focus();
	            return false;
	        }
