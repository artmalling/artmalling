...
public class PCod004DAO extends AbstractDAO {
	...
	public List searchMaster(ActionForm form) throws Exception {
		...
		if( !strStrCd.equals("%") && !strStrCd.equals("")){
			...
			strQuery += svc.getQuery("SEL_BUYERMST_WHERE_STR_CD") + "\n";
		}
		if( !strOrgFlag.equals("%") && !strOrgFlag.equals("")){
			sql.setString(i++, strOrgFlag);
			strQuery += svc.getQuery("SEL_BUYERMST_WHERE_ORG_FLAG") + "\n";
		}
		if( !strUseYn.equals("%") && !strUseYn.equals("")){
			sql.setString(i++, strUseYn);
			strQuery += svc.getQuery("SEL_BUYERMST_WHERE_USE_YN") + "\n";
		}
		if( !strDeptCd.equals("%") && !strDeptCd.equals("")){
			sql.setString(i++, strDeptCd);
			strQuery += svc.getQuery("SEL_BUYERMST_WHERE_DEPT_CD") + "\n";
		}
		if( !strTeamCd.equals("%") && !strTeamCd.equals("")){
			sql.setString(i++, strTeamCd);
			strQuery += svc.getQuery("SEL_BUYERMST_WHERE_TEAM_CD") + "\n";
		}
		if( !strPcCd.equals("%") && !strPcCd.equals("")){
			sql.setString(i++, strPcCd);
			strQuery += svc.getQuery("SEL_BUYERMST_WHERE_PC_CD") + "\n";
		}
