 ...
 */
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}    name="NO"              width=30    align=center</FC>'
                      + '<FC>id=VEN_CD      name="협력사"          width=70     align=center suppress=1 SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} SubSumText=""</FC>'
                      + '<FC>id=VEN_NM      name="협력사명"        width=120    align=left   suppress=1 SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} SubSumText={decode(curlevel,1,"협력사소계",2,"사업자별 소계")}</FC>'
                      + '<FC>id=PUMBUN_CD   name="브랜드코드"        width=70     align=center Edit=none suppress=1 SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} SubSumText=""</FC>'
                      + '<FC>id=PUMBUN_NM   name="브랜드명"          width=120    align=left Edit=none suppress=1 SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} SubSumText=""</FC>' 
                      + '<FC>id=PAY_DT      name="지불일자"        width=150    align=center SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}</FC>'
                      + '<FC>id=J_TOT_AMT   name="전월이월액"      width=90     align=right  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=TOT_AMT     name="당월지불대상액"  width=90     align=right  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=IN_AMT     name="매입금액"        width=110     align=right  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=SUP_AMT     name="공급가액"        show=false   width=80     align=right  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=VAT_AMT     name="부가세액"        show=false   width=80     align=right  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=PAY_GUBUN   name="구분"           width=60    align=left SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}</FC>'                      
                      + '<FC>id=DEBIT_AMT   name="지불액"          width=110     align=right  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} subsumtext={subsum(SUM_DEBIT_AMT)} sumtext={subsum(SUM_DEBIT_AMT)}</FC>'
                      + '<FC>id=PAY_AMT     name="당월잔액"        width=110     align=right  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
...
//     GR_MASTER.ColumnProp("VEN_NM", "SubSumText")        = "협력사별 소계";  
 ...
 */
 function btn_Print() {
    ...
    var strBizTypeNm = LC_S_BIZ_TYPE.Text;                        // 거래형태명
    ...
    var strPumCd     = EM_S_PUMBUN_CD.Text;                          // 협력사       
    var strPumNm     = EM_S_PUMBUN_NM.Text;                          // 협력사명     
 ...
 //    var strUserId     = '<c:out value="${sessionScope.sessionInfo.USER_ID}" />';
    ...
    var parameters = "&strStrCd="+strStrCd  
                   ...
                   + "&strPayCnt="+strPayCnt
                   + "&strPayDt="+strPayDt
                   + "&strPayDtNm="+encodeURIComponent(strPayDtNm)
                   + "&strBizType="+strBizType   
                   + "&strBizTypeNm="+encodeURIComponent(strBizTypeNm)
                   + "&strVenCd="+strVenCd
                   + "&strVenNm="+encodeURIComponent(strVenNm)
                   + "&strPumCd="+strPumCd
                   + "&strPumNm="+encodeURIComponent(strPumNm)
                   + "&strPayInfo="+encodeURIComponent(strPayInfo); 
    window.open("/dps/ppay309.pp?goTo=print"+parameters,"OZREPORT", 1000, 700);
 }
 ...
 */
 function getMaster(){
    ...
    var strSaleEdt = EM_S_SALE_E_DT.Text;                       // 매출기간종료 
    var strVenCd   = EM_S_VEN_CD.Text;                          // 협력사
    var strPumCd   = EM_S_PUMBUN_CD.Text;                       // 브랜드
    ...
    TR_MAIN.Action  = "/dps/ppay309.pp?goTo="+goTo+parameters;  
  ...
  function getPayDt() {
	...
	var goTo         = "getPayDt" ;    
 	...
 	var parameters = "&strStrCd="+encodeURIComponent(strStrCd)   
 	 ...
 	 TR_DETAIL.Action="/dps/ppay309.pp?goTo="+goTo+parameters;  
 ...
 function getVenInfo(code, name, btnFlag){
   ...
   function setGrid(){
       ...
       if(strBizType == 3){  //임대을A일경우
    	   GR_MASTER.ColumnProp("IN_AMT",      "show")   = false;   // 매입금액
       }else{
           GR_MASTER.ColumnProp("IN_AMT",      "show")   = true;   // 매입금액
       }
   }
...
<Param Name="SubsumExpr" VALUE="2:COMP_NO,1:VEN_CD">
