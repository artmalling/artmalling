<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 발주관리 > 신선단품 매입발주 등록
 * 작 성 일 : 2010.02.21
 * 작 성 자 : 김 경 은
 * 수 정 자 : 
 * 파 일 명 : pord1030.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 신선단품으로 관리하는 상품의 매입/반품 발주 등록한다.
 * 이    력 :
 * 
 ******************************************************************************/
-->
 ...
 */
 ...
 function doInit(){
    ...
    registerUsingDataset("pord103","DS_IO_MASTER,DS_IO_DETAIL" );
 ...
 function gridCreate2(){
      var hdrProperies = '<FC>id={currow}         name="NO"           width=30    align=center  sumtext=""</FC>'    
                      ...
                      + '</FG> '                                                    
                      + '<FG> name="매가 (부가세 포함)"'                                       
                      + '<FC>id=NEW_SALE_PRC      name="단가"         width=80    align=right   Edit=Numberic </FC>'                        
...
function btn_Delete() {
    ...
    if(showMessage(Question, YESNO, "USER-1023") == 1){  
        ...
        TR_MAIN.Action="/dps/pord103.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){  
        ...
        TR_SAVE.Action   = "/dps/pord103.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
...
function getList(){
   ...
   TR_S_MAIN.Action  = "/dps/pord103.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord103.po?goTo="+goTo+parameters;  
...
function getSkuInfo(){
   ...
   var strPumbunCd  = EM_PUMBUN_CD.Text;                                    // 점
   ...
   if(DS_O_SKU_INFO.CountRow > 0){
       ...
       if(strBizType == "1"){                // 직매입
    	   GR_DETAIL.ColumnProp("RATE", "name") = "목표이익율";
       //    DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "NEW_GAP_RATE") = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "GOAL_PROF_RATE"); 
    	   ...
    	   GR_DETAIL.ColumnProp("RATE", "name") = "마진율";
...
<script language=JavaScript for=DS_O_LIST event=onRowPosChanged(row)>
    ...
    if(row != 0){
        ...
        if(strSlipNo != ""){
            ...
            if(strBizType == "1"){                // 직매입
                GR_DETAIL.ColumnProp("RATE", "name") = "목표;이익율";
            }else if(strBizType == "2"){          // 특정
                GR_DETAIL.ColumnProp("RATE", "name") = "마진율";
            }
