 ...
 */
 ...
 function doInit(){
     ...
     DS_I_SEARCH2.setDataHeader('<gauce:dataset name="H_SEARCH"/>');
     ...
     initEmEdit(EM_YYYYMMDD2,		"TODAY",	NORMAL);		// 매입/매출일자
    
     //콤보 초기화
     initComboStyle(LC_STR_CD,DS_IO_STR_CD,        "CODE^0^30,NAME^0^140", 1, NORMAL);    // 점(조회)	
     
     initComboStyle(LC_STR_CD2,DS_IO_STR_CD,        "CODE^0^30,NAME^0^140", 1, NORMAL);    // 점(조회)
 ...
 *************************************************************************/
 ...
 function checkValidation(Gubun) {
      ...
      if(Gubun == "Save"){
    	  ...
    	  if(EM_YYYYMMDD.Text.length == 0){                                // 매입/매출일자
              showMessage(INFORMATION, OK, "USER-1002", "매출일자");
              EM_YYYYMMDD.Focus();
              return false;
          }
          return true;
    	  ...
    	  if(LC_STR_CD2.Text.BindColVal == "" ){
              showMessage(EXCLAMATION, OK, "USER-1002", "점");
              LC_STR_CD2.Focus();
              
              return false;
          }
    	  if(EM_YYYYMMDD2.Text.length == 0){                                // 매입/매출일자
              showMessage(INFORMATION, OK, "USER-1002", "매출일자");
              EM_YYYYMMDD2.Focus();
              alert("b");
              return false;
  ...
  */
  ...
  function CloseCheck(){
     ...
     TR_VALCHECK.Action="/dps/psal538.ps?goTo=valCheck"; 
 ...
 function process(){
	  ...
	  //변경또는 신규 내용을 저장하시겠습니까?
        if( showMessage(QUESTION, YESNO, "USER-1204") == 1 ){    // validation 체크
            TR_MAIN.Action="/dps/psal538.ps?goTo=save&strFlag=save"; 
 ...
 function process2(){
	    ...
	    if(!checkValidation("Save2"))
	        return;
	    
	    
	  //변경또는 신규 내용을 저장하시겠습니까?
     if( showMessage(QUESTION, YESNO, "USER-1204") == 1 ){    // validation 체크
         TR_MAIN.Action="/dps/psal538.ps?goTo=save2&strFlag=save"; 
         TR_MAIN.KeyValue="SERVLET(I:DS_I_SEARCH=DS_I_SEARCH2)"; //조회는 O
...
<comment id="_NSID_"><object id="DS_I_SEARCH2"     classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
...
<!--공통 타이틀/버튼 // -->
  ...
  <tr>
  	<br><br><br><b><font face="굴림">LG CNS POS 정산처리</font></b>
  </tr>
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          ...
          <tr>
            ...
            <td>
                    <comment id="_NSID_">
                        <object id=LC_STR_CD2 classid=<%= Util.CLSID_LUXECOMBO %> width=100% tabindex=1 align="absmiddle">
                        </object>
                    </comment><script>_ws_(_NSID_);</script>
          ...
          <tr>
            ...
            <td>
                  <comment id="_NSID_">
                      <object id=EM_YYYYMMDD2 classid=<%=Util.CLSID_EMEDIT%>  width=75 tabindex=1 align="absmiddle">
                      </object>
                  </comment>
                  <script> _ws_(_NSID_);</script>
                  <img src="/<%=dir%>/imgs/btn/date.gif" onclick="javascript:openCal('G',EM_YYYYMMDD2)"  align="absmiddle"/>
            </td>
          </tr> 
         <tr>
             <td colspan=2 align="Right">
               <img src="/<%=dir%>/imgs/btn/process.gif" id="IMG_porcess" align="absmiddle" onclick="javascript:process2();" /> 
             </td>
         </tr>
        </table></td>
      </tr>
...
<comment id="_NSID_">
    <object id=BD_SEARCH2 classid=<%=Util.CLSID_BIND%>>
        <param name=DataID          value=DS_I_SEARCH2>
        <param name="ActiveBind"    value=true>
        <param name=BindInfo        value='
            <c>Col=STR_CD              Ctrl=LC_STR_CD2             param=BindColVal</c>
            <c>Col=PAY_YM              Ctrl=EM_YYYYMMDD2           param=Text</c>           
