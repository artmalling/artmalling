 ...
 * 개    요 : 외상매출현황
 * 이    력 :2010.05.19 박종은
 *        2010.07.19 박종은  조회조건 변경 및 데이터 컬럼 변경
 ******************************************************************************/
-->
...
function doInit(){
    ...
    getEtcCode("DS_CREDIT_FLAG"      , "D", "P607", "Y");
    ...
    registerUsingDataset("psal427","DS_O_MASTER" );
    
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"             width=30    align=center    </FC>'
                     + '<FC>id=SALE_DT                 name="일자"            width=100    align=center      mask="XXXX/XX/XX" suppress="1" sumtext="합계"</FC>'
                     + '<FC>id=POS_NO                  name="POS번호"         width=70    align=center     suppress="2"</FC>'
                     + '<FC>id=TRAN_NO                 name="거래번호"         width=80    align=center      suppress="3"</FC>'
                     + '<FC>id=GUBUN                   name="외상구분"         width=100    align=left     </FC>'
                     + '<FC>id=SALE_AMT                name="매출액"           width=100    align=right    mask="###,###"    sumtext={subsum(SALE_AMT)}</FC>'
                     + '<FC>id=BALANCE_AMT             name="(사용/임대)금액"   width=100    align=right    mask="###,###"    sumtext={subsum(BALANCE_AMT)}</FC>'
                     + '<C>id=DM_NAME                  name="담당자명"          width=100    align=left     </C>'
                     + '<C>id=DM_SABUN                 name="담당자사번"         width=100    align=center     </C>'
                     ;
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal427.ps?goTo="+goTo+parameters;  
...
function searchPosNo(strPosNo){
    ...
    TR_DETAIL.Action="/dps/psal427.ps?goTo="+goTo+parameters;  
