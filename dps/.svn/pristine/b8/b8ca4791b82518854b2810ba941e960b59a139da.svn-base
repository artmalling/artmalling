...
function doInit(){
    ...
    initEmEdit(EM_O_SKU_NAME,        "GEN^40",      READ);            //단품(조회)
    ...
    registerUsingDataset("psal404","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"           width=30    align=center    </FC>'
                     ...
                     + '<FC>id=PUMBUN_CD               name="브랜드코드"        width=50    align=center      suppress=1    </FC>'
                     + '<FC>id=PUMBUN_NAME             name="브랜드명"         width=80    align=left      suppress=1   sumtext="합계"</FC>'
                     + '<G>                            name="마진"  '
                     + '<C>id=EVENT_FLAG               name="행사구분"      width=60    align=center   </C>'
                     + '<C>id=EVENT_RATE               name="행사율"        width=60    align=right   </C>'
                     + '</G>'
                     + '<FC>id=MG_RATE                 name="마진율"        width=60    align=right     mask="#,###.00"    </FC>'
                     + '<FC>id=SALE_AMT_ORIGIN         name="총매출"      width=90    align=right    mask="###,###"      sumtext={subsum(SALE_AMT_ORIGIN)}</FC>'
                     ...
                     + '<FC>id=SALE_PROF_AMT           name="이익액"        width=90    align=right     mask="###,###"    sumtext={subsum(SALE_PROF_AMT)}</FC>'
                     + '<FC>id=PROFRATE                name="이익율"        width=60    align=right     mask="#,###.00"   sumtext={subsum(NET_SALE_AMT)/subsum(SALE_AMT)*100}</FC>'
                     ;
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal404.ps?goTo="+goTo+parameters;  
...
<script language=JavaScript for=EM_O_SKU_CD event=onKillFocus()>
    ...
    EM_O_SKU_NAME.Text = "";
    if(EM_O_SKU_CD.text.length != 0){            
        var rtnMap = setStrSkuNmWithoutPop("DS_SEARCH_NM",EM_O_SKU_CD.Text,EM_O_SKU_NAME.Text, 'Y', '0','','','','1');
        //setStrSkuNmWithoutPop("DS_SEARCH_NM",EM_O_SKU_CD.Text,EM_O_SKU_NAME.Text,scrFlag=="I"?'N':'Y',scrFlag=="I"?1:0,'','',scrFlag=="I"?'2':'','',scrFlag=="I"?'Y':'','',scrFlag=="I"?'':'1');
        if (rtnMap != null){
            EM_O_SKU_CD.Text = rtnMap.get("EM_O_SKU_CD");
            EM_O_SKU_NAME.Text = rtnMap.get("EM_O_SKU_NAME");
        }else{
            EM_O_SKU_CD.Text = "";
            EM_O_SKU_NAME.Text = "";
        }
         ...
         <tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
		            ...
		            <td colspan="7">
		              <comment id="_NSID_">
		                <object id=EM_O_SKU_CD classid=<%=Util.CLSID_EMEDIT%> width=107 tabindex=1 align="absmiddle"></object>
		              </comment><script> _ws_(_NSID_);</script>
		              <img src="/<%=dir%>/imgs/btn/detail_search_s.gif"  onclick="javascript:strSkuPop(EM_O_SKU_CD,EM_O_SKU_NAME,'Y','','','','','','','1');"  align="absmiddle" />
		              <comment id="_NSID_">
