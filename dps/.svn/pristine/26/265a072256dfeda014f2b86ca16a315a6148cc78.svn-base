...
public class PSal104DAO extends AbstractDAO {
	 ...
	 * @throws Exception
	 */
	public List searchDetail(ActionForm form, String OrgFlag) throws Exception {
		...
		connect("pot");
		sql.put(svc.getQuery("SEL_PLANTEAMDETAIL"));
	 ...
	 * @throws Exception
	 */
	public List searchTeam(ActionForm form, String OrgFlag) throws Exception {
	...
	public List searchDetailCheck(ActionForm form, String OrgFlag) throws Exception {
		...
		sql.put(svc.getQuery("SEL_PLANTEAM_DATA_CHECK"));
			...
			while (mi[0].next()) {//DateSet의 현제 Row의 상태를 체크한다 IS_INSERT, IS_UPDATE, IS_DELETE
				...
				if (mi[0].IS_UPDATE()) {
					if(mi[0].getString("CHK_FLAG").equals("T")){
						...
						sql.setString(8, mi[0].getString("PLAN_YEAR"));
						sql.setString(9, "N");		
