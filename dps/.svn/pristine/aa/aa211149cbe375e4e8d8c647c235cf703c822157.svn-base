<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 발주관리 > 의류단품 대출입발주
 * 작 성 일 : 2010.03.08
 * 작 성 자 : 김 경 은
 * 수 정 자 : 
 * 파 일 명 : pord1070.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 의류단품으로 관리하는 상품의 대출입 발주 등록한다.
 * 이    력 :
 * 
 ******************************************************************************/
-->
...
var strCStyleType     = "";                            // 대출의류단품구분(1:A타입, 2:B타입)
var strDStyleType     = "";                            // 대출의류단품구분(1:A타입, 2:B타입)
 ...
 */
 ...
 function doInit(){
    ...
    registerUsingDataset("pord107","DS_IO_MASTER,DS_IO_DETAIL1,DS_IO_DETAIL2");
 } 
 ...
 function gridCreate2(){
     var hdrProperies = '<FC>id={currow}          name="NO"          width=30     align=center  sumtext=""</FC>'                           
                      ...
                      + '<FC>id=STYLE_CD          name="스타일"       width=82    align=center   Edit=none </FC>'
...
function btn_Delete() {
    ...
    if(showMessage(Question, YESNO, "USER-1023") == 1){  
        ...
        TR_MAIN.Action="/dps/pord107.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){
       ...
       TR_SAVE.Action="/dps/pord107.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
...
function getList(){
   ...
   TR_S_MAIN.Action="/dps/pord107.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord107.po?goTo="+goTo+parameters;  
   ...
   if(strSlipFlag == "C"){
       strCStyleType = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"STYLE_TYPE1");
       strDStyleType = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"STYLE_TYPE2");
   }else{
       strCStyleType = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"STYLE_TYPE2")
       strDStyleType = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"STYLE_TYPE1");
   }
...
function getPumbunInfo(gubun){
    ...
    if(rtnMap != null){
          ...
          if(gubun == "C"){
              strCStyleType = rtnMap.get("STYLE_TYPE");
              if(strCStyleType == "1"){
              ...
              strDStyleType = rtnMap.get("STYLE_TYPE");
              if(strDStyleType == "1"){
                  GR_DETAIL.ColumnProp("COLOR_CD", "SHOW") = "True";
                  GR_DETAIL.ColumnProp("SIZE_CD" , "SHOW") = "True";
              }else if(strDStyleType == "2"){
 ...
 */
 function setStrPbnNmWithoutPopCall(gubun){
    ...
    if(DS_O_RESULT.CountRow == 1){
        ...
        if(gubun == "C"){
            strCStyleType = DS_O_RESULT.NameValue(DS_O_RESULT.RowPosition, "STYLE_TYPE");
            if(strCStyleType == "1"){
                GR_DETAIL.ColumnProp("COLOR_CD", "SHOW") = "True";
                GR_DETAIL.ColumnProp("SIZE_CD" , "SHOW") = "True";
            }else if(strCStyleType == "2"){
            ...
            strDStyleType = DS_O_RESULT.NameValue(DS_O_RESULT.RowPosition, "STYLE_TYPE")
            if(strDStyleType == "1"){
                GR_DETAIL.ColumnProp("COLOR_CD", "SHOW") = "True";
                GR_DETAIL.ColumnProp("SIZE_CD" , "SHOW") = "True";
            }else if(strDStyleType == "2"){
...
function getSkuPop(row, colid, gubun){
    ...
    var strBizType      = BIZ_TYPE.Text;                                             // 거래형태(1:직매입)
    var strUseYn        = "Y";                                                       // 사용여부
    var strMultiGb      = "M";
    var strStyleType    = "";
    if(gubun == "C"){
    	strStyleType = strCStyleType;
        ...
        if(rtnList != null){
            for(var i = 0; i < rtnList.length; i++){
                ...
                DS_DETAIL.NameValue(row+i, "ORD_UNIT_CD") = rtnList[i].SALE_UNIT_CD;
                getSkuInfo(gubun);
            }
            if(rtnList.length > 1)
    	...
    	strStyleType = strDStyleType;
    	if(strStyleType == "1"){
            rtnMap = skuATypeToGridPordPop(strSkuCd, "", "Y"
                                             , strUsrCd, strStrCd, strPumbunCd, "", ""
                                             , "", strPumbunType, strBizType, strUseYn);
        }else{    
        	rtnMap = skuBTypeToGridPordPop(strSkuCd, "", "Y"
                                             , strUsrCd, strStrCd, strPumbunCd, "", ""
                                             , "", "", "", strPumbunType, strBizType
                                             , strUseYn, "", "");
        }
    	...
    	if(rtnMap != null){
            ...
            DS_DETAIL.NameValue(DS_DETAIL.RowPosition, "ORD_UNIT_CD") = rtnMap.get("CMP_SPEC_CD");
            getSkuInfo(gubun);
    	}
    }
}
...
function getSkuInfo(gubun){  
   ...
   DS_DETAIL.NameValue(DS_DETAIL.RowPosition, "STYLE_CD")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "STYLE_CD");        // 스타일
   DS_DETAIL.NameValue(DS_DETAIL.RowPosition, "COLOR_CD")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "COLOR_CD");        // 칼라
   DS_DETAIL.NameValue(DS_DETAIL.RowPosition, "SIZE_CD")       = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SIZE_CD");         // 사이즈
   DS_DETAIL.NameValue(DS_DETAIL.RowPosition, "ORD_UNIT_CD")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_UNIT_CD");    // 단위명
 ...
 */
 function setGapCalc(row, colid){
	...
	var cost_amt = getRoundDec(roundFlag, ord_qty*cost_prc);      // 원가금액
	var sale_amt = getRoundDec(roundFlag, ord_qty*sale_prc);      // 매가금액
	var idx = 0; 
	...
	if (colid == "ORD_QTY" || colid == "NEW_COST_PRC" || colid == "NEW_SALE_PRC") {
    	DS_IO_DETAIL1.NameValue(row, "NEW_COST_AMT") = cost_amt;                    // 원가금액
   	    ...
   	    totCostAmt = DS_IO_DETAIL1.Sum(19,0,0);
        totSaleAmt = DS_IO_DETAIL1.Sum(21,0,0);
        ...
        EM_GAP_TOT_AMT.Text  = DS_IO_DETAIL1.Sum(15,0,0);
        
        EM_TOT_QTY.Text      = DS_IO_DETAIL1.Sum(17,0,0);
        EM_TOT_COST_AMT.Text = DS_IO_DETAIL1.Sum(19,0,0);
        EM_TOT_SALE_AMT.Text = DS_IO_DETAIL1.Sum(21,0,0);
        DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VAT_TAMT")  = DS_IO_DETAIL1.Sum(27,0,0);
   	    
        DS_IO_DETAIL2.NameValue(DS_IO_DETAIL2.RowPosition, "NEW_COST_AMT") = DS_IO_DETAIL1.Sum(19,0,0);          // 대입원가금액
        DS_IO_DETAIL2.NameValue(DS_IO_DETAIL2.RowPosition, "NEW_SALE_AMT") = DS_IO_DETAIL1.Sum(21,0,0);          // 대입매가금액
        DS_IO_DETAIL2.NameValue(DS_IO_DETAIL2.RowPosition, "VAT_AMT")      = DS_IO_DETAIL1.Sum(27,0,0);          // 대입부가세
    }
  ...
  */
  function setGapCalc2(row, colid){
	  ...
	  if(colid == "ORD_QTY" || colid == "NEW_COST_AMT" || colid == "NEW_SALE_AMT"){
	      ...
	      var cost_amt = DS_IO_DETAIL1.Sum(19,0,0);                                  // 원가금액
	      var sale_amt = DS_IO_DETAIL1.Sum(21,0,0);                                  // 매가금액
	      var cost_prc = DS_IO_DETAIL1.Sum(19,0,0)/ord_qty;                          // 원가단가
	      var sale_prc = DS_IO_DETAIL1.Sum(21,0,0)/ord_qty;                          // 매가단가
	      ...
	      if(strTaxFlag == "1"){        // 과세
	          if(DS_IO_DETAIL2.Sum(21,0,0) > 0){
	              ...
	              EM_P_GAP_RATE.Text = ((DS_IO_DETAIL2.Sum(21,0,0)/1.1) - DS_IO_DETAIL2.Sum(19,0,0))/(DS_IO_DETAIL2.Sum(21,0,0)/1.1) * 100;     // 총차익율
	              strSumText = "{((SUM(NEW_SALE_AMT)/1.1) - SUM(NEW_COST_AMT))/(SUM(NEW_SALE_AMT)/1.1) * 100}";
	              EM_P_GAP_TOT_AMT.Text  = getRoundDec(roundFlag,DS_IO_DETAIL2.Sum(21,0,0)/1.1 - DS_IO_DETAIL2.Sum(19,0,0));
	           }
	          ...
	          if(DS_IO_DETAIL2.Sum(21,0,0) > 0){
	              ...
	              EM_P_GAP_RATE.Text = (DS_IO_DETAIL2.Sum(21,0,0) - DS_IO_DETAIL2.Sum(19,0,0))/DS_IO_DETAIL2.Sum(21,0,0) * 100;                // 총차익율
	              strSumText = "{(SUM(NEW_SALE_AMT) - SUM(NEW_COST_AMT))/SUM(NEW_SALE_AMT) * 100}";
	              EM_P_GAP_TOT_AMT.Text  = DS_IO_DETAIL2.Sum(21,0,0) - DS_IO_DETAIL2.Sum(19,0,0);
	          }
	      }
	      GR_DETAIL2.ColumnProp("NEW_GAP_RATE", "SumText") = strSumText;
	      EM_P_TOT_QTY.Text = DS_IO_DETAIL2.Sum(17,0,0);
	      EM_P_TOT_COST_AMT.Text = DS_IO_DETAIL2.Sum(19,0,0);
	      EM_P_TOT_SALE_AMT.Text = DS_IO_DETAIL2.Sum(21,0,0);
                            ...
                            <tr valign="top">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                                    <tr>
                                        <td>
                                          <comment id="_NSID_"> 
                                              <OBJECT id=GR_DETAIL1 width=100% height=125 classid=<%=Util.CLSID_GRID%>>
                            ...
                            <tr valign="top">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                                    <tr>
                                        <td>
                                          <comment id="_NSID_"> 
                                              <OBJECT id=GR_DETAIL2 width=100% height=70 classid=<%=Util.CLSID_GRID%>>
