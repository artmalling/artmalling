...
public class PPay108DAO extends AbstractDAO { 
	/**
	...
	public List getList(ActionForm form) throws Exception {
		...
		if(!strFlag.equals("")){		
			sql.put(svc.getQuery("SEL_TAX_ISSUE_FLAG"));
	...
	public String save(ActionForm form, MultiInput mi1, String userId, String org_flag)
		...
		try {
			...
			while (mi1.next()) {
				...
				strKey = mi1.getString("STR_CD") + mi1.getString("TAX_ISSUE_DT")+ mi1.getString("TAX_ISSUE_SEQ_NO");
			}
		} catch (Exception e) {
			rollback();
			throw e;
		...
		try {
			...
			while (mi1.next()) {
				...
				if(mi1.IS_DELETE()){// 삭제
					...
					updateTaxInfo(form, userId, mi1.getString("STR_CD"), mi1.getString("TAX_ISSUE_DT"), mi1.getString("TAX_ISSUE_SEQ_NO"));
	...
	/**
	 *  DPS.PO_SLPMST    세금계산서발행일, 순번 UPDATE
	 *  DPS.PP_PREDEFDED 세금계산서발행일, 순번 UPDATE
	 *  DPS.PP_VENPAYDTL 세금계산서발행일, 순번 UPDATE
	 * 
	 * @param form
	 * @param mi1
	 * @param strID
	...
	public int updateTaxInfo(ActionForm form, String userId, String strStrCd 
										    , String strTaxIssueDt, String strTaxIssueSeqNo)
			throws Exception {
		int res = 0;
		SqlWrapper sql = null;
		Service svc = null;
		...
		try {
			...
			sql.close();
			i = 1;
			sql.put(svc.getQuery("UPD_SLPMST_DEL")); 
			...
			sql.close();
			i = 1;
			sql.put(svc.getQuery("UPD_PREDEFDED")); 
			...
			sql.close();
			i = 1;
			sql.put(svc.getQuery("UPD_VENPAYDTL")); 
