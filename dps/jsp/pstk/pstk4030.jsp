<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 재고수불 > 저장품 > 저장품출고등록 
 * 작 성 일 : 2010.06.02
 * 작 성 자 : 정진영
 * 수 정 자 : 
 * 파 일 명 : pstk4030.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 저장품 출고/출고반품 등록관리를  한다.
 * 이    력 :
 *        2010.06.02 (정진영) 신규작성
 ******************************************************************************/
-->
...
function doInit(){
    ...
    initEmEdit(EM_OUT_TOT_QTY , "NUMBER^7^0" , READ);    //출고총수량
    initEmEdit(EM_OUT_TOT_AMT , "NUMBER^13^0", READ);    //출고총금액
    ...
    getEtcCode("DS_O_SLIP_FLAG"  , "D", "P804", "Y");
    
    getEtcCode("DS_SLIP_FLAG"    , "D", "P804", "N");
    ...
    registerUsingDataset("pstk403","DS_MASTER,DS_DETAIL" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}      width=30   align=center name="NO"           </FC>'
                     ...
                     + '<FC>id=SLIP_FLAG     width=90   align=left   name="출고/반품구분 " EditStyle=Lookup data="DS_SLIP_FLAG:CODE:NAME" </FC>';
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}      width=25   align=center  edit=none     sumtext=""   name="NO"          </FC>'
                     ...
                     + '<FC>id=OUT_QTY       width=75   align=right   edit=Numeric  sumtext=@sum name="*수량"        </FC>'
                     + '<FC>id=OUT_PRC       width=85   align=right   edit=Numeric  sumtext=@sum name="*원가"        </FC>'
                     + '<FC>id=OUT_AMT       width=85   align=right   edit=none     sumtext=@sum name="금액"         </FC>';
...
function btn_Delete() {
    ...
    btnSaveYn = true;
    TR_MAIN.Action="/dps/pstk403.pt?goTo=delete";  
...
function btn_Save() {
    ...
    btnSaveYn = true;
    TR_MAIN.Action="/dps/pstk403.pt?goTo=save";  
...
function searchMaster(){
    ...
    TR_SUB.Action="/dps/pstk403.pt?goTo="+goTo+parameters;      
...
function searchDetail(){
    ...
    var parameters = "&strStrCd="+encodeURIComponent(strStrCd)
                   ...
                   + "&strPumbunCd="+encodeURIComponent(strPumbunCd);
    TR_MAIN.Action="/dps/pstk403.pt?goTo="+goTo+parameters;      
...
function getCalDt(evnFlag, obj, scvFlag){
    ...
    if(obj.Text < getTodayFormat("YYYYMMDD")){  
        ...
        showMessage(INFORMATION, OK, "USER-1030","출고/반품일자");    
    }
}
/**
 * getPumbunCode()
...
function setSkuCodeToGrid(evnFlag, row){
    ...
    if( appDt == ""){
        showMessage(EXCLAMATION, OK, "USER-1003", "출고/반품일자");
    ...
    if( codeStr == "" && evnFlag != 'POP'){
        ...
        DS_DETAIL.NameValue(row,"OUT_QTY")     = "";
        DS_DETAIL.NameValue(row,"OUT_PRC")     = "";
        DS_DETAIL.NameValue(row,"OUT_AMT")     = "";
        return;
    }
    var rtnMap;
    if( evnFlag == 'POP'){
    ...
    if( detailData ){	
        ...
        if( DS_SEARCH_NM.CountRow == 1){
            DS_DETAIL.NameValue(row,"OUT_PRC")      = DS_SEARCH_NM.NameValue(1,"COL1");
            setAmtValue(row);
            setTotalValue();
            return;
        }
...
function enableCnt( mode ){
    ...
    enableControl(EM_OUT_TOT_QTY     , readFlag);
    enableControl(EM_OUT_TOT_AMT     , readFlag);
...
function setAmtValue(row){
	var qty = Number(DS_DETAIL.NameValue(row,"OUT_QTY"));
    var prc = Number(DS_DETAIL.NameValue(row,"OUT_PRC"));
	DS_DETAIL.NameValue(row,"OUT_AMT") = qty * prc;
}
/**
 * setTotalValue()
 * 작 성 자 : 
...
function setTotalValue(){
	EM_OUT_TOT_QTY.Text = DS_DETAIL.Sum(DS_DETAIL.ColumnIndex("OUT_QTY"),0,0);
	EM_OUT_TOT_AMT.Text = DS_DETAIL.Sum(DS_DETAIL.ColumnIndex("OUT_AMT"),0,0);
}
/**
...
function checkMasterValidation(){
    ...
    var totalQty = DS_DETAIL.Sum(DS_DETAIL.ColumnIndex("OUT_QTY"),0,0);
    var totalAmt = DS_DETAIL.Sum(DS_DETAIL.ColumnIndex("OUT_AMT"),0,0);
    ...
    if( LC_STR_CD.BindColVal == ""){
        ...
        EM_VEN_CD.Focus();
        return false;     
    }else if( totalQty != EM_OUT_TOT_QTY.Text ){
        showMessage(EXCLAMATION, OK, "USER-1000", "총수량과 전표상세의 수량 합계가 일치 하지 않습니다.");
        GD_DETAIL.Focus();
        return false;         	
    }else if( totalAmt != EM_OUT_TOT_AMT.Text ){
...
function checkDetailValidation(){
    ...
    for(var i=1; i<=DS_DETAIL.CountRow; i++){
        ...
        if( DS_DETAIL.NameValue(i,"OUT_QTY")==""){
            showMessage(EXCLAMATION, OK, "USER-1008", "수량", 0);
            colid = "OUT_QTY";    
            errYn = true;
            break;
        }
        if( DS_DETAIL.NameValue(i,"OUT_PRC")==""){
              ...
              <tr>
                <td ><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
                  <tr>
                    ...
                    <th width="75" class="point">출고/반품일자</th>
                    <td width="95">
                      <comment id="_NSID_">
                  ...
                  <tr>
                    ...
                    <td >
                      <comment id="_NSID_">
                        <object id=EM_OUT_TOT_QTY classid=<%= Util.CLSID_EMEDIT %> width=95 tabindex=1 align="absmiddle"></object>
                      </comment><script>_ws_(_NSID_);</script>
                    </td>
                    <th >총금액</th>
                    <td colspan="3">
                      <comment id="_NSID_">
                        <object id=EM_OUT_TOT_AMT classid=<%=Util.CLSID_EMEDIT%>  width=95  tabindex=1 align="absmiddle"></object>
                      </comment><script> _ws_(_NSID_);</script>
                    </td>
                  </tr>
                </table></td>
...
<object id=BO_HEADER classid=<%= Util.CLSID_BIND %>>
        ...
        <param name=BindInfo        value='
            ...
            <c>col=OUT_TOT_QTY     ctrl=EM_OUT_TOT_QTY    param=Text </c>
            <c>col=OUT_TOT_AMT     ctrl=EM_OUT_TOT_AMT    param=Text </c>
        '>
</object>
</comment><script>_ws_(_NSID_);</script>
</body>
