 ...
 */
 ...
 function doInit(){
     ...
     // EMedit에 초기화
     initEmEdit(EM_YYYYMM,       "THISMN",   READ);         // 매출년월
     initEmEdit(EM_CALC_DT,    "TODAY", NORMAL);            // 손익계산일자

     //콤보 초기화
     initComboStyle(LC_STR_CD,DS_IO_STR_CD,        "CODE^0^30,NAME^0^140", 1, NORMAL);    // 점(조회)
     initComboStyle(LC_AUTO_SLIP_FLAG,DS_O_AUTO_SLIP_FLAG,  "CODE^0^30,NAME^0^120", 1, NORMAL);    // 자동전표처리구분
     
     getStore("DS_IO_STR_CD",     "Y", "", "N");
     getEtcCode("DS_O_AUTO_SLIP_FLAG",  "D", "PA02", "N");            // 자동전표처리구분

     LC_STR_CD.index   = 0;
     LC_AUTO_SLIP_FLAG.index   = 0;
     EM_YYYYMM.Focus(); 
 ...
 function process(){
	  ...
	  //변경또는 신규 내용을 저장하시겠습니까?
        if( showMessage(QUESTION, YESNO, "USER-1204") == 1 ){    // validation 체크
        	...
        	DS_I_SEARCH.NameValue(DS_I_SEARCH.CountRow,"AUTO_SLIP_FLAG")     = LC_AUTO_SLIP_FLAG.BindColVal;
        	DS_I_SEARCH.NameValue(DS_I_SEARCH.CountRow,"CLOSE_TASK_FLAG") = "PSAL";
        	DS_I_SEARCH.NameValue(DS_I_SEARCH.CountRow,"CLOSE_UNIT_FLAG") = "41";
        	DS_I_SEARCH.NameValue(DS_I_SEARCH.CountRow,"CLOSE_YM")        = EM_YYYYMM.Text;
        	
            TR_MAIN.Action="/dps/pstk504.pt?goTo=save"; 
...
<comment id="_NSID_"><object id="DS_O_AUTO_SLIP_FLAG"   classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
            ...
            <td>
                    <comment id="_NSID_">
                        <object id=LC_AUTO_SLIP_FLAG classid=<%= Util.CLSID_LUXECOMBO %> width=100% tabindex=1 align="absmiddle">
                        </object>
                    </comment><script>_ws_(_NSID_);</script>
