<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 발주관리 > 규격단품 매가인상하발주 등록
 * 작 성 일 : 2010.03.28
 * 작 성 자 : 김 경 은
 * 수 정 자 : 
 * 파 일 명 : pord1130.jsp
...
//    LC_INC_RSN_CD.Index    = 0;
    ...
    registerUsingDataset("pord113","DS_O_LIST,DS_IO_MASTER,DS_IO_DETAIL");
 } 
...
function btn_Delete() {
    ...
    if(showMessage(QUESTION, YESNO, "USER-1023") == 1){  
        ...
        TR_MAIN.Action="/dps/pord113.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){
        ...
        TR_SAVE.Action="/dps/pord113.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
     ...
     if(Gubun == "Save"){
         ...
         if(intRowCount > 0){
             for(var i=1; i <= intRowCount; i++){
                 ...
                 fltNewGapAmt  = DS_IO_DETAIL.NameValue(i, "GAP_SALE_AMT");
                ...
                if(fltNewGapAmt <= 0 && strIncFlag == 1){
                ...
                if(fltNewGapAmt >= 0 && strIncFlag == 2){
...
function getList(){
   ...
   TR_S_MAIN.Action  = "/dps/pord113.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord113.po?goTo="+goTo+parameters;  
     ...
     /*if(strSkuCd != ""){
         ...
         if(rtnList != null){
             for(var i = 0; i < rtnList.length; i++){
                 ...
                 DS_IO_DETAIL.NameValue(row+i, "SKU_NM")      = rtnList[i].SKU_NM;
 ...
 */
 function setGapCalc(row, colid){
	 ...
	 if(colid == "ORD_QTY" || colid == "OLD_COST_PRC" || colid == "NEW_SALE_PRC"){
         ...
         DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"OLD_GAP_TAMT") = DS_IO_DETAIL.Sum(31,0,0); // 구차익액
         
         DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"OLD_GAP_RATE") 
                             = getCalcPord("GAP_RATE", DS_IO_DETAIL.Sum(30,0,0), DS_IO_DETAIL.Sum(15,0,0), "", strTaxFlag, roundFlag); // 구차익율
         DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"NEW_GAP_RATE")                             
                             = getCalcPord("GAP_RATE", DS_IO_DETAIL.Sum(28,0,0), DS_IO_DETAIL.Sum(11,0,0), "", strTaxFlag, roundFlag); // 신차익율

         DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"GAP_COST_TAMT") = DS_IO_DETAIL.Sum(33,0,0); // 원가차익액합
         DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"GAP_SALE_TAMT") = DS_IO_DETAIL.Sum(34,0,0); // 매가차익액합
                             
         DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"OLD_COST_TAMT") = DS_IO_DETAIL.Sum(28,0,0);  // 구원가금액합
         DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition,"NEW_COST_TAMT") = DS_IO_DETAIL.Sum(30,0,0);  // 신원가금액합
...
<script language=JavaScript for=DS_IO_MASTER event=OnColumnChanged(row,colid)>
    ...
    case "OLD_PRC_APP_DT":
        // 구행사코드
        getEventCode("DS_O_OLD_EVENT_CD", LC_STR.BindColVal, EM_PUMBUN_CD.Text, EM_OLD_PRC_APP_DT.Text, "N");   
        break; 
    case "NEW_PRC_APP_DT":
        // 구행사코드
