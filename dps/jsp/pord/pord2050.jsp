...
//    initEmEdit(EM_GAP_RATE,        "NUMBER^3^2" , READ);           // 차익율
    ...
    registerUsingDataset("pord205","DS_O_LIST" );
 } 
...
function btn_Conf() {
	...
	//승인취소 하려면
    if(DS_O_LIST.Namevalue(DS_O_LIST.RowPosition ,"SLIP_PROC_STAT") == "01"){ //SM 확정상태
        ...
        if( showMessage(STOPSIGN, YESNO, "USER-1198") != 1 )
            ...
            if( showMessage(STOPSIGN, YESNO, "USER-1212", "취소") == 1 ){        //정말 취소 하시겠습니까.
                strSlipProcStat = "01" //승인상태의 플래그로 승인을 취소하기 위함.
                    sendFlag = "2";        //승인취소
                    //alert("승인취소한다");     
            }else{
                return;
    ...
    //승인하려면
        ...
        if(returnv == 2){
            if( showMessage(STOPSIGN, YESNO, "USER-1212", "반려") == 1 ){        //정말 반려 하시겠습니까. 
                ...
                vanMsgFlag = true;
                sendFlag = "1";        //반려
                //return false;   //SM 확정전 단계는 없으므로 리턴시킴
                //alert("반려");     
            }else{
                return;
            }      
...
//	        var strSlipProcStat = DS_O_LIST.NameValue(i + 1,"SLIP_PROC_STAT");
			...
			var params = "&strSlipProcStat="+encodeURIComponent(strSlipProcStat)
					   ...
					   TR_MAIN.Action="/dps/pord205.po?goTo=conf"+params;
					   TR_MAIN.Post(); 	
					   k = k+1;
	     }       
	}    
    ...
    if(strSlipProcStat=="01"){//확정상태였던 전표의 취소된 건수
...
function getList(){
  ...
  // var strSlip_flag     = RD_S_SLIP_FLAG.CodeValue;              // 전표구분 
   ...
   TR_L_MAIN.Action  = "/dps/pord205.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord205.po?goTo="+goTo+parameters;  
