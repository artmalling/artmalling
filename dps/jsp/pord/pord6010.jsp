 ...
 */
 ...
 function doInit(){
    ...
    DS_IO_DETAIL_EXCEL.setDataHeader('<gauce:dataset name="H_SKU_EXCEL_INFO"/>');
    ...
    registerUsingDataset("pord601","DS_IO_MASTER,DS_IO_DETAIL");
 } 
 ...
 function gridCreate2(){
     var hdrProperies = '<FC>id={currow}          name="NO"           width=30    align=center sumtext="" color = {if(SKU_CD="","RED")} </FC>'
                      + '<FC>id=SEL               name="선택"         width=50    align=center HeadCheckShow=true EditStyle=CheckBox color = {if(SKU_CD="","RED")} </FC>'                 
                      + '<FC>id=SKU_CD            name="*단품코드"    width=120   align=center EditStyle=Popup sumtext="합계" color = {if(SKU_CD="","RED")} </FC>'                        
                      + '<FC>id=MODEL_NO          name="브랜드단품코드" width=120   align=center Edit=none color = {if(SKU_CD="","RED")} </FC>'                           
                      + '<FC>id=SKU_NM            name="단품명"       width=100   align=left   Edit=none color = {if(SKU_CD="","RED")} </FC>'                           
                      + '<FC>id=ORD_UNIT_NM       name="단위"         width=40    align=left   Edit=none color = {if(SKU_CD="","RED")} </FC>'                           
                      + '<FC>id=ORD_QTY           name="*수량"        width=50    align=right  Edit=Numeric sumtext=@sum color = {if(SKU_CD="","RED")} </FC>'                           
                      + '<FG> name="원가 (부가세 제외)"'                                       
                      + '<FC>id=NEW_COST_PRC      name="*단가"        width=80    align=right  edit={IF(BIZ_TYPE="1", IF(COST_ZERO="Y","true", "false"),"false")}color = {if(SKU_CD="","RED")}  </FC>'                        
                      + '<FC>id=NEW_COST_AMT      name="금액"         width=120   align=right  Edit=none sumtext=@sum  color = {if(SKU_CD="","RED")} </FC>'                        
                      + '</FG> '                                                    
                      + '<FG> name="매가 (부가세 포함)"'         
//                      + '<FC>id=NEW_SALE_PRC      name="*단가"        width=80    align=right  edit={IF(BIZ_TYPE="1", IF(SALE_ZERO="Y","true", "false"),"false")} </FC>'    
                        // 특정일때도 매가단가가 0이면 수정가능하게..2010-08-17
                      + '<FC>id=NEW_SALE_PRC      name="*단가"        width=80    align=right  edit={IF(BIZ_TYPE="1", IF(SALE_ZERO="Y","true", "false"),IF(BIZ_TYPE="2", IF(SALE_ZERO="Y","true", "false"), "false"),"false")} color = {if(SKU_CD="","RED")}  </FC>'                        
                      + '<FC>id=NEW_SALE_AMT      name="금액"         width=120   align=right  Edit=none sumtext=@sum color = {if(SKU_CD="","RED")}  </FC>'                        
                      + '</FG> '                                                    
                      + '<FC>id=NEW_GAP_RATE      name="차익율"       width=80    align=right  Edit=none  color = {if(SKU_CD="","RED")}  </FC>'
                      + '<FC>id=MG_RATE           name="마진율"       width=80    align=right  Edit=none show=false color = {if(SKU_CD="","RED")} </FC>'
                      + '<FC>id=AVG_SALE_QTY      name="평균판매량"   width=80    align=right  Edit=none sumtext=@sum color = {if(SKU_CD="","RED")} </FC>'                           
                      + '<FC>id=AVG_SALE_AMT      name="평균판매액"   width=110   align=right  Edit=none sumtext=@sum color = {if(SKU_CD="","RED")} </FC>'                           
                      + '<FC>id=ORD_STK_QTY       name="현재고"       width=50    align=right  Edit=none sumtext=@sum color = {if(SKU_CD="","RED")}  </FC>'
    ...
    if(DS_IO_MASTER.CountRow > 0){
         ...
         if(slip_no.length == 0){
             ...
             if (!orderCloseCheck()) {
             	...
             	EM_ORD_DT.Text = tomorrow;                                    // 발주일
 	            var date = addDate("d", 2, strToday);                         // 오늘날짜 + 2
...
function btn_Delete() {
    ...
    if(showMessage(Question, YESNO, "USER-1023") == 1){  
        ...
        TR_MAIN.Action="/dps/pord601.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){    // validation 체크
        ...
        TR_SAVE.Action="/dps/pord601.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
...
//     enableControl(IMG_ADD         ,flag);
    ...
    enableControl(IMG_FILE_SEARCH ,flag);
    enableControl(A_EXCEL_DOWN    ,flag);
...
function getList(){
   ...
   TR_L_MAIN.Action  = "/dps/pord601.po?goTo="+goTo+parameters;  
...
function getSkuInfo(nRow){  
   ...
   var strPrcEndDt  = "99991231";                                   // 가격종료일
   var strSkuCd     = DS_IO_DETAIL.NameValue(nRow, "SKU_CD");       // 단품코드
   var nNewCostPrc  = DS_IO_DETAIL.NameValue(nRow, "NEW_COST_PRC");	// 판매원가
   var nNewSalePrc  = DS_IO_DETAIL.NameValue(nRow, "NEW_SALE_PRC");	// 판매매가
   var strBizType   = EM_BIZ_TYPE.Text;                             // 거래형태
   ...
   var parameters =  "&strStrCd="	 + encodeURIComponent(strStrCd)    
                   ...
                   + "&strPrcEndDt=" + encodeURIComponent(strPrcEndDt)     
                   + "&strSkuCd="	 + encodeURIComponent(strSkuCd)   
                   + "&nNewCostPrc=" + encodeURIComponent(nNewCostPrc)   
                   + "&nNewSalePrc=" + encodeURIComponent(nNewSalePrc)   
                   + "&strBizType="	 + encodeURIComponent(strBizType)
                   ;
   TR_S_MAIN.Action   = "/dps/pord601.po?goTo="+goTo+parameters;  
   TR_S_MAIN.KeyValue = "SERVLET("+action+":DS_O_SKU_INFO=DS_O_SKU_INFO)"; //조회는 O
   TR_S_MAIN.Post();

   if(DS_O_SKU_INFO.CountRow > 0){
       
	   DS_IO_DETAIL.NameValue(nRow, "SEL")           = 'F';
	   DS_IO_DETAIL.NameValue(nRow, "SLIP_NO")       = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "SLIP_NO");
	   DS_IO_DETAIL.NameValue(nRow, "STR_CD")        = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "STR_CD");
	   DS_IO_DETAIL.NameValue(nRow, "PUMBUN_CD")     = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "PUMBUN_CD");
       DS_IO_DETAIL.NameValue(nRow, "VEN_CD")        = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VEN_CD");
       DS_IO_DETAIL.NameValue(nRow, "SLIP_FLAG")     = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "SLIP_FLAG");

       DS_IO_DETAIL.NameValue(nRow, "SALE_ZERO")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_ZERO");
       DS_IO_DETAIL.NameValue(nRow, "COST_ZERO")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "COST_ZERO");

       DS_IO_DETAIL.NameValue(nRow, "PUMBUN_TYPE")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "PUMBUN_TYPE");
       DS_IO_DETAIL.NameValue(nRow, "PUMMOK_CD")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "PUMMOK_CD");       // 품목
       DS_IO_DETAIL.NameValue(nRow, "SKU_CD")        = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SKU_CD");          // 단품코드
       DS_IO_DETAIL.NameValue(nRow, "SKU_NM")        = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SKU_NAME");        // 단품명
       DS_IO_DETAIL.NameValue(nRow, "MODEL_NO")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "MODEL_NO");        // 단품명
       DS_IO_DETAIL.NameValue(nRow, "ORD_UNIT_CD")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_UNIT_CD");    // 단위
       DS_IO_DETAIL.NameValue(nRow, "ORD_UNIT_NM")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_UNIT_NM");    // 단위명
       DS_IO_DETAIL.NameValue(nRow, "ORD_STK_QTY")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "STORK_QTY");     // 재고
       DS_IO_DETAIL.NameValue(nRow, "AVG_SALE_QTY")  = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "AVG_SALE_QTY");    // 평균판매량
       DS_IO_DETAIL.NameValue(nRow, "AVG_SALE_AMT")  = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "AVG_SALE_PRC");    // 평균판매액
       DS_IO_DETAIL.NameValue(nRow, "BUY_S_DT")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BUY_S_DT");
       DS_IO_DETAIL.NameValue(nRow, "BUY_E_DT")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BUY_E_DT");
       
       //var nNewCostPrc = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SAL_COST_PRC");    // 원가단가
       //var nNewSalePrc = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_PRC");    	// 매가단가
       //DS_IO_DETAIL.NameValue(nRow, "NEW_COST_PRC")  = nNewCostPrc;    		// 원가단가
       //DS_IO_DETAIL.NameValue(nRow, "NEW_SALE_PRC")  = nNewSalePrc;        	// 매가단가
       
       
       var nNewCostPrc = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_PRC");    // 원가단가
       //var nNewSalePrc = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_PRC");    	// 매가단가
       DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_SALE_PRC")  = nNewCostPrc;    		// 원가단가
       //DS_IO_DETAIL.NameValue(nRow, "NEW_SALE_PRC")  = nNewSalePrc;        	// 매가단가

       
       // 마진율
       var strTaxFlag = EM_TAX_FLAG.Text;
       var nMgRate = getSaleMgRate(nNewCostPrc, nNewSalePrc, roundFlag, strTaxFlag);
       DS_IO_DETAIL.NameValue(nRow, "MG_RATE")       = nMgRate;   
       
       DS_IO_DETAIL.NameValue(nRow, "BOTTLE_CD")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BOTTLE_CD");       // 공병단품번호
       DS_IO_DETAIL.NameValue(nRow, "BIZ_TYPE")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BIZ_TYPE");
      
       // 특정단품일 경우 가격정보의 마진율을 세팅한다.
       if(strBizType == "2"){
    	   DS_IO_DETAIL.NameValue(nRow, "NEW_GAP_RATE") = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_MG_RATE");
       }
   }else{
	   // 단품데이터 없으면
       DS_IO_DETAIL.NameValue(nRow, "SEL")    = 'T';
//        DS_IO_DETAIL.NameValue(nRow, "SKU_NM") = "존재하지않는 단품";        // 단품명
   }   
}


function getSkuInfo2(nRow){  
	   ...
	   var strPrcAppDt  = EM_PRC_APP_DT.Text;                           // 가격적용일
	   var strPrcEndDt  = "99991231";                                   // 가격종료일
	   var strSkuCd     = DS_IO_DETAIL_EXCEL.NameValue(nRow, "MODEL_NO");       // 단품코드
	   var nNewCostPrc  = DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_COST_PRC");	// 판매원가
	   var nNewSalePrc  = DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_SALE_PRC");	// 판매매가
	   var nQty			=DS_IO_DETAIL_EXCEL.NameValue(nRow, "ORD_QTY");	//수량
	   var strBizType   = EM_BIZ_TYPE.Text;                             // 거래형태
	   ...
	   if(DS_O_SKU_INFO.CountRow > 0){
	       ...
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "SEL")           = 'F';
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "SLIP_NO")       = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "SLIP_NO");
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "STR_CD")        = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "STR_CD");
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "PUMBUN_CD")     = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "PUMBUN_CD");
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "VEN_CD")        = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VEN_CD");
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "SLIP_FLAG")     = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "SLIP_FLAG");

	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "SALE_ZERO")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_ZERO");
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "COST_ZERO")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "COST_ZERO");

	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "PUMBUN_TYPE")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "PUMBUN_TYPE");
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "PUMMOK_CD")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "PUMMOK_CD");       // 품목
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "SKU_CD")        = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SKU_CD");          // 단품코드
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "SKU_NM")        = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SKU_NAME");        // 단품명
	       //DS_IO_DETAIL_EXCEL.NameValue(nRow, "MODEL_NO")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "MODEL_NO");        // 단품명
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "ORD_UNIT_CD")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_UNIT_CD");    // 단위
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "ORD_UNIT_NM")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_UNIT_NM");    // 단위명
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "ORD_STK_QTY")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "STORK_QTY");     // 재고
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "AVG_SALE_QTY")  = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "AVG_SALE_QTY");    // 평균판매량
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "AVG_SALE_AMT")  = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "AVG_SALE_PRC");    // 평균판매액
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "BUY_S_DT")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BUY_S_DT");
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "BUY_E_DT")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BUY_E_DT");
	       ...
	       //DS_IO_DETAIL.NameValue(nRow, "NEW_SALE_PRC")  = nNewSalePrc;        	// 매가단가
	        ...
	        nNewCostPrc = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_PRC");    // 원가단가
	       var nNewCostAmt = nNewCostPrc * nQty;    // 원가단가
	       //var nNewSalePrc = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_PRC");    	// 매가단가
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_SALE_PRC")  = nNewCostPrc;    		// 원가단가
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_SALE_AMT")  = nNewCostAmt;    		// 원가단가
	       //DS_IO_DETAIL.NameValue(nRow, "NEW_SALE_PRC")  = nNewSalePrc;        	// 매가단가
	       ...
	       var strTaxFlag = EM_TAX_FLAG.Text;
	       var nMgRate = getSaleMgRate(nNewCostPrc, nNewSalePrc, roundFlag, strTaxFlag);
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "MG_RATE")       = nMgRate;   
	       
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "BOTTLE_CD")     = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BOTTLE_CD");       // 공병단품번호
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "BIZ_TYPE")      = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "BIZ_TYPE");
	      
	       // 특정단품일 경우 가격정보의 마진율을 세팅한다.
	       if(strBizType == "2"){
	    	   DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_GAP_RATE") = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_MG_RATE");
	       }
	   }else{
		   // 단품데이터 없으면
	       DS_IO_DETAIL_EXCEL.NameValue(nRow, "SEL")    = 'T';
//	        DS_IO_DETAIL.NameValue(nRow, "SKU_NM") = "존재하지않는 단품";        // 단품명
	   }   
	}
 ...
 */
 function setGapCalc(row, colid){
	 if (colid == "ORD_QTY" || colid == "NEW_COST_PRC" || colid == "NEW_SALE_PRC") {
	     ...
	     var totGapAmt   = 0;                                              // 원가합계
	     var totGapRate  = 0;                                              // 매가합계
	     var cost_amt_vat = getCalcPord("COST_PRC", "", sale_amt, rate, "2", roundFlag); // 원가금액(부가세포함)
	     ...
	     case "1":
	    	 //alert(cost_amt);
	    	 //alert(sale_amt);
	         DS_IO_DETAIL.NameValue(row, "NEW_GAP_RATE")  = getCalcPord("GAP_RATE2", cost_amt, sale_amt,  "", strTaxFlag, roundFlag);
	         //alert(DS_IO_DETAIL.NameValue(row, "NEW_GAP_RATE"));
	         DS_IO_DETAIL.NameValue(row, "MG_RATE")       = getSaleMgRate(cost_amt, sale_amt, roundFlag, strTaxFlag);
	         break;
	     ...
	     ////totGapAmt  = DS_IO_DETAIL.Sum(9,0,0);
	     totGapAmt  = getCalcPord("GAP_AMT",  totCostAmt,   totSaleAmt,   "", strTaxFlag, roundFlag);
	     switch (strBizType) {
	     case "1":
	    	 totGapRate = getCalcPord("GAP_RATE2", totCostAmt, totSaleAmt, "", strTaxFlag, roundFlag);
	         break;
	         
	     case "2":
	    	 totGapRate = getCalcPord("GAP_RATE", totCostAmt, totSaleAmt, "", strTaxFlag, roundFlag);
	         break;
	     }
	     ...
	     // MARIO OUTLET
	     DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VAT_TAMT")  = totVatAmt;
 ...
 */
 function setGapCalc2(row, colid){
	 if (colid == "ORD_QTY" || colid == "NEW_COST_PRC" || colid == "NEW_SALE_PRC") {
		 ...
		 var strTaxFlag  = EM_TAX_FLAG.Text;
	     var strBizType  = EM_BIZ_TYPE.Text;                               // 거래형태
	     var ord_qty     = DS_IO_DETAIL_EXCEL.NameValue(row, "ORD_QTY");         // 발주수량
	     var cost_prc    = DS_IO_DETAIL_EXCEL.NameValue(row, "NEW_COST_PRC");    // 원가단가
	     var sale_prc    = DS_IO_DETAIL_EXCEL.NameValue(row, "NEW_SALE_PRC");    // 매가단가
	     var cost_amt    = ord_qty * cost_prc;                             // 원가금액
	     var sale_amt    = ord_qty * sale_prc;                             // 매가금액
	     var rate        = DS_IO_DETAIL_EXCEL.NameValue(row, "MG_RATE");         // 마진율
	     var totGapAmt   = 0;                                              // 원가합계
	     var totGapRate  = 0;                                              // 매가합계
	     var cost_amt_vat = getCalcPord("COST_PRC", "", sale_amt, rate, "2", roundFlag); // 원가금액(부가세포함)
	     //DS_IO_DETAIL_EXCEL.NameValue(row, "NEW_COST_AMT") = cost_amt;           // 원가금액
	     //DS_IO_DETAIL_EXCEL.NameValue(row, "NEW_SALE_AMT") = sale_amt;           // 매가금액
	     //alert(strBizType);
	     // 거래형태 (1:직매입,2:특정)
	     switch (strBizType) {
	     case "1":
	    	 //alert(cost_amt);
	    	 //alert(sale_amt);
	         DS_IO_DETAIL_EXCEL.NameValue(row, "NEW_GAP_RATE")  = getCalcPord("GAP_RATE2", cost_amt, sale_amt,  "", strTaxFlag, roundFlag);
	         //alert(DS_IO_DETAIL.NameValue(row, "NEW_GAP_RATE"));
	         DS_IO_DETAIL_EXCEL.NameValue(row, "MG_RATE")       = getSaleMgRate(cost_amt, sale_amt, roundFlag, strTaxFlag);
	         break;
	         
	     case "2":
	    	 DS_IO_DETAIL_EXCEL.NameValue(row, "NEW_GAP_RATE")  = getCalcPord("GAP_RATE", cost_amt, sale_amt,  "", strTaxFlag, roundFlag);
	    	 DS_IO_DETAIL_EXCEL.NameValue(row, "MG_RATE")       = getSaleMgRate(cost_amt, sale_amt, roundFlag, strTaxFlag);
	         break;
	     }
         ...
         DS_IO_DETAIL_EXCEL.NameValue(row, "NEW_GAP_AMT")  = gapAmt;      // 차익액
         DS_IO_DETAIL_EXCEL.NameValue(row, "VAT_AMT")      = vatAmt;      // 부가세
	     ...
	     EM_TOT_QTY.Text      = DS_IO_DETAIL_EXCEL.Sum(14,0,0);
	     EM_TOT_COST_AMT.Text = DS_IO_DETAIL_EXCEL.Sum(16,0,0);
	     EM_TOT_SALE_AMT.Text = DS_IO_DETAIL_EXCEL.Sum(18,0,0);
	     // MARIO OUTLET
	     DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VAT_TAMT")  = totVatAmt;
 ...
 function getVenInfo(code, name, btnFlag){
     ...
     if(code.id == "EM_S_VEN_CD") {
          ...
          strBizType      = EM_BIZ_TYPE.Text;                                         // 거래형태
     }
  ...
  function setStrPbnNmWithoutPopCall(){
    ...
    if(DS_O_RESULT.CountRow == 1){
        ...
        // 거래형태가 직매입(1), 특정(2)일경우만 등록가능하다.
      	if(DS_O_RESULT.NameValue(DS_O_RESULT.RowPosition, "BIZ_TYPE") != '1' && DS_O_RESULT.NameValue(DS_O_RESULT.RowPosition, "BIZ_TYPE") != '2'){
            EM_PUMBUN_CD.TEXT = "";
            EM_PUMBUN_NM.TEXT = "";
            clearPumbunInfo();
            showMessage(EXCLAMATION, OK, "USER-2003");  
...
//  	DS_O_EXCELTEMP.ClearData();
 	searchSetWait("B");
 	DS_IO_DETAIL.ClearData();
 	DS_IO_DETAIL_EXCEL.ClearData();
 	
 	// Excel파일 DateSet에 저장               
     DS_IO_DETAIL_EXCEL.Do("Excel.Application", strOption);
...
//  	var strTrFlag = LC_TRAN_FLAG.BindColVal;
 	...
 	for(nRow = 1; nRow <= DS_IO_DETAIL_EXCEL.CountRow; nRow++){
 		/* 행추가 */
 		//DS_IO_DETAIL.Addrow();
 		/* 엑셀 업로드 Master 데이터셋에 import */ 
 		var strSkuCd    = DS_IO_DETAIL_EXCEL.NameValue(nRow, "MODEL_NO");
 		//alert(strSkuCd);
 		var nOrdQty     = DS_IO_DETAIL_EXCEL.NameValue(nRow, "ORD_QTY");
 		var nNewCostPrc = DS_IO_DETAIL_EXCEL.NameValue(nRow, "ORD_COST_PRC");
 		//var nNewSalePrc = DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_SALE_PRC");
 		
 		//var nOrdCostPrc =  nNewCostPrc/1.1; // 부가세 별도 금액
 		var nOrdCostPrc =  nNewCostPrc;
 		
 		
 		DS_IO_DETAIL_EXCEL.NameValue(nRow, "MODEL_NO")       = strSkuCd;
 		//DS_IO_DETAIL.NameValue(nRow, "ORD_QTY")      = nOrdQty;
 		//DS_IO_DETAIL.NameValue(nRow, "NEW_COST_PRC") = nNewCostPrc;
 		//DS_IO_DETAIL.NameValue(nRow, "NEW_COST_AMT") = nNewCostPrc * nOrdQty;
 		DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_COST_PRC") = nOrdCostPrc;
 		//alert(nOrdCostPrc);
 		DS_IO_DETAIL_EXCEL.NameValue(nRow, "NEW_COST_AMT") = nOrdCostPrc * nOrdQty;
 		//DS_IO_DETAIL.NameValue(nRow, "NEW_SALE_PRC") = nNewSalePrc;
 		//DS_IO_DETAIL.NameValue(nRow, "NEW_SALE_AMT") = nNewSalePrc * nOrdQty;
 		...
 		// 차익액, 차익율을 계산한다.
 		setGapCalc2(nRow, "ORD_QTY");
 	}
 	
  	var strData = DS_IO_DETAIL_EXCEL.ExportData(1,DS_IO_DETAIL_EXCEL.CountRow, true );
...
<comment id="_NSID_"><object id="DS_IO_DETAIL_EXCEL" classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr valign="top">
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                    ...
                    <td>
                          <comment id="_NSID_"> 
                              <OBJECT id=GR_LIST width=100% height="725" classid=<%=Util.CLSID_GRID%>>
...
<%--                                     <img src="/<%=dir%>/imgs/btn/add_row.gif" id="IMG_ADD"  width="52" height="18" hspace="2" onclick="javascript:addDetailRow();" /> --%>
									...
									<a href="/dps/samplefiles/PORD_SKUCD_UPLOAD(Sample).xls" align="absmiddle">
										<img src="/<%=dir%>/imgs/btn/excel_down.gif" border="0" align="absmiddle" id=A_EXCEL_DOWN>
									</a>
                            ...
                            <tr valign="top">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                                    <tr>
                                        <td>
                                          <comment id="_NSID_"> 
                                              <OBJECT id=GR_DETAIL width=100%  height="476"  classid=<%=Util.CLSID_GRID%>>
