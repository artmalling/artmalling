 ...
 */
 ...
 function doInit(){
    ...
    registerUsingDataset("pord108","DS_IO_MASTER,DS_IO_DETAIL1,DS_IO_DETAIL2" );
...
function btn_Delete() {
    ...
    if(showMessage(Question, YESNO, "USER-1023") == 1){  
	    ...
	    TR_MAIN.Action="/dps/pord108.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){
        ...
        TR_SAVE.Action="/dps/pord108.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
 ...
 */
 function getList(){
   ...
   TR_S_MAIN.Action  = "/dps/pord108.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord108.po?goTo="+goTo+parameters;  
   ...
   // 협력사별 반올림 구분을 받는다
   strVenCd  = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "VEN_CD");            // 대출협력사코드
...
function getPumbunInfo(gubun){
    ...
    var strFStrCd       = LC_P_STR.BindColVal;                                       // 점입
    var strOrgCd        = "";                                                        // 조직코드
    var strVenCd        = "";                                                        // 협력사
    var strBuyerCd      = "";                                                        // 바이어
...
function skuValCheck(row, colid, gubun){
    setObjName(gubun);
    var intRow          = 1;
    var strSkuCd        = DS_IO_DETAIL1.NameValue(row, "SKU_CD");
    var strUsrCd        = '<c:out value="${sessionScope.sessionInfo.USER_ID}" />';   // 세션 사원번호
    var strStrCd        = "";                                                        // 점
 ...
 */
  ...
  */
  function setGapCalc(row, colid){
      ...
      //EM_GAP_TOT_AMT.Text  = getCalcPord("GAP_AMT",  totCostAmt, totSaleAmt, "", strTaxFlag, roundFlag); // 차익액합계
      EM_GAP_TOT_AMT.Text  = DS_IO_DETAIL1.Sum(9,0,0); // 차익액합계
...
<script lanaguage=JavaScript for=EM_PRC_APP_DT event=OnKillFocus()>
    ...
    var nextDay = addDate("d", 1, EM_P_DELI_DT.Text);
    var setDate = "";
