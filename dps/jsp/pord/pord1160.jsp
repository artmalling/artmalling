<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 발주관리 > 의류단품 통합발주 등록
 * 작 성 일 : 2010.04.06
 * 작 성 자 : 김 경 은
 * 수 정 자 : 
 * 파 일 명 : pord1160.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 의류단품으로 관리하는 상품의 통합 발주 등록한다.
 * 이    력 :
 * 
 ******************************************************************************/
-->
 ...
 */
 ...
 function doInit(){
    ...
    registerUsingDataset("pord116","DS_O_LIST,DS_IO_MASTER,DS_IO_DETAIL,DS_O_SKU_INFO" );
...
function btn_Delete() {
    ...
    if(!checkValidation("Delete"))
     ...
     if(showMessage(Question, YESNO, "USER-1023") == 1){  
        ...
        TR_MAIN.Action="/dps/pord116.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){    // validation 체크
        ...
        TR_SAVE.Action="/dps/pord116.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
...
function checkValidation(Gubun) {
     ...
     if(Gubun == "Delete"){
         ...
         var strSlipProcStat = getSlipProcStat("DS_O_RESULT", strStrCd, strSlipNo);            // 전표상태T"); 
...
function getList(){
   ...
   TR_S_MAIN.Action  = "/dps/pord116.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord116.po?goTo="+goTo+parameters;  
...
function skuValCheck(row, colid, gubun){
    ...
    if(strStyleType == "1"){
        rtnMap = setSkuATypeNmWithoutToGridPordPop("DS_O_RESULT", strSkuCd, "", strAuthGb, popFlag
                                                    , strUsrCd, strStrCd, strPumbunCd, "", ""
                                                    , "", strPumbunType, strBizType, strUseYn );   
     
    }else{
        rtnMap = setSkuBTypeNmWithoutToGridPordPop("DS_O_RESULT", strSkuCd, "", strAuthGb, popFlag
                                                    , strUsrCd, strStrCd, strPumbunCd, "", ""
                                                    , "", "", "",  strPumbunType, strBizType
                                                    , strUseYn, "", "" );  
    }
...
//      if(DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "NEW_COST_PRC") == "100000")
//    	  DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "COST_ZERO") = "Y";
 ...
 */
  ...
  * 사용방법 : clearDataSetRow(DS_IO_MASTER);
  **/
