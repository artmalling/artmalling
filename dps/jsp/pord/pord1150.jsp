<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 발주관리 > 신선단품 통합발주 등록
 * 작 성 일 : 2010.04.04
 * 작 성 자 : 김 경 은
 * 수 정 자 : 
 * 파 일 명 : pord1150.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 신선단품으로 관리하는 상품의 통합 발주 등록한다.
 * 이    력 :
 * 
 ******************************************************************************/
-->
 ...
 */
 ...
 function doInit(){
    ...
    registerUsingDataset("pord115","DS_O_LIST,DS_IO_MASTER,DS_IO_DETAIL,DS_O_SKU_INFO" );
 ...
 function gridCreate2(){
      var hdrProperies = '<FC>id={currow}          name="NO"           width=30    align=center sumtext=""</FC>'    
                      ...
                      + '<FC>id=RATE              name="목표;이익율"    width=80    align=right  Edit=none  </FC>'
                      + '<FG> name="원가 (부가세 제외)"'                                       
...
function btn_New() {
    ...
    // 전표마스터 추가후 행추가,K 삭제 버튼 활성화 (저장되지 않은 내용이 있을경우 추가되지 않음)
...
function btn_Delete() {
    ...
    if(DS_O_LIST.NameValue(DS_O_LIST.RowPosition, "SLIP_NO").length == 0){
    	DS_IO_DETAIL.ClearData();
        DS_IO_MASTER.ClearData();
    ...
    if(showMessage(Question, YESNO, "USER-1023") == 1){  
        ...
        TR_MAIN.Action="/dps/pord115.po?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){    // validation 체크
        ...
        TR_SAVE.Action   = "/dps/pord115.po?goTo=save&strFlag=save&strBuyerYN="+strBuyerYN; 
...
function getList(){
   ...
   TR_S_MAIN.Action  = "/dps/pord115.po?goTo="+goTo+parameters;  
...
function getDetail(){
   ...
   TR_MAIN.Action="/dps/pord115.po?goTo="+goTo+parameters;  
...
function getSkuInfo(){
   ...
   if(DS_O_SKU_INFO.CountRow > 0){
       ...
       DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "SKU_NM")        = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SKU_NAME");
       DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "ORD_UNIT_CD")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_UNIT_CD");
       DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "ORD_UNIT_NM")   = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_UNIT_NM");
       DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "NEW_COST_PRC")  = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SAL_COST_PRC");
       DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "NEW_SALE_PRC")  = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "SALE_PRC");
       ...
       DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "RATE")          = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "RATE");
       
//       if(strBizType == "1"){                // 직매입
//           DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition, "NEW_GAP_RATE") = DS_O_SKU_INFO.NameValue(DS_O_SKU_INFO.RowPosition, "GOAL_PROF_RATE"); 
//       }else if(strBizType == "2"){          // 특정
 ...
 */
 function setGapCalc(row, colid){
	if(colid == "NEW_COST_PRC" || colid == "ORD_QTY" || colid == "NEW_SALE_PRC"){   
	    ...
	    var cost_prc    = 0;                                                // 원가단가
	    var sale_prc    = 0;                                                // 매가단가
	    var gapRate     = DS_IO_DETAIL.NameValue(row, "NEW_GAP_RATE");      // 차익율
	    var cost_amt    = 0;                                                // 원가금액
	    var sale_amt    = 0;                                                // 매가금액
	    ...
	    var vatAmt      = 0;                                                // 부가세
	    var totVatAmt   = 0;                                                // 총부가세
	    var rate        = DS_IO_DETAIL.NameValue(row, "RATE");              // 목표이익율
	        
	    // 거래형태 (1:직매입,2:특정)
	    switch (strBizType) {
	    case "1":
	        cost_prc = DS_IO_DETAIL.NameValue(row, "NEW_COST_PRC"); 
	        if(colid != "NEW_SALE_PRC"){
	        	DS_IO_DETAIL.NameValue(row, "NEW_SALE_PRC") = getCalcPord("SALE_PRC", cost_prc, "", rate, strTaxFlag, roundFlag);   // 매가단가
	        }
	        GR_DETAIL.ColumnProp("NEW_COST_PRC", "Edit") = "Numeric";
	        GR_DETAIL.ColumnProp("NEW_SALE_PRC", "Edit") = "Numeric";
	        //GR_DETAIL.ColumnProp("NEW_GAP_RATE", "Edit") = "true";
	        break;
	        
	    case "2":
	        sale_prc = DS_IO_DETAIL.NameValue(row, "NEW_SALE_PRC");
	        cost_prc = getCalcPord("COST_PRC", "", sale_prc, rate, strTaxFlag, roundFlag);  // 원가단가
	        DS_IO_DETAIL.NameValue(row, "NEW_COST_PRC") = cost_prc;
	        
	        GR_DETAIL.ColumnProp("NEW_COST_PRC", "Edit") = "none";
	        GR_DETAIL.ColumnProp("NEW_SALE_PRC", "Edit") = "Numeric";
	        //GR_DETAIL.ColumnProp("NEW_GAP_RATE", "Edit") = "none";
	        break;
	    }
	    ...
	    totCostAmt   = DS_IO_DETAIL.Sum(13,0,0);
	    totSaleAmt   = DS_IO_DETAIL.Sum(15,0,0);
	    totVatAmt    = DS_IO_DETAIL.Sum(21,0,0)
	    
	    //EM_GAP_TOT_AMT.Text  = getCalcPord("GAP_AMT",  totCostAmt, totSaleAmt, "", strTaxFlag, roundFlag);
	    EM_GAP_TOT_AMT.Text  = DS_IO_DETAIL.Sum(9,0,0);
	    ...
	    EM_TOT_QTY.Text      = DS_IO_DETAIL.Sum(11,0,0);
	    EM_TOT_COST_AMT.Text = DS_IO_DETAIL.Sum(13,0,0);
	    EM_TOT_SALE_AMT.Text = DS_IO_DETAIL.Sum(15,0,0);
	}
 }
...
<script language=JavaScript for=DS_IO_MASTER event=OnColumnChanged(row,colid)>
    ...
    case "BIZ_TYPE":
        ...
        if(strBizType == "1"){
            ...
            GR_DETAIL.ColumnProp("NEW_COST_PRC", "Edit") = "number";
            GR_DETAIL.ColumnProp("NEW_SALE_PRC", "Edit") = "number";
        }else{
            LC_PAY_COND.BindColVal = "";
...
<comment id="_NSID_"><object id="DS_O_STR1"          classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
<comment id="_NSID_"><object id="DS_O_STR2"          classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
            ...
            <tr>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0"
                    ...
                    <tr>
                        ...
                        <td width="110">
                          <comment id="_NSID_"> 
                              <object id=LC_S_STR classid=<%=Util.CLSID_LUXECOMBO%> height=100 width=100 tabindex=1 align="absmiddle"> </object> 
                          </comment><script>_ws_(_NSID_);</script>
                        </td>
                        <th width="70">팀</th>
                        <td width="110">
                          <comment id="_NSID_"> 
                              <object id=LC_S_BUMUN classid=<%=Util.CLSID_LUXECOMBO%> width=100 tabindex=1 align="absmiddle"> </object> 
                          </comment><script>_ws_(_NSID_);</script>
                        </td>
                        <th width="70">파트</th>
                        <td width="110">
                          <comment id="_NSID_"> 
                              <object id=LC_S_TEAM classid=<%=Util.CLSID_LUXECOMBO%> width=100 tabindex=1 align="absmiddle"> </object> 
                          </comment><script>_ws_(_NSID_);</script>
                        </td>
                        <th width="70">PC</th>
                        <td>
                          <comment id="_NSID_"> 
                              <object id=LC_S_PC classid=<%=Util.CLSID_LUXECOMBO%> width=100 tabindex=1 align="absmiddle"> </object> 
                          </comment><script>_ws_(_NSID_);</script>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td width="110">
                          <comment id="_NSID_"> 
                              <object id=LC_S_GJDATE_TYPE classid=<%=Util.CLSID_LUXECOMBO%> height=100 width=100 tabindex=1 align="absmiddle" > </object> 
                          </comment><script>_ws_(_NSID_);</script>
                        </td>
                        ...
                        <td>
                          <comment id="_NSID_"> 
                              <object id=LC_S_S_PROC_STAT classid=<%=Util.CLSID_LUXECOMBO%> width=100 tabindex=1 align="absmiddle"> </object> 
                          </comment><script>_ws_(_NSID_);</script>
                        </td>
                    </tr>
                    <tr>
            ...
            <tr valign="top">
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0"
                    ...
                    <tr>
                        <td>
                          <comment id="_NSID_"> 
                              <OBJECT id=GR_LIST width=100% height=432 classid=<%=Util.CLSID_GRID%>>
                    ...
                    <tr>
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
                            ...
                            <tr>
                                ...
                                <td >
                                    <comment id="_NSID_"> 
                                        <object id=LC_PAIR_STR classid=<%=Util.CLSID_LUXECOMBO%> width=100 tabindex=1 align="absmiddle"> </object> 
                                    </comment><script>_ws_(_NSID_);</script>
                                </td>
                                <th >점입전표</th>
                                <td >
                                    <comment id="_NSID_"> 
                                        <object id=EM_PAIR_SLIP_NO classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1 align="absmiddle"> </object> 
                                    </comment><script> _ws_(_NSID_);</script>
                                </td>
                                <th >점입전표상태</th>
                                <td>
                                    <comment id="_NSID_"> 
                                        <object id=EM_P_SLIP_PROC_STAT_NM classid=<%=Util.CLSID_EMEDIT%> width=115 tabindex=1 align="absmiddle"> </object> 
                                    </comment><script> _ws_(_NSID_);</script>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    <comment id="_NSID_"> 
                                        <object id=EM_ORD_FLAG classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1 align="absmiddle"> </object> 
                                    </comment><script> _ws_(_NSID_);</script>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <comment id="_NSID_"> 
                                        <object id=EM_BUYER_NM classid=<%=Util.CLSID_EMEDIT%> width=62 tabindex=1 align="absmiddle"> </object> 
                                    </comment><script> _ws_(_NSID_);</script>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <comment id="_NSID_"> 
                                        <object id=EM_GAP_TOT_AMT classid=<%=Util.CLSID_EMEDIT%> width=65 tabindex=1 align="absmiddle"> </object> 
                                    </comment><script> _ws_(_NSID_);</script>
                                    <comment id="_NSID_"> 
                            ...
                            <tr valign="top">
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                                    <tr>
                                        <td>
                                          <comment id="_NSID_"> 
                                              <OBJECT id=GR_DETAIL width=100% height=156 classid=<%=Util.CLSID_GRID%>>
