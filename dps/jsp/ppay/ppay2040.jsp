 ...
 */
 ...
 function doInit(){
	...
	//Master 그리드 세로크기자동조정  2013-07-17
     ...
     initEmEdit(EM_YYYYMM_CP        ,"THISMN"    ,PK    		 );        // 지불년월
     ...
     initComboStyle(LC_PAY_FLAG,DS_O_PAY_FLAG    			,"CODE^0^20,NAME^0^130"  ,1  ,PK);       // 공제구분
     ...
     LC_TEAM.index = 0;
     LC_STR_CD.Focus();
     EM_YYYYMM_CP.text  = addDate("M", "-1", '<%=strToMonth%>');
 ...
 function gridCreate1(){
     ...
     var hdrProperies = '<FC>id={currow}     name="NO"               titleheight=60 width=30   align=center</FC>'
                      ...
                      + '<FC>id=STR_CD       name="*점"                       width=90   align=left   Edit=none EditStyle=Lookup   Data="DS_IO_STR_CD:CODE:NAME"</FC>'   
                      ...
                      + '<FC>id=VEN_CD       name="*공제지불;협력사코드"         width=80   align=center EditStyle=Popup</FC>'                      
                      + '<FC>id=VEN_NM       name="공제지불;협력사명"           width=120   align=left   Edit=none</FC>'
                      + '<FC>id=DED_VEN_CD   name="*공제계산서;협력사코드"       width=110   align=center EditStyle=Popup</FC>'
                      + '<FC>id=DED_VEN_NM   name="공제계산서;협력사명"          width=120   align=left   Edit=none</FC>'
                      ...
                      + '<FC>id=INPUT_DT     name="공제등록일"                 width=80    align=center  Edit=none Mask="XXXX/XX/XX" </FC>'   
                      + '<FC>id=REASON_CD    name="*공제"                     width=140   align=left   EditStyle=Lookup Data="DS_I_REASON_CD:CODE:NAME"</FC>'
                      +	'<FC>id=CASH_IN_YN   name="입금여부"                   width=70    align=center EditStyle=Lookup Data="DS_I_CASH_IN_YN:CODE:NAME"  </FC>' //신규 추가
                      + '<FC>id=VAT_YN       name="VAT여부"                   width=90    align=left   Edit=none EditStyle=Lookup Data="DS_I_VAT_YN:CODE:NAME"</FC>' 
                      + '<FC>id=INPUT_AMT    name="*공제금액"        gte_columntype="number:0:true" gte_Summarytype="number:0:true"          width=100   align=right  Edit={IF(CONF_FLAG="N","true","false")} sumtext={SUM(INPUT_AMT)} </FC>'  //{IF(CONF_FLAG="N","true","false")}   
                      + '<FC>id=SUP_AMT      name="공급가"           gte_columntype="number:0:true" gte_Summarytype="number:0:true"          width=100   align=right  Edit=none  sumtext={SUM(SUP_AMT)}</FC>'   
                      + '<FC>id=VAT_AMT      name="부가세"           gte_columntype="number:0:true" gte_Summarytype="number:0:true"         width=100   align=right  Edit=none  sumtext={SUM(VAT_AMT)}</FC>'   
 ...
 *************************************************************************/
 ...
 function btn_Search() {
      ...
      if(checkValidation("Search")){
          ...
          var strPayFlag       = LC_PAY_FLAG.BindColVal;                 // 공제구분
          var strVenCd         = EM_S_VEN_CD.Text;                      // 협력사코드
          var strPumCd         = EM_S_PUMBUN_CD.Text;                   // 브랜드코드
          var strTeam          = LC_TEAM.BindColVal;                   

          getMaster("DS_IO_MASTER",strStrCd, strYyyymm, strPayCyc, strPayCnt, strVenCd, strPumCd, strPayFlag, strTeam );
  ...
  * return값 : void
  */
function getMaster(dataSet, strStrCd, strYyyymm, strPayCyc, strPayCnt, strVenCd, strPumCd, strPayFlag, strTeam){
	var goTo       = "getMaster" ;    
    var action     = "O";     
    ...
    TR_MAIN.Action  = "/dps/ppay204.pp?goTo="+goTo+parameters;  
...
function fn_pre_copy() {

	if( showMessage(QUESTION, YESNO, "USER-1000", EM_YYYYMM_CP.TEXT +" -> "+ EM_YYYYMM.Text +" 복사 하시겠습니까?") != 1 )
        return;
    
    
    var strStrCd   = LC_STR_CD.BindColVal;
    var	parameters = "&S_YYYYMM="			+ EM_YYYYMM_CP.TEXT
    		       + "&I_YYYYMM="			+ EM_YYYYMM.TEXT
    		       + "&strStrCd="			+ strStrCd
    ;          

    
    TR_MAIN.Action  = "/dps/ppay204.pp?goTo=savecopy"+parameters;  
...
/*	
    ...
    if(intDelCnt > 0){
	    ...
	    TR_MAIN.Action="/dps/ppay204.pp?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(STOPSIGN, YESNO, "USER-1010") == 1 ){
        ...
        TR_MAIN.Action="/dps/ppay204.pp?goTo=save&strFlag=save&strYYYYMM="+strYYYYMM; 
        TR_MAIN.KeyValue="SERVLET(I:DS_IO_MASTER=DS_IO_MASTER)"; //조회는 O
        TR_MAIN.Post();
...
function btn_Print() {
	    ...
	    var strPayFlag_Cd    = LC_PAY_FLAG.BindColVal;
	    var strPayFlag_Nm    = LC_PAY_FLAG.Text;
	    var strTeam			 = LC_TEAM.BindColVal;
	    var strTeam_Nm		 = LC_TEAM.Text;
	     ...
	     var parameters =  "&strStrCd="+strStrCd
				         ...
				         + "&strPayFlag_Nm="+encodeURIComponent(strPayFlag_Nm)
				         + "&strTeam="+strTeam
				         + "&strTeam_Nm="+strTeam_Nm
				         + "&strGubn="+"2"
				         ;
	    window.open("/dps/ppay204.pp?goTo=print"+parameters,"OZREPORT", 1000, 700);   
 ...
 *************************************************************************/
 ...
 function addRow(){
	  if(checkValidation("Save")){
          ...
          DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "CASH_IN_YN")    = "N";
...
//         alert("btnFlag = " + btnFlag);
 ...
 function checkKeyVlaue(row) {
     ...
     TR_MAIN.Action="/dps/ppay204.pp?goTo=checkKeyVlaue&strKeyValue="+strKeyValue; 
...
//     alert("DS_O_RESULT = " + DS_O_RESULT.NameValue(DS_O_RESULT.RowPosition, "KEYVALUE"));
 ...
 function setVatXYN(row) {
     ...
     TR_MAIN.Action="/dps/ppay204.pp?goTo=setVatXYN&strReasonCd="+strReasonCd; 
       ...
       if(Gubun == "Save"){          
           ...
           if(intRowCount > 0){
               for(var i=1; i <= intRowCount; i++){
                   ...
                   if(intInputAmt == 0){
                       showMessage(EXCLAMATION, OK, "USER-1000", "입력금액은 0은 아닌 값을 입력해야 합니다.");
...
//            return true;
    ...
    case "DED_VEN_CD":
        if(!checkDedVenCd(row)){
            showMessage(EXCLAMATION, OK, "USER-1003", "공제계산서 협력사코드");
            DS_IO_MASTER.NameValue(row, "DED_VEN_NM") = "";
            DS_IO_MASTER.NameValue(row, "SEL") = 'T';
            GR_MASTER.SetColumn("DED_VEN_CD");
            GR_MASTER.Focus();
            return false;           
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
            ...
            <td rowspan=3>
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
			          <tr>
			            <th class="point" width="80">전월복사</th>
			         </tr>
			         <tr>
			         	<td>기준년월
			         	<comment id="_NSID_">
		                      <object id=EM_YYYYMM_CP classid=<%=Util.CLSID_EMEDIT%>  width=75 tabindex=1 align="absmiddle">
		                      </object>
		                  </comment>
		                  <script> _ws_(_NSID_);</script>
		                  <img src="/<%=dir%>/imgs/btn/date.gif" onclick="javascript:javascript:openCal('M',EM_YYYYMM_CP)"  align="absmiddle"/>
		                  <img src="/<%=dir%>/imgs/btn/copy_last.gif" onclick="javascript:javascript:fn_pre_copy()"  align="absmiddle"/>
			         	</td>
			         </tr>
			         <TR>
			         	<TD>기준년월에서 지불년월로 권한있는 브랜드에 한하여 복사가 진행됩니다.</TD>
			         </TR>
			    </table>
			</td>
          </tr>
          ...
          <tr>
            ...
            <th class="point" width="80">공제구분</th>
            <td>
                    <comment id="_NSID_">
                        <object id=LC_PAY_FLAG classid=<%= Util.CLSID_LUXECOMBO %> width=150 tabindex=1  align="absmiddle">
                        </object>
                    </comment><script>_ws_(_NSID_);</script>
            </td>
            ...
            <td>
                    <comment id="_NSID_">
                        <object id=LC_TEAM classid=<%= Util.CLSID_LUXECOMBO %> width=100 tabindex=1  align="absmiddle">
                        </object>
                    </comment><script>_ws_(_NSID_);</script>
            </td>
           </tr>
