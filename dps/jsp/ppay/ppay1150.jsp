 ...
 */
 ...
 function doInit(){
     ...
     // EMedit에 초기화
     initEmEdit(EM_FROM_DT, "SYYYYMMDD", PK);            // 매입기간 시작일
     initEmEdit(EM_TO_DT, 	"EYYYYMMDD", PK);            // 매입기간 종료일
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}    name="NO"			width=30    Edit=none sumtext="합계"  align=center</FC>'
                      + '<FC>id=VEN_CD      name="협력사;코드"    width=55    Edit=none align=center</C>'
                      + '<FC>id=VEN_NAME     name="협력사명"      width=130    Edit=none align=left</C>'
                      + '<FC>id=COMP_NO		name="사업자번호"    width=90    Edit=none Mask="XXX-XX-XXXXX" align=center</C>'
                      + '<FC>id=COMP_FLAG     name="법인구분"      width=70    Edit=none align=center</C>'
                      + '<FC>id=TAX_FLAG     name="구분"      width=40    Edit=none align=center</C>'
                      + '<G> name="매입"'
                      + '<C>id=DPS_SUP_AMT  name="공급가액"     width=110    Edit=none sumtext=@sum align=right</C>'
                      + '<C>id=DPS_VAT_AMT  name="세액"         width=110    Edit=none sumtext=@sum align=right</C>'
                      + '<C>id=DPS_TOT_AMT  name="계"          	width=110    Edit=none sumtext=@sum align=right</C>'
                      + '</G>'
                      + '<G> name="접수"'
                      + '<C>id=ACC_SUP_AMT	name="공급가액"      width=110    Edit=none sumtext=@sum align=right</C>'
                      + '<C>id=ACC_VAT_AMT  name="세액"          width=110    Edit=none sumtext=@sum align=right</C>'
                      + '<C>id=ACC_TOT_AMT  name="계"          	 width=110    Edit=none sumtext=@sum align=right</C>'
                      + '</G>'
                      + '<G> name="매입/접수 차액"'
                      + '<C>id=SUP_AMT      name="공급가액"       width=110    Edit=none sumtext=@sum align=right</C>'
                      + '<C>id=VAT_AMT      name="세액"           width=110    Edit=none sumtext=@sum align=right</C>'
                      + '<C>id=TOT_AMT      name="계"          	  width=110    Edit=none sumtext=@sum align=right</C>'
                      + '</G>'
                      + '<C>id=B_FLAG       name="비고"          width=30    Edit=none align=center</C>'
                      ;
...
function btn_Search() {
    ...
    TR_MAIN.Action  = "/dps/ppay115.pp?goTo="+goTo+parameters;  
...
function btn_Excel() {
	...
	var parameters = "";
		parameters  = "점="+strStrCd                    
			    	...
			    	+ "~"+ strToDt
			    	;
	openExcel2(GR_MASTER, "세금계산서비교현황", parameters, true );
}
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
            ...
            <td width="170">
                    <comment id="_NSID_">
                        <object id=LC_STR_CD classid=<%= Util.CLSID_LUXECOMBO %> width=170 tabindex=1 align="absmiddle">
                        </object>
                    </comment><script>_ws_(_NSID_);</script>
            ...
            <td >
                  <comment id="_NSID_">
                      <object id=EM_FROM_DT classid=<%=Util.CLSID_EMEDIT%>  width=119 tabindex=1 align="absmiddle">
                      </object>
                  </comment><script> _ws_(_NSID_);</script>
                  <img src="/<%=dir%>/imgs/btn/date.gif" id="IMG_FROM_DT" onclick="javascript:openCal('G',EM_FROM_DT)" align="absmiddle" />
                  ~
                  <comment id="_NSID_">
                      <object id=EM_TO_DT classid=<%=Util.CLSID_EMEDIT%>  width=119 tabindex=1 align="absmiddle">
                      </object>
                  </comment><script> _ws_(_NSID_);</script>
