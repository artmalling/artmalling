<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 대금지불 > 특정/임대을 지불대상액 조회
 * 작 성 일 : 2010.05.11
 * 작 성 자 : 박래형
 * 수 정 자 : 
 * 파 일 명 : ppay3050.jsp
 ...
 */
 ...
 function doInit(){
	//Master 그리드 세로크기자동조정  2013-07-17
     ...
     getEtcCode("DS_O_BIZ_TYPE",  "D", "P416", "Y");            // 조회거래형태
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}          name="NO"              width=30     align=center</FC>'
    	  			  + '<FC>id=PAY_DT            name="지불일자"        width=80     align=center  sumtext=""  mask="XXXX/XX/XX"</FC>'
                      ...
                      + '<FC>id=TAX_FLAG_NM       name="과세구분"         width=70     align=left</FC>'
                      + '<FC>id=SALE_TOT_AMT      name="총매출"           gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=100     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=REDU_AMT          name="할인"             gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=90     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=SALE_TAMT         name="매출"             gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=100     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=SALE_COMIS_TAMT   name="이익액"           gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=90     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=PAY_TRG_TAMT      name="지불대상액"       gte_columntype="number:0:true" gte_Summarytype="number:0:true"	 width=90     align=right  sumtext=@sum  </FC>';
 ...
 function gridCreate2(){
     var hdrProperies = '<FC>id={currow}          name="NO"              width=30     align=center</FC>'
                      ...
                      + '<FC>id=MG_RATE           name="마진율"           gte_columntype="number:2:true" gte_Summarytype="number:2:true"	width=50    align=right  sumtext=@avg  </FC>'
                      + '<FC>id=SALE_TOT_AMT      name="총매출"           gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=100     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=REDU_AMT          name="할인"             gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=90     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=SALE_AMT          name="매출"             gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=100     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=SALE_COMIS_AMT    name="이익액"           gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=90     align=right  sumtext=@sum  </FC>'
                      + '<FC>id=PAY_TRG_AMT       name="지불대상액"       gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=90     align=right  sumtext=@sum  </FC>';
...
function btn_Excel() {
	if(DS_O_MASTER.CountRow <= 0){
	    ...
	    var strTitle = "특정/임대을 지불대상액";
	     ...
	     var strPayDt   = LC_PAY_DT.BindColVal;
 ...
 *************************************************************************/
  ...
  */
  function getMaster(){
     ...
     TR_MAIN.Action  = "/dps/ppay305.pp?goTo="+goTo+parameters;  
   ...
   */
   function getDetail(){
      ...
      var strYyyymm  = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "PAY_YM");                    // 매출년월
      var strPayCyc  = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "PAY_CYC");                   // 지불주기
      var strPayCnt  = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "PAY_CNT");                   // 지불회차
      var strVenCd   = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "VEN_CD");                    // 협력사    
      var strPumbunCd   = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "PUMBUN_CD");                    // 협력사            
      ...
      TR_DETAIL.Action  = "/dps/ppay305.pp?goTo="+goTo+parameters;  
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr valign="top">
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                    ...
                    <td>
                    <comment id="_NSID_"> 
                        <OBJECT id=GR_MASTER width=100% height=210 classid=<%=Util.CLSID_GRID%>>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr valign="top">
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A">
                    ...
                    <td>
                          <comment id="_NSID_"> 
                              <OBJECT id=GR_DETAIL width=100% height=209 classid=<%=Util.CLSID_GRID%>>
