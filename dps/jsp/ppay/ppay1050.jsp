<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 대금지불 > 공제협력사 매출세금계산서 일괄 생성
 * 작 성 일 : 2010.04.18
 * 작 성 자 : 김경은
 * 수 정 자 : 
 * 파 일 명 : ppay1050.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 공제협력사 매출세금계산서 일괄 생성
 * 이    력 :
 * 
 ******************************************************************************/
-->
 ...
 */
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}          name="NO"            width=30    Edit=none sumtext="합계"  align=center</FC>'
    	              ...
    	              + '<FC>id=VEN_NM            name="협력사명"      width=110   Edit=none align=left</FC>'
    	              + '<FC>id=BIZ_TYPE_NM       name="거래형태"      width=80    Edit=none align=left</FC>'
                      + '<FC>id=INPUT_DT          name="공제일자"      width=80    Edit=none Mask="XXXX/XX/XX" align=center</FC>'
                      + '<FC>id=REASON_CD         name="공제"          width=30    Edit=none align=Center</FC>'
                      + '<FC>id=REASON_NM         name="공제명"        width=90    Edit=none align=left</FC>'
                      + '<FC>id=INPUT_AMT         name="공제금액"      width=100   Edit=none sumtext=@sum align=right</FC>'
                      + '<FG>                     name="세금계산서"'
                      ...
                      + '<FC>id=TOT_AMT           name="합계액"         width=110  Edit=none sumtext=@sum align=right</FC>'
...
function btn_Save() {
    ...
    if( showMessage(QUESTION, YESNO, "USER-1010") == 1 ){    // validation 체크
        ...
        TR_MAIN.Action="/dps/ppay105.pp?goTo=save&strFlag=save"+parameters; 
 ...
 */
 function getMaster(){
    ...
    TR_MAIN.Action  = "/dps/ppay105.pp?goTo="+goTo+parameters;  
 ...
 function checkValidation(Gubun) {
      ...
      if(Gubun == "Save"){
          ...
          if(ppayCloseCheck(strStrCd,strPayYm,strPayCyc,strPayCnt) == "TRUE"){
              showMessage(INFORMATION, OK, "USER-1150", "대금지불월", "공제협력사 매출세금계산서");
              return false;
          }
          return true;
      }
...
<script language="javascript"  for=GR_MASTER event=OnHeadCheckClick(Col,Colid,bCheck)>
    ...
    for( var i=1 ; i<=dataSet.CountRow; i++){
    	if(dataSet.NameValue(i, "TAX_ISSUE_DT") != " ")
    		continue;
    	else
