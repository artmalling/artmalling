 ...
 */
 ...
 function doInit(){
	...
	//Master 그리드 세로크기자동조정  2013-07-17
     ...
     initComboStyle(LC_PAY_FLAG,DS_O_PAY_FLAG    			,"CODE^0^30,NAME^0^80"  ,1  ,PK);       // 공제구분
     ...
     getEtcCode("DS_I_REASON_CD"  ,"D"   ,"P411"  ,"N");         // 보류
     getEtcCode("DS_O_PAY_FLAG"    ,"D"   ,"P411"  ,"Y" );         // 공제구분
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}     name="NO"               width=30    align=center</FC>'
                      ...
                      + '<FC>id=INPUT_DT     name="보류등록일"         width=80    align=center Edit=none Mask="XXXX/XX/XX" </FC>'
                      + '<FC>id=REASON_CD    name="*보류"            width=140   align=left   EditStyle=Lookup Data="DS_I_REASON_CD:CODE:NAME"</FC>'
                      + '<FC>id=INPUT_AMT    name="*보류금액"        width=110   align=right  Edit={IF(CONF_FLAG="N","true","false")}</FC>'
...
function getMaster(dataSet, strStrCd, strYyyymm, strPayCyc, strPayCnt, strVenCd, strPumCd , strPayFlag , strTeam){
    ...
    TR_MAIN.Action  = "/dps/ppay203.pp?goTo="+goTo+parameters;  
...
/*	
    ...
    if(intDelCnt > 0){
	    ...
	    TR_MAIN.Action="/dps/ppay203.pp?goTo=save"+params; 
...
function btn_Save() {
    ...
    if( showMessage(STOPSIGN, YESNO, "USER-1010") == 1 ){
        ...
        TR_MAIN.Action="/dps/ppay203.pp?goTo=save&strFlag=save&strYYYYMM="+strYYYYMM; 
        TR_MAIN.KeyValue="SERVLET(I:DS_IO_MASTER=DS_IO_MASTER)"; //조회는 O
        TR_MAIN.Post();
...
//         alert("btnFlag = " + btnFlag);
 ...
 function checkKeyVlaue(row) {
     ...
     TR_MAIN.Action="/dps/ppay203.pp?goTo=checkKeyVlaue&strKeyValue="+strKeyValue; 
           ...
           if(intRowCount > 0){
               for(var i=1; i <= intRowCount; i++){
                   ...
                   if(strReasonCd.length <= 0){
                       showMessage(INFORMATION, OK, "USER-1003", "보류");
...
*/
  ...
  */
  function closeCheck(row, strPayCyc, strPayCnt){
      ...
      if(closeFlag == "TRUE"){
          showMessage(EXCLAMATION, OK, "USER-1068", "대금지불","보류등록");
          return false;
      }else{
          return true;
      }    
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          ...
          <tr>
            ...
            <td>
                    <comment id="_NSID_">
                        <object id=LC_PAY_FLAG classid=<%= Util.CLSID_LUXECOMBO %> width=100 tabindex=1  align="absmiddle">
                        </object>
                    </comment><script>_ws_(_NSID_);</script>
            </td>
