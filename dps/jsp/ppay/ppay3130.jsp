 ...
 */
 ...
 function doInit(){
     ...
     //initEmEdit(EM_SALE_S_DT,    "TODAY", READ);            // 매입/매출기간 시작일   
     ...
     //콤보 초기화
     initComboStyle(LC_STR_CD,DS_IO_STR_CD,        "CODE^0^30,NAME^0^140", 1, PK);    // 점(조회)
     initComboStyle(LC_SEND_FLAG,DS_IO_SEND_FLAG,  "CODE^0^30,NAME^0^120", 1, PK);    // 회계전송구분
     ...
     initComboStyle(LC_PAY_DT,      DS_O_PAY_DT,    "CODE^0^0,NAME^0^80",  1, PK);    // 지불일자
     ...
     LC_PAY_CNT.index  = 0;
     EM_YYYYMM.Focus(); 
     //EM_SALE_S_DT.Text = setDateAdd("D", -1, EM_SALE_S_DT.Text);
 ...
 *************************************************************************/
  ...
  */
  function getPayDt() {
	...
	var strYyyymm  = EM_YYYYMM.Text;                          // 매출년월
	...
	var goTo         = "getPayDt" ;    
 	...
 	var parameters = "&strStrCd="+encodeURIComponent(strStrCd)   
 	 ...
 	 TR_DETAIL.Action="/dps/ppay313.pp?goTo="+goTo+parameters;  
 ...
 function checkValidation(Gubun) {
      ...
      if(Gubun == "Save"){
    	  ...
    	  if(LC_PAY_CYC.BindColVal == ""){                                
              showMessage(INFORMATION, OK, "USER-1002", "지불주기");
              LC_PAY_CYC.Focus();
              return false;
          }
    	  
    	  if(LC_PAY_CNT.BindColVal == ""){                                
              showMessage(INFORMATION, OK, "USER-1002", "지불회차");
              LC_PAY_CNT.Focus();
              return false;
          }
    	  
          if(LC_PAY_DT.BindColVal == "" ){
              showMessage(EXCLAMATION, OK, "USER-1002", "지불일자");
              LC_PAY_DT.Focus();
              return false;
          }
          */
  ...
  function CloseCheck(){
     ...
     TR_VALCHECK.Action="/dps/ppay313.pp?goTo=valCheck"; 
 ...
 function process(){
	    ...
	    /*
	    if(LC_SEND_FLAG.BindColVal == "05"){
		}else{
			if(EM_YYYYMM.Text < strToday.substring(0, 6)){
				showMessage(EXCLAMATION, OK, "USER-1000", LC_SEND_FLAG.Text + "은 당월 이후만 입력가능 합니다.");
				EM_YYYYMM.Text = strToday.substring(0, 6);
				EM_YYYYMM.Focus();
				return;
			}
		}     */ 
	  ...
	  //변경또는 신규 내용을 저장하시겠습니까?
        if( showMessage(QUESTION, YESNO, "USER-1204") == 1 ){    // validation 체크
            TR_MAIN.Action="/dps/ppay313.pp?goTo=save&strFlag=save"; 
...
<script language=JavaScript for=DS_IO_SEND_FLAG event=OnFilter(row)>
    if (DS_IO_SEND_FLAG.NameValue(row,"REFER_CODE") == "3"){
    	return true;
    }
	else 
		return false;
...
<script language=JavaScript for=LC_SEND_FLAG event=OnSelChange()>
/*
	if(LC_SEND_FLAG.BindColVal == "05"){
		return;
	}else{
...
<script language=JavaScript for=EM_YYYYMM event=onKillFocus()>
    ...
    if(LC_SEND_FLAG.BindColVal == "05"){    	
    	...
    	if(this.Text < strToday.substring(0, 6)){
    		showMessage(EXCLAMATION, OK, "USER-1000", LC_SEND_FLAG.Text + "은 당월 이후만 입력가능 합니다.");
    		this.Text = strToday.substring(0, 6);
    		this.Focus();
    		return false;
    	}
    }*/
    ...
    <td class="PT01 PB03"><table width="50%" border="0" cellspacing="0" cellpadding="0" class="o_table">
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
            <th width="80" class="point">회계전송구분</th>
            <td>
                    <comment id="_NSID_">
          ...
          <tr>
            ...
            <td>
                  <comment id="_NSID_">
                      <object id=EM_YYYYMM classid=<%=Util.CLSID_EMEDIT%>  width=280 tabindex=1 align="absmiddle">
                      </object>
                  </comment>
          ...
          <tr  style="display:none">
            <th width="80" class="point">지불일자</th>
            <td>
                    <comment id="_NSID_">
...
<comment id="_NSID_">
    <object id=BD_SEARCH classid=<%=Util.CLSID_BIND%>>
        ...
        <param name=BindInfo        value='
            ...
            <c>Col=PAY_DT              Ctrl=LC_PAY_DT             param=BindColVal</c>            
