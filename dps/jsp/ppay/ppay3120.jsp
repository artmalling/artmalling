<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 대금지불 > 대금지불명세서(보고용) 조회/출력
 * 작 성 일 : 2010.05.12
 * 작 성 자 : 김경은
 * 수 정 자 : 
 * 파 일 명 : ppay3120.jsp
 ...
 */
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}          name="NO"             width=30      align=center</FC>'
                      + '<FC>id=VEN_CD            name="협력사"         width=70      align=center sumtext="합계" show=false</FC>'
                      + '<FC>id=VEN_NM            name="협력사명"        width=130     align=left  suppress=1   SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext="합계"</FC>'
                      + '<FC>id=PUMBUN_CD    	  name="브랜드코드"         width=70     align=center SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} Edit=none show=false</FC>'
                      + '<FC>id=PUMBUN_NM    	  name="브랜드명"           width=120     align=left SubSumText={decode(curlevel,1,"협력사 소계","")} SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} Edit=none </FC>' 
                      + '<FC>id=REAL_PAY_DT       name="지불일자"        width=80     align=center MASK="XXXX/XX/XX"  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} show=false</FC>'
                      + '<FC>id=STR_NM            name="점"              width=60      align=left SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} show=false</FC>'
                      + '<FC>id=BIZ_TYPE_NM       name="거래형태"        width=70      align=left SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} show=false</FC>'
                      + '<FC>id=BTIME_BAL_AMT     name="전월이월잔액"    width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum show=false</FC>'
                      + '<FC>id=SUM_AMT           name="매입금액"        width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum show=false</FC>'
                      + '<FC>id=NTIME_SUP_AMT     name="공급가"          width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=NTIME_VAT_AMT     name="부가세"          width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=SALE_TOT_AMT      name="총매출"          width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=REDU_AMT          name="할인"            width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum show=false</FC>'
                      + '<FC>id=SALE_AMT          name="매출"            width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum show=false</FC>'
                      + '<FC>id=COMIS_SALE_AMT    name="이익액"          width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=SALE_COMIS_AMT    name="매출수수료"      width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum  show=false</FC>'
                      + '<FC>id=NTIME_PAY_AMT     name="지불대상액"      width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=NTIME_BFPAY_AMT   name="선급금액"        width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=NTIME_HOLD_AMT    name="보류액"          width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=NTIME_DED_AMT     name="공제액"          width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=NTIME_DED_CASH_AMT  name="공제액(현금입금)"  width=110  show=false   align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=NTIME_RLPAY_AMT   name="실지불액"        width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
                      + '<FC>id=NTIME_BAL_AMT     name="지불후잔액"      width=110     align=right SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")} sumtext=@sum</FC>'
     ...
     DS_IO_MASTER.SubSumExpr  = "1:VEN_CD"; 
     //GR_MASTER.ColumnProp("VEN_CD", "SubSumText")        = "소계";  
     GR_MASTER.ColumnProp("VEN_NM", "sumtext")           = "합계";  
 }
...
function btn_Excel() {
	    ...
	    var strTitle = "대금지불 명세서";
	    if (LC_S_BIZ_TYPE.BindColVal == '1') {
	    	strTitle = "대금지불 명세서(직매입)";	
	    } else if (LC_S_BIZ_TYPE.BindColVal == '2') {
	    	strTitle = "대금지불 명세서(특정)";	
	    } else if (LC_S_BIZ_TYPE.BindColVal == '3') {
	    	strTitle = "대금지불 명세서(임대을)";	
	    } else {
	    	strTitle = "대금지불 명세서";
	    }
 ...
 */
 ...
 //    var strUserId     = '<c:out value="${sessionScope.sessionInfo.USER_ID}" />';
    ...
    window.open("/dps/ppay312.pp?goTo=print"+parameters,"OZREPORT", 1000, 700);
 }
 ...
 */
 function getMaster(){
    ...
    TR_MAIN.Action  = "/dps/ppay312.pp?goTo="+goTo+parameters;
  ...
  function getPayDt() {
	...
	var strPayCnt  = LC_S_PAY_CNT.BindColVal;                   // 지불회차
 	...
 	var parameters = "&strStrCd="+encodeURIComponent(strStrCd)   
 	 ...
 	 TR_DETAIL.Action="/dps/ppay312.pp?goTo="+goTo+parameters;  
