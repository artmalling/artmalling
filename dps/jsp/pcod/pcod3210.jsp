...
function doInit(){
    ...
    registerUsingDataset("pcod321","DS_IO_DETAIL, DS_IO_MASTER" );
}
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}     name="NO"         width=25   align=center   edit="none"</FC>'
                     ...
                     + '<C>id=PUMBUN_NAME   name="명"       width=130   align=left   edit="none" </C>'
                     + '</FG>'
                     ...
                     + '<C>id=REMARK        name="구분명"         width=40   align=left   edit="none" </C>';
...
function gridCreate3(){
    var hdrProperies = '<FC>id={currow}         name="NO"             width=30   align=center</FC>'
        ...
        + '<FC>id=EVENT_CD         name="행사코드"         width=110   align=center</FC>'
...
function btn_Save() {
    ...
    TR_MAIN.Action="/dps/pcod321.pc?goTo=save"+parameters;
 ...
 */
 function searchMaster(){
    ...
    TR_MAIN.Action="/dps/pcod321.pc?goTo="+goTo+parameters;  
 ...
 */
 function searchDetail(){
     ...
     TR_DETAIL.Action="/dps/pcod321.pc?goTo="+goTo+parameters;  
 ...
 */
 function searchDetail2(){
     ...
     TR_DETAIL2.Action="/dps/pcod321.pc?goTo="+goTo+parameters;  
...
function btn_Add1(){
    ...
    if(DS_IO_MASTER.RowPosition > 0) {
        ...
        //maxAppSDt = DS_IO_DETAIL.Max(DS_IO_DETAIL.ColumnIndex("APP_S_DT"),0,0);
        //maxAppSDt = getRawData(addDate('d',1,(maxAppSDt==''?getTodayFormat("YYYYMMDD"):maxAppSDt)));
        maxAppSDt = getRawData(addDate('d',0,(maxAppSDt==''?getTodayFormat("YYYYMMDD"):maxAppSDt)));
...
//                if (DS_IO_DETAIL.NameValue(i,"APP_S_DT") <= varToday && DS_IO_DETAIL.NameValue(i,"APP_E_DT") == "99991231"){
				//if (DS_IO_DETAIL.NameValue(i,"APP_S_DT") <= varToday ){
		            ...
		            /*
				if (DS_IO_DETAIL.NameValue(i,"APP_S_DT") < varToday ){
		            showMessage(Information, OK, "USER-1000", "적용시작일은 오늘 일자보다 크거나 같아야 합니다.");
...
//                if (isBetweenFromTo(DS_IO_DETAIL.NameValue(i,"APP_S_DT"), DS_IO_DETAIL.NameValue(i,"APP_E_DT")) == "false"){
                ...
                for(k=1; k<= DS_IO_DETAIL.CountRow; k++){
                	if(DS_IO_DETAIL.NameValue(i, "APP_S_DT") <= DS_IO_DETAIL.NameValue(k, "APP_S_DT") && i != k && DS_IO_DETAIL.RowStatus(i) == "1"){
                		...
                		if(DS_IO_DETAIL.NameValue(i, "APP_S_DT") <  varToday && DS_IO_DETAIL.RowStatus(i) == "1"){
                            showMessage(Information, OK, "USER-1000", "적용시작일은 당일보다  크거나같아야 합니다.");
                            ...
                            DS_IO_DETAIL.NameValue(i, "APP_S_DT") = getRawData(addDate('d',0,(varToday==''?getTodayFormat("YYYYMMDD"):varToday)));;
                            return false;
                        }
                		else{
	                		if(	DS_IO_DETAIL.RowStatus(i) == "3" &&
	                			...
	                			if(DS_IO_DETAIL.NameValue(i, "APP_S_DT") <= DS_IO_DETAIL.NameValue(k, "APP_S_DT") && i != k && 
	                                ...
	                                setFocusGrid(GD_DETAIL, DS_IO_DETAIL,i,"APP_S_DT");
	                                //DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition,"APP_S_DT") = getRawData(addDate('D',1,DS_IO_DETAIL.NameValue(k, "APP_S_DT")));
	                                DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition,"APP_S_DT") = getRawData(addDate('D',0,DS_IO_DETAIL.NameValue(k, "APP_S_DT")));
	                                return false;
	                			}
	                		}
                		}
...
function searchMgInfo(vMgFlag){
	...
	TR_SEARCH.Action   = "/dps/pcod321.pc?goTo=" + goTo + parameters;
...
function searchPumbunCheck(vPumbunCd){
	...
	TR_SEARCH2.Action   = "/dps/pcod321.pc?goTo=" + goTo + parameters;
...
<!-- 			                  </td> -->
                              <td  colspan="4" class="FS10">정상:00~10,행사:11~69,기획:70~75,균일:76~80,인하:81~89,타사상품권:90,온라인매출:91~99
			                  </td>
			                </tr>
						</table>
						</td>
