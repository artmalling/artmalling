...
function doInit(){
    ...
    initEmEdit(EM_BRAND_CD2              ,"000000"        ,NORMAL);    // 조회용 브랜드코드
    initEmEdit(EM_BRAND_NM2              ,"GEN"          ,READ);    // 조회용 브랜드명
	...
	//EM_BRAND_CD2.text = "000663";
	//EM_BRAND_NM2.text = "나이키";
    
    registerUsingDataset("pcod208","DS_IO_MASTER" );
}
 ...
 function gridCreate1(){
     var hdrProperies =   '<FC>id=SEL               name=""            		 width=30    EditStyle=CheckBox HeadCheckShow="true" align=center</FC>'
	        			...
	        			+ '<FC>id=VEN_NAME          name="협력사명"              width=130   edit = none align=left</FC>'
				        + '<FC>id=REP_PUMBUN_CD     name="대표브랜드"            width=70    edit = none align=center</FC>'
				        + '<FC>id=REP_PUMBUN_NAME   name="대표브랜드명"          width=130   edit = none align=left</FC>'
	        			+ '<FC>id=PUMBUN_CD         name="브랜드코드"            width=70    edit = none align=center</FC>'
				        + '<FC>id=PUMBUN_NAME       name="브랜드명"              width=150   edit = none align=left</FC>'
				        + '<FC>id=MG_RATE           name="마진"            		 width=50    edit = none align=right</FC>'
				         ;
...
function btn_Search() {
    ...
    TR_MAIN.Action   = "/dps/pcod208.pc?goTo=" + goTo + parameters;
...
function btn_Delete() {
	...
	if (!DS_IO_MASTER.IsUpdated){
	   ...
	   if( showMessage(QUESTION, YESNO, "USER-1023") == 1 ){
			...
			var parameters   = "&strBrandCd=" + encodeURIComponent(EM_BRAND_CD2.text);  //브랜드 코드;
			
			TR_MAIN.Action="/dps/pcod208.pc?goTo=del" + parameters;
...
function btn_Save() {
    ...
    if(EM_BRAND_CD2.text == "" || EM_BRAND_NM2.text == "") {
    	showMessage(INFORMATION, OK, "USER-1000","대표브랜드코드를 입력하세요.");
    	EM_BRAND_CD2.setfocus;
    	return;
    }
    ...
    TR_MAIN.Action="/dps/pcod208.pc?goTo=save" + parameters;
    TR_MAIN.KeyValue="SERVLET(I:DS_IO_MASTER=DS_IO_MASTER)"; //조회는 O
    TR_MAIN.Post();
...
<script language=JavaScript for=EM_BRAND_CD2 event=OnKillFocus()>
//변경된 내역이 없으면 무시
if(!this.Modified)
    return;
	setPumbunCode('NAME','S',EM_BRAND_CD2,EM_BRAND_NM2);
</script>
...
<!--공통 타이틀/버튼 // -->
  ...
  <tr valign="top">
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr valign="top">
        <td><table width="700" border="0" cellspacing="0" cellpadding="0"  class="BD4A">
          <tr>
            <td width="700">
                <comment id="_NSID_">
            	...
            	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
					...
					<tr>
				      <th width="80" >대표브랜드</th>
				            <td width="300">
				              <comment id="_NSID_">
				                <object id=EM_BRAND_CD2 classid=<%=Util.CLSID_EMEDIT%> width=50 tabindex=1 align="absmiddle"></object>
				              </comment><script> _ws_(_NSID_);</script>
				              <img src="/<%=dir%>/imgs/btn/detail_search_s.gif"  onclick="javascript:strPbnPop(EM_BRAND_CD2,EM_BRAND_NM2, 'Y');"  align="absmiddle" />
				              <comment id="_NSID_">
				                <object id=EM_BRAND_NM2 classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1 align="absmiddle"></object>
				              </comment><script> _ws_(_NSID_);</script>
					</td>
					</tr>
					<tr>
...
<pre>
  ...
  - 대표브랜드 등록방법
    ...
    위 3개브랜드를 묶을 경우 체크박스 선택 후 
    대표브랜드에 311520 정규코드를 입력하여 저장
 
 
  - 삭제버튼
    삭제기능은 묶었던 대표브랜드에서 개별로 조회되도록 
    푸는 기능입니다.
    
    
  - 마진이 0.00 인것은 행사기간이 지난 브랜드입니다.
