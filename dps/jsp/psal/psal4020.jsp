...
function doInit(){
    ...
    registerUsingDataset("psal402","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"           width=30    align=center    </FC>'
                     ...
                     + '<FC>id=TEAM_CD                 name="파트코드"        width=60    align=center      suppress=1    </FC>'
                     ...
                     + '<C>id=ORIGIN_SALE_TAMT        name="매출목표"      width=100    align=right    mask="###,###"     gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(ORIGIN_SALE_TAMT/UNIT)} sumtext={round(subsum(ORIGIN_SALE_TAMT)/UNIT)}</C>'
                     + '<C>id=SALE_QTY                name="매출수량"      width=100    align=right    mask="###,###"     gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(SALE_QTY/UNIT)} sumtext={round(subsum(SALE_QTY)/UNIT)} show=false </C>'
                     + '<C>id=TOT_SALE_AMT            name="총매출"      width=100    align=right    mask="###,###"       gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(TOT_SALE_AMT/UNIT)} sumtext={round(subsum(TOT_SALE_AMT)/UNIT)}</C>'
                     + '<C>id=ACHIEVERATE             name="달성율"        width=60    align=right     mask="#,###.00"   gte_columntype="number:2:true" gte_Summarytype="number:2:true"	sumtext={decode(subsum(ORIGIN_SALE_TAMT),0,0.00,round(subsum(TOT_SALE_AMT) / subsum(ORIGIN_SALE_TAMT) * 100,2))}</C>'
                     + '<C>id=REDU_AMT                name="할인"          width=90    align=right    mask="###,###"     gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(REDU_AMT/UNIT)} sumtext={round(subsum(REDU_AMT)/UNIT)}</C>'
                     + '<C>id=SALE_AMT                name="매출"          width=100    align=right    mask="###,###"     gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(SALE_AMT/UNIT)} sumtext={round(subsum(SALE_AMT)/UNIT)}</C>'
                     + '<C>id=DC_AMT                  name="에누리"        width=100    align=right    mask="###,###"     gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(DC_AMT/UNIT)} sumtext={round(subsum(DC_AMT)/UNIT)}</C>'
                     ...
                     + '<C>id=SALE_PROF_AMT           name="이익액"        width=100    align=right     mask="###,###"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(SALE_PROF_AMT/UNIT)} sumtext={round(subsum(SALE_PROF_AMT)/UNIT)}</C>'
                     + '<C>id=PROFRATE                name="이익율"        width=60    align=right     mask="#,###.00"   gte_columntype="number:2:true" gte_Summarytype="number:2:true"	sumtext={decode(subsum(NORM_SALE_AMT_VAT),0,0.00,round((subsum(SALE_PROF_AMT)/subsum(NORM_SALE_AMT_VAT))*100,2))} </C>'
                     ...
                     + '<C>id=CUST_DANGA              name="객단가"        width=100    align=right     mask="###,###"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(CUST_DANGA/UNIT)} sumtext={round(decode(subsum(CUST_CNT),0,0,round(subsum(TOT_SALE_AMT)/subsum(CUST_CNT)))/UNIT)} </C>'
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}               name="NO"           width=30    align=center    </FC>'
                     + '<FC>id=ORG_CD                 name="브랜드코드"      width=70    align=center    suppress=1</FC>'
                     + '<FC>id=ORG_NAME               name="브랜드명"        width=130    align=left      suppress=1   sumtext="합계"</FC>'
                     + '<C>id=ORIGIN_SALE_TAMT        name="매출목표"      width=100    align=right    mask="###,###"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(ORIGIN_SALE_TAMT/UNIT)} sumtext={round(subsum(ORIGIN_SALE_TAMT)/UNIT)}</C>'
                     + '<C>id=SALE_QTY                name="매출수량"      width=100    align=right    mask="###,###"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={subsum(SALE_QTY)} show=false </C>'
                     + '<C>id=TOT_SALE_AMT            name="총매출"      width=100    align=right    mask="###,###"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(TOT_SALE_AMT/UNIT)} sumtext={round(subsum(TOT_SALE_AMT)/UNIT)}</C>'
                     + '<C>id=ACHIEVERATE             name="달성율"        width=60    align=right     mask="#,###.00"  gte_columntype="number:2:true" gte_Summarytype="number:2:true"	    sumtext={decode(subsum(ORIGIN_SALE_TAMT),0,0,round(subsum(TOT_SALE_AMT) / subsum(ORIGIN_SALE_TAMT) * 100,2))}  </C>'
                     + '<C>id=REDU_AMT                name="할인"          width=100    align=right    mask="###,###"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(REDU_AMT/UNIT)} sumtext={round(subsum(REDU_AMT)/UNIT)}</C>'
                     + '<C>id=SALE_AMT                name="매출"          width=100    align=right    mask="###,###"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(SALE_AMT/UNIT)} sumtext={round(subsum(SALE_AMT)/UNIT)}</C>'
                     + '<C>id=DC_AMT                  name="에누리"        width=100    align=right    mask="###,###"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(DC_AMT/UNIT)} sumtext={round(subsum(DC_AMT)/UNIT)}</FC>'
                     + '<C>id=NET_SALE_AMT            name="순매출"        width=100    align=right     mask="###,###"   gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(NET_SALE_AMT/UNIT)} sumtext={round(subsum(NET_SALE_AMT)/UNIT)} </C>'
                     + '<C>id=SALE_PROF_AMT           name="이익액"        width=100    align=right     mask="###,###"   gte_columntype="number:0:true" gte_Summarytype="number:0:true"	value= {round(SALE_PROF_AMT/UNIT)} sumtext={round(subsum(SALE_PROF_AMT)/UNIT)}</C>'
                     + '<C>id=PROFRATE                name="이익율"        width=60    align=right     mask="#,###.00"  gte_columntype="number:2:true" gte_Summarytype="number:2:true"	sumtext={decode(subsum(NORM_SALE_AMT_VAT),0,0.00,round((subsum(SALE_PROF_AMT)/subsum(NORM_SALE_AMT_VAT))*100,2))} </C>'
                     + '<C>id=CUST_CNT                name="객수"         width=60    align=right      mask="###,###"   gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={subsum(CUST_CNT)}</C>'
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal402.ps?goTo="+goTo+parameters;  
 ...
 */
 function searchDetail(){
     ...
     TR_DETAIL.Action="/dps/psal402.ps?goTo="+goTo+parameters;  
