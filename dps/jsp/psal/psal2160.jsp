...
function doInit(){
    ...
    getEtcCode("DS_TEAM_CD"    , "D", "D216", "Y"); 																		//팀
    ...
    registerUsingDataset("psal216","DS_IO_MASTER,DS_IO_DETAIL" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"             width=30     align=center    BgColor={decode(ERROR_CHECK,"","white","yellow")} </FC>'
                     ...
                     + '<FC>id=PRT_CD                  name="파트"           width=40     align=center    Edit=none    </FC>'
                     + '<FC>id=PRT_NM                  name="파트명"         width=60     align=center    Edit=none    </FC>'
                     ...
                     + '<G>                            name="PC별월매출목표합계"'
                     + '<C>id=ORIGIN_NORM_SAMT         name="정상매출"       width=130     align=right     SumText={SubSum(ORIGIN_NORM_SAMT)}   Edit=none</C>'
...
//                      + '<C>id=ORIGIN_PROF_TAMT         name="이익"          width=100     align=right     SumText={SubSum(ORIGIN_PROF_TAMT)}   Edit=none</C>'
                     + '<C>id=ORIGIN_SALE_CRATE        name="매출구성비"     width=100     align=right     Dec=2                                    Edit=none</C>'
...
function gridCreate2(lastday){
        ...
        var hdrProperies =
      	...
      	//+ '<G> name="파트"'
 		// + '<FC>id=PRT_CD    width=65    align=center   name="코드"    EDIT=NONE</FC>'
 		// + '<FC>id=PRT_NM  	 width=80   align=left     name="명"   EDIT=NONE   </FC>'
        // + '</G> '
      	 + '<G> name="브랜드"'
  		 + '<FC>id=PUMBUN_CD    width=65    align=center   name="코드"    </FC>'
  		 + '<FC>id=PUMBUN_NAME  width=110   align=left     name="명"     SumText="합계" </FC>'
         + '</G> '
 ...
 */
 ...
 function changecolumn(){
	...
	if(DS_IO_MASTER.countrow != 0) {
		...
		for(var a=1; a<=DS_IO_DETAIL.CountRow; a++){
			
			for(i=1; i<=31; i++){
				if(i < 10){
					Tot_sum += Number(DS_IO_DETAIL.NameValue(a,"SALE0"+i))
				} else {
					Tot_sum += Number(DS_IO_DETAIL.NameValue(a,"SALE"+i))
				}
			}

			DS_IO_DETAIL.NameValue(a,"TOT_AMT") = Number(Tot_sum);
			Tot_sum = 0;
			
			nMasterTotSum += DS_IO_DETAIL.NameValue(a,"TOT_AMT");
		}
		
		DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "ORIGIN_NORM_SAMT") = Number(nMasterTotSum);
	}
	...
	var nPcTotAmt = 0;
	var nAmtRate  = 0;
	for(b=1; b<=DS_IO_MASTER.CountRow; b++){
		nPcTotAmt += DS_IO_MASTER.NameValue(b, "ORIGIN_NORM_SAMT");
	}
	
// 	alert(nPcTotAmt);
	
	for(b=1; b<=DS_IO_MASTER.CountRow; b++){
		nAmtRate = (Number(DS_IO_MASTER.NameValue(b, "ORIGIN_NORM_SAMT")) / Number(nPcTotAmt)) * 100;
		DS_IO_MASTER.NameValue(b, "ORIGIN_SALE_CRATE") = nAmtRate;		
	}	
 }
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal216.ps?goTo="+goTo+parameters;  
...
function btn_Save() {
    ...
    TR_MAIN.Action = "/dps/psal216.ps?goTo=save"+parameters;
 ...
 */
 ...
 function btn_Conf() {
	...
	// 저장할 데이터 없는 경우
     ...
     TR_MAIN.Action="/dps/psal216.ps?goTo=saveconf"+parameters;
...
function searchDetail(){
    var strStrCd        = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "STR_CD");           //점
    //var strDeptCd       = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "DEPT_CD");          //팀
    var strTeamCd       = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "PRT_CD");          //파트
    //var strPCCd         = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "PC_CD");            //PC
    var strYYYYMM       = DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "YYYYMM");           //년월
    ...
    TR_DETAIL.Action="/dps/psal216.ps?goTo="+goTo+parameters;  
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  ...
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=180 classid=<%=Util.CLSID_GRID%>>
