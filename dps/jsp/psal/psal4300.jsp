...
function doInit(){
    ...
    //현재년도 셋팅
    EM_SALE_DT_S.text =  varYesterday;
    	//varToMon+"01";

    EM_SALE_DT_E.text =  varYesterday;
    ...
    getDept2("DS_DEPT_CD", "Y", LC_STR_CD.BindColVal, "Y", "", strOrgFlag);
    ...
    registerUsingDataset("psal430","DS_O_MASTER" );
     
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}       name="NO"           width=60    align=center    </FC>'
                     + '<FC>id=POS_JUM        name="점"           width=40    align=center SHOW=FALSE  </FC>'
                     + '<FC>id=POS_YMD        name="매출일자"         width=70    align=center        SUMTEXT = "정상계 : " </FC>'
                     + '<FC>id=POS_NO         name="POS번호"         width=60    align=center        sumtext = {sum(if( POS_GB = "1",SALE_AMT,0))}</FC>'
                     + '<FC>id=POS_SN         name="거래번호"         width=60    align=center        SUMTEXT = "반품계 : "</FC>'
                     + '<FC>id=POS_SEQ        name="순번"            width=40    align=center         sumtext = {sum(if( POS_GB <> "1",SALE_AMT,0))}</FC>'
                     + '<FC>id=POS_GB         name="정상반품오타"         width=80    align=center     SUMTEXT = "총계 : "</FC>'
                     + '<FC>id=GOOD_CD        name="상품코드"        width=100    align=center        sumtext = {sum(if( POS_GB <> "",SALE_AMT,0))}</FC>'
                     + '<FC>id=GOOD_NM        name="상품명"          width=140    align=left        </FC>'
                     + '<FC>id=SALE_QTY       name="판매수량"        gte_columntype="number:0:true" gte_Summarytype="number:0:true"	  width=50    align=right    mask="###,###"  sumtext={sum(SALE_QTY)} </FC>'
                     + '<FC>id=SALE_UP        name="판매단가"        gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=70    align=right    mask="###,###"  </FC>'
                     + '<FC>id=SALE_AMT       name="판매금액"        gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=70    align=right     mask="###,###" sumtext={sum(SALE_AMT)} </FC>'
                     + '<FC>id=SIJE_GB        name="시재구분"        width=50    align=center      </FC>'
                     + '<FC>id=POS_W_SN       name="원영수증번호"    width=120    align=center   </FC>'
                     + '<FC>id=POS_TIME       name="발생시간"    width=60    align=center   </FC>'
                     + '<FC>id=EVENT_CD       name="행사코드"    width=70    align=center  </FC>'
                     + '<FC>id=SALE_NORMAL    name="정상가"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=70    align=right     mask="###,###" </FC>'
                     + '<FC>id=SALE_EVENT     name="행사가"    gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=70    align=right     mask="###,###"  </FC>'
                     ;
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal430.ps?goTo="+goTo+parameters;  
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
                  ...
                  <td colspan="3"><comment id="_NSID_"> <object id=EM_PUMBUN_CD
                      ...
                      align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script>&nbsp &nbsp&nbsp <input type="checkbox" id="chk_online">네이버샵
                  </td>  
               </tr>
            </table>
            </td>
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=438 classid=<%=Util.CLSID_GRID%> >
