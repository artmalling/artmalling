...
function doInit(){
	var obj   = document.getElementById("GD_COU_SAL_DTL"); 
...
function gridCreate(){
    ...
    var hdrProperies = '<C>id={currow}    width=30  align=center name="NO"</C>'
                     ...
                     + '<C>id=COUPON_CD   width=80  align=center name="쿠폰코드"  show=false </C>'
                     + '<C>id=COUPON_NAME width=180 align=left   name="쿠폰명"  show=false </C>'
    ...
    var hdrProperies1 = '<C>id={currow}    width=35  align=center name="NO"</C>'
                      ...
                      + '<C>id=COUPON_CNT  width=60  align=right  name="수량"</C>'
                      + '<C>id=COUPON_AMT  width=80  align=right  name="단가"</C>'
                      + '<C>id=PAY_AMT     width=100 align=right  name="금액"</C>'
...
function btn_Search() {
    ...
    var parameters = "&strStoreCd="      + encodeURIComponent(LC_STR_CD.BindColVal)              //점구분코드
                   ...
                   + "&strPosNo="        + encodeURIComponent(EM_POS_NO.text)                    //POS번호
                   + "&strCouponType="   + encodeURIComponent(LC_COUPON_TYPE.BindColVal)         //쿠폰유형
                   + "&strGiftTypeCd="   + encodeURIComponent(EM_GIFT_TYPE_CD.Text);
                   ;
    DS_COU_SAL_MST.DataID = "/dps/psal541.ps?goTo="+goTo+parameters;
...
function subQuery(row){
    ...
    var action     = "O";
    var parameters = "&strStoreCd="      + encodeURIComponent(DS_COU_SAL_MST.NameValue(row,"STR_CD"))    //점코드
                   + "&strStartDt="      + encodeURIComponent(DS_COU_SAL_MST.NameValue(row,"SALE_DT"))   //매출일자
                   + "&strGiftTypeCd="   + encodeURIComponent(EM_GIFT_TYPE_CD.Text);
                   ;
   
    DS_COU_SAL_DTL.DataID = "/dps/psal541.ps?goTo="+goTo+parameters;
...
function btn_Excel() {
    ...
    openExcel5(GD_COU_SAL_DTL, strTitle, parameters, true , "",g_strPid );
