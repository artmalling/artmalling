...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}     width=30  align=center name="NO"          </FC>'
                     ...
                     + '<FC>id=STR_NM       width=100 align=center name="점" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}    </FC>'
                     + '<FC>id=FLOR_CD      width=40  align=center name="층" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}         </FC>'
                     + '<FC>id=POS_FLAG     width=60  align=center name="POS구분" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}         </FC>'
                     + '<FC>id=POS_FLAG_NM  width=140 align=center name="POS구분" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=POSCNT		width=60  align=right  name="전체"  mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=BFCNT        width=60  align=right  name="개설전"	mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=USECNT      	width=120 align=right  name="사용(영업중+마감)"	mask="###,###" sumtext=@sum  color=limegreen sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}      </FC>'
                     + '<FC>id=OPENCNT      width=80  align=right  name="영업중"	mask="###,###" color=limegreen  sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}    </FC>'
                     + '<FC>id=MAGAMCNT     width=80  align=right  name="마감"	mask="###,###" color=blue   sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}   </FC>';
...
function drawPosInfo(){
	...
	//Master 그리드 세로크기자동조정  2014-17-18
...
function strPosAcount( strPosNo )
{
    ...
    var returnVal = window.showModalDialog("/dps/psal515.ps?goTo=strPosAcount",
                                           arrArg,
 ...
 */
 function searchDetail(){
	    ...
	    var parameters = "&strStrCd="	+encodeURIComponent(strStrCd)
	                   ...
	                   + "&strGB="  	+encodeURIComponent(RD_GB.CodeValue)
	                   ;
	    TR_DETAIL.Action="/dps/psal515.ps?goTo="+goTo+parameters;  
                ...
                <object id=EM_MAGAM classid=<%=Util.CLSID_EMEDIT%>  width=75  tabindex=1 align="absmiddle"></object>
					...
					<tr>
						...
						<td colspan="8"><comment id="_NSID_"> <object
							...
							<param name=Format value="A^A:전체,B^B:개설전,U^U:사용,O^O:영업중,C^C:마감">
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr valign="top">
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD2A">
          <tr>
            <td><div  style="height:270px;width:100%;overflow:auto;" id="DIV_POS_LIST" onresize="javascript:drawPosInfo();"></div>
            </td>
          </tr>
        </table></td>
      </tr>
