...
function doInit(){
    ...
    LC_POS_FLAG.Index = 0;
    //외상구분
    initComboStyle(LC_CREDIT_FLAG,DS_CREDIT_FLAG, "CODE^0^30,NAME^0^80", 1, NORMAL);              
    getEtcCode("DS_CREDIT_FLAG"      , "D", "P608", "Y");
    LC_CREDIT_FLAG.Index = 0;
    ...
    registerUsingDataset("psal442","DS_O_MASTER" );
    
}
 
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"             width=30    align=center    </FC>'
    	             + '<FC>id=CREDIT_FLAG             name="미결구분"         width=70    align=left     suppress="1" </FC>'
                     + '<FC>id=STR_CD                  name="점"             width=70    align=center     suppress="1" show=false</FC>'
                     + '<FC>id=SALE_DT                 name="일자"            width=80    align=center      mask="XXXX/XX/XX" suppress="2" sumtext="합계"</FC>'
                     + '<FC>id=POS_NO                  name="POS번호"         width=60    align=center     suppress="3"</FC>'
                     + '<FC>id=TRAN_NO                 name="거래번호"         width=60    align=center      suppress="4"</FC>'
                     + '<FC>id=END_PROC_DT             name="결말예정일"       width=80    align=center      mask="XXXX/XX/XX" </FC>'
                     + '<FC>id=STATUS                  name="상태"            width=60    align=center     </FC>'
                     + '<C>id=SALE_TOT_AMT             name="순매출"           width=90    align=right    mask="###,###"    sumtext={subsum(SALE_TOT_AMT)}</C>'
                     + '<C>id=SP_SUNSUKUM              name="선수금"           width=80    align=right    mask="###,###"    sumtext={subsum(SP_SUNSUKUM)}</C>'
                     + '<C>id=SP_JANKUM                name="잔금"            width=70    align=right    mask="###,###"    sumtext={subsum(SP_JANKUM)}</C>'
                     + '<C>id=SP_IPAMK                 name="입금액"           width=80    align=right    mask="###,###"    sumtext={subsum(SP_IPAMK)}</C>'
                     + '<C>id=SP_JANAEK                name="잔액"            width=70    align=right    mask="###,###"    sumtext={subsum(SP_JANAEK)}</C>'
                     + '<C>id=DM_NAME                  name="담당자명"         width=80    align=left     </C>'
                     + '<C>id=DM_SABUN                 name="담당자사번"       width=100    align=left     </C>'
                     + '<C>id=BIGO                     name="비고"            width=100    align=left     </C>'
                     ;
...
function btn_Search() {
    ...
    var strCreditFlag   = LC_CREDIT_FLAG.BindColVal;         //거래번호
    ...
    var parameters = "&strStrCd="           +encodeURIComponent(strStrCd)
                   ...
                   + "&strCreditFlag="      +encodeURIComponent(strCreditFlag);
    
    TR_MAIN.Action="/dps/psal442.ps?goTo="+goTo+parameters;  
...
function searchPosNo(strPosNo){
    ...
    TR_DETAIL.Action="/dps/psal442.ps?goTo="+goTo+parameters;  
 ...
 * 작 성 자 : 박종은
 * 작 성 일 : 2010-07-20
 * 개    요 :  미결구분 변경시 그리드 컬럼 show true/false
 * return값 : 
 */
function creditFlagChange(){
    if(LC_CREDIT_FLAG.BindColVal == "02" || LC_CREDIT_FLAG.BindColVal == "03"){
    	GD_MASTER.ColumnProp("END_PROC_DT", "Show") = false;
        GD_MASTER.ColumnProp("DM_NAME", "Show")     = true;
        GD_MASTER.ColumnProp("DM_SABUN", "Show")    = true;
    }
    else{
    	if(LC_CREDIT_FLAG.BindColVal == "%"){
    		GD_MASTER.ColumnProp("END_PROC_DT", "Show") = true;
            GD_MASTER.ColumnProp("DM_NAME", "Show")     = true;
            GD_MASTER.ColumnProp("DM_SABUN", "Show")    = true;
    	}
    	else{
    		GD_MASTER.ColumnProp("END_PROC_DT", "Show") = true;
            GD_MASTER.ColumnProp("DM_NAME", "Show")     = false;
            GD_MASTER.ColumnProp("DM_SABUN", "Show")    = false;
    	}
...
<!-- 미결구분변경시  변경시  -->
<script language=JavaScript for=LC_CREDIT_FLAG event=OnSelChange()>
...
<comment id="_NSID_">
<object id="DS_CREDIT_FLAG" classid=<%=Util.CLSID_DATASET%>></object>
</comment>
<script>_ws_(_NSID_);</script>
         ...
         <tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
              ...
              <tr>
                  ...
                  <th>미결구분</th>
                  <td><comment id="_NSID_"> <object
                     id=LC_CREDIT_FLAG classid=<%=Util.CLSID_LUXECOMBO%> width=140 tabindex=1 
                     align="absmiddle"> </object> </comment><script>_ws_(_NSID_);</script></td>
               </tr>
            </table>
            </td>
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=457 classid=<%=Util.CLSID_GRID%> tabindex=1>
