...
function doInit(){
    ...
    initEmEdit(EM_SALE_S_DT, "YYYYMMDD", PK);      //조회시작일자    
    //EM_SALE_S_DT.text  = varToday;
    initEmEdit(EM_SALE_E_DT, "YYYYMMDD", NORMAL);      //조회종료일자
    EM_SALE_E_DT.text  = varToday;
    EM_SALE_S_DT.text = varToMon + "01";
    ...
    registerUsingDataset("psal558","DS_IO_MASTER" );
}
 ...
 function gridCreate1(){
     var hdrProperies = '<FC>id={currow}                name="NO"               width=30    align=center    Edit=none</FC>'
                      ...
                      + '<FC>id=SALE_DT                 name="입금일자"          width=100   align=center    Edit=none    mask="XXXX/XX/XX" suppress=1 </FC>'
                      + '<FC>id=POS_NO                  name="장비번호"          width=70    align=center    Edit=none  suppress=1  </FC>'
                      + '<FC>id=ETC_CD                  name="부자재코드"        width=70    align=center    Edit=none     </FC>'
                      + '<FC>id=ETC_QTY                 name="수량"             width=70    align=right    Edit=none    dec=0 </FC>'  
                      + '<FC>id=ETC_AMT                 name="금액"             width=100    align=right     Edit=none  dec=0 </FC>';
...
function btn_Search() {
	...
	var strSDate      = EM_SALE_S_DT.text;       // 조회 시작일
	var strEDate      = EM_SALE_E_DT.text;       // 조회 종료일
    ...
    TR_MAIN.Action   = "/dps/psal558.ps?goTo=" + goTo + parameters;
...
function btn_Delete() {
    ...
    TR_MAIN.Action   = "/dps/psal558.ps?goTo=del"   + parameters;
...
function btn_Save() {
    ...
    TR_MAIN.Action = "/dps/psal558.ps?goTo=save";
    TR_MAIN.KeyValue = "SERVLET(I:DS_IO_MASTER=DS_IO_MASTER)"; 
    TR_MAIN.Post();
...
function btn_Excel() {
	if(DS_IO_MASTER.CountRow	<= 0){
		...
		var	strTitle = "부자재 입금 내역";

		
		var	parameters = " - 점코드 " + strStrCd + "  - 조회기간 : " + strSDate + " ~ " + strEDate ;
 ...
 *************************************************************************/
 ...
 function chkValidation(gbn){
     ...
     case "search" :  
    	    ...
    	    //조회시작일자
    	    if (isNull(EM_SALE_S_DT.text) ==true ) {
    	        showMessage(INFORMATION, OK, "USER-1003","조회시작일자"); 
    	        return false;
    	    }
    	    //조회시작일자 자릿수, 공백 체크
    	    if (EM_SALE_S_DT.text.replace(" ","").length != 8 ) {
    	        showMessage(INFORMATION, OK, "USER-1027","조회시작일자","8");
    	        return false;
    	    }
    	    //년월일형식체크
    	    if (!checkYYYYMMDD(EM_SALE_S_DT.text) ) {
                showMessage(INFORMATION, OK, "USER-1069","조회시작일자");
                return false;
            }
    	    
    	    //영업조회월
    	    if (isNull(EM_SALE_E_DT.text) ==true				//null이거나,
    	     || EM_SALE_E_DT.text.replace(" ","").length != 8   // 자릿수가 틀리거나,
    	     || !checkYYYYMMDD(EM_SALE_E_DT.text) )				// 형식이 틀릴경우 금일자 자동입력.
    	     {
    	    	EM_SALE_E_DT.text = varToday;
            }    	    
    	    return true;
