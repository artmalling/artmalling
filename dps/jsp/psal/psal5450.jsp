...
function doInit(){
    ...
    registerUsingDataset("psal545","DS_O_MASTER" );
    
    EM_SALE_FLAG.Enable =	"false";
    
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"                width=60    align=center    </FC>'
                     + '<FC>id=STR_CD                  name="점코드"             width=60    align=center    show=false </FC>'
                     + '<FC>id=SALE_DT                 name="매출일자"           width=80    align=center     </FC>'
                     + '<FC>id=POS_NO                  name="POS번호"           width=60    align=center     suppress="2"    sumtext="합계"</FC>'
                     + '<FC>id=TRAN_NO                 name="거래번호"           width=60    align=center       </FC>'
                     + '<FC>id=TRAN_TYPE               name=""                  show=false  width=60    align=right       </FC>'
                     + '<FC>id=TRAN_MODE               name="거래모드"            width=80    align=left     </FC>'
                     ...
                     + '<FC>id=REDU_TOT_AMT            name="할인"               gte_columntype="number:0:true" gte_Summarytype="number:0:true"		width=80    align=right     mask="###,###"   sumtext={subsum(REDU_TOT_AMT)}</FC>'
                     + '<FC>id=SALE_AMT                name="매출"               gte_columntype="number:0:true" gte_Summarytype="number:0:true"		width=80    align=right     mask="###,###"   sumtext={subsum(SALE_AMT)}</FC>'
                     + '<FC>id=DC_TOT_AMT              name="에누리"             gte_columntype="number:0:true" gte_Summarytype="number:0:true"		width=80    align=right     mask="###,###"   sumtext={subsum(DC_TOT_AMT)}</FC>'
                     + '<FC>id=REAL_SALE_AMT           name="순매출"             gte_columntype="number:0:true" gte_Summarytype="number:0:true"		width=80    align=right     mask="###,###"   sumtext={subsum(REAL_SALE_AMT)}</FC>'
                     ...
                     + '<FC>id=GIFT_AMT                name="상품권"             gte_columntype="number:0:true" gte_Summarytype="number:0:true"		width=80    align=right     </FC>'//mask="###,###"   sumtext={subsum(GIFT_AMT)}
                     + '<FC>id=CUPON_AMT               name="쿠폰"               gte_columntype="number:0:true" gte_Summarytype="number:0:true"		width=80    align=right    </FC>'// mask="###,###"   sumtext={subsum(CUPON_AMT)}
                     + '<FC>id=OKCASHBAG_AMT           name="OK캐쉬백"           gte_columntype="number:0:true" gte_Summarytype="number:0:true"		width=80    align=right    </FC>'// mask="###,###"   sumtext={subsum(OKCASHBAG_AMT)}
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal545.ps?goTo="+goTo+parameters;  
...
function btn_Excel() {
    ...
    var strSaleDtE      = EM_SALE_DT_E.text;         //시작일자
...
function btn_Print() {
   ...
   /*  strPayInfo = "월" + strPayCyc +"회, " +  LC_S_PAY_CNT.Text;
    ...
    window.open("/dps/psal545.ps?goTo=print"+parameters,"OZREPORT", 1000, 700);   
}
...
function chkValidation(gbn){
    ...
    case "search" :
    	...
    	if (isNull(EM_CARD_NO.text)==true ) {
...
function searchPosNo(strPosNo){
    ...
    TR_DETAIL.Action="/dps/psal545.ps?goTo="+goTo+parameters;  
         ...
         <tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
	               ...
	               <td colspan="3" >
	                  <comment id="_NSID_">
	                    <object id="EM_SALE_FLAG" classid=<%=Util.CLSID_RADIO%> tabindex=1 style="height:20; width:550" >
	                    <param name=Cols    value="7">
	                    <param name=Format  value="%^전체 ,0^현금 ,1^카드 ,2^상품권,3^쿠폰,4^타사상품권,5^OK캐쉬백 ">    
	                    <param name=CodeValue  value="1">
	                    </object>  	
	                    </comment><script> _ws_(_NSID_);</script> 
	                    </td>
	                    
						<th width="70" class="point"><b>카드번호</b></th>
