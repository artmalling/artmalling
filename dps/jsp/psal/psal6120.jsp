...
function doInit(){
    ...
    initEmEdit(EM_GIFT_NO,      "GEN",      NORMAL);	// POS 번호
    ...
    registerUsingDataset("psal612","DS_MASTER" );
...
function gridCreate(){
	...
	var hdrProperies = '<FC>id={currow}           name="NO"        width=40      align=center            </FC>'
	                 ...
	                 + '<FC> id=POS_NM            name="POS명"     width=120     align=left   show=true   </FC>'
	                 + '<FC> id=GIFTCARD_NO       name="상품권번호"    width=120     align=left   show=true   </FC>'
	                 ;
...
function btn_Search() {
    ...
    var parameters   = "&strStrCd="  + encodeURIComponent(strCd)
				     ...
				     ;
    TR_SEARCH_MASTER.Action   ="/dps/psal612.ps?goTo="+goTo+parameters;  
...
function fnChkProcess(strCd, strDate) {
	...
	var parameters =  "&strStrCd="		+encodeURIComponent(strCd)
					...
					;
	var action 	   = "O";
	TR_MAIN.Action  = "/dps/psal612.ps?goTo="+goTo+parameters;  
...
function fnRunProcess(strCd, strDate, strGb) {
	...
	var parameters =  "&strStrCd="		+encodeURIComponent(strCd)
					...
					;
	var action 	   = "O";
	TR_SAVE.Action  = "/dps/psal612.ps?goTo="+goTo+parameters;  
...
// 	alert("arrArg = " + arrArg);
	...
	var returnVal = window.showModalDialog("/dps/jsp/psal/psal6121.jsp?"
			                              , arrArg
					...
					<td>
						<table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
							<tr>
								...
								<th width="70">상품권번호</th>
								<td>
									<comment id="_NSID_">
										<object id=EM_GIFT_NO classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1 align="absmiddle">
										</object>
									</comment><script> _ws_(_NSID_);</script></td>
								</td>
								<th  width="60" style="background-color:#F6CECE;color:#FE2E2E">자료이관 처리일자</th>
								<td>
									<comment id="_NSID_">
									...
									　　<img src="/<%=dir%>/imgs/btn/process.gif" onclick="javascript:fnChkProcess(LC_STR_CD.BindColVal,EM_PROC_DT.TEXT);" align="absmiddle" />
								</td>
