...
function doInit(){
	...
	//단위
	initComboStyle(LC_UNIT, DS_UNIT, "CODE^0^30,NAME^0^80", 1, NORMAL); 
...
//     var strCmprdt = searchCmpr();
    ...
    registerUsingDataset("psal302","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"           width=30     align=center    </FC>'
    				 + '<FC>id=STR_CD                 name="점코드"        width=100    align=center    suppress=1 show=false</FC>'
		 			 + '<FC>id=STR_NAME               name="점명"          width=80    align=center    suppress=1</FC>'
                     + '<FC>id=ORG_CD                  name="조직코드"      width=80   align=center    suppress=2 show=false</FC>'
                     + '<FC>id=ORG_NAME                name="조직명"        width=80    align=left      suppress=2   sumtext="합계"</FC>'
                     + '<G>                            name="매출일"'
                     + '<C>id=TARGET_SALE_AMT          name="매출목표"      width=100    align=right    mask="###,###"     value= {TARGET_SALE_AMT} sumtext={subsum(TARGET_SALE_AMT)}</C>'
                     + '<C>id=TOT_SALE_AMT             name="매출실적"      width=100    align=right    mask="###,###"     value= {TOT_SALE_AMT} sumtext={subsum(TOT_SALE_AMT)}</C>'
                     + '<C>id=ACHIEVERATE              name="매출;달성율"   width=60    align=right    mask="#,###.00"    </C>'
                     + '<C>id=TOT_SALE_AMT_CMPR        name="대비매출"      width=100    align=right    mask="###,###"     value= {TOT_SALE_AMT_CMPR} sumtext={subsum(TOT_SALE_AMT_CMPR)}</C>'
                     + '<C>id=SALEIRATE                name="매출;신장율"    width=60   align=right    mask="#,###.00"    </C>'
                     + '</G>'
                     + '<G>                            name="당월누계"'
                     + '<C>id=TARGET_SALE_AMT_M        name="매출목표"      width=100    align=right    mask="###,###"     value= {TARGET_SALE_AMT_M} sumtext={subsum(TARGET_SALE_AMT_M)}</C>'
                     + '<C>id=TOT_SALE_AMT_M           name="매출실적"      width=100    align=right    mask="###,###"     value= {TOT_SALE_AMT_M} sumtext={subsum(TOT_SALE_AMT_M)}</C>'
                     + '<C>id=ACHIEVERATE_M            name="매출;달성율"   width=60    align=right    mask="#,###.00"    </C>'
                     + '<C>id=TOT_SALE_AMT_M_CMPR      name="대비매출"      width=100    align=right    mask="###,###"     value= {TOT_SALE_AMT_M_CMPR} sumtext={subsum(TOT_SALE_AMT_M_CMPR)}</C>'
                     + '<C>id=SALEIRATE_M              name="매출;신장율"   width=60    align=right    mask="#,###.00"    </C>'
                     + '</G>'
                     + '<G>                            name="당년누계"'
                     + '<C>id=TARGET_SALE_AMT_Y        name="매출목표"      width=100    align=right    mask="###,###"     value= {TARGET_SALE_AMT_Y} sumtext={subsum(TARGET_SALE_AMT_Y)}</C>'
                     + '<C>id=TOT_SALE_AMT_Y           name="매출실적"      width=100    align=right    mask="###,###"     value= {TOT_SALE_AMT_Y} sumtext={subsum(TOT_SALE_AMT_Y)}</C>'
                     + '<C>id=ACHIEVERATE_Y            name="매출;달성율"   width=60    align=right    mask="#,###.00"    </C>'
                     + '<C>id=TOT_SALE_AMT_Y_CMPR      name="대비매출"      width=100    align=right    mask="###,###"     value= {TOT_SALE_AMT_Y_CMPR} sumtext={subsum(TOT_SALE_AMT_Y_CMPR)}</C>'
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal302.ps?goTo="+goTo+parameters;  
...
function changeUnit(){
    /* 단위구분  01:원 , 02:천원  */
    if(LC_UNIT.BindColVal == "01"){
    	// 단위가 원일때
    	GD_MASTER.ColumnProp("TARGET_SALE_AMT",     "value")   = "{TARGET_SALE_AMT}"
    	GD_MASTER.ColumnProp("TOT_SALE_AMT",        "value")   = "{TOT_SALE_AMT}"
    	GD_MASTER.ColumnProp("TOT_SALE_AMT_CMPR",   "value")   = "{TOT_SALE_AMT_CMPR}"
    	GD_MASTER.ColumnProp("TARGET_SALE_AMT_M",   "value")   = "{TARGET_SALE_AMT_M}"
    	GD_MASTER.ColumnProp("TOT_SALE_AMT_M",      "value")   = "{TOT_SALE_AMT_M}"
    	GD_MASTER.ColumnProp("TOT_SALE_AMT_M_CMPR", "value")   = "{TOT_SALE_AMT_M_CMPR}"
    	GD_MASTER.ColumnProp("TARGET_SALE_AMT_Y",   "value")   = "{TARGET_SALE_AMT_Y}"
    	GD_MASTER.ColumnProp("TOT_SALE_AMT_Y",      "value")   = "{TOT_SALE_AMT_Y}"
    	GD_MASTER.ColumnProp("TOT_SALE_AMT_Y_CMPR", "value")   = "{TOT_SALE_AMT_Y_CMPR}"
    	// 소계
        GD_MASTER.ColumnProp("TARGET_SALE_AMT",     "sumtext") = "{subsum(TARGET_SALE_AMT)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT",        "sumtext") = "{subsum(TOT_SALE_AMT)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_CMPR",   "sumtext") = "{subsum(TOT_SALE_AMT_CMPR)}"
        GD_MASTER.ColumnProp("TARGET_SALE_AMT_M",   "sumtext") = "{subsum(TARGET_SALE_AMT_M)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_M",      "sumtext") = "{subsum(TOT_SALE_AMT_M)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_M_CMPR", "sumtext") = "{subsum(TOT_SALE_AMT_M_CMPR)}"
        GD_MASTER.ColumnProp("TARGET_SALE_AMT_Y",   "sumtext") = "{subsum(TARGET_SALE_AMT_Y)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_Y",      "sumtext") = "{subsum(TOT_SALE_AMT_Y)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_Y_CMPR", "sumtext") = "{subsum(TOT_SALE_AMT_Y_CMPR)}"
    	
    }else{
    	// 단위가 천원일때
    	GD_MASTER.ColumnProp("TARGET_SALE_AMT",     "value")   = "{round(TARGET_SALE_AMT/1000)    }"
        GD_MASTER.ColumnProp("TOT_SALE_AMT",        "value")   = "{round(TOT_SALE_AMT/1000)       }"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_CMPR",   "value")   = "{round(TOT_SALE_AMT_CMPR/1000)  }"
        GD_MASTER.ColumnProp("TARGET_SALE_AMT_M",   "value")   = "{round(TARGET_SALE_AMT_M/1000)  }"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_M",      "value")   = "{round(TOT_SALE_AMT_M/1000)     }"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_M_CMPR", "value")   = "{round(TOT_SALE_AMT_M_CMPR/1000)}"
        GD_MASTER.ColumnProp("TARGET_SALE_AMT_Y",   "value")   = "{round(TARGET_SALE_AMT_Y/1000)  }"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_Y",      "value")   = "{round(TOT_SALE_AMT_Y/1000)     }"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_Y_CMPR", "value")   = "{round(TOT_SALE_AMT_Y_CMPR/1000)}"
        
        // 소계
        GD_MASTER.ColumnProp("TARGET_SALE_AMT",     "sumtext") = "{round(subsum(TARGET_SALE_AMT)/1000)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT",        "sumtext") = "{round(subsum(TOT_SALE_AMT)/1000)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_CMPR",   "sumtext") = "{round(subsum(TOT_SALE_AMT_CMPR)/1000)}"
        GD_MASTER.ColumnProp("TARGET_SALE_AMT_M",   "sumtext") = "{round(subsum(TARGET_SALE_AMT_M)/1000)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_M",      "sumtext") = "{round(subsum(TOT_SALE_AMT_M)/1000)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_M_CMPR", "sumtext") = "{round(subsum(TOT_SALE_AMT_M_CMPR)/1000)}"
        GD_MASTER.ColumnProp("TARGET_SALE_AMT_Y",   "sumtext") = "{round(subsum(TARGET_SALE_AMT_Y)/1000)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_Y",      "sumtext") = "{round(subsum(TOT_SALE_AMT_Y)/1000)}"
        GD_MASTER.ColumnProp("TOT_SALE_AMT_Y_CMPR", "sumtext") = "{round(subsum(TOT_SALE_AMT_Y_CMPR)/1000)}"
    }
}
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
				  ...
				  <th width = "70"> 단위</th>
				  	<td colspan= "3">
						<comment id="_NSID_"> 
							<object	id=LC_UNIT classid=<%=Util.CLSID_LUXECOMBO%> width=95	tabindex=1 align="absmiddle"> </object> 
						</comment>
						<script>_ws_(_NSID_);</script>
					</td>
               </tr>
