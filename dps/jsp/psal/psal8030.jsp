 ...
 */
 ...
 function doInit(){
     ...
     EM_YYYYMMDD.Focus(); 
     EM_YYYYMMDD.Text = EM_SALE_S_DT.Text;
 }
 ...
 *************************************************************************/
 ...
 function checkValidation(Gubun) {
  ...
  function CloseCheck(){
     ...
     TR_VALCHECK.Action="/dps/psal803.ps?goTo=valCheck"; 
 ...
 function process(){
	  ...
	  //변경또는 신규 내용을 저장하시겠습니까?
        if( showMessage(QUESTION, YESNO, "USER-1204") == 1 ){    // validation 체크
            TR_MAIN.Action="/dps/psal803.ps?goTo=save&strFlag=save"; 
...
<script language=JavaScript for=DS_IO_SEND_FLAG event=OnFilter(row)>
    if (DS_IO_SEND_FLAG.NameValue(row,"REFER_CODE") == "5"){
    	return true;
    }
	else 
		return false;
