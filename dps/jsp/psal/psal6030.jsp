<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 매출관리 > POS정산 > 사은품수령후매출취소현황
 * 작 성 일 : 2010.04.19
 * 작 성 자 : 이정식
 * 수 정 자 : 
 * 파 일 명 : psal6030.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : 사은품수령후매출취소현황을 조회한다
 * 이    력 :
 *        2010.04.19 (이정식) 신규작성
 ******************************************************************************/
-->
 ...
 * 작 성 자 : 이정식
 * 작 성 일 : 2010-04-19
 * 개    요 : 사은품수령후매출취소현황 그리드 초기화 
 * return값 : void
*/
function gridCreate(){
    var hdrProperies = '<FC>id={currow}    width=30  align=center name="NO"</FC>'
    	             + '<FC>id=SALE_DT     width=75  align=center name="취소일자" Mask="XXXX/XX/XX"</FC>'
                     + '<FC>id=POS_NO      width=65  align=center name="POS번호"</FC>'
                     + '<FC>id=TRAN_NO     width=65  align=center name="거래번호"</FC>'
                     + '<FC>id=PUMBUN_CD   width=65  align=center name="브랜드코드"</FC>'
                     + '<FC>id=PUMBUN_NM   width=90  align=left   name="브랜드명"</FC>'
                     + '<C>id=POSSALE_AMT  width=80  align=right  name="증정매출금액"</C>'
                     + '<C>id=SAL_CAN_AMT  width=80  align=right  name="매출취소금액"</C>'
                     + '<C>id=EST_RTN_AMT  width=80  align=right  name="예상회수금액"</C>'
                     + '<C>id=PAY_AMT      width=80  align=right  name="실회수금액"</C>'
                     + '<C>id=RTN_AMT      width=80  align=right  name="미회수금액"</C>'
                     + '<C>id=BUY_COST_PRC width=80  align=right  name="사은품단가"</C>'
                     + '<C>id=SKU_NAME     width=100 align=left   name="사은품명"</C>'
                     ;
                     
    initGridStyle(GD_GIFT_CANCEL, "common", hdrProperies, false);
}
/*************************************************************************
  * 2. 공통버튼
...
function btn_Search() {
    ...
    //2. 데이터셋 초기화
    DS_GIFT_CANCEL.ClearData();
    
    //3. 조회시작
    var goTo       = "searchGiftCancel";
    ...
    var parameters = "&strStoreCd=" + LC_STR_CD.BindColVal //점구분코드
                   ...
                   ;
    DS_GIFT_CANCEL.DataID = "/dps/psal603.ps?goTo="+goTo+parameters;
    DS_GIFT_CANCEL.Reset();
}
...
<!--------------------- 3. DataSet Success 메세지 처리  ----------------------->
<script language=JavaScript for=DS_GIFT_CANCEL event=OnLoadCompleted(rowcnt)>
    if(!isNull(this.ErrorMsg)) 
        showMessage(INFORMATION, OK, "USER-1000", this.ErrorMsg);
    
    setPorcCount("SELECT", rowcnt);
    GD_GIFT_CANCEL.Focus();
</script>
<!--------------------- 4. DataSet Fail 메세지 처리  -------------------------->
<script language=JavaScript for=DS_GIFT_CANCEL  event=OnLoadError()>
    dsFailed(this);
</script>
<!--------------------- 5. DataSet 이벤트 처리  ------------------------------->
<!--------------------- 6. 컴포넌트 이벤트 처리  ------------------------------->
<!-- Grid sorting event 처리 -->
<script language=JavaScript for=GD_GIFT_CANCEL event=OnClick(row,colid)>
...
<!-- ====================Output용================== -->
    ...
    <!--[그리드]사은품수령후매출취소현황 -->
    <object id="DS_GIFT_CANCEL"   classid=<%= Util.CLSID_DATASET %>></object>
<!-- ====================Input용================== -->
      ...
      <tr valign="top">
        <td>
          <table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
            <tr>
              <td>
                <comment id="_NSID_"><object id=GD_GIFT_CANCEL width="100%" height=506 classid=<%=Util.CLSID_GRID%>>
                  <param name="DataID" value="DS_GIFT_CANCEL">
