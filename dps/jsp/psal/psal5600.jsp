...
function doInit(){
   ...
   //콤보 초기화
    ...
    initComboStyle(PROC_FLAG,DS_PROC_FLAG, "CODE^0^30,NAME^0^140", 1, PK);
    
    getEtcCode("DS_ERR_FLAG" ,"D"   ,"P617"  ,"Y" );
    getEtcCode("DS_PROC_FLAG" ,"D"   ,"P628"  ,"Y" );
	...
	initEmEdit(EM_SALE_DT_S,                      "YYYYMMDD",                READ);   //시작일자
   ...
   //현재년도 셋팅
    ...
    registerUsingDataset("psal560","DS_MASTER" );
...
function gridCreate1(){
    /*var hdrProperies = '<FC>id={currow}            width=30  name="NO"     Edit=none              align=center</FC>' 
                     + '<FC>id=SEL                 width=50  align=center          sumtext="합계"    name="확인" EditStyle=CheckBox HeadCheckShow=true Edit={IF(PROC_FLAG="N","true","false")} </FC>'// Pointer=Hand
                     + '<FC>id=N_NO             width=90  align=right   Edit=none   name="NAVER_NO" align="center"  Edit={IF(PROC_FLAG="N","true","false")} </FC>'
                     + '<FC>id=PUMBUN          width=150 align=left    Edit=none   name="브랜드번호" </FC>'
                     + '<FC>id=ORDER_NO            width=90  align=left    Edit=none   name="주문번호" </FC>'
                     + '<FC>id=GOOD_ORDER_NO            width=90  align=left    Edit=none   name="상품주문번호" </FC>'
                     + '<FC>id=GUBUN            width=90  align=left    Edit=none   name="구분" </FC>'
                     + '<FC>id=GOOD_NM            width=90  align=left    Edit=none   name="상품명" </FC>'
                     + '<FC>id=RECIEVER            width=90  align=center    Edit=none   name="구매자명" </FC>'
                     + '<FC>id=CAL_DT1            width=90  align=center    Edit=none   name="정산예정일" </FC>'
                     + '<FC>id=CAL_DT2            width=90  align=center    Edit=none   name="정산완료일" </FC>'
                     + '<FC>id=CAL_DT3            width=90  align=center    Edit=none   name="정산기준일" </FC>'
                     + '<FC>id=CAL_GUBUN            width=90  align=left    Edit=none   name="정산구분" </FC>'
                     + '<FC>id=PRICE                 width=90  align=right   Edit=none   name="가격" sumtext={sum(PRICE)} </FC>'
                     + '<FC>id=CHARGE                 width=90  align=right   Edit=none   name="결재수수료" sumtext={sum(CHARGE)} </FC>'
                     + '<FC>id=BENE_PRICE                 width=90  align=right   Edit=none   name="혜택금액" sumtext={sum(BENE_PRICE)} </FC>'
                     + '<FC>id=CAL_PRICE                 width=90  align=right   Edit=none   name="정산예정금액" sumtext={sum(CAL_PRICE)} </FC>'
                     ...
                     + '<FC>id=PROC_FLAG           width=90  align=left    Edit=none   name="확정여부" EditStyle="Combo" Data="Y:확정,N:미확정" </FC>'
       ...
       var hdrProperies = '<FC>id={currow}            width=30  name="NO"     Edit=none              align=center</FC>' 
                         + '<FC>id=SEL                 width=50  align=center          sumtext="합계"    name="확인" EditStyle=CheckBox HeadCheckShow=true Edit={IF(PROC_FLAG="N","true","false")} </FC>'// Pointer=Hand
                         + '<FC>id=CAL_DT1             width=90  align=right   Edit=none   name="등록일" align="center"  Edit={IF(PROC_FLAG="N","true","false")} </FC>'
                         + '<FC>id=PUMBUN          width=150 align=left    Edit=none   name="브랜드번호" </FC>'
                         + '<FC>id=PUMBUN_NM          width=150 align=left    Edit=none   name="브랜드명" </FC>'
                         + '<FC>id=GOOD_ORDER_NO            width=90  align=left    Edit=none   name="상품주문번호" </FC>'
                         + '<FC>id=ORDER_NO            width=90  align=left    Edit=none   name="주문번호" </FC>'
                         + '<FC>id=GOOD_NM            width=90  align=left    Edit=none   name="상품명" </FC>'
                         + '<FC>id=PRICE                 width=90  align=right   Edit=none   name="가격" sumtext={sum(PRICE)} </FC>'
                         + '<FC>id=CAL_DT2            width=90  align=center    Edit=none   name="발송처리일" </FC>'
                         + '<FC>id=RECIEVER            width=90  align=center    Edit=none   name="구매자명" </FC>'
                         + '<FC>id=CAL_GUBUN            width=90  align=left    Edit=none   name="구매자ID" </FC>'
                         + '<FC>id=CAL_DT3            width=90  align=center    Edit=none   name="수취인명" </FC>'
                         + '<FC>id=ERR_YN              width=50  align=center  Edit=none   name="오류여부"  </FC>'//value={IF(ERR_CD="","N","Y")}
                         + '<FC>id=ERR_MSG             width=300 align=left    Edit=none   name="오류내용"  </FC>'//EditStyle="Combo" Data="'+errorMsgData+'"
                         + '<FC>id=PROC_FLAG           width=90  align=left    Edit=none   name="확정여부" EditStyle="Combo" Data="Y:확정,N:미확정" </FC>'
                         + '<FC>id=SEQ_NO              width=90  align=left    Edit=none   name="데이터순번" show=false </FC>'
...
function btn_Search() {
    ...
    var strProcYN        = PROC_FLAG.BindColval;
    ...
    var parameters = "&strStrCd="           +encodeURIComponent(strStrCd)
                   ...
                   + "&strProcYN="           +encodeURIComponent(strProcYN)
                   ;
    
    TR_MAIN.Action="/dps/psal560.ps?goTo="+goTo+parameters;  
...
function btn_Delete() {
   /*   var dataSetId = "";
    ...
    DS_MASTER.ImportData( dataSet.ExportData(1, dataSet.CountRow, true));
    ...
    GD_MASTER.ColumnProp('SEL','HeadCheck')= "FALSE";
     ...
     var goTo       = "deleteData" ;    
    var parameters = "strCheckType="+encodeURIComponent(strCheckType)
    			   + "&strStrCd="+encodeURIComponent(strStrCd)
    			   + "&strStrDt="+encodeURIComponent(strStrDt)
    				;
    TR_MAIN.Action="/dps/psal527.ps?goTo="+goTo+parameters;  
    TR_MAIN.KeyValue="SERVLET(I:DS_MASTER=DS_MASTER,O:DS_RESULT="+dataSetId+")"; //조회는 O
    TR_MAIN.Post();
...
function btn_Save() {
    ...
    if(strStrDt<varToday){//DS_MASTER.NameValue(0,"RELEASE_DT")!
   ...
   //var intRowCount   = DS_MASTER.CountRow;      
    ...
    /* for(var i=0; i<DS_MASTER.CountRow; i++){
    	...
    	if(DS_MASTER.NameValue(i,"SAVECHECK")!="N"){
...
function delRow(){
   ...
   if(showMessage(Question, YESNO, "USER-1023") != 1)
       return;
   for(var j=1; j<DS_MASTER.CountRow+1; j++){
	   
	   if(DS_MASTER.NameValue(j, "SEL")=="T"&&DS_MASTER.NameValue(j, "PROC_FLAG")=="Y"){
		   showMessage(INFORMATION, OK, "USER-1226"); 
		   return false;
   		}
   }
...
function excelDown(){
	...
	var sampleUrl = "/dps/samplefiles/ONLINE_DAYSALE(Sample).xls";
...
function errorchk(){
	 ...
	 var i = 0;
	   ...
	   for(i=1; i <= DS_MASTER.CountRow; i++){
			var strStrCd           = LC_STR_CD.BindColVal;
			var strStrDt           = EM_SALE_DT_S.text;
		    var strOrdererNm       = DS_MASTER.NameValue(i,"PUMBUN");            
		    var strOrderNo         = DS_MASTER.NameValue(i,"ORDER_NO");
		    var strPumbunNm        = DS_MASTER.NameValue(i,"PUMBUN")));
		    var strReleaseDt       = strStrDt;//DS_MASTER.NameValue(i,"RELEASE_DT");                
		    var strOrderSeq        = DS_MASTER.NameValue(i,"PUMBUN");
		    var strPumbunCd        = DS_MASTER.NameValue(i,"PUMBUN");
		    ...
		    var parameters  =						 
						      ...
						      + "&strReleaseDt="           +encodeURIComponent(strReleaseDt)
						      + "&strOrderSeq="            +encodeURIComponent(strOrderSeq) 
						      + "&strPumbunNm="            +encodeURIComponent(strPumbunNm)
						      + "&strStrDt="               +encodeURIComponent(strStrDt)
						      + "&strPumbunCd="            +encodeURIComponent(strPumbunNm)
					          + "&strStrCd="               +encodeURIComponent(strStrCd)
					          ; 
	            ...
	            DS_MASTER.NameValue(i, "PROC_FLAG") = "N"//확정 기본값으로 N을 준다.
	            ...
	            /* if(DS_MASTER.NameValue(i,"ORDER_SEQ")==""){
	   	       ...
	   	       if(DS_MASTER.NameValue(i,"PRICE")==""||DS_MASTER.NameValue(i,"PRICE")=="0"||DS_MASTER.NameValue(i,"PRICE")==0){
	   	    		DS_MASTER.NameValue(i,"ERR_YN")="Y";
	          ...
	          if( DS_CHKORGMST.NameValue(0,"ERR_CD")    == "001") {
	        	  DS_MASTER.NameValue(i,"ERR_YN") = "Y";
	        	  DS_MASTER.NameValue(i,"ERR_MSG")= "등로된 브랜드가 아닙니다.";
	        	  continue;
	          }
	          if(DS_CHKORGMST.NameValue(0,"ERR_CD")     == "002") {
	        	  
		          	DS_MASTER.NameValue(i,"ERR_YN") = "Y";
		          	DS_MASTER.NameValue(i,"ERR_MSG")= "네이버 브랜드명이 아닙니다.";
		          	continue;
		      }
	          ...
	          if( DS_CHKORGMST.NameValue(0,"ERR_CD")     == "004") {
		        	 
		          	DS_MASTER.NameValue(i,"ERR_YN") = "Y";
		          	DS_MASTER.NameValue(i,"ERR_MSG")= "주문번호에 이상이 있습니다.";
		          	continue;
		      } 
...
function process(){
	 var strStrDt=EM_SALE_DT_S.text;
	   ...
	   if( showMessage(STOPSIGN, YESNO, "USER-1205") == 1 ){
...
function CloseCheck(){
    ...
    /*  DS_I_SEARCH_TMP.ClearData();
     ...
     TR_VALCHECK.Action="/dps/psal560.ps?goTo=valCheck"+parameters; 
...
function headercheck(){
	 for(var i=1; i<DS_MASTER.CountRow+1; i++){
		if(DS_MASTER.NameValue(i,"PROC_FLAG")=="Y")DS_MASTER.NameValue(i,"SEL")="F";
	} 
}
-->
</script>
    ...
    for(i=0;i<TR_VALCHECK.SrvErrCount('UserMsg');i++) {
        if(TR_VALCHECK.SrvErrMsg('UserMsg',i) != "OK"){
            ...
            if( showMessage(QUESTION, YESNO, "USER-1204") == 1 ){    // validation 체크
            	TR_SAVE.Action="/dps/psal560.ps?goTo=save&strFlag=save"; 
...
<comment id="_NSID_">
<object id="DS_PROC_FLAG" classid=<%=Util.CLSID_DATASET%>>
</object>
</comment>
<script>_ws_(_NSID_);</script>
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
                  ...
                  <td width="150" >
                    <comment id="_NSID_">
                    <object id="PROC_FLAG" classid=<%=Util.CLSID_LUXECOMBO%> tabindex=1 style="height:20; width:150">
        ...
        <td valign="top">
          ...
          <div id=tab_page1 width="100%" >
            <table border="0" cellspacing="0" cellpadding="0" class="BD4A" width="100%">
              <tr>
                <td>
                  <comment id="_NSID_"><object id=GD_MASTER width="100%" height=445 classid=<%=Util.CLSID_GRID%> tabindex=1>
