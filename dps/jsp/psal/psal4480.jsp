...
function doInit(){
    ...
    initEmEdit(EM_SEARCH_CNT,         "NUMBER^9^0",    PK);         //조회건수
    ...
    */
    ...
    registerUsingDataset("psal448","DS_O_MASTER" );
    
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"        	    width=30     align=center    show=false </FC>'
                     + '<FC>id=NUM                     name="순위"          	width=40     align=center </FC>'
                     + '<FC>id=PUMBUN_CD               name="브랜드코드"        width=60     align=center    suppress=2 </FC>'
                     + '<FC>id=PUMBUN_NAME             name="브랜드명"          width=140    align=left      suppress=2       	sumtext="합계" </FC>'
                     + '<G>                            name="매출"  '
                     + '<C>id=SALE_QTY                 name="수량"  	        gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=70     align=right     mask="###,###"   	sumtext={subsum(SALE_QTY)} </C>'
                     + '<C>id=NORM_SALE_AMT            name="매출"           	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=110    align=right     mask="###,###"   	value = {round(NORM_SALE_AMT/UNIT)}		sumtext={round(subsum(NORM_SALE_AMT)/UNIT)} </C>'
                     + '</G>'
                     + '<FC>id=AVG_QTY                 name="일평균;수량"       gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=70     align=right     mask="#,###" </FC>'
                     + '<FC>id=AVG_AMT                 name="일평균매가"       	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=110    align=right     mask="###,###"  	value={round(AVG_AMT/UNIT)}</FC>'
                     + '<FC>id=AREA_SIZE               name="면적(㎡)"          gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=80     align=right </FC>'
                     + '<FC>id=AREA_AMT                name="㎡당매출"          gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=110    align=right 	 value={round(AREA_AMT/UNIT)} </FC>'
                     ;
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal448.ps?goTo="+goTo+parameters;  
...
function btn_Excel() {
    ...
    if(GD_MASTER.CountRow <= 0){
          showMessage(INFORMATION, OK, "USER-1000","다운 할 내용이 없습니다. 조회 후 엑셀다운 하십시오.");
            return;
        }
        var strTitle = "BEST/WORST브랜드별";
