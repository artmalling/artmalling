<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 신용카드대금청구 > 청구관리 > 카드매출 점검조회
 * 작  성  일  : 2010.05.31 
 * 작  성  자  : 
 * 수  정  자  : 
 * 파  일  명  : psal9180.jsp
...
function doInit(){
    ...
    initEmEdit(EM_S_DT,        "today",            NORMAL);           //조회from
    initEmEdit(EM_E_DT,        "today",            NORMAL);           //조회 to
    ...
    initEmEdit(EM_JBRCH_ID,     "GEN^30",              NORMAL);    //가맹점번호
    ...
    EM_S_DT.text   = addDate("D", "-1", EM_S_DT.text);
...
function gridCreate1(){
    var hdrProperies    = '<FC>id={currow}     name="NO"             width=30   align=center</FC>'
    					+ '<FC>id=GB           name="구분"         width=40   align=center </FC>'
 ...
 *************************************************************************/
 ...
 function saveData(){
    ...
    TR_MAIN.Action  ="/dps/psal918.ps?goTo="+goTo+parameters;   
...
function searchVanId(strStrCd){
	 ...
	 TR_VANID.Action  = "/dps/psal918.ps?goTo="+goTo+parameters;
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td colspan="5"><comment id="_NSID_"> <object
							...
							style="height: 40; width: 700" tabindex=1>
							<param name=Cols value="7">
							<param name=Format value="0^전체,A^A:전일취소,B^B:당일취소(원거래포함),C^C:수기입력,D^D:승인번호중복,
							                          E^E:발급사중복,F^F:매입사중복,G^G:카드빈없음,H^H:카드빈발급사차이,
							                          I^I:카드빈매입사차이,J^J:당일수기취소(원거래포함)맟원거래무,K^K:전일수기취소">
							<param name="AutoMargin" value="true">
							<param name=CodeValue value="J">
						</object> </comment><script> _ws_(_NSID_);</script></td>
					</tr>
				</table>
				</td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				<td><comment id="_NSID_"> <object id=GR_MASTER
					width="100%" height=280 classid=<%=Util.CLSID_GRID%> tabindex=1>
