<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 매출관리 > POS정산 > POS거래건수현황
 * 작 성 일 : 2010.07.07
 * 작 성 자 : 정진영
 * 수 정 자 : 
 * 파 일 명 : psal5160.jsp
...
function doInit(){
	//Master 그리드 세로크기자동조정  2013-07-17
	 var obj   = document.getElementById("DIV_POS_LIST"); 
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}     width=30  align=center name="NO"          </FC>'
                     + '<FC>id=STR_CD       width=40  align=center name="점" show=false SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}    </FC>'
                     + '<FC>id=STR_NM       width=80 align=center name="점" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}    </FC>'
                     + '<FC>id=FLOR_CD      width=30  align=center name="층" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}         </FC>'
                     + '<FC>id=POS_NO       width=50  align=center name="POS번호" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}         </FC>'
                     + '<FC>id=POS_FLAG     width=30  align=center name="POS구분" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}         </FC>'
                     + '<FC>id=POS_FLAG_NM  width=100 align=center name="POS구분" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=POSCNT		width=30  align=right  name="전체"  mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=BFCNT        width=40  align=right  name="개설전"	mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=USECNT      	width=110 align=right  name="사용(영업중+마감)" show=false	mask="###,###" sumtext=@sum  color=limegreen sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}      </FC>'
                     + '<FC>id=OPENCNT      width=40  align=right  name="영업중"	mask="###,###" color=limegreen  sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}    </FC>'
                     + '<FC>id=MAGAMCNT     width=30  align=right  name="마감"	mask="###,###" color=blue   sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}   </FC>'
                     + '<FC>id=MIN_TRAN_NO  width=60 align=center name="최초거래" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}    </FC>'
                     + '<FC>id=MAX_TRAN_NO  width=60 align=center name="최종거래" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}    </FC>'
                     + '<FC>id=TRAN_CNT		width=50  align=right  name="거래"  mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=PROC_CNT		width=50  align=right  name="처리"  mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=LOST_CNT		width=30  align=right  name="결락"  color=red mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=ERR_CNT		width=30  align=right  name="오류"  color=red mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>'
                     + '<FC>id=NPROC_CNT	width=30  align=right  name="대기"  color=green mask="###,###" sumtext=@sum SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD")}       </FC>';
    ...
    DS_DETAIL.SubSumExpr  = "2:STR_NM, 1:FLOR_CD" ; 
    GD_DETAIL.ColumnProp("STR_NM", "sumtext")        = "합계";
}
...
function btn_Search() {
    ...
    var parameters = "&strStrCd="	+encodeURIComponent(strStrCd)
                   ...
                   + "&strGB="  	+encodeURIComponent(RD_GB.CodeValue)
                   ;
    TR_MAIN.Action="/dps/psal516.ps?goTo="+goTo+parameters;  
...
function btn_Excel() {
	...
	//openExcel2(GD_DETAIL, "", "", true );
    openExcel5(GD_DETAIL, "", "", true , "",g_strPid );
...
function strPosAcount( strPosNo )
{
    ...
    var returnVal = window.showModalDialog("/dps/psal516.ps?goTo=strPosAcount",
                                           arrArg,
 ...
 */
 function searchDetail(){
	    ...
	    var parameters = "&strStrCd="	+encodeURIComponent(strStrCd)
	                   ...
	                   + "&strGB="  	+encodeURIComponent(RD_GB.CodeValue)
	                   ;
	    TR_DETAIL.Action="/dps/psal516.ps?goTo="+goTo+parameters;  
                ...
                <object id=EM_MAGAM classid=<%=Util.CLSID_EMEDIT%>  width=75  tabindex=1 align="absmiddle"></object>
					...
					<tr>
						...
						<td colspan="8"><comment id="_NSID_"> <object
							id=RD_GB classid=<%=Util.CLSID_RADIO%>
							style="height: 20; width: 670" tabindex=1>
							<param name=Cols value="7">
							<param name=Format value="A^A:전체,B^B:개설전,U^U:사용,O^O:영업중,C^C:마감,D^D:결락,E^E:오류">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  ...
                  <comment id="_NSID_"> <object
                     id=GD_DETAIL width=100% height=182 classid=<%=Util.CLSID_GRID%> >
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr valign="top">
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD2A">
          <tr>
            <td><div  style="height:250px;width:100%;overflow:auto;" id="DIV_POS_LIST" onresize="javascript:drawPosInfo();"></div>
            </td>
          </tr>
        </table></td>
      </tr>
