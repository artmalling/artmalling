...
function doInit(){
    ...
    registerUsingDataset("psal439","DS_O_MASTER" );
    
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}               name="NO"           width=30    align=center    </FC>'
                     + '<FC>id=SALE_DT                name="일자"          width=100    align=center   mask="XXXX/XX/XX"  sumtext="합계"  </FC>'
                     + '<G>                           name="유니폼"'
                     + '<C>id=UNIFORM_QTY             name="수량"          width=40    align=right     mask="###,###"     sumtext={subsum(UNIFORM_QTY)}</C>'
                     + '<C>id=UNIFORM_AMT             name="금액"          width=90    align=right    mask="###,###"     sumtext={subsum(UNIFORM_AMT)}</C>'
                     + '</G>'
                     + '<G>                           name="명찰"'
                     + '<C>id=NAMETAG_QTY             name="수량"          width=40    align=right     mask="###,###"     sumtext={subsum(NAMETAG_QTY)}</C>'
                     + '<C>id=NAMETAG_AMT             name="금액"          width=90    align=right    mask="###,###"     sumtext={subsum(NAMETAG_AMT)}</C>'
                     + '</G>'
                     + '<G>                           name="포장박스"'
                     + '<C>id=PAKBOX_QTY              name="수량"          width=40    align=right     mask="###,###"     sumtext={subsum(PAKBOX_QTY)}</C>'
                     + '<C>id=PAKBOX_AMT              name="금액"          width=90    align=right    mask="###,###"     sumtext={subsum(PAKBOX_AMT)}</C>'
                     + '</G>'
                     + '<G>                           name="주차권"'
                     + '<C>id=PARKED_QTY              name="수량"          width=40    align=right     mask="###,###"     sumtext={subsum(PARKED_QTY)}</C>'
                     + '<C>id=PARKED_AMT              name="금액"          width=90    align=right    mask="###,###"     sumtext={subsum(PARKED_AMT)}</C>'
                     + '</G>'
                     + '<G>                           name="합계"'
                     + '<C>id=TOTAL_QTY               name="수량"          width=40    align=right     mask="###,###"     sumtext={subsum(PARKED_QTY)}</C>'
                     + '<C>id=TOTAL_AMT               name="금액"          width=90    align=right    mask="###,###"     sumtext={subsum(PARKED_AMT)}</C>'
                     + '</G>'
                     ;
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal439.ps?goTo="+goTo+parameters;  
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=507 classid=<%=Util.CLSID_GRID%> tabindex=1>
