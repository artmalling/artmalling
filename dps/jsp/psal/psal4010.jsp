...
function doInit(){
    ...
    GD_DETAIL.GTitleHeight = 20;
    GD_DETAIL.TitleHeight  = 40;
    ...
    */
    ...
    registerUsingDataset("psal401","DS_O_MASTER" );
    
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"            width=30     align=center    </FC>'
                     + '<FC>id=ORG_CD                  name="조직코드"      width=90    align=center   suppress=1</FC>'
                     + '<FC>id=ORG_NAME                name="조직명"        width=80     align=left     suppress=1   	   sumtext="합계"</FC>'
                     + '<G>                            name="매출일"'
                     + '<C>id=ORIGIN_SALE_TAMT         name="매출목표"      width=100     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(ORIGIN_SALE_TAMT/UNIT)} 		sumtext={round(subsum(ORIGIN_SALE_TAMT)/UNIT)}</C>'
                     + '<C>id=TOT_SALE_AMT             name="총매출실적"    width=100     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(TOT_SALE_AMT/UNIT)} 			sumtext={round(subsum(TOT_SALE_AMT)/UNIT)}</C>'
                     + '<C>id=ACHIEVERATE              name="매출;달성율"   width=60     gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right    mask="#,###.00"    sumtext={decode(subsum(ORIGIN_SALE_TAMT),0,0.00,round((subsum(TOT_SALE_AMT)/subsum(ORIGIN_SALE_TAMT))*100,2))} </C>'
                     + '<C>id=SALE_AMT_CMPR            name="대비매출"      width=90     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(SALE_AMT_CMPR/UNIT)} 		    sumtext={round(subsum(SALE_AMT_CMPR)/UNIT)}</C>'
                     + '<C>id=SALEIRATE                name="매출;신장율"   width=60     gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right    mask="#,###.00"    sumtext={decode(subsum(SALE_AMT_CMPR),0,0.00,round(((subsum(TOT_SALE_AMT)-subsum(SALE_AMT_CMPR))/subsum(SALE_AMT_CMPR))*100,2))} </C>'
                     + '</G>'
                     + '<G>                            name="당월누계"'
                     + '<C>id=ORIGIN_SALE_TAMT_M       name="매출목표"      width=100     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(ORIGIN_SALE_TAMT_M/UNIT)} 		sumtext={round(subsum(ORIGIN_SALE_TAMT_M)/UNIT)}</C>'
                     + '<C>id=TOT_SALE_AMT_M           name="총매출실적"    width=100     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(TOT_SALE_AMT_M/UNIT)} 		    sumtext={round(subsum(TOT_SALE_AMT_M)/UNIT)}</C>'
                     + '<C>id=ACHIEVERATE_M            name="매출;달성율"   width=70     gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right    mask="#,###.00"    sumtext={decode(subsum(ORIGIN_SALE_TAMT_M),0,0.00,round((subsum(TOT_SALE_AMT_M)/subsum(ORIGIN_SALE_TAMT_M))*100,2))} </C>'
                     + '<C>id=SALE_AMT_CMPR_M          name="대비매출"      width=90     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(SALE_AMT_CMPR_M/UNIT)} 		sumtext={round(subsum(SALE_AMT_CMPR_M)/UNIT)}</C>'
                     + '<C>id=SALEIRATE_M              name="매출;신장율"   width=60     gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right    mask="#,###.00"    sumtext={decode(subsum(SALE_AMT_CMPR_M),0,0.00,round(((subsum(TOT_SALE_AMT_M)-subsum(SALE_AMT_CMPR_M))/subsum(SALE_AMT_CMPR_M))*100,2))} </C>'
                     + '</G>'
                     + '<G>                            name="당년누계"'
                     + '<C>id=ORIGIN_SALE_TAMT_Y       name="매출목표"      width=100     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(ORIGIN_SALE_TAMT_Y/UNIT)} 		sumtext={round(subsum(ORIGIN_SALE_TAMT_Y)/UNIT)}</C>'
                     + '<C>id=TOT_SALE_AMT_Y           name="총매출실적"    width=100     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(TOT_SALE_AMT_Y/UNIT)} 			sumtext={round(subsum(TOT_SALE_AMT_Y)/UNIT)}</C>'
                     + '<C>id=ACHIEVERATE_Y            name="매출;달성율"   width=70     gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right    mask="#,###.00"    sumtext={decode(subsum(ORIGIN_SALE_TAMT_Y),0,0.00,round((subsum(TOT_SALE_AMT_Y)/subsum(ORIGIN_SALE_TAMT_Y))*100,2))} </C>'
                     + '<C>id=SALE_AMT_CMPR_Y          name="대비매출"      width=100     gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right    mask="###,###"     value= {round(SALE_AMT_CMPR_Y/UNIT)} 		sumtext={round(subsum(SALE_AMT_CMPR_Y)/UNIT)}</C>'
                     + '<C>id=SALEIRATE_Y              name="매출;신장율"   width=60     gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right    mask="#,###.00"    sumtext={decode(subsum(SALE_AMT_CMPR_Y),0,0.00,round(((subsum(TOT_SALE_AMT_Y)-subsum(SALE_AMT_CMPR_Y))/subsum(SALE_AMT_CMPR_Y))*100,2))} </C>'
                     + '</G>'
                     + '<C>id=SALEDT                   name="매출일자"      width=100    align=center   Show="false"</C>'
                     + '<C>id=CMPRDT                   name="대비일자"      width=80     align=left     Show="false"</C>'
                     ;
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}                name="NO"            width=30    align=center    </FC>'
                     + '<FC>id=ORG_CD                  name="브랜드코드"      width=80    align=center    suppress=1</FC>'
                     + '<FC>id=ORG_NAME                name="브랜드명"        width=130   align=left      suppress=1   	   sumtext="합계"</FC>'
                     + '<G>                            name="매출일"'
                     + '<C>id=ORIGIN_SALE_TAMT         name="매출목표"      width=100    gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right     mask="###,###"     value= {round(ORIGIN_SALE_TAMT/UNIT)} 		sumtext={round(subsum(ORIGIN_SALE_TAMT)/UNIT)}</C>'
                     + '<C>id=TOT_SALE_AMT             name="총매출실적"    width=100    gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right     mask="###,###"     value= {round(TOT_SALE_AMT/UNIT)} 			sumtext={round(subsum(TOT_SALE_AMT)/UNIT)}</C>'
                     + '<C>id=ACHIEVERATE              name="매출;달성율"   width=70    gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right     mask="#,###.00"    sumtext={decode(subsum(ORIGIN_SALE_TAMT),0,0.00,round((subsum(TOT_SALE_AMT)/subsum(ORIGIN_SALE_TAMT))*100,2))} </C>'
                     + '<C>id=SALE_AMT_CMPR            name="대비매출"      width=100    gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right     mask="###,###"     value= {round(SALE_AMT_CMPR/UNIT)} 			sumtext={round(subsum(SALE_AMT_CMPR)/UNIT)}</C>'
                     + '<C>id=SALEIRATE                name="매출;신장율"   width=60    gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right     mask="#,###.00"    sumtext={decode(subsum(SALE_AMT_CMPR),0,0.00,round(((subsum(TOT_SALE_AMT)-subsum(SALE_AMT_CMPR))/subsum(SALE_AMT_CMPR))*100,2))} </C>'
                     + '</G>'
                     + '<G>                            name="당월누계"'
                     + '<C>id=ORIGIN_SALE_TAMT_M       name="매출목표"      width=100    gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right     mask="###,###"     value= {round(ORIGIN_SALE_TAMT_M/UNIT)} 		sumtext={round(subsum(ORIGIN_SALE_TAMT_M)/UNIT)}</C>'
                     + '<C>id=TOT_SALE_AMT_M           name="총매출실적"    width=100    gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right     mask="###,###"     value= {round(TOT_SALE_AMT_M/UNIT)} 			sumtext={round(subsum(TOT_SALE_AMT_M)/UNIT)}</C>'
                     + '<C>id=ACHIEVERATE_M            name="매출;달성율"   width=70    gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right     mask="#,###.00"    sumtext={decode(subsum(ORIGIN_SALE_TAMT_M),0,0.00,round((subsum(TOT_SALE_AMT_M)/subsum(ORIGIN_SALE_TAMT_M))*100,2))} </C>'
                     + '<C>id=SALE_AMT_CMPR_M          name="대비매출"      width=100    gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right     mask="###,###"     value= {round(SALE_AMT_CMPR_M/UNIT)} 		sumtext={round(subsum(SALE_AMT_CMPR_M)/UNIT)}</C>'
                     + '<C>id=SALEIRATE_M              name="매출;신장율"   width=60    gte_columntype="number:2:true" gte_Summarytype="number:2:true"		align=right     mask="#,###.00"    sumtext={decode(subsum(SALE_AMT_CMPR_M),0,0.00,round(((subsum(TOT_SALE_AMT_M)-subsum(SALE_AMT_CMPR_M))/subsum(SALE_AMT_CMPR_M))*100,2))} </C>'
                     + '</G>'
                     + '<G>                            name="당년누계"'
                     + '<C>id=ORIGIN_SALE_TAMT_Y       name="매출목표"      width=100    gte_columntype="number:0:true" gte_Summarytype="number:0:true"		align=right     mask="###,###"     value= {round(ORIGIN_SALE_TAMT_Y/UNIT)} 		sumtext={round(subsum(ORIGIN_SALE_TAMT_Y)/UNIT)}</C>'
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal401.ps?goTo="+goTo+parameters;  
 ...
 */
 function searchDetail(){
     ...
     var strSaleDtS      = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "SALEDT");         //매출일자
     var strSaleDtE      = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "CMPRDT");         //대비일자
     var strEmUnit 		 = EM_UNIT.BindColVal;		 //단위
     ...
     TR_DETAIL.Action="/dps/psal401.ps?goTo="+goTo+parameters;  
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=290 classid=<%=Util.CLSID_GRID%> tabindex=1>
