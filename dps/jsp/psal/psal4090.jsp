...
function doInit(){
    ...
    registerUsingDataset("psal409","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"           width=30    align=center    </FC>'
                     ...
                     + '<FG>                           name="매입사" '
                     + '<FC>id=ACQU_COMP_CD            name="코드"          width=30   align=center       suppress="1"</FC>'
                     + '<FC>id=ACQU_COMP               name="명"            width=100   align=left        sumtext="합계" suppress="1"</FC>'
                     + '</FG>' 
                     + '<FG>                           name="발급사" '
                     + '<FC>id=ACQU_COMP2              name="코드"          width=30   align=center       suppress="1"</FC>'
                     + '<FC>id=ACQU_COMP_CD2           name="명"            width=100   align=left         sumtext="합계" suppress="1"</FC>'
                     + '</FG>'
                     + '<FC>id=SALE_AMT                name="매출금액"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right     mask="###,###"    sumtext={subsum(SALE_AMT)}</FC>'
                     + '<FC>id=CNT                     name="매출건수"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=60     align=right    mask="###,###"    sumtext={subsum(CNT)}       </FC>'
                     + '<FC>id=NORM_SALE_AMT           name="정상금액"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right     mask="###,###"    sumtext={subsum(NORM_SALE_AMT)}</FC>'
                     + '<FC>id=NORM_CNT                name="정상매출건수"  gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right    mask="###,###"    sumtext={subsum(NORM_CNT)}       </FC>'
                     + '<FC>id=RET_SALE_AMT            name="반품액"        gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right     mask="###,###"    sumtext={subsum(RET_SALE_AMT)}</FC>'
                     + '<FC>id=RET_CNT                 name="반품건수"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right    mask="###,###"    sumtext={subsum(RET_CNT)}       </FC>'
    ...
    GD_MASTER.ColumnProp("ACQU_COMP", "SubSumText")        = "소계";  
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}                name="NO"           width=30    align=center    sumtext = "" subsumtext= ""</FC>'
                     ...
                     + '<FC>id=SALE_AMT                name="매출금액"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right    mask="###,###"    sumtext={subsum(SALE_AMT)} SubBgColor=#FFFFE0 </FC>'
                     + '<FC>id=CNT                     name="매출건수"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=60    align=right    mask="###,###"    sumtext={subsum(CNT)}      SubBgColor=#FFFFE0 </FC>'
                     + '<FC>id=NORM_SALE_AMT           name="정상금액"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right     mask="###,###"    sumtext={subsum(NORM_SALE_AMT)}</FC>'
                     + '<FC>id=NORM_CNT                name="정상매출건수"  gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=100    align=right    mask="###,###"    sumtext={subsum(NORM_CNT)}       </FC>'
                     + '<FC>id=RET_SALE_AMT            name="반품액"        gte_columntype="number:0:true" gte_Summarytype="number:0:true"  width=100    align=right     mask="###,###"    sumtext={subsum(RET_SALE_AMT)}</FC>'
                     + '<FC>id=RET_CNT                 name="반품건수"      width=80    align=right    mask="###,###"    sumtext={subsum(RET_CNT)}       </FC>'                     
                     ;
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal409.ps?goTo="+goTo+parameters;  
 ...
 *************************************************************************/
 ...
 function searchDetail() {
     ...
     var strCardsa       = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "ACQU_COMP2");         //카드사
     var strBcomp        = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition, "ACQU_COMP_CD");      //매입사
     ...
     TR_DETAIL.Action="/dps/psal409.ps?goTo="+goTo+parameters;  
...
function searchPosTran(strDt, strPosNo, strStrCd, strCardNo )
{
    ...
    var returnVal = window.showModalDialog("/dps/psal409.ps?goTo=searchTran",
                                           arrArg,
