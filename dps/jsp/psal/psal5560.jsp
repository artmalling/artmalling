<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 매출관리 > POS정산 > OK캐쉬백포인트사용(마케팅)
 * 작 성 일 : 2015.06.22
 * 작 성 자 : 강연식
 * 수 정 자 : 
 * 파 일 명 : psal5560.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : OK캐쉬백포인트사용현황 을 조회한다(마케팅 요청 화면)
 * 이    력 :
 *        2015.06.22 (강연식) 신규작성
 ******************************************************************************/
-->
...
function doInit(){
	...
	var obj   = document.getElementById("GD_OK_SAL_MST"); 
...
function gridCreate(){
    ...
    var hdrProperies = '<C>id={currow}    width=30  align=center name="NO"</C>'
                     + '<C>id=SALE_DT     width=140  align=center name="매출일자" mask="XXXX/XX/XX" SUMTEXT="합계"</C>'
                     + '<C>id=TOT_PLUS_AMT     width=140  align=right  name="정상매출" SUMTEXT=@SUM<</C>'
                     + '<C>id=TOT_MINUS_AMT     width=140  align=right  name="반품매출" SUMTEXT=@SUM<</C>'
                     + '<C>id=AMT     width=140  align=right  name="순매출" SUMTEXT=@SUM<</C>'
                     + '<C>id=OK_COUNT     width=100  align=right  name="건수" SUMTEXT=@SUM<</C>'
                     + '<C>id=OK_AMT     width=140  align=right  name="포인트" SUMTEXT=@SUM<</C>'
                     ;
...
function btn_Search() {
    ...
    var parameters = "&strStoreCd="      + LC_STR_CD.BindColVal              //점구분코드
                  ...
                  // + "&strPosNo="        + EM_POS_NO.text                    //POS번호
                   ;
    DS_OK_SAL_MST.DataID = "/dps/psal556.ps?goTo="+goTo+parameters;
    DS_OK_SAL_MST.Reset();
}
...
function subQuery(row){
    ...
    DS_OK_SAL_DTL.DataID = "/dps/psal556.ps?goTo="+goTo+parameters;
    DS_OK_SAL_DTL.Reset();
}
