<!-- 
/*******************************************************************************
 * 시스템명 : 영업관리 > 매출관리 > POS정산 > OK캐쉬백매출현황
 * 작 성 일 : 2010.04.08
 * 작 성 자 : 이정식
 * 수 정 자 : 
 * 파 일 명 : psal5500.jsp
 * 버    전 : 1.0 (버전은 1.0부터 시작하며 0.1씩 증가)
 * 개    요 : OK캐쉬백매출현황 을 조회한다
 * 이    력 :
 *        2010.04.08 (이정식) 신규작성
 ******************************************************************************/
-->
...
function gridCreate(){
    ...
    var hdrProperies = '<C>id={currow}    width=30  align=center name="NO"</C>'
                     ...
                     + '</G>'
                     + '<G>name="OK캐쉬백매출"'
                     + '<C>id=PAY_CNT     width=70  align=right  name="수량" SUMTEXT=@SUM</C>'
                     + '<C>id=PAY_AMT     width=90  align=right  name="금액" SUMTEXT=@SUM</C>'
                     + '<C>id=BRCH_FEE     width=90  align=right  name="가맹점수수료" SUMTEXT=@SUM</C>'
                     + '</G>'
                     + '<G>name="OK캐쉬백반품"'
                     + '<C>id=RTN_CNT     width=70  align=right  name="수량" SUMTEXT=@SUM</C>'
                     + '<C>id=RTN_AMT     width=90  align=right  name="금액" SUMTEXT=@SUM</C>'
...
function btn_Search() {
    ...
    var parameters = "&strStoreCd="      + LC_STR_CD.BindColVal              //점구분코드
                   ...
                   ;
    DS_OK_SAL_MST.DataID = "/dps/psal550.ps?goTo="+goTo+parameters;
    DS_OK_SAL_MST.Reset();
}
...
function subQuery(row){
    ...
    DS_OK_SAL_DTL.DataID = "/dps/psal550.ps?goTo="+goTo+parameters;
    DS_OK_SAL_DTL.Reset();
}
