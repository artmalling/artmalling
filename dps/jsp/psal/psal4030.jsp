...
function doInit(){
    ...
    registerUsingDataset("psal403","DS_O_MASTER" );
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}				name="NO"		width=30    align=center    SubSumText="" SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     ...
                     + '</FG>'
                     + '<G>						name="브랜드"'
                     + '<FC>id=PUMBUN_CD			name="코드"		width=50    align=center	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=PUMBUN_NAME			name="명"		width=80    align=left		SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '</G>'
                     + '<G>							name="행사"'
                     + '<C>id=EVENT_FLAG			name="구분"		width=40    align=center	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</C>'
                     + '<C>id=EVENT_RATE			name="율"		width=40    align=center	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</C>'
                     + '</G>'
                     + '<FC>id=MG_RATE				name="마진율" 	width=60    align=right		mask="#,###.00"	gte_columntype="number:2:true" gte_Summarytype="number:2:true"	SubSumText=""  SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=TOT_SALE_AMT			name="총매출"	width=90    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(TOT_SALE_AMT)/UNIT)} 	 	value={round(TOT_SALE_AMT/submax(UNIT))}	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=REDU_AMT				name="할인"		width=90    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={subsum(REDU_AMT)} 				   	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=SALE_AMT				name="매출"		width=90    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(SALE_AMT)/UNIT)} 	   	value={round(SALE_AMT/submax(UNIT))}		SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=DC_AMT				name="에누리"	width=90    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={subsum(DC_AMT)} 				   	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=NET_SALE_AMT			name="순매출"	width=90    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(NET_SALE_AMT)/UNIT)} 	value={round(NET_SALE_AMT/submax(UNIT))}	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=SALE_PROF_AMT		name="이익액"	width=90    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(SALE_PROF_AMT)/UNIT)} value={round(SALE_PROF_AMT/submax(UNIT))}		SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '<FC>id=PROFRATE				name="이익율"	width=60    align=right     mask="#,###.00"	gte_columntype="number:2:true" gte_Summarytype="number:2:true"	SubSumText={decode(subsum(SALE_AMT),0,0.00,round((subsum(SALE_PROF_AMT)/subsum(SALE_AMT)*100),2))} sumtext={decode(subsum(SALE_AMT),0,0.00,round((subsum(SALE_PROF_AMT)/subsum(SALE_AMT)*100),2))} SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
...
function btn_Search() {
    ...
    TR_MAIN.Action="/dps/psal403.ps?goTo="+goTo+parameters;  
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
                  ...
                  <td colspan="3"><comment id="_NSID_"> <object id=EM_PUMBUN_CD
                      ...
                      align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script>  &nbsp<input type="checkbox" id="chk_online">네이버샵
                  </td>
               </tr>
            </table>
            </td>
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=472 classid=<%=Util.CLSID_GRID%> tabindex=1>
