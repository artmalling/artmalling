...
function doInit(){
    ...
    initComboStyle(EM_UNIT, DS_UNIT, "CODE^0^30,NAME^0^80", 1, NORMAL);				//단위(01:원,02:천원)
    
    //구분 콤보 초기화
    initComboStyle(EM_PUMMOK_FLAG,DS_PUMMOK_FLAG, "CODE^0^30,NAME^0^80", 1, NORMAL);               
    //행사구분
    initComboStyle(LC_HS_MG, DS_HS_MG,   "CODE^0^30,NAME^0^100", 1, NORMAL); // [조회용]행사구분
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}             name="NO"		width=30    align=center    SubSumText="" 	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     ...
                     + '<FC>id=PC_NAME	        	name="명"		width=100   align=left		suppress=1		SubSumText={decode(curlevel,1,"브랜드별소계","")} SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1")}</FC>'
                     + '</FG>'
                     + '<FG>                        name="협력사"'
                     + '<FC>id=VEN_CD               name="코드"		width=60    align=center	suppress=1		SubSumText=""										SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FC>id=VEN_NAME             name="명"		width=130    align=left		suppress=1		SubSumText={decode(curlevel,1,"협력사소계","")} SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '</FG>'
                     + '<FG>                        name="대표브랜드"'
                     + '<FC>id=REP_PUMBUN_CD        name="코드"		width=60    align=center	SubSumText=""	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FC>id=REP_PUMBUN_NAME      name="명"		width=130    align=left		sumtext="합계"	SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '</FG>'                    
                     + '<FG>                        name="브랜드"   '  
                     ...
                     + '</FG>'
                     + '<FC>id=FLOOR_NM		        name="층"		width=60    align=center					SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FG>                        name="품목"     '
                     ...
                     + '<FC>id=TOT_SALE_AMT         name="총매출"	width=100    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(TOT_SALE_AMT)/UNIT)}		value={round(TOT_SALE_AMT/submax(UNIT))}		SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FC>id=REDU_AMT             name="할인"		width=100    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={subsum(REDU_AMT)}			SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FC>id=SALE_AMT             name="매출"		width=100    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(SALE_AMT)/UNIT)}			value={round(SALE_AMT/submax(UNIT))}			SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FC>id=DC_AMT               name="에누리"	width=100    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={subsum(DC_AMT)}			SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FC>id=NET_SALE_AMT         name="순매출"	width=100    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(NET_SALE_AMT)/UNIT)}		value={round(NET_SALE_AMT/submax(UNIT))}		SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
                     + '<FC>id=SALE_PROF_AMT        name="이익액"	width=100    align=right		mask="###,###"	gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext={round(subsum(SALE_PROF_AMT)/UNIT)}		value={round(SALE_PROF_AMT/submax(UNIT))}		SubBgColor={decode(curlevel,1,"#FFFFE0",2,"#FFFACD",3,"#FFF7B1",4,"#FFF29A",5,"#FFF29A")}</FC>'
...
function btn_Search() {
    ...
    if(LC_HS_MG.BindColVal != ""){
    	var strHsMg  = LC_HS_MG.BindColVal;          //행사구분
    	strEventFlag = strHsMg.substring(0,2);
    	strEventRate = strHsMg.substring(2,4);
    }
    ...
    TR_MAIN.Action="/dps/psal446.ps?goTo="+goTo+parameters;  
...
function btn_Print() {
	    ...
	    var strEmUnit		= EM_UNIT.BindColVal;				//단위
	    var strEmUnitNm     = EM_UNIT.text;
	    var strPumbunNm     = EM_PUMBUN_NAME.text;        //브랜드
	    
	    var strUnit = "1";		// '원'단위(01:원,02:천원)
	    if(strEmUnit=="02"){	
	    	strUnit = "1000";
	    }
	    ...
	    var parameters =  "&strStrCd="+strStrCd
				         ...
				         + "&strVenCd="+strVenCd
					 	 ...
					 	 + "&strUserId="+strUserId
					 	 + "&strEmUnit="+strUnit
					 	 + "&strEmUnitNm="+encodeURIComponent(strEmUnitNm)
	    ...
	    return;
	    */
	    window.open("/dps/psal446.ps?goTo=print"+parameters,"OZREPORT", screen.availWidth, screen.availHeight);
	    //window.open("/dps/psal446.ps?goTo=print"+parameters,"OZREPORT", 1024, 768);   
...
//         enableControl(LC_HS_MG, 		false);
    ...
    if( evnflag == "POP" ){
    	 strPbnPop(codeObj,nameObj,'Y','', strCd,'','','','0','','');
         codeObj.Focus();
         /* 행사구분 콤보 조회 */
         searchMgHs();
         enableControl(LC_HS_MG, 		true);
         LC_HS_MG.index = 0;
    }else if( evnflag == "NAME" ){
    	 setStrPbnNmWithoutPop("DS_SEARCH_NM",codeObj,nameObj,"Y",1,'', strCd,'','','','0','','');
   	     /* 행사구분 콤보 조회 */
   	     searchMgHs();
   	     enableControl(LC_HS_MG, 		true);
   	     LC_HS_MG.index = 0;
...
function searchMgHs() {
    ...
    var strSdate     = EM_SALE_DT_S.Text;
    var strEdate     = EM_SALE_DT_E.Text;
    ...
    TR_MG.Action="/dps/psal446.ps?goTo="+goTo+parameters;  
    TR_MG.KeyValue="SERVLET("+action+":DS_HS_MG=DS_HS_MG)"; //조회는 O
    TR_MG.Post();
...
<script language=JavaScript for=EM_PUMMOK_FLAG event=OnSelChange()>
if(EM_PUMMOK_FLAG.BindColVal == "1"){
	EM_PUMBUN_CD.Text = "";
	EM_PUMBUN_NAME.Text = "";
	LC_HS_MG.BindColVal = "%";
...
<script>_ws_(_NSID_);</script>
<comment id="_NSID_">
<object id="DS_HS_MG"         classid=<%=Util.CLSID_DATASET%>></object>
</comment>
<script>_ws_(_NSID_);</script>
...
<comment id="_NSID_">
<object id="TR_MG" classid=<%=Util.CLSID_TRANSACTION%>>
   <param name="KeyName" value="Toinb_dataid4">
</object>
</comment>
<script>_ws_(_NSID_);</script>
         ...
         <tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
                  ...
                  <th width="80">협력사</th>
                  <td width="200"><comment id="_NSID_"> <object id=EM_S_VEN_CD
                  ...
                  <th width="80">브랜드</th>
		            <td  width="250">
		              ...
		              <img src="/<%=dir%>/imgs/btn/detail_search_s.gif"  class="PR03" onclick="javascript:setPumbunCode('POP');"  align="absmiddle" id=IMG_PUMBUN />
		              <comment id="_NSID_">
		                <object id=EM_PUMBUN_NAME classid=<%=Util.CLSID_EMEDIT%> width=140 tabindex=1 align="absmiddle"></object>
		              </comment><script> _ws_(_NSID_);</script>
		            </td>
                  <th width="100">OP</th>
                  <td colspan="5" width="200" ><comment id="_NSID_">
                  <object
                     id="LC_HS_MG"  classid=<%=Util.CLSID_LUXECOMBO%> width=95 tabindex=1 
                     align="absmiddle" >
