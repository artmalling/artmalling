...
function doInit(){
    ...
    registerUsingDataset("psal585","DS_O_MASTER" );
    
    // 이후 내용 17.05.05 수정 K,J,Y
    
    document.getElementById("LC_POS_FLAG").style.display = "none"; // 필요 없어서 숨김.
    document.getElementById("EM_SALE_USER_ID").style.display = "none";
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"                width=60    align=center    suppress="2" BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'
                     + '<FC>id=STR_CD                  name="점코드"             width=60    align=center   suppress="2"  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'
                     + '<FC>id=SALE_DT                 name="매출일자"           width=80    align=right      suppress="2" BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'
                     + '<FC>id=POS_NO                  name="POS번호"           width=60    align=center     suppress="2"    sumtext="합계" BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'       
                     + '<FC>id=TRAN_NO                 name="거래번호"           width=60    align=center     suppress="2" BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}  Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'
                     + '<FC>id=PUMBUN_CD               name="브랜드코드"         width=100    align=center    suppress="2"  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}  Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'
                     + '<FC>id=PUMBUN_NAME             name="브랜드명"           width=200    align=center     suppress="2"  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}   Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'              
                     + '<FC>id=TRAN_FLAG               name="거래구분"            width=80    align=center   BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'
                     + '<FC>id=RCP_AMT                 name="영수증금액"         width=80    gte_columntype="number:0:true" sumtext=@sum gte_Summarytype="number:0:true"		align=right     mask="###,###"   sumtext={subsum(SALE_TOT_AMT)} BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'
                     + '<FC>id=ITEM_AMT                name="물품금액"               gte_columntype="number:0:true" sumtext=@sum gte_Summarytype="number:0:true"		width=80    align=right    BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'// mask="###,###"   sumtext={subsum(CASH_TOT_AMT)}
                     + '<FC>id=CASH_AMT                name="현금매출"               gte_columntype="number:0:true" sumtext=@sum gte_Summarytype="number:0:true"		width=80    align=right  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}   Color={decode(TRAN_FLAG,"반품","#FF0000")} </FC>'//mask="###,###"   sumtext={subsum(CARD_AMT)}
                     + '<FC>id=CARD_AMT                name="카드매출"              gte_columntype="number:0:true" sumtext=@sum gte_Summarytype="number:0:true"		width=80    align=right  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}   Color={decode(TRAN_FLAG,"반품","#FF0000")} </FC>'//mask="###,###"   sumtext={subsum(GIFT_AMT)}
                     + '<FC>id=IGIFT_AMT               name="자사상품권"               gte_columntype="number:0:true" sumtext=@sum gte_Summarytype="number:0:true"		width=80    align=right   BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")} </FC>'// mask="###,###"   sumtext={subsum(CUPON_AMT)}
                     + '<FC>id=OGIFT_AMT               name="타사상품권"               gte_columntype="number:0:true" sumtext=@sum gte_Summarytype="number:0:true"		width=80    align=right  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}  Color={decode(TRAN_FLAG,"반품","#FF0000")} </FC>'// mask="###,###"   sumtext={subsum(CUPON_AMT)}
                     + '<FC>id=ETC_AMT                 name="외상매출;(외상+온라인+기타)"   gte_columntype="number:0:true" sumtext=@sum gte_Summarytype="number:0:true"	width=200    align=right  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")}</FC>'// mask="###,###"   sumtext={subsum(OKCASHBAG_AMT)}
                     + '<FG> 						   name="현금영수증정보" '
                     + '<FC>id=CASH_RECP_AMT           name="승인금액"   		gte_columntype="number:0:true" gte_Summarytype="number:0:true"	sumtext=@sum width=80    align=right  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")} Color={decode(TRAN_FLAG,"반품","#FF0000")} show=false</FC>'// mask="###,###"   sumtext={subsum(OKCASHBAG_AMT)}
                     + '<FC>id=CASH_RECP_ID            name="영수번호"           width=100    align=center     suppress="2"  BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}   Color={decode(TRAN_FLAG,"반품","#FF0000")} show=false </FC>'
                     + '<FC>id=MSKD_CASH_RECP_ID       name="영수번호"           width=100    align=center     suppress="2"  sumtext="승인건수" BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}   Color={decode(TRAN_FLAG,"반품","#FF0000")} show=false </FC>'
                     + '<FC>id=CASH_RECP_APPR_NO       name="승인번호"           width=100    align=center     suppress="2"  sumtext={sum(APPR_CNT)} BgColor={if(RCP_AMT<>ITEM_AMT,"#F6CED8")}   Color={decode(TRAN_FLAG,"반품","#FF0000")} show=false </FC>'
                     + '</FG>'
                     ;
...
function btn_Search() {
    ...
    var strSaleDtE      = EM_SALE_DT_E.text;         //일자
    ...
    TR_MAIN.Action="/dps/psal585.ps?goTo="+goTo+parameters;  
...
function btn_Print() {
   ...
   /*  strPayInfo = "월" + strPayCyc +"회, " +  LC_S_PAY_CNT.Text;
    ...
    window.open("/dps/psal585.ps?goTo=print"+parameters,"OZREPORT", 1000, 700);   
}
...
function searchPosNo(strPosNo){
    ...
    TR_DETAIL.Action="/dps/psal585.ps?goTo="+goTo+parameters;  
...
function showCashRecp(tfChkbox) {
	
	GD_MASTER.ColumnProp("CASH_RECP_AMT", "Show") = tfChkbox;
	//GD_MASTER.ColumnProp("CASH_RECP_ID", "Show") = tfChkbox;
	GD_MASTER.ColumnProp("MSKD_CASH_RECP_ID", "Show") = tfChkbox;
	GD_MASTER.ColumnProp("CASH_RECP_APPR_NO", "Show") = tfChkbox;
...
var strOrgCd = LC_STR_CD.bindcolval + "000000";
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
                ...
                <th width="80" >영수증금액</th>   
               		<td  colspan="1" width="280" ><comment id="_NSID_"> <object
                     ...
                     align="absmiddle"> </object> </comment><script>_ws_(_NSID_);</script></td>            
                <td  colspan="2">
                	<input type="checkbox" id=CHK_CASHRECP onclick="showCashRecp(this.checked)">현금영수증내역조회
                </td>
               </tr>
               <!--  tr>
	               <th width="80">결제형태</th>
