...
var g_strPid           = "<%=pageName%>"; // PID
	...
	function doInit() {
		...
		obj.style.height = (parseInt(window.document.body.clientHeight) - top)
				+ "px";
		...
		initEmEdit(EM_APPRNO, "CODE^20^0", NORMAL); //승인번호
		...
		//POS층
		initComboStyle(LC_POS_FLOOR, DS_POS_FLOOR, "CODE^0^30,NAME^0^80", 1,
				NORMAL);
		getEtcCode("DS_POS_FLOOR", "D", "P061", "Y");
		LC_POS_FLOOR.Index = 0;
		//POS구분
		initComboStyle(LC_POS_FLAG, DS_POS_FLAG, "CODE^0^30,NAME^0^140", 1,
				NORMAL);
		...
		registerUsingDataset("psal426", "DS_O_MASTER");

		var strUsrId = '<c:out value="${sessionScope.sessionInfo.USER_ID}" />'; // 로그인 사번
		//alert(strUsrId);
		if (strUsrId != "17010138" && strUsrId != '17021201'
				&& strUsrId != '17021202' && strUsrId != "fkl") {

			document.getElementById("SEARCH_POINT").style.display = "none";
	...
	function gridCreate1() {
		var hdrProperies = '<FC>id={currow}                name="NO"           width=30    align=center    </FC>'
				+ '<FC>id=SALE_DT                 name="매출일자"         width=80    align=center   mask="XXXX/XX/XX"   suppress="1" </FC>'
				+ '<FC>id=POS_NO                  name="POS번호"       width=55    align=center     suppress="2"</FC>'
				+ '<FC>id=TRAN_NO                 name="거래번호"        width=55    align=center     suppress="3" </FC>'
				+ '<FC>id=SALE_TIME               name="매출시간"         width=80    align=center   mask="XX:XX:XX"     suppress="1" sumtext = "합계"</FC>'
				+ '<FC>id=TRAN_MODE               name="거래모드"        width=65    align=center     suppress="4" color={decode(TRAN_MODE,"일괄취소","#FF0000")}</FC>'
				+ '<FC>id=TRAN_FLAG               name="거래구분"        width=55    align=center     suppress="5" </FC>'
				+ '<FC>id=SALE_AMT_TOT            name="총매출"        width=100    align=right    mask="###,###"      sumtext={subsum(SALE_AMT_TOT)}</FC>'
				+ '<FC>id=REDU_PRC                name="할인"          width=70    align=right    mask="###,###"    sumtext={subsum(REDU_PRC)}</FC>'
				+ '<FC>id=SALE_AMT                name="매출"          width=100    align=right    mask="###,###"    sumtext={subsum(SALE_AMT)}</FC>'
	...
	function gridCreate2() {
		var hdrProperies = '<FC>id={currow}                name="NO"           width=30     align=center    </FC>'
				...
				+ '<FC>id=SKU_CD                  name="단품코드"       width=110    align=center     suppress="4" </FC>'
				+ '<FC>id=SKU_NAME                name="단품명"        width=100     align=left       suppress="4" </FC>'
				+ '<FC>id=SALE_AMT_TOT            name="판매금액"       width=90   align=right      mask="###,###"      sumtext={subsum(SALE_AMT_TOT)}</FC>'
	...
	function gridCreate3() {
		var hdrProperies = '<FC>id={currow}				name="NO"					width=30	align=center				</FC>'
				+ '<FC>id=DIS_NAME				name="구분"					width=60	align=left	suppress="2" sumtext = "합계"	</FC>'
	...
	function btn_Search() {
		...
		var parameters = "&strStrCd=" + encodeURIComponent(strStrCd)
				...
				+ "&strPosNoS=" + encodeURIComponent(strPosNoS) + "&strPosNoE="
				+ encodeURIComponent(strPosNoE) + "&strSaleDtS="
				+ encodeURIComponent(strSaleDtS) + "&strSaleDtE="
				+ encodeURIComponent(strSaleDtE) + "&strDealNo="
				+ encodeURIComponent(strDealNo) + "&strSkuCd="
				+ encodeURIComponent(strSkuCd) + "&strPumbunCd="
				+ encodeURIComponent(strPumbunCd) + "&strFromSaleAmt="
				+ encodeURIComponent(strFromSaleAmt) + "&strToSaleAmt="
				+ encodeURIComponent(strToSaleAmt) + "&strFromSaleTime="
				+ encodeURIComponent(strFromSaleTime) + "&strToSaleTime="
				+ encodeURIComponent(strToSaleTime) + "&strCreditCardNo="
				+ encodeURIComponent(strCreditCardNo) + "&strApprNo="
				+ encodeURIComponent(strApprNo);
	...
	function btn_Excel() {
		...
		var parameters = "점 " + strStrCd + " ,   POS층 " + strPosFloor
				+ " ,   POS구분 " + strPosFlag + " ,   매출기간 " + strSaleDtS
				+ " ~ " + strSaleDtE + " ,   매출시간 " + strFromSaleTime + " ~ "
				+ strToSaleTime + " ,   판매금액 " + strFromSaleAmt + " ~ "
				+ strToSaleAmt + " ,   브랜드명 " + strPumbunCd + " ,   단품명 "
				+ strSkuCd + " ,   POS번호 " + strPosNoS + " ~ " + strPosNoE
				+ " ,   거래번호 " + strDealNo + " ,   카드번호 " + strCreditCardNo;
		+" ,   조회건수 " + strSearchCnt;
	...
	function search_Detail() {
		...
		var strTranNo = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition,
				"TRAN_NO"); //거래번호
		var strSaleDt = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition,
				"SALE_DT"); //매출일자

		var goTo = "searchDetail";
		var action = "O";
		var parameters = "&strStrCd=" + encodeURIComponent(strStrCd)
				+ "&strPosNo=" + encodeURIComponent(strPosNo) + "&strTranNo="
				+ encodeURIComponent(strTranNo) + "&strSaleDt="
				+ encodeURIComponent(strSaleDt);

		TR_DETAIL.Action = "/dps/psal426.ps?goTo=" + goTo + parameters;
	...
	function search_Detail2() {
		...
		TR_DETAIL.Action = "/dps/psal426.ps?goTo=" + goTo + parameters;
		TR_DETAIL.KeyValue = "SERVLET(" + action
				+ ":DS_O_DETAIL2=DS_O_DETAIL2)"; //조회는 O
		TR_DETAIL.Post();
	...
	function chkValidation(gbn) {
		...
		case "search":
			...
			if (EM_POSNO_S.text.replace(" ", "").length > 4
					&& EM_POSNO_S.text != "") {
			...
			if (EM_POSNO_E.text.replace(" ", "").length > 4
					&& EM_POSNO_E.text != "") {
			...
			if (EM_DEALNO.text != "") {
				if (EM_POSNO_E.text != EM_POSNO_S.text) {
					showMessage(Information, OK, "USER-1000",
							"거래번호로 조회시 POS시작번호와 POS종료번호가 동일해야 합니다.");
					EM_POSNO_S.focus();
					return false;
				}
				if (LC_POS_FLOOR.BindColVal != "%") {
					showMessage(Information, OK, "USER-1000",
							"거래번호로 조회시 POS층은 전체여야 합니다.");
					LC_POS_FLOOR.focus();
					return false;
				}
				if (LC_POS_FLAG.BindColVal != "%") {
					showMessage(Information, OK, "USER-1000",
							"거래번호로 조회시 POS구분은 전체여야 합니다.");
					LC_POS_FLAG.focus();
					return false;
				}
			}

			if (EM_SALE_TIME_S.Text.replace(" ", "").length > 0
					&& EM_SALE_TIME_S.Text.replace(" ", "").length < 4) {
				showMessage(Information, OK, "USER-1027", "판매시작시간", "4");
				EM_SALE_TIME_S.focus();
				return false;
			}

			if (EM_SALE_TIME_E.Text.replace(" ", "").length > 0
					&& EM_SALE_TIME_E.Text.replace(" ", "").length < 4) {
	...
	function searchPosNoMM() {
		...
		TR_DETAIL.Action = "/dps/psal426.ps?goTo=" + goTo + parameters;
		TR_DETAIL.KeyValue = "SERVLET(" + action
				+ ":DS_O_POSNOMM=DS_O_POSNOMM)"; //조회는 O
		TR_DETAIL.Post();
	...
	function searchPoint() {
		...
		TR_DETAIL.Action = "/dps/psal426.ps?goTo=" + goTo + parameters;
		TR_DETAIL.KeyValue = "SERVLET(" + action + ":DS_O_POINT=DS_O_POINT)"; //조회는 O
		TR_DETAIL.Post();

		if (DS_O_POINT.CountRow > 0) {

			var cust_id = "- 회원번호 : " + DS_O_POINT.NameValue(1, "CUST_ID");
			var cust_name = " - 회 원 명 : "
					+ DS_O_POINT.NameValue(1, "CUST_NAME");
			var card_no = " - 카드번호 : " + DS_O_POINT.NameValue(1, "CARD_NO");
			var hp = " - 전화번호 : " + DS_O_POINT.NameValue(1, "HP");
			var occ_point = " - 적립점수 : " + DS_O_POINT.NameValue(1, "OCC_POINT");

			showMessage(Information, OK, "USER-1000", cust_id + "<br>"
					+ cust_name + "<br>" + card_no + "<br>" + hp + "<br>"
					+ occ_point + "<br>");
		} else {

			showMessage(Question, OK, "USER-1000", "적립 내역이 없습니다.");
	...
	function openReciept(obj, rownum) {
		//alert(rownum);
		var rtnMap = new Map();
		var arrArg = new Array();
		var strStrCd = "000" + LC_STR_CD.BindColVal;
		var strSaleDt = obj.NameValue(rownum, "SALE_DT");
		var strPosNo = obj.NameValue(rownum, "POS_NO");
		var strTranNo = obj.NameValue(rownum, "TRAN_NO");
		var strTranFlag = obj.NameValue(rownum, "TRAN_FLAG");
		...
		arrArg.push(rtnMap);
		arrArg.push(strSaleDt + strStrCd + strPosNo + strTranNo + strTranFlag); // 바코드

		var returnVal = window
				.showModalDialog(
						"/dps/psal426.ps?goTo=popUpList",
						arrArg,
						"dialogWidth:450px;dialogHeight:800px;scroll:yes;"
...
<script language=JavaScript for=GD_MASTER event=OnDblClick(row)>
	//sortColId( eval(this.DataID), row, colid); //헤더 클릭시 정렬
	openReciept(DS_O_MASTER, row);
</script>
...
<script language=JavaScript for=EM_PUMBUN_CD event=onKillFocus()>
	...
	if (EM_PUMBUN_CD.text.length != 0) {
		setStrPbnNmWithoutPop("DS_O_PUMBUN_CD", EM_PUMBUN_CD, EM_PUMBUN_NAME,
				'Y', '1');
	}
</script>
...
<comment id="_NSID_"> <object id="DS_O_POSNOMM"
	classid=<%=Util.CLSID_DATASET%>></object> </comment>
<script>
	_ws_(_NSID_);
</script>
<comment id="_NSID_"> <object id="DS_POS_FLOOR"
	classid=<%=Util.CLSID_DATASET%>></object> </comment>
<script>
	_ws_(_NSID_);
</script>

<comment id="_NSID_"> <object id="DS_POS_FLAG"
	classid=<%=Util.CLSID_DATASET%>></object> </comment>
<script>
	_ws_(_NSID_);
</script>
...
<comment id="_NSID_"> <object id="DS_O_DETAIL"
	classid=<%=Util.CLSID_DATASET%>> </object> </comment>
<script>
	_ws_(_NSID_);
</script>

<comment id="_NSID_"> <object id="DS_O_DETAIL2"
	classid=<%=Util.CLSID_DATASET%>> </object> </comment>
<script>
	_ws_(_NSID_);
</script>

<comment id="_NSID_"> <object id="DS_O_POSNO"
	classid=<%=Util.CLSID_DATASET%>></object> </comment>
<script>
	_ws_(_NSID_);
</script>
...
<comment id="_NSID_"> <object id="DS_O_SKU_CD"
	classid=<%=Util.CLSID_DATASET%>></object> </comment>
<script>
	_ws_(_NSID_);
</script>

<comment id="_NSID_"> <object id="DS_O_POINT"
	classid=<%=Util.CLSID_DATASET%>></object> </comment>
<script>
	_ws_(_NSID_);
</script>
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0"
									...
									<tr>
										...
										<td width="150"><comment id="_NSID_"> <object
												id=LC_POS_FLOOR classid=<%=Util.CLSID_LUXECOMBO%> width=140
										...
										<td colspan="3"><comment id="_NSID_"> <object
												id=LC_POS_FLAG classid=<%=Util.CLSID_LUXECOMBO%> width=140
												tabindex=1 align="absmiddle"> </object> </comment>
											<script>
												_ws_(_NSID_);
											</script></td>
									...
									<tr>
										...
										<th>승인번호</th>
										<td><comment id="_NSID_"> <object id=EM_APPRNO
									...
									<tr>
										...
										<td><comment id="_NSID_"> <object
											...
											~ <comment id="_NSID_"> <object id=EM_SALE_TIME_E
										...
										<td colspan="5"><comment id="_NSID_"> <object
											...
											~ <comment id="_NSID_"> <object id=EM_SALE_AMT_E
									...
									<tr>
										...
										<td colspan="5"><comment id="_NSID_"> <object
											...
											onclick="javascript:setSkuCdCombo('POP');" align="absmiddle" />
											<comment id="_NSID_"> <object id=EM_SKU_NAME
									...
									<tr>
										<th>POS시작번호</th>
										<td><comment id="_NSID_"> <object id=EM_POSNO_S
										...
										<th>POS종료번호</th>
										<td><comment id="_NSID_"> <object id=EM_POSNO_E
												classid=<%=Util.CLSID_EMEDIT%> width=140 tabindex=1
												align="absmiddle"> </object> </comment>
											<script>
												_ws_(_NSID_);
										...
										<th>거래번호</th>
										<td colspan="5"><comment id="_NSID_"> <object
												id=EM_DEALNO classid=<%=Util.CLSID_EMEDIT%> width=140
												tabindex=1 align="absmiddle"> </object> </comment>
											<script>
												_ws_(_NSID_);
											</script> * 적립내용 확인 <a id=SEARCH_POINT>
												<img src="/<%=dir%>/imgs/btn/detail_search_s.gif"
												onclick="javascript:searchPoint();" align="absmiddle" /> <font
												color="red">※ 확인하고자 하는 영수증 선택 후 돋보기 클릭.</font>
										</a></td>
									</tr>
								</table>
							</td>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0"
									...
									<tr>
										<td width="100%"><comment id="_NSID_"> <object
												id=GD_MASTER width=100% height=280
									...
									<tr valign="top">
										<td>
											<table width="100%" border="0" cellspacing="0"
												...
												<tr>
													<td width="100%"><comment id="_NSID_"> <object
															id=GD_DETAIL width=100% height=280
															classid=<%=Util.CLSID_GRID%> tabindex=1>
						...
						<tr valign="top">
							...
							<td width="100%">
								<table width="100%" border="0" cellspacing="0" cellpadding="0"
									...
									<tr>
										<td width="100%"><comment id="_NSID_"> <object
												id=GD_DETAIL2 width=100% height=200
												classid=<%=Util.CLSID_GRID%> tabindex=1>
