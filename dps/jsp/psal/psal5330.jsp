...
function doInit(){
    ...
    registerUsingDataset("psal533","DS_O_MASTER" );
...
function gridCreate2(){
    ...
    var hdrProperies = '<FC>id={currow}     name="NO"              width=30    align=center</FC>'
                     ...
                     + '<FC>id=SALE_YM      name="*매출월"          width=80    align=center EditStyle=Popup Edit=Numeric Mask="XXXX/XX" </FC>'
                     //제휴몰 코드 제휴몰 이름
                     + '<FC>id=ORDERER_CD   name="*제휴몰코드"          width=80    align=center EditStyle=Popup  </FC>'
                     + '<FC>id=ORDERER_NM   name="*제휴몰"          width=80   Edit=none  align=center   </FC>'
                     + '<FC>id=DED_CD       name="*공제코드"           width=80   align=left   EditStyle=Popup </FC>'//Data="DS_I_REASON_CD:CODE:NAME"
                     + '<FC>id=DED_NM       name="*공제이름"           width=140   align=left   </FC>'//Data="DS_I_REASON_CD:CODE:NAME"
                     + '<FC>id=VAT_YN       name="VAT여부"         width=90    align=left    EditStyle=Lookup Data="DS_I_VAT_YN:CODE:NAME"</FC>' 
                     + '<FC>id=INPUT_AMT    name="*공제금액"        width=100   align=right   </FC>'  //{IF(CONF_FLAG="N","true","false")}   
                     + '<FC>id=SUP_AMT      name="공급가"          width=100   align=right  </FC>'   
                     + '<FC>id=VAT_AMT      name="부가세"          width=100   align=right  </FC>'
                     + '<FC>id=ETC          name="비고"            width=300   align=left   </FC>'//Edit={IF(CONF_FLAG="N","true","false")}
                     
                     + '<FC>id=SEQ_NO          name="순번"        width=300   align=left SHOW=FALSE  </FC>'
...
function btn_Search() {
    ...
    var strOriginAreaName = EM_I_ORDERER_NAME.text;
    ...
    var parameters = "&strStrCd="           +encodeURIComponent(strStrCd)
                   ...
                   + "&strOriginAreaName="  +encodeURIComponent(strOriginAreaName)
                   ;
    
    TR_MAIN.Action="/dps/psal533.ps?goTo="+goTo+parameters;  
...
function btn_Save() {
    ...
    if( showMessage(STOPSIGN, YESNO, "USER-1010") == 1 ){
        ...
        TR_MAIN.Action="/dps/psal533.ps?goTo=save&strFlag=save"; 
 ...
 *************************************************************************/
 function searchDitail(){
	    ...
	    TR_DETAIL.Action="/dps/psal533.ps?goTo="+goTo+parameters;  
 ...
 function setVatXYN(row) {
     ...
     var strReasonCd = DS_IO_MASTER.NameValue(row, "DED_CD");     
     
     TR_MAIN.Action="/dps/psal533.ps?goTo=setVatXYN&strReasonCd="+strReasonCd; 
  ...
  */
  function calcAmt(row){
      ...
      var strVenCd    = DS_IO_MASTER.NameValue(row, "VEN_CD");               // 협력사코드
          if(strVatYn == "Y"){
          //roundFlag = getVenRoundFlag("DS_O_RESULT", varToday, strStrCd, strVenCd);
     ...
     if(Gubun == "Save"){          
         ...
         if(intRowCount > 0){
             for(var i=1; i <= intRowCount; i++){
               ...
               strDedCd        = DS_IO_MASTER.NameValue(i, "DED_CD");
          	   ...
          	   if(strSaleYm.length <= 0){
                     showMessage(INFORMATION, OK, "USER-1003", "매출월");
                     GR_MASTER.SetColumn("SALE_YM");  
                     DS_IO_MASTER.RowPosition = i;  
                     return false;
                 }
          	   
          	  if(strOrdererCd <= 0){
	                 showMessage(EXCLAMATION, OK, "USER-1008", "제휴몰");
	                 GR_MASTER.SetColumn("ORDERER_CD");  
	                 DS_IO_MASTER.RowPosition = i;  
	                 return false;
                 }   
                
              if(strDedCd.length <= 0){
                     showMessage(INFORMATION, OK, "USER-1003", "공제");
                     GR_MASTER.SetColumn("DED_CD");  
                     DS_IO_MASTER.RowPosition = i;  
                     return false;
                 }        
...
function checkKeyVlaue(row) {
    ...
    var strDedCd       = DS_IO_MASTER.NameValue(row, "DED_CD");
    
    var parameter   = "&strStrCd="     +encodeURIComponent(strStrCd)
				    + "&strSaleYm="    +encodeURIComponent(strSaleYm)
				    + "&strOrdererCd=" +encodeURIComponent(strOrdererCd) 
				    + "&strDedCd="     +encodeURIComponent(strDedCd) 
				    ;
    
    TR_MAIN.Action="/dps/psal533.ps?goTo=checkKeyVlaue"+parameter; 
...
function addRow(){
	  if(checkValidation("Save")){
         ...
         //DS_IO_MASTER.NameValue(DS_IO_MASTER.RowPosition, "STR_CD") 		= LC_STR_CD.Text;
...
<script language=JavaScript for=GR_MASTER event=OnPopup(row,colid,data)>
    ...
    case "ORDERER_CD":
      ...
      var map= setIndiCommonPop('제휴몰', 'P613', 'POP', strOrdererCd, strOrdererNm,'I');
      if (map==""||map==null){
     ...
     case "DED_CD":
    	 var strDedCd = DS_IO_MASTER.NameValue(row, "DED_CD");
    	 var strDedNm = DS_IO_MASTER.NameValue(row, "DED_NM");
        
    	 var map = setIndiCommonPop('공제', 'P618', 'POP', "", "",'I');
    	 if(map==null||map==""){
    		 return false;
    	 }else{
	    	 DS_IO_MASTER.NameValue(row, "DED_CD") = map.get("CODE_CD");
	  	     DS_IO_MASTER.NameValue(row, "DED_NM") = map.get("CODE_NM");
...
<object id="DS_O_MASTER_CROSSTAB" classid=<%=Util.CLSID_DATASET%>>
    ...
    <param name=GroupExpr   value="ORDERER_NM:,DED_NM,:DED_AMT"></object>
</comment>
<script>_ws_(_NSID_);</script>
      ...
      <tr valign="top">
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" class="BD4A" >
          <tr>
            <td width="100%">
                <comment id="_NSID_">
                    <OBJECT id=GR_MASTER width=100% height=223 classid=<%=Util.CLSID_GRID%>>
