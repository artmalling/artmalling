...
function doInit(){
    ...
    initEmEdit(EM_I_VEN, "CODE^6^0", NORMAL);  //협력사
    ...
    registerUsingDataset("psal303","DS_O_MASTER" );
    
}

function gridCreate1(){
    var hdrProperies = '<FC>id={currow}                name="NO"           width=30    align=center    subsumtext="" SubBgColor=#FFFFE0 </FC>'
                     + '<FC>id=BUYER_CD                name="바이어"        width=60    align=center    suppress=1    subsumtext=""  SubBgColor=#FFFFE0</FC>'
                     + '<FC>id=BUYER_NAME              name="바이어명"       width=80    align=left      suppress=1   SubSumText={decode(curlevel,0,"",1,"바이어소계")} SubBgColor=#FFFFE0</FC>'
                     + '<C>id=PC_CD                   name="PC"           width=85    align=center      suppress=2   subsumtext="" SubBgColor=#FFFFE0</C>'
                     + '<C>id=PC_NAME                 name="PC명"         width=80    align=left      suppress=2     subsumtext="" SubBgColor=#FFFFE0</C>'
                     + '<C>id=PUMBUN_CD               name="브랜드"         width=60    align=center      suppress=3   subsumtext="" SubBgColor=#FFFFE0</C>'
                     + '<C>id=PUMBUN_NAME             name="브랜드명"        width=100    align=left      suppress=3   subsumtext="" SubBgColor=#FFFFE0</C>'
                     + '<C>id=VEN_CD                  name="협력사코드"     width=70    align=center      suppress=4  subsumtext=""  SubBgColor=#FFFFE0</C>'
                     + '<C>id=VEN_NAME                name="협력사명"      width=80    align=left      suppress=4   subsumtext="" SubBgColor=#FFFFE0</C>'
                     + '<C>id=ORIGIN_SALE_TAMT        name="매출목표"      width=90    align=right    mask="###,###"     value= {ORIGIN_SALE_TAMT/1000} sumtext={subsum(ORIGIN_SALE_TAMT)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=TOT_SALE_AMT            name="총매출"      width=90    align=right    mask="###,###"     value= {TOT_SALE_AMT/1000} sumtext={subsum(TOT_SALE_AMT)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=ACHIEVERATE             name="달성율"        width=60    align=right     mask="#,###.00"    </C>'
                     + '<C>id=SALE_AMT_CMPR           name="대비총매출"      width=90    align=right    mask="###,###"     value= {SALE_AMT_CMPR/1000} sumtext={subsum(SALE_AMT_CMPR)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=SALEIRATE               name="신장율"        width=60    align=right     mask="#,###.00"    subsumtext="" SubBgColor=#FFFFE0</C>'
                     + '<C>id=REDU_AMT                name="할인"          width=90    align=right    mask="###,###"    value= {REDU_AMT/1000} sumtext={subsum(REDU_AMT)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=SALE_AMT                name="매출"          width=90    align=right    mask="###,###"    value= {SALE_AMT/1000} sumtext={subsum(SALE_AMT)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=DC_AMT                  name="에누리"        width=90    align=right    mask="###,###"    value= {DC_AMT/1000} sumtext={subsum(DC_AMT)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=NET_SALE_AMT            name="순매출"        width=90    align=right     mask="###,###"    value= {NET_SALE_AMT/1000} sumtext={subsum(NET_SALE_AMT)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=SALE_PROF_AMT           name="이익액"        width=90    align=right     mask="###,###"    value= {SALE_PROF_AMT/1000} sumtext={subsum(SALE_PROF_AMT)/1000} SubBgColor=#FFFFE0</C>'
                     + '<C>id=PROFRATE                name="이익율"        width=60    align=right     mask="#,###.00"   subsumtext="" SubBgColor=#FFFFE0</C>'
                     + '<C>id=CUST_CNT                name="객수"         width=60    align=right      mask="###,###"    sumtext={subsum(CUST_CNT)} SubBgColor=#FFFFE0</C>'
                     + '<C>id=CUST_DANGA              name="객단가"        width=80    align=right     mask="###,###"     value= {CUST_DANGA/1000} SubBgColor=#FFFFE0</C>';
    ...
    DS_O_MASTER.SubSumExpr  = "1:BUYER_CD" ; 
    GD_MASTER.ColumnProp("BUYER_NAME", "sumtext")        = "합계";
}
...
function btn_Search() {
    ...
    var strVenCd        = EM_I_VEN.text;             //협력사코드
    ...
    TR_MAIN.Action="/dps/psal303.ps?goTo="+goTo+parameters;  
...
function btn_Excel() {
    ...
    var strSaleDtS      = EM_SALE_DT_S.text;         //매출일자
    var strSaleDtE      = EM_SALE_DT_E.text;         //대비일자
    var strVenCd        = EM_I_VEN_NM.text;             //협력사코드
...
function setVenCode(evnFlag, svcFlag, codeObj, nameObj){
    ...
    setVenNmWithoutPop("DS_SEARCH_NM", codeObj, nameObj , 1,'',svcFlag=='I'?'Y':'');
}
...
if(!this.Modified)
    return;
setVenCode('NAME','S',EM_I_VEN,EM_I_VEN_NM);
         ...
         <tr>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0"
               ...
               <tr>
                  ...
                  <th>협력사</th>
                  <td colspan="2"><comment id="_NSID_"> <object id=EM_I_VEN
                      classid=<%=Util.CLSID_EMEDIT%> width=45 tabindex=1
                      align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script> <img
                      src="/pot/imgs/btn/detail_search_s.gif" id=IMG_VEN
                      class="PR03"
                      onclick="javascript:setVenCode('POP','S',EM_I_VEN,EM_I_VEN_NM);"
                      align="absmiddle" /> <comment id="_NSID_"> <object
                      id=EM_I_VEN_NM classid=<%=Util.CLSID_EMEDIT%> width=105
                      tabindex=1 align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script>
                  </td>
                  <td >단위 : 천원, 명, %
                  </td>
         ...
         <tr valign="top">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
               ...
               <tr>
                  <td width="100%">
                  <comment id="_NSID_"> <object
                     id=GD_MASTER width=100% height=483 classid=<%=Util.CLSID_GRID%> tabindex=1>
