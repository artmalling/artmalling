...
public class PPay107DAO extends AbstractDAO { 
	/**
	 * 사용자 권한 조회
	 *       
	 * @param form   
	 ...
	 * @throws Exception
	 */
	public List getAttribute(ActionForm form, String userId) throws Exception {
		...
		sql.put(svc.getQuery("SEL_ACCOM12"));
		ret = select2List(sql);
	...
	public List getList(ActionForm form) throws Exception {
		...
		// 매입세금계산서일 경우 역발행만 조회
		if("1".equals(strTaxInvFlag))
			sql.put(svc.getQuery("SEL_RVS_ISSUE_FLAG"));
		...
		try {
			...
			while (mi.next()) {
				...
				if ( mi.IS_UPDATE()){
					...
					sql.put(svc.getQuery("SEL_TSMILETAX_DEL_CNT"));							
					sql.setString(++i, mi.getString("EDI_SEQ_NO"));
					...
					if(orgCdCnt.equals("0")) {
						throw new Exception("[USER]"+"삭제 불가능한 데이터 입니다.");
					}
					
					sql.close();
					i = 0;
					System.out.println(mi.getString("EDI_SEQ_NO"));
