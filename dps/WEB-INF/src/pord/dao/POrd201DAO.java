...
public class POrd201DAO extends AbstractDAO {    	
	/**
	 * 규격단품 매입발주 리스트 내역 조회
	 * 
	 * @param form
...
//		System.out.println("strquery : "+ strQuery);
	...
	*/
	public int conf(ActionForm form, String userId, String org_flag)
	    ...
	    String strSlipProcStat  = "";		// 전표상태flag
...
//			strOrgSlipProcStat = form.getParam("strOrgSlipProcStat");
			...
			//확정취소 
			if("01".equals(strSlipProcStat)){ 
				sql.put(svc.getQuery("UPD_MASTER_CAN"));
				sql.setString(i++, "        ");
				sql.setString(i++, null);
				sql.setString(i++, "00");
				sql.setString(i++, "01");			//반려취소상태		
				sql.setString(i++, userId); 
		    ...
		    //확정 
				...
				sql.setString(i++, form.getParam("strToday"));
				sql.setString(i++, userId);  
				sql.setString(i++, "01");		
				sql.setString(i++, strSlipFlag); //발주확정일자에 필요한 전표구분
				sql.setString(i++, form.getParam("strToday")); //발주확정일자에 필요한 전표구분
				sql.setString(i++, userId); 
				sql.setString(i++, form.getParam("strStrCd"));
				sql.setString(i++, form.getParam("strSlipNo"));
			...
			if("0".equals(sendFlag)){		//승인
				strConfSlipProcStat = "01";
			}else{
				strConfSlipProcStat = strOrgSlipProcStat;
			}
			*/
			strConfSlipProcStat = "01";
			...
			System.out.println("strStrCd = " + strStrCd);
			System.out.println("strSlipNo = " + strSlipNo);
			System.out.println("strConfSlipProcStat = " + strConfSlipProcStat);
			...
			map.put("strConfSlipProcStat",  strConfSlipProcStat);    
	...
	public int sendSMS(ActionForm form, HashMap map, String userId, String org_flag, String sendFlag) 
		...
		String strConfSlipProcStat     = "";	// 전표진행상태		
		...
		try {
			...
			strConfSlipProcStat     = (String)map.get("strConfSlipProcStat");
            ...
            psql.setString(i++, strConfSlipProcStat);       // 전표상태
