...
public class POrd601DAO extends AbstractDAO {
	 ...
	 * @throws Exception
	 */
	public List getSkuInfo(ActionForm form, String userId, String org_flag) throws Exception {
		...
		String strOrdYm    = String2.nvl(form.getParam("strOrdYm"));
		String strPrcAppDt = String2.nvl(form.getParam("strPrcAppDt"));
		String strPrcEndDt = String2.nvl(form.getParam("strPrcEndDt"));
		String strSkuCd    = String2.nvl(form.getParam("strSkuCd"));
		String nNewCostPrc = String2.nvl(form.getParam("nNewCostPrc"));
		String nNewSalePrc = String2.nvl(form.getParam("nNewSalePrc"));
//		String strBizType  = String2.nvl(form.getParam("strBizType"));
		...
		sql.setString(i++, strPrcEndDt);
		//sql.setString(i++, nNewCostPrc);
		//sql.setString(i++, nNewSalePrc);
		sql.setString(i++, strStrCd);    
		sql.setString(i++, strOrdYm);
		...
		try {
			...
			while (mi2.next()) {
				...
				if (mi2.IS_INSERT()) { // 저장
					...
					sql.setString(i++, mi2.getString("ORD_STK_QTY"));   		
					sql.setString(i++, mi2.getString("AVG_SALE_QTY"));   	
					sql.setString(i++, mi2.getString("AVG_SALE_AMT"));   	
					...
					/***********************************************************************************/
					/******************** 20170202 점별단품등록 로직 추가 시작 PRH *********************/
					/***********************************************************************************/
					// 3. 점별단품D[ 존재여부 체크
					i = 1; 
					sql = new SqlWrapper();
					sql.put(svc.getQuery("SEL_CHK_STRSKUCD"));
					sql.setString(i++, mi2.getString("STR_CD"));
					sql.setString(i++, mi2.getString("SKU_CD"));
					sql.setString(i++, mi2.getString("SKU_CD"));		
					Map mapSkuCdCnt = (Map)selectMap(sql);
					strCnt = mapSkuCdCnt.get("CNT").toString();
					sql.close();
					
					// 업로드한 상품이 점별단품에 없을 경우
					if(strCnt.equals("0")){
						//3-1. 점별단품 등록 시작
						i = 1; 
						sql = new SqlWrapper();
						...
						sql.setString(i++, mi2.getString("STR_CD"));  
						sql.setString(i++, mi2.getString("SKU_CD"));  
						sql.setString(i++, mi2.getString("SKU_NM"));  
						sql.setString(i++, mi2.getString("SKU_NM"));  
						sql.setString(i++, mi2.getString("PUMBUN_TYPE"));   
						sql.setString(i++, mi2.getString("PUMBUN_CD"));   
						sql.setString(i++, mi2.getString("VEN_CD"));  
						sql.setDouble(i++, mi2.getDouble("NEW_COST_PRC"));
						sql.setDouble(i++, mi2.getDouble("NEW_SALE_PRC"));
						sql.setDouble(i++, mi2.getDouble("MG_RATE"));
						sql.setString(i++, mi2.getString("BUY_S_DT"));   	     
						sql.setString(i++, mi2.getString("BUY_E_DT"));  
						sql.setString(i++, mi2.getString("BUY_S_DT"));   	     
						sql.setString(i++, mi2.getString("BUY_E_DT"));  
						sql.setString(i++, userId); 
						sql.setString(i++, userId);              
	...
	public int delete(ActionForm form, String userId)
		...
		try {
			...
			sql.close();
			sql.put(svc.getQuery("DEL_FILE_ALL")); 
