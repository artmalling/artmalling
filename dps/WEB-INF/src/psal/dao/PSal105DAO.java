...
public class PSal105DAO extends AbstractDAO {
	...
	public List searchDetail(ActionForm form, String OrgFlag, String userId) throws Exception {
		...
		sql.put(svc.getQuery("SEL_PLANPC_SELORIGIN"));
		...
		if(ret.isEmpty() == true){
			...
			svc 	= null;
			svc = (Service) form.getService();
			sql2.put(svc.getQuery("SEL_PLANPC_DETAIL"));
			...
			if(ret.isEmpty() == true){
				...
				strQuery  = svc.getQuery("SEL_PLANPC_SEL")+ "\n ";
				for(j=1; j <= 12 ; j++){
					...
					i 			= 1;
					strQuery += svc.getQuery("SEL_PLANPCYM")+ "\n ";
					sql3.setString(i++, strPlanYear);
					sql3.setString(i++, strMonth);
					...
					if(j < 12){
						strQuery  += svc.getQuery("SEL_PLANPC_UNION")+ "\n ";
					}
					
				}
				strQuery  += svc.getQuery("SEL_PLANPC_ORDERBY")+ "\n ";
	...
	/**
	 * 당초PC별월매출계획 확정데이터  조회한다.
	 * 
	 * @param form
	 * @return
	 * @throws Exception
	 */
	public List searchConfFlagPC(ActionForm form, String OrgFlag) throws Exception {
