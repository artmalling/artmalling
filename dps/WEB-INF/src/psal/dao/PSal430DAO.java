...
public class PSal430DAO extends AbstractDAO {
	...
	public List searchMaster(ActionForm form, String OrgFlag, String userid) throws Exception {
		...
		if(strOrgFlag.equals("1")){
			sql.put(svc.getQuery("SEL_SALE_ORGSKU_01"));   
			...
			sql.put(svc.getQuery("SEL_SALE_ORGSKU_02"));	
			sql.setString(i++, userid);
		}else{
			sql.put(svc.getQuery("SEL_SALE_ORGSKU2_01"));   
			...
			sql.put(svc.getQuery("SEL_SALE_ORGSKU2_02"));
			sql.setString(i++, userid);
		}
		ret = select2List(sql);
