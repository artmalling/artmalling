...
public class PSal217DAO extends AbstractDAO {
	...
	public int save(ActionForm form, MultiInput mi[], String strId)
		...
		try {
			...
			while (mi[0].next()) {
				...
				// TR데이터 생성 프로시저 호출
				prs = callProc(mi[0], nSeqNo, strId);
	...
	private int insertMaster(ActionForm form, MultiInput mi, String strId, int nSeqNo) throws Exception {
	    ...
	    try {
	        ...
	        sql.setString(++i, mi.getString("CARD_PBLSH_CD"));
	        ret = update(sql);
			sql.close();
	...
	public ProcedureResultSet callProc(MultiInput mi, int nSeqNo, String strId)
		...
		int i = 0;
	        ...
	        // 처리로직 프로시저 호출
			psql.put("DPS.PR_PSDAY_ETC_PAY", 7);
	 ...
	 * @throws Exception
	 */
	public List searchPayType(ActionForm form, String userid) throws Exception {
		try{
		List ret 		= null;
		SqlWrapper sql 	= null;
		Service svc 	= null;
		...
		connect("pot");
		sql.put(svc.getQuery("SEL_PAY_TYPE"));
		//sql.setString(++i, strStrCd);
		//sql.setString(++i, strPumBunCd);
		//sql.setString(++i, strSaleDt);
