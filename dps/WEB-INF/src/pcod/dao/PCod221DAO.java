...
public class PCod221DAO extends AbstractDAO {
		...
		try {
			...
			while (mi.next()) {
				...
				if ( mi.IS_INSERT()) {		
					...
					sql.setString(++i, mi.getString("TEL_NO1"));
					sql.setString(++i, mi.getString("TEL_NO2"));
					sql.setString(++i, mi.getString("TEL_NO3"));
				...
				// 점별 브랜드 신규입력시 브랜드별 인정률 기본세팅 20170102
				if ( mi.IS_INSERT()) {
					sql.close();
					i = 0;			
					sql = new SqlWrapper();
					svc = (Service) form.getService();
					sql.put(svc.getQuery("INS_MC_APPRATE"));
					sql.setString(++i, mi.getString("STR_CD"));
					sql.setString(++i, pumbunCd);
					sql.setString(++i, strID);
					sql.setString(++i, strID);
