...
public class PCod101DAO extends AbstractDAO {
		...
		try {
			...
			while (mi[0].next()) {//DateSet의 현제 Row의 상태를 체크한다 IS_INSERT, IS_UPDATE, IS_DELETE
				...
				if (mi[0].IS_INSERT()) {
					...
					sql.setString(24, mi[0].getString("BUY_ACC_VEN_CD"));				
					sql.setString(25, mi[0].getString("SALE_ACC_VEN_CD"));
					sql.setString(26, strID);
					sql.setString(27, mi[0].getString("VEN_CD"));
