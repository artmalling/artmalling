...
public class ECmn105Action extends DispatchAction {
	/*
	 * Java Pattern 에서 지원하는 logger를 사용할 수 있도록 객체를 선언
	 */
	//private Logger logger = Logger.getLogger(ECmn105Action.class);
     ...
     * 
     */
	public ActionForward getChkSced(ActionMapping mapping, ActionForm form,
            HttpServletRequest request, HttpServletResponse response)
    throws Exception {
        ECmn105DAO dao		= null;
        String json 		= null;
		try{
			dao = new ECmn105DAO();
			json = (String) dao.getChkSced(form);
		}catch(Exception e){
			e.printStackTrace();
		}
		ActionUtil.sendAjaxResponse(response, json);
