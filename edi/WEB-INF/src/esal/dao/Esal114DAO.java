package esal.dao;

import java.util.List;

import ecom.util.Util;
import kr.fujitsu.ffw.control.ActionForm;
import kr.fujitsu.ffw.control.cfg.svc.Service;
import kr.fujitsu.ffw.model.AbstractDAO;
import kr.fujitsu.ffw.model.SqlWrapper;
import kr.fujitsu.ffw.util.String2;


public class Esal114DAO extends AbstractDAO{
	public String getMaster(ActionForm form) throws Exception { 
		int i = 0;
		SqlWrapper sql = null;
		Service svc = null;
		List list = null;
		String rtJson = null;
		
		try{ 
			sql = new SqlWrapper();
			svc = (Service) form.getService(); 
  
			String strcd 		= String2.nvl(form.getParam("strcd"));			//점코드
			String strPumbuncd 	= String2.nvl(form.getParam("strPumbuncd"));	//품번코드
			String sDate 		= String2.nvl(form.getParam("sDate"));			//기간FROM 
			String eDate 		= String2.nvl(form.getParam("eDate"));			//기간TO
			String strGrpGb		= String2.nvl(form.getParam("strGrpGb"));		//그룹 구분

			connect("pot");
			
			if (strGrpGb.equals("0")) {
				sql.put(svc.getQuery("SEL_MASTER_HEAD"));
			}
			else {
				sql.put(svc.getQuery("SEL_MASTER_HEAD_GRP"));
				sql.setString(++i, strPumbuncd);
				sql.setString(++i, strPumbuncd);
			}
			
			sql.put(svc.getQuery("SEL_MASTER_BODY")); 
			sql.setString(++i, strcd);
			sql.setString(++i, strcd);
			sql.setString(++i, sDate);
			sql.setString(++i, eDate);
			sql.setString(++i, strPumbuncd);
			
			sql.setString(++i, strcd);
			sql.setString(++i, sDate);
			sql.setString(++i, eDate);
			sql.setString(++i, strPumbuncd);
			
			if (!strGrpGb.equals("0")) {
				sql.put(svc.getQuery("SEL_MASTER_TAIL_GRP"));
			}
			
			sql.put(svc.getQuery("SEL_MASTER_ORD"));
			
			
			list = select2List(sql);
			
			Util util = new Util(); 
			String	cols= "SALE_DT,POS_NO,POS_NM,CAL_01,NORM_01,DIFF_01,CAL_11,NORM_11,DIFF_11,CAL_21,NORM_21,DIFF_21";
			rtJson = util.listToJsonOBJ(list, cols);
			System.out.println(">>>>>>>>>>rtJson  :" + rtJson);
			
		}catch(Exception e){
			throw e;
		}
		
		return rtJson;
	}
}
