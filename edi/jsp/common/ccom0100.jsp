...
function doOnClick( row ){
	...
	if( rowCnt > 0 ){
		var objJson = "";
			...
			objJson += ""+strPummokCd[row].value+":"+strPummokNm[row].value+":"+strUnitCd[row].value+":"+strTagflag[row].value+":"+strTagOwnFlag[row].value+"";
...
function btn_Search(){
    ...
    var param = "&goTo=getPummok" + "&strPummokcd=" + strPummokcd
                                      + "&pumbunCd=" + pumbunCd;
     
	<ajax:open callback="on_getPummokXML" 
						param="param" 
						method="POST" 
						urlvalue="/edi/ccom010.cc"/>
	
	<ajax:callback function="on_getPummokXML">
	    var content = "";
		if( rowsNode.length > 0 ){
			   ...
			   for( i = 0; i < rowsNode.length; i++ ){
				    var strPummok = rowsNode[i].childNodes[1].childNodes[0].nodeValue == "null" ? "" : rowsNode[i].childNodes[1].childNodes[0].nodeValue;  //품목 코드
				    var strPummokNm = rowsNode[i].childNodes[2].childNodes[0].nodeValue == "null" ? "" : rowsNode[i].childNodes[2].childNodes[0].nodeValue;  //품목명
				    var strUnit_cd = rowsNode[i].childNodes[9].childNodes[0].nodeValue == "null" ? "" : rowsNode[i].childNodes[9].childNodes[0].nodeValue;  //단위
				    var strTag_flag = rowsNode[i].childNodes[10].childNodes[0].nodeValue == "null" ? "" : rowsNode[i].childNodes[10].childNodes[0].nodeValue;  //택구분
				    var strTag_own_flag = rowsNode[i].childNodes[11].childNodes[0].nodeValue == "null" ? "" : rowsNode[i].childNodes[11].childNodes[0].nodeValue;  //택구분
				    ...
				    content += "<td class='r1' id='1tdId"+i+"' onclick='doOnClick("+i+");' style='cursor:hand;' ><input type='hidden' name='d_pummokcd' id='d_pummokcd"+i+"' value='"+strPummok+"' /> "+strPummok+"</td>";
				    content += "<td class='r3' id='2tdId"+i+"' onclick='doOnClick("+i+");' style='cursor:hand;'><input type='hidden' name='d_pummokNm' id='d_pummokNm"+i+"' value='"+strPummokNm+"'  /> "+strPummokNm+"</td>";
				    content += "</tr>";
			   }      
                                ...
                                <td>
                                    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
                                        ...
                                        <tr>
                                            <th width="50">품목코드</th>
                                            <td width="80">
                                                <input type="text" name="PUMMOKCD" id="PUMMOKCD" size="11" maxlength="8" onkeypress="javascript:onlyNumber();" style='text-align:left;IME-MODE: disabled' onblur="btn_Search();"/>
                                            </td>
                                            <th width="50">품목명</th>
                                            <td >
