...
var _ErrorMessage  = [
 // 서버에서 발생하는 에러메세지 (DataSet을 사용하는 Error Message
  ["SERVER-1001", "<font color=red><b>요청하신 작업을 수행할 수 없습니다.</b></font><br>전산정보팀으로 연락바랍니다.<br><label style='cursor:hand;' onClick='viewDetail();'><font color=yellow><b>[상세보기]</b></font></label>를 누르면 자세한 내용을 확인 할 수 있습니다."],
  ...
  ["USER-1024", "확정하시겠습니까?"], // Question    , YesNo
  ...
  ["USER-1037", "확정 취소하시겠습니까?"], // Question    , YesNo
  ...
  ["USER-1044", "Row Num (#{1})에 중복된 데이터가 있습니다."], // StopSign    , Ok
  ...
  ["USER-1064", "#{1}을/를 변경하면 #{2}이 삭제됩니다. <br> #{1}을/를 변경  하시겠습니까?"], // StopSign    , YESNO
  ...
  ["USER-1068", "#{1}마감되어 #{2}등록/수정이 불가능합니다."], // StopSign    , YESNO
  ...
  ["USER-1081", "브랜드 최저마진이 #{1}%입니다.<br>&nbsp;#{1}% 이상 입력하세요."], // StopSign    , Ok
  ...
  ["USER-1189", "동일 브랜드에 동일 단품은 등록할 수 없습니다."], // StopSign    , Ok
  ...
  ["USER-1218", "이미 확정된 발주전표 입니다. <br> 확정된 발주전표는 수정/삭제 할 수 없습니다."], // StopSign    , Ok
  ["USER-1219", "(#{1})상태인 전표입니다.<br> 재 조회하시기 바랍니다."], // StopSign    , Ok
  ["USER-1220", "#{1}마감되어 검품확정 할 수 없습니다."], // StopSign    , Ok
  ["USER-1221", "브랜드유형이 정상, F&B일 경우만 등록 가능합니다."], // StopSign    , Ok    
...
function showMessage() {
	...
	if (strMsgCode.indexOf("-") == -1){
		...
		if(blnFocus) setTimeout("LASTFOCUSEDOBJ.Focus()");
		return strReturn;
	}
	...
	switch (strErrorType) {
        case "SERVER" :	// SERVER TYPE ERROR
            ...
            if(hasFocusObject()) blnFocus = true;
			...
			for(var i=4,n=arguments.length;i<n;i++){ objParseMsg.push(arguments[i]);}
            ...
            rtn = showModalDialog("/edi/jsp/error.jsp", poparg,"dialogWidth:400px;dialogHeight:" + strErrDialogHeight + "px;scroll:no;resizable:yes;help:no;unadorned:no;center:yes;status:no;dialogHide:yes");
            if (blnSessionError) {
      ...
      if(hasFocusObject()) blnFocus = true;
        ...
        if(hasFocusObject()) blnFocus = true;
				...
				strMsg = _parseMessage(objParseMsg);
				strMsg = "&nbsp;" + strMsg;
				strServerMsg += "메세지 : " + error[0] + '\n';
				strServerMsg += "URL    : " + error[1] + '\n';
				strServerMsg += "라인   : " + error[2] + '\n';
  			poparg.push(strIcon);
  			poparg.push(strButton);
  			poparg.push(strMsgCode);
  			poparg.push(strMsg);
        ...
        if(hasFocusObject()) blnFocus = true;
				...
				var strServerMsg = "";
				objParseMsg.push(strMsgCode);
				for(var i=3, n=arguments.length;i<n;i++){ objParseMsg.push(arguments[i]); }
				strMsg = _parseMessage(objParseMsg);
				strMsg = "&nbsp;" + strMsg;
				poparg.push(strIcon);
  			poparg.push(strButton);
