...
//  }
  ...
  grName.UrlImages  = "<I>Type='PopupBotton', Url='search_s', Fit='Original', Flat='True'</I>" +
    "<I>Type='PopupCalendar', Url='/edi/imgs/icon/ico_calender.gif', Fit='Original', Flat='True'</I>" +
    "<I>Type='DisableIndicator', Url='/edi/imgs/icon/icon_blue.gif' Fit='Original'</I>" +
    "<I>Type='FocusIndicator', Url='/edi/imgs/icon/icon_red.gif' Fit='Original'</I>" +
    "<I>Type='NonFocusIndicator', Url='/edi/imgs/icons/icon_gray.gif' Fit='Original'</I>";
...
function openExcel(objGrid, strTitle, blnSupp){
    ...
    return window.showModalDialog("/edi/jsp/excel.jsp", obj,"dialogWidth:430px;dialogHeight:" + h + "px;scroll:no;resizable:no;help:no;unadorned:yes;center:yes;status:no");
}
...
function openExcel2(objGrid, strTitle, strCond, blnSupp){
    var obj = new Array();
    var dataset = eval(objGrid.DataID);
    if (dataset.CountRow == 0){
        showMessage(Information, Ok, "SCRIPT-1014");
...
function openExcel3(objGrid1, objGrid2, strTitle, strCond, strMasterInfo){
    ...
    return window.showModalDialog("/edi/jsp/gfbuy/excel.jsp", obj,"dialogWidth:430px;dialogHeight:" + h + "px;scroll:no;resizable:no;help:no;unadorned:yes;center:yes;status:no");
}
...
function openExcelM(objGrid, strTitle, strCond, blnSupp){
    ...
    var h = windowsXPSP2 == true ? "230" : "195";
    return window.showModalDialog("/edi/jsp/excelM.jsp", obj,"dialogWidth:430px;dialogHeight:" + h + "px;scroll:no;resizable:no;help:no;unadorned:yes;center:yes;status:no");
}
...
function openExcelS(objGrid, strTitle, strCond, blnSupp){
    ...
    var h = windowsXPSP2 == true ? "230" : "195";
    return window.showModalDialog("/edi/jsp/excelS.jsp", obj,"dialogWidth:430px;dialogHeight:" + h + "px;scroll:no;resizable:no;help:no;unadorned:yes;center:yes;status:no");
}
...
function getStore(strDataSet, authGb, bonsaGb, allGb) {
    ...
    TR_MAIN.Action    = "/edi/ccom901.cc?goTo=getStore";
...
function getDept(strDataSet, authGb, strStrCd, allGb, strMngOrgYn) {
    ...
    TR_MAIN.Action    = "/edi/ccom901.cc?goTo=getDept";
...
function getTeam(strDataSet, authGb,strStrCd, strDept, allGb, strMngOrgYn)  {    
    ...
    TR_MAIN.Action    = "/edi/ccom901.cc?goTo=getTeam";
...
function getPc(strDataSet, authGb,strStrCd, strDept, strTeam, allGb, strMngOrgYn)  {
    ...
    DS_I_CONDITION.NameValue(DS_I_CONDITION.RowPosition, "MNG_ORG_YN")  = strMngOrgYn;
    TR_MAIN.Action    = "/edi/ccom901.cc?goTo=getPc";
...
function getPumbun(strDataSet, authGb, strStrCd, strJojikGb, strBu, strGwa, strKye) {
    ...
    TR_MAIN.Action    = "/edi/tcom999.tc?goTo=getPumbun";
...
function getCorner(strDataSet, strStrCd, strDept, strTeam, strPc, allGb)  {
    ...
    TR_MAIN.Action    = "/edi/ccom901.cc?goTo=getCorner";
...
* 작 성 자 : 이종철(공통)
* 작 성 일 : 2006-08-17
* 개    요 : 브랜드 또는 PLU코드를 입력받아 권한에 적합한지를 판단
* 사용방법 : isAuthOK_PLU_PUMBUN(strDataSet, "Y", "0001", "0", "88", "01", "A" )
*            arguments[0] -> 저장할 DataSet의 ID
*            arguments[1] -> 점
*            arguments[2] -> 브랜드코드 또는 plu 코드
* return값 : void
*/
function isAuthOK_PLU_PUMBUN(strDataSet, strStrCd, strCode) {
    ...
    TR_MAIN.Action    = "/edi/tcom999.tc?goTo=isAuthOK_PLU_PUMBUN";
...
function getCorp(strDataSet) {
    TR_MAIN.Action    = "/edi/tcom999.tc?goTo=getCorp";
...
function initImgDataSet( imgDataSet ) {
    ...
    imgDataSet.DataId = "/edi/jsp/imageLoad.jsp";
    imgDataSet.Reset();  
}
...
function getEventCode(strDataSet, strCd, pumbunCd, prcAppDt, allGb) {
    ...
    TR_MAIN.Action    = "/edi/ccom000.cc?goTo=getEventCode";
...
*            arguments[2] -> 주거/비주거구분(2:주거, 1:비주거[아파트,CES])
...
* return값 : 코드/명
*/
function getFlc( strDataSet, strStaGbn, strHusFlag, strAuthGbn ,strAllGbn ){
    ...
    TR_MAIN.Action="/edi/ccom901.cc?goTo=getFlc";
...
function getBcompCode(strDataSet, sysPart, commPart, allGb, nulGb, objComboNm) {  
    ...
    TR_MAIN.Action  = "/edi/ccom901.cc?goTo=getBcompCode";
    TR_MAIN.KeyValue= "SERVLET(I:DS_I_COMMON=DS_I_COMMON,O:DS_O_COMMON="+strDataSet+")";
    TR_MAIN.Post();
}    
...
function getDcardCode(strDataSet, sysPart, commPart, allGb, nulGb, objComboNm) {  
    ...
    TR_MAIN.Action  = "/edi/ccom901.cc?goTo=getDcardCode";
    TR_MAIN.KeyValue= "SERVLET(I:DS_I_COMMON=DS_I_COMMON,O:DS_O_COMMON="+strDataSet+")";
    TR_MAIN.Post();
}    
...
function getVcardCode(strDataSet, sysPart, commPart, allGb, nulGb, objComboNm) {  
    ...
    TR_MAIN.Action  = "/edi/ccom901.cc?goTo=getVcardCode";
    TR_MAIN.KeyValue= "SERVLET(I:DS_I_COMMON=DS_I_COMMON,O:DS_O_COMMON="+strDataSet+")";
    TR_MAIN.Post();
}   
...
function getChrgCode(strDataSet, sysPart, commPart, allGb, nulGb, objComboNm) {  
    ...
    TR_MAIN.Action  = "/edi/ccom901.cc?goTo=getChrgCode";
    TR_MAIN.KeyValue= "SERVLET(I:DS_I_COMMON=DS_I_COMMON,O:DS_O_COMMON="+strDataSet+")";
    TR_MAIN.Post();
}    
...
function getBrchUserAuth(BRCH_ID) {
    ...
    var parameters  = "&dsName="  + dsName ;
        parameters += "&sqlId=GET_USER_AUTH";
        parameters += "&BRCH_ID=" + BRCH_ID ;
...
function getBizCdCombo(){
    ...
    dsSet.ClearData();          
    dsSet.DataID   = "/edi/ccom221.cc?goTo=getBizCdCombo"+parameters;
    dsSet.SyncLoad = "true";
    dsSet.Reset();
}
 ...
 *            arguments[1] -> 년월
 *            arguments[2] -> seq
 *            arguments[3] -> "Y" 전체 표시 "N" DEFAULT  
 *
 * 실행  예) getDivCombo(DS_DIV_CD, "201101", "1", "Y")
 * return값 :
*/
function getDivCombo(){
    ...
    dsSet.ClearData();          
    dsSet.DataID   = "/edi/ccom221.cc?goTo=getDivCombo"+parameters;
    dsSet.SyncLoad = "true";
    dsSet.Reset();
}
