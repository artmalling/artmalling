...
function closeCalendar() {
	oPopup.hide();
	try{
		if(document.getElementsByName("join1")[0].checked){
			ctlToPlaceValue.value =	(constructDate(dateSelected,monthSelected,yearSelected)).substring(0,4);
		}else if(document.getElementsByName("join1")[1].checked){
			ctlToPlaceValue.value =	(constructDate(dateSelected,monthSelected,yearSelected)).substring(0,7);
		}else{
...
function selectMonth(nMonth) {
	...
	//tp
	var sMon;
	//if(document.f.join1[1].checked)	{
	try{
		if(document.getElementsByName("join1")[1].checked)	{
			if( (nMonth + nStartingMonth) < 10  )	sMon = '0'+(nMonth + nStartingMonth);
			else sMon = (nMonth + nStartingMonth);
		
			ctlToPlaceValue.value = (yearSelected+'-'+sMon );
		}
	}catch(x){}
	...
	try{
		if(document.getElementsByName("join1")[1].checked)	oPopup.hide();
	}catch(x){}
}
...
function selectYear(nYear) {
	...
	//tp
	try{
		if(document.getElementsByName("join1")[0].checked)	ctlToPlaceValue.value = (nYear+nStartingYear);
	}catch(x){}
	...
	try{
		if(document.getElementsByName("join1")[0].checked)	oPopup.hide();
	}catch(x){}		
...
function popUpCalendar(ctl, ctl2, format) {
	...
	if(bPageLoaded) {
		...
		//tp
		var ctl2_NEW = '';
		if(ctl2.value.length == 4){
			ctl2_NEW = ctl2.value+"-01-01";
		}else if (ctl2.value.length == 7){
			ctl2_NEW = ctl2.value+"-01";
		}
		...
		aFormat	= dateFormat.split(formatChar);
			...
			if(formatChar != "") {
				aData =	ctl2_NEW.split(formatChar);
