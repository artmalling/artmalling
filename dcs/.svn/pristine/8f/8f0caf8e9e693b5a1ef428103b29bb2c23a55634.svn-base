<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 멤버쉽관리 > 포인트관리 > 부정적립예상조회(결제수단)
 * 작 성 일 : 2010.05.29
 * 작 성 자 : kj
 * 수 정 자 : 
 * 파 일 명 : dmbo6300.jsp
...
function doInit(){
    ...
    getEtcCode("DS_O_CUST_TYPE", "D", "D115", "Y");
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}     name="NO"               width=30    align=center  </FC>'
                     ...
                     + '<FC>id=CUST_GRADE   name="고객등급"        width=110   align=left     </FC>'
                     + '<FC>id=CUST_TYPE    name="고객유형"        width=110   align=left     </FC>'
                     + '<FC>id=CASH_CNT      name="현금결제건수"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"    width=120    align=right SumText=@sum </FC>'
                     + '<FC>id=CARD_CNT      name="카드종류건수"      gte_columntype="number:0:true" gte_Summarytype="number:0:true"   width=120   align=right SumText=@sum </FC>'
                     ;
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}      name="NO"          width=30    align=center  </FC>'
                     ...
                     + '<FC>id=TYPE_NAME     name="결제수단"      width=324   align=left</FC>'
...
function btn_Search() {
    ...
    if (trim(EM_CNT_S.text).length == 0){
        showMessage(EXCLAMATION, OK, "USER-1003",  "현금결제건수");
        EM_CNT_S.Focus();
        return;
    }
    
    if(parseInt(trim(EM_CNT_S.Text)) < 0){
        showMessage(EXCLAMATION, OK, "USER-1004",  "현금결제건수");
        EM_CNT_S.Focus();
        return;
    }
    EXCEL_PARAMS  = "시작일자=" + EM_S_DT_S.text;
    EXCEL_PARAMS += "-종료일자=" + EM_E_DT_S.text;
    EXCEL_PARAMS += "-현금결제건수=" + EM_CNT_S.text;
    
    EXCEL_PARAMS += "-회원유형=" + LC_CUST_TYPE.BindColVal;    
    ...
    parameters      = "&strSdt="      + encodeURIComponent(EM_S_DT_S.text)
    				...
    				+ "&strCustType=" + encodeURIComponent(LC_CUST_TYPE.BindColVal)
    				;    				
    				
    TR_MAIN.Action  = "/dcs/dmbo630.do?goTo="+goTo+parameters;  
...
function btn_Print() {
    ...
    window.open("/dcs/dmbo630.do?goTo=print"+parameters,"OZREPORT", 1000, 700);
}
 ...
 *************************************************************************/
 ...
 function doClick(row){
     ...
     TR_DETAIL.Action  = "/dcs/dmbo630.do?goTo="+goTo+parameters;  
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr valign="top">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			class="BD4A">
			<tr>
				<td><object id=GD_DETAIL width="100%" height=257
					classid=<%=Util.CLSID_GRID%> tabindex=1>
