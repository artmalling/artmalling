<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 멤버쉽운영 > VIP 라운지 > VIP 방문 등록
 * 작 성 일    : 2010.06.29
 * 작 성 자    : 김영진
 * 수 정 자    : 
 * 파 일 명    : dmbo7110.jsp
...
function doInit(){
    ...
    initEmEdit(EM_CRM_GRADE,   "GEN^40",     READ);       //CRM 회원등급
    initEmEdit(EM_VIP_REASON,  "GEN^100",    READ);       //회원등급사유

    initEmEdit(EM_BRCH_NM_D,   "GEN^50",     READ);        //가맹점명
    initEmEdit(EM_IN_DATE_D,   "0000/00/00 00:00:00", READ);//방문일시
    initEmEdit(EM_MEMBER_QTY_D,"NUMBER^1^0", PK);          //동반 고객수 
    initEmEdit(TXT_CUST_MEMO_D,"GEN^400",    PK);          //회원메모
    ...
    TXT_CUST_MEMO_D.Enable  = false;
    EM_MEMBER_QTY_D.Enable  = false;
    
    EM_CARD_NO_S.Focus();
    
    //화면에서 사용자가 입력,수정,삭제 한 데이터가 있는지 확인하는 함수-해당 페이지를 닫을 때 ‘저장하시겠습니까?’ 로직 처리를 위해 필요함
    registerUsingDataset("dmbo711","DS_O_MASTER,DS_IO_DETAIL");
}

function gridCreate1(){      
    var hdrProperies = '<FC>id={currow}        name="NO"           width=30    align=center</FC>'
                     + '<FC>id=BRCH_ID         name="가맹점ID"      width=100   align=center show=false</FC>' 
                     + '<FC>id=BRCH_NAME       name="가맹점명"      width=140   align=center</FC>' 
                     + '<FC>id=IN_DATE         name="방문일자"      width=110   align=center  mask="XXXX/XX/XX"</FC>'
                     + '<FC>id=IN_TIME         name="방문시간"      width=110   align=center  mask="XX:XX:XX"</FC>'
                     + '<FC>id=MEMBER_QTY      name="동반고객수"    width=90    align=right</FC>'
                     + '<FC>id=CUST_MEMO       name="회원메모"      width=297   align=left</FC>'
                     + '<FC>id=VISIT_IN_DATE   name="방문일자시간"  width=100   align=center show=false</FC>'
                     + '<FC>id=CUST_ID         name="회원"          width=100   align=center show=false</FC>';
...
function btn_Search() {
    ...
    if (DS_O_MASTER.IsUpdated || DS_IO_DETAIL.IsUpdated || strChangFlag != false){
        if(showMessage(EXCLAMATION, YESNO, "USER-1073") != 1 ){
            setTimeout("EM_MEMBER_QTY_D.Focus();",50);
            return false;
        }else{
            strChangFlag = true;
        }
    }
    if(trim(EM_SS_NO_S.Text).length == 0 && trim(EM_CUST_ID_S.Text).length == 0 && trim(EM_CARD_NO_S.Text).length == 0){
...
function btn_New() {
	if(strErrorMsg == "Y"){
		showMessage(EXCLAMATION, OK, "USER-1000",  "VIP 회원이 아닙니다.");
		EM_CARD_NO_S.Focus();
        return;
	}
    ...
    if (DS_O_MASTER.IsUpdated || DS_IO_DETAIL.IsUpdated){
        if(showMessage(EXCLAMATION, YESNO, "USER-1072") != 1 ){
            ...
            strChangFlag = true;
            if(DS_O_MASTER.NameValue(DS_O_MASTER.CountRow, "IN_DATE") == ""){
    ...
    for(i = 0 ; i < DS_O_MASTER.CountRow ; i++){
        var strGridDt = DS_O_MASTER.NameValue(i ,"IN_DATE");
        if("<%=strToDate%>" == strGridDt){
            intGridDtCnt++;
        }
    }
    if(intGridDtCnt > 1){
        showMessage(EXCLAMATION, OK, "USER-1000",  "하루 1회만 이용 가능합니다.");
        GR_MASTER.Focus();
        return;
    }
    DS_O_MASTER.Addrow(); 
    ...
    DS_IO_DETAIL.AddRow();
    EM_MEMBER_QTY_D.Enable = true;
    TXT_CUST_MEMO_D.Enable = true;
    EM_BRCH_NM_D.TEXT = '<c:out value="${sessionScope.sessionInfo.BRCH_ID}" />'; 
    DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition ,"BRCH_ID") = '<c:out value="${sessionScope.sessionInfo.BRCH_ID}" />';
    DS_IO_DETAIL.NameValue(DS_IO_DETAIL.RowPosition ,"CUST_ID") = strCustId;
    strChangFlag = false;
    EM_MEMBER_QTY_D.Focus();
    intChangRow = 1;
    bfListRowPosition = 0;
}
...
function btn_Save() {
    ...
    if(trim(EM_MEMBER_QTY_D.Text).length == 0){
        showMessage(EXCLAMATION, OK, "USER-1002",  "동반 고객수");
        EM_MEMBER_QTY_D.Focus();
        return;
    }
    if(parseInt(EM_MEMBER_QTY_D.Text) > 4){
         showMessage(EXCLAMATION, OK, "USER-1000",  "동반 고객수는 VIP회원 포함 4인까지 가능합니다.");
         EM_MEMBER_QTY_D.Focus();
         return;
    }
    if(trim(TXT_CUST_MEMO_D.Text).length == 0){
        showMessage(EXCLAMATION, OK, "USER-1002",  "회원메모");
        TXT_CUST_MEMO_D.Focus();
        return;
    }
...
function showMaster(){
    ...
    TR_MAIN.Action  ="/dcs/dmbo711.do?goTo="+goTo+parameters;  
    TR_MAIN.KeyValue="SERVLET("+action+":DS_O_MASTER=DS_O_MASTER,"+action+":DS_O_GRADE=DS_O_GRADE)"; //조회는 O
    TR_MAIN.Post();
...
function doClick(row){
    ...
    var strInDate   = DS_O_MASTER.NameValue(row ,"VISIT_IN_DATE");
    var strBrchId   = DS_O_MASTER.NameValue(row ,"BRCH_ID");
    var goTo        = "searchDetail";    
    var action      = "O";     
    var parameters  = "&strCustId="   + encodeURIComponent(strCustId)
                    + "&strInDate="   + encodeURIComponent(strInDate)
                    + "&strBrchId="   + encodeURIComponent(strBrchId);
    TR_DETAIL.Action  = "/dcs/dmbo711.do?goTo="+goTo+parameters;  
    TR_DETAIL.KeyValue= "SERVLET("+action+":DS_IO_DETAIL=DS_IO_DETAIL)"; //조회는 O
    TR_DETAIL.Post();
    
    for(i = 0 ; i < DS_O_MASTER.CountRow ; i++){
        var strGridDt = DS_O_MASTER.NameValue(DS_O_MASTER.RowPosition ,"IN_DATE");
        if("<%=strToDate%>" == strGridDt){
        	 TXT_CUST_MEMO_D.Enable  = true;
        	 EM_MEMBER_QTY_D.Enable  = true;
        }else{
...
function saveData(){
    var goTo        = "save";
    var action      = "I";  //조회는 O
    TR_MAIN.Action  ="/dcs/dmbo711.do?goTo="+goTo;   
...
<script language=JavaScript for=DS_O_MASTER event=CanRowPosChange(row)>
    if ( strChangFlag == false && (DS_O_MASTER.IsUpdated || DS_IO_DETAIL.IsUpdated)){
        if( showMessage(QUESTION, YESNO, "USER-1074") != 1 ){
            ...
            intChangRow = 0;
            if(DS_O_MASTER.NameValue(row, "IN_DATE") == "")
...
<script>_ws_(_NSID_);</script>
<comment id="_NSID_">
<object id="DS_O_GRADE" classid=<%=Util.CLSID_DATASET%>></object>
</comment>
<script>_ws_(_NSID_);</script>
<!-- ===============- 공통함수용 -->
<comment id="_NSID_">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0"
                    class="s_table">
                    <tr>
                        <th width="80">회원등급사유</th>
                        <td width="420"><comment id="_NSID_"> <object
                            id=EM_VIP_REASON classid=<%=Util.CLSID_EMEDIT%> width=401>
                        </object> </comment> <script> _ws_(_NSID_);</script></td>
                        <th width="80">CRM회원등급</th>
                        <td><comment id="_NSID_"> <object id=EM_CRM_GRADE
                            classid=<%=Util.CLSID_EMEDIT%> width=175> </object> </comment> <script> _ws_(_NSID_);</script></td>
                    </tr>
                </table>
                </td>
            </tr>
    ...
    <tr>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0"
            ...
            <tr>
                <th width="80">가맹점명</th>
                <td width="160"><comment id="_NSID_"> <object
                    id=EM_BRCH_NM_D classid=<%=Util.CLSID_EMEDIT%> width=140>
                </object> </comment> <script> _ws_(_NSID_);</script> </td>
                <th width="80">방문일시</th>
                <td width="160"><comment id="_NSID_"> <object
                    id=EM_IN_DATE_D classid=<%=Util.CLSID_EMEDIT%> width=140>
                </object> </comment> <script> _ws_(_NSID_);</script></td>
                <th width="80" class="point">동반 고객수</th>
                <td><comment id="_NSID_"> <object
                    id=EM_MEMBER_QTY_D classid=<%=Util.CLSID_EMEDIT%> width=60 tabindex=1>
                </object> </comment> <script> _ws_(_NSID_);</script>&nbsp;(VIP 포함 인원수)</td>
            </tr>
            <tr>
                <th width="80" class="point">회원메모</th>
                <td colspan="5"><comment id="_NSID_"> <object
                    id=TXT_CUST_MEMO_D classid=<%=Util.CLSID_TEXTAREA%>
                    style="width: 100%; height: 45px;" tabindex=1
                    onkeyup="javascript:checkByteStr(TXT_CUST_MEMO_D, 400,'Y');">
                </object> </comment> <script> _ws_(_NSID_);</script></td>
            </tr>
        </table>
        </td>
...
<object id=BD_DETAIL classid=<%=Util.CLSID_BIND%>>
    ...
    <param name=BindInfo
        value='
            <c>col=BRCH_NAME      ctrl=EM_BRCH_NM_D         Param=Text</c>
            <c>col=IN_DATE        ctrl=EM_IN_DATE_D         Param=Text</c>
            <c>col=MEMBER_QTY     ctrl=EM_MEMBER_QTY_D      Param=Text</c>
            <c>col=CUST_MEMO      ctrl=TXT_CUST_MEMO_D      Param=Text</c>
        '>
</object>
</comment>
<script>_ws_(_NSID_);</script>
...
<comment id="_NSID_">
<object id=BD_GRADE classid=<%=Util.CLSID_BIND%>>
    <param name=DataID value=DS_O_GRADE>
    <param name="ActiveBind" value=true>
    <param name=BindInfo
        value='
            <c>col=CRM_GRADE      ctrl=EM_CRM_GRADE        Param=Text</c>
            <c>col=VIP_REASON     ctrl=EM_VIP_REASON       Param=Text</c>
        '>
</object>
</comment>
<script>_ws_(_NSID_);</script>
