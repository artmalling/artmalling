<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 멤버쉽운영 > 승인처리 > 상품권교환 반품
 * 작  성  일  : 2016.11.07
 * 작  성  자  : KHJ
 * 수  정  자  : 
 * 파  일  명  : dmbo7190.jsp
...
function doInit(){
	...
	var obj   = document.getElementById("GR_MASTER2"); 
...
//     initEmEdit(EM_SUM_POINT,   "NUMBER^9^0", READ);       //누적+발생포인트
    ...
    initEmEdit(EM_GIFTCERT_AMT,   "NUMBER^9^0",     NORMAL);       //
    initEmEdit(EM_GIFT_TYPE_CD,   "GEN^40",     NORMAL);       //
    initEmEdit(EM_ISSUE_TYPE,   "GEN^40",     NORMAL);       //
    initEmEdit(EM_GIFT_AMT_TYPE,   "GEN^40",     NORMAL);       //
    initEmEdit(EM_SALE_STR,   "GEN^40",     NORMAL);       //
    initEmEdit(EM_SALE_DT,   "GEN^40",     NORMAL);       //
    

    EM_GIFTCERT_AMT.style.display     = "none";
    EM_GIFT_TYPE_CD.style.display     = "none";
    EM_ISSUE_TYPE.style.display     = "none";
    EM_GIFT_AMT_TYPE.style.display     = "none";
    EM_SALE_STR.style.display     = "none";
    EM_SALE_DT.style.display     = "none";
    

    initEmEdit(EM_PWD_NO,      "0000",       PK);         //비밀번호
    
    getEtcCode("DS_PROC_GB","M",  "M020", "N");   //교환반품구분
    ...
    registerUsingDataset("dmbo719","DS_I_POINT,DS_I_MASTER");
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}         name="NO"             width=30    align=center   edit=none</FC>'
			    	 + '<FC>id=SAVE_CHK         name="선택"           width="40"  align=center   EditStyle=CheckBox  Edit={IF(STATUS_GB="T", "false",  IF(PROC_GBN="01","true","false"))} </FC>'
			         + '<FC>id=PROC_DT          name="교환일자"       width=100   align=center   edit=none   mask="XXXX/XX/XX"</FC>'
    	             + '<FC>id=SEQ_NO           name="순번"           width=100   align=center   edit=none </FC>'
                     + '<FC>id=CONV_GIFT_AMT   	name="상품권금액"     width=100   align=right    edit=none </FC>'
                     + '<FC>id=CONV_POINT       name="교환포인트"     width=100   align=right    edit=none </FC>'
                     + '<FC>id=STATUS_GB        name="반품여부"       width=80    align=center   edit=none  EditStyle=CheckBox </FC>'
                     + '<FC>id=PROC_GBN         name="교환/반품구분"  width=80    align=center   edit=none  EditStyle=Lookup  Data="DS_PROC_GB:CODE:NAME"</FC>'
                     + '<FC>id=RTN_DT           name="반품일자"       width=100   align=center   edit=none   mask="XXXX/XX/XX"</FC>'
    	             + '<FC>id=RTN_SEQ_NO       name="반품순번"       width=100   align=center   edit=none </FC>'
                     ;
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}         name="NO"                    width=30     align=center</FC>'
                     ...
                     + '<FC>id=PROC_DT          name="반품일자"              width=120    align=center   edit=none   mask="XXXX/XX/XX"</FC>'
                     + '<FC>id=BRCH_ID          name="가맹점코드"            width=100    show=false  align=left     edit=none   show=false</FC>'
                     + '<FC>id=BRCH_NAME        name="가맹점명"              width=186    show=false  align=left     edit=none</FC>'
                     + '<FC>id=GIFT_TYPE_CD     name="상품권종류"            width=90     show=false  align=left     edit=none</FC>'
                     + '<FC>id=ISSUE_TYPE       name="발행형태"              width=60     show=false  align=left     edit=none</FC>'
                     + '<FC>id=GIFT_AMT_TYPE    name="금종"                  width=90     show=false  align=left     edit=none</FC>'
                     + '<FC>id=SALE_STR         name="판매점"                width=90     align=left     edit=none   EditStyle=Lookup  Data="DS_STR_CD:CODE:NAME"  </FC>'
                     + '<FC>id=SALE_DT          name="판매일"                width=90     align=center     edit=none    mask="XXXX/XX/XX"</FC>'
                     + '<FC>id=GIFT_SYS_YN      name="상품권발급등록여부"    width=150    show=false   align=center   edit=none</FC>';
...
function btn_Save() {
    ...
    if (!CARD_NO_EXIST) {
        showMessage(StopSign, OK, "USER-1000",  "유효한 카드가 존재하지 않는 회원입니다. 상품권교환반품을 할 수 없습니다.");
        return;
    }
    ...
    // 상품권 번호 자리수 확인 / 상품권마스터 존재여부(금액이씨으면존재)
    for(var i=1; i<=DS_I_MASTER.CountRow; i++){ 
        ...
        if ( parseInt(DS_I_MASTER.NameValue(i, "GIFTCERT_AMT")) == 0) {
        ...
        intGiftCardAmt = intGiftCardAmt + parseInt(DS_I_MASTER.NameValue(i, "GIFTCERT_AMT"));
    ...
    for (i=1; i < DS_O_MASTER.CountRow+1 ; i++){
	    if(DS_O_MASTER.NameValue(i, "SAVE_CHK") == 'T'){
	        intFlagCnt++; 
	        intConvGiftAmt 	= intConvGiftAmt + (parseInt(DS_O_MASTER.NameValue(i, "CONV_GIFT_AMT")));
	    }
    }
    
    if(intFlagCnt == 0){
        showMessage(EXCLAMATION, OK, "USER-1000", " 상품권 교환 내역을 선택하세요.");
        return;
    }
    ...
    if (strGiftNoYn == "Y") {
        showMessage(StopSign, OK, "USER-1000",  "동일한 상품권번호가 존재합니다. 상품권교환반품을 할 수 없습니다.");
        strGiftNoYn = "N";
        return;
    }
    ...
    if( showMessage(QUESTION, YESNO, "USER-1000", "상품권교환반품을 처리하시겠습니까?") != 1 ){
        return;
    }
    //saveData(); 11.08.11    
    realSave();
...
function btn_New() {
     ...
     EM_GIFTCERT_AMT.Text   = "";
     EM_GIFT_TYPE_CD.Text   = "";
     EM_ISSUE_TYPE.Text   = "";
     EM_GIFT_AMT_TYPE.Text   = "";
     EM_SALE_STR.Text   = "";
     EM_SALE_DT.Text   = "";
...
function saveData(){
    ...
    var goTo        = "chkSave";
    var action      = "I";  //조회는 O
    TR_MAIN2.Action  ="/dcs/dmbo719.do?goTo="+goTo;   
 ...
 function realSave(){
    ...
    for (i=1; i < DS_O_MASTER.CountRow+1 ; i++){
    	if(DS_O_MASTER.NameValue(i, "SAVE_CHK") == 'T'){
	    	...
	    	intConvPoint 	= intConvPoint + (parseInt(DS_O_MASTER.NameValue(i, "CONV_POINT")));
	    	intConvGiftAmt 	= intConvGiftAmt + (parseInt(DS_O_MASTER.NameValue(i, "CONV_GIFT_AMT")));
    	}
    }

	DS_I_POINT.NameValue(DS_I_POINT.RowPosition, "CARD_NO") 		= strCardNo;
	DS_I_POINT.NameValue(DS_I_POINT.RowPosition, "CONV_POINT") 		= intConvPoint * (-1);
    ...
    var goTo        = "realSave";
    var action      = "I";  //조회는 O
    TR_MAIN.Action  ="/dcs/dmbo719.do?goTo="+goTo;   
    TR_MAIN.KeyValue="SERVLET("+action+":DS_I_MASTER=DS_I_MASTER,"+action+":DS_I_POINT=DS_I_POINT,"+action+":DS_O_MASTER=DS_O_MASTER)"; 
    TR_MAIN.Post();
}
...
function showMaster(){
    ...
    TR_MAIN.Action  ="/dcs/dmbo719.do?goTo="+goTo+parameters;  
...
function searchAmt(){
    ...
    TR_MAIN3.Action  ="/dcs/dmbo719.do?goTo="+goTo+parameters;  
    ...
    if(intTRow > 0){
	   	for (i=1; i < DS_IO_RULE.CountRow+1 ; i++){
	   		for(j=1; j < parseInt(DS_IO_RULE.NameValue(i, "CONV_QTY"))+1 ; j++){
	            ...
	            DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "GIFTCERT_AMT") 	= parseInt(DS_IO_RULE.NameValue(i, "CONV_GIFT_AMT"));   			
	   		} 
	   	}
    }   	
...
<script language=JavaScript for=TR_MAIN3 event=onSuccess>
	DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "GIFTCERT_AMT") = EM_GIFTCERT_AMT.Text;
	DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "GIFT_TYPE_CD") = EM_GIFT_TYPE_CD.Text;
	DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "ISSUE_TYPE") = EM_ISSUE_TYPE.Text;
	DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "GIFT_AMT_TYPE") = EM_GIFT_AMT_TYPE.Text;
	DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "SALE_STR") = EM_SALE_STR.Text;
	DS_I_MASTER.NameValue(DS_I_MASTER.RowPosition, "SALE_DT") = EM_SALE_DT.Text;
</script>
<!--------------------- 2. TR Fail 메세지 처리  ------------------------------->
...
<script>_ws_(_NSID_);</script>
<!-- ===============- Output용 -->
<comment id="_NSID_"><object id="DS_STR_CD" classid=<%=Util.CLSID_DATASET%>>	<param name=UseFilter value=true></object></comment><script>_ws_(_NSID_);</script>
...
<comment id="_NSID_"><object id="DS_PROC_GB" classid=<%= Util.CLSID_DATASET %>></object></comment><script>_ws_(_NSID_);</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
        <td class="sub_title PT07 PB02"><img
            ...
            align="absmiddle" class="PR03" />상품권 교환 내역</td>
    </tr>
    <tr valign="top">
        <td class="PB03">
        <table width="100%" border="0" cellspacing="0" cellpadding="0"
            ...
            <tr>
                <td><comment id="_NSID_"> <object id=GR_MASTER2
                    width="100%" height=150 classid=<%=Util.CLSID_GRID%> tabindex=1>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td class="right"><img src="/pot/imgs/btn/add_row.gif"
				    ...
				    <comment id="_NSID_"> <object id=EM_GIFTCERT_AMT classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1> 
					</object> </comment> <script> _ws_(_NSID_);</script>
				    <comment id="_NSID_"> <object id=EM_GIFT_TYPE_CD classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1> 
					</object> </comment> <script> _ws_(_NSID_);</script>
				    <comment id="_NSID_"> <object id=EM_ISSUE_TYPE classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1> 
					</object> </comment> <script> _ws_(_NSID_);</script>
				    <comment id="_NSID_"> <object id=EM_GIFT_AMT_TYPE classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1> 
					</object> </comment> <script> _ws_(_NSID_);</script>
				    <comment id="_NSID_"> <object id=EM_SALE_STR classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1> 
					</object> </comment> <script> _ws_(_NSID_);</script>
				    <comment id="_NSID_"> <object id=EM_SALE_DT classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1> 
					</object> </comment> <script> _ws_(_NSID_);</script>
...
<object id=BD_AMT classid=<%=Util.CLSID_BIND%>>
    ...
    <param name=BindInfo
        value='
            <c>col=GIFTCERT_AMT       ctrl=EM_GIFTCERT_AMT       Param=Text</c>
            <c>col=GIFT_TYPE_CD       ctrl=EM_GIFT_TYPE_CD       Param=Text</c>
            <c>col=ISSUE_TYPE         ctrl=EM_ISSUE_TYPE         Param=Text</c>
            <c>col=GIFT_AMT_TYPE      ctrl=EM_GIFT_AMT_TYPE      Param=Text</c>
            <c>col=SALE_STR           ctrl=EM_SALE_STR           Param=Text</c>
