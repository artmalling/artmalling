 ...
 * @caused   by 
 */
public class DMbo619DAO extends AbstractDAO {
    ...
    public int realSave(ActionForm form, MultiInput mi[], HttpServletRequest request) throws Exception {
        ...
        try {
			...
			if(result == 0){
				...
				System.out.println("DMbo619DAO saveData : rtnCode = " + rtnCode);
			}else{

				rollback();
			    throw new Exception("|[USER]" +result + "상품권교환 : 포인트 사용에 실패하였습니다");	
			}
			//rollback();
			...
			if(rtnCode > 0){	
				...
				rollback();
			    throw new Exception("|[USER]" +result + "상품권교환 : 상품권 등록에 실패하였습니다");	
			}
			
			System.out.println("DMbo619DAO proCall : retstr = " + retstr);
