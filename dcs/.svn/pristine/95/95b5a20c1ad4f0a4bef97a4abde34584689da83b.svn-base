<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 멤버쉽운영 > VIP 라운지 > 일자별 VIP 방문고객조회
 * 작 성 일    : 2010.06.30
 * 작 성 자    : 김영진
 * 수 정 자    : 
 * 파 일 명    : dmbo7120.jsp
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}       name="NO"             width=30      align=center</FC>'
    	             ...
    	             + '<FC>id=IN_DATE        name="일자"           width=78      align=center   Mask="XXXX/XX/XX"  SumText= "합계"</FC>'
    	             + '<FC>id=VISIT_IN_DATE  name="방문시간"       width=100     align=center   show=false</FC>'
                     + '<FC>id=VIP_QTY        name="VIP방문횟수"    width=75      align=right    SumText=@sum</FC>'
                     + '<FC>id=MEMBET_TQTY    name="총방문자수"     width=70      align=right    SumText=@sum</FC>';
...
function gridCreate2(){
    var hdrProperies = '<FC>id={currow}       name="NO"                 width=30      align=center</FC>'
                     ...
                     + '<FC>id=CUST_GRADE     name="카드회원등급"       width="80"    align=center</FC>'
                     + '<FC>id=CRM_GRADE      name="CRM회원등급"        width="80"    align=center</FC>'
                     + '<FC>id=IN_DATE        name="방문일자"           width="80"    align=center    mask="XXXX/XX/XX" show=false</FC>'
                     + '<FC>id=IN_TIME        name="방문시간"           width="70"    align=center    mask="XX:XX:XX"</FC>'
                     + '<FC>id=MEMBER_QTY     name="방문자수"           width="60"    align=right</FC>';
...
function showMaster(){
    ...
    TR_MAIN.Action  ="/dcs/dmbo712.do?goTo="+goTo+parameters;  
...
function doClick(row){
    ...
    var strVisitInDate = DS_O_MASTER.NameValue(row ,"IN_DATE");
    var strBrchId      = DS_O_MASTER.NameValue(row ,"BRCH_ID");
    
    var goTo        = "searchDetail";    
    var action      = "O";     
    var parameters  = "&strVisitInDate=" + encodeURIComponent(strVisitInDate)
                    + "&strBrchId="      + encodeURIComponent(strBrchId);
    TR_DETAIL.Action  = "/dcs/dmbo712.do?goTo="+goTo+parameters;  
