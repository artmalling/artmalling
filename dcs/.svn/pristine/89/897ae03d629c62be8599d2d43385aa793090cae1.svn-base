...
//     initEmEdit(EM_SUM_POINT,   "NUMBER^9^0",          READ);       //누적+발생포인트
    ...
    registerUsingDataset("dctm213","DS_IO_MASTER");
}
 
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}         name="NO"        width=30     align=center  </FC>'
                     + '<FC>id=SAVE_CHK         name="선택"      width=50     align=center  Edit={IF(FLAG="Y","true","false")}  EditStyle=CheckBox </FC>'
                     + '<FC>id=CARD_NO          name="카드번호"   width=165   align=center  Edit=none  mask="XXXX-XXXX-XXXX-XXXX"</FC>'
                     + '<FC>id=REP_CARD_YN      name="대표카드"   width=55    align=center  show=false  Edit=none  </FC>'
                     + '<FC>id=CARD_TYPE_NM     name="카드종류"   width=85    align=left    Edit=none</FC>'
                     + '<FC>id=CARD_STAT_NM     name="카드상태"   width=78    align=center  Edit=none</FC>'
                     + '<FC>id=ISSUE_DT         name="발급일자"   width=90    align=center  Edit=none mask="XXXX/XX/XX"</FC>'
                     + '<FC>id=BRCH_NAME        name="가입경로"   width=90   align=left     Edit=none</FC>'
                     + '<FC>id=CUST_NAME        name="신고자"     width=100   align=left    Edit=none</FC>'
                     + '<FC>id=LOSS_DT          name="신고일자"   width=90    align=center  Edit=none mask="XXXX/XX/XX"</FC>';
                     
                     + '<FC>id=FLAG             name="hidden"    width="165" show=false  Edit=none </FC>';
                     + '<FC>id=CARD_TYPE_CD     name="hidden"    width="165" show=false  Edit=none </FC>';
                     + '<FC>id=CARD_STAT_CD     name="hidden"    width="165" show=false  Edit=none </FC>';
...
function btn_Search() {
    ...
    TR_MAIN.Action  = "/dcs/dctm213.dm?goTo="+goTo+parameters;  
    ...
    if (DS_IO_MASTER.CountRow > 0 ) {
    	...
    	setFocusGrid(GD_MASTER, DS_IO_MASTER,1,'')
    }
    else 
        EM_CARD_NO_S.Focus();   
}
...
function btn_Save() {
    ...
    var goTo        = "saveData";
    var action      = "I";  //조회는 O
    TR_MAIN.Action  ="/dcs/dctm213.dm?goTo="+goTo;   
...
<script language=JavaScript for=RD_COMP_PERS_FLAG_S event=OnClick()>
    ...
    if (strCompPersFlag != this.CodeValue) {
        if (this.CodeValue == "P") {
            GD_MASTER.ColumnProp("CARD_NO",      "Width") += 15;
            GD_MASTER.ColumnProp("CARD_STAT_NM", "Width") += 10;
            GD_MASTER.ColumnProp("ISSUE_DT",     "Width") += 10;
            GD_MASTER.ColumnProp("BRCH_NAME",    "Width") += 10;
            GD_MASTER.ColumnProp("LOSS_DT",      "Width") += 10;

            GD_MASTER.ColumnProp("REP_CARD_YN", "Show") = false;
        } else {
            GD_MASTER.ColumnProp("CARD_NO",      "Width") -= 15;
            GD_MASTER.ColumnProp("CARD_STAT_NM", "Width") -= 10;
            GD_MASTER.ColumnProp("ISSUE_DT",     "Width") -= 10;
            ...
            GD_MASTER.ColumnProp("REP_CARD_YN", "Show") = true;
        }
    }
    if (strCompPersFlag == this.CodeValue) return;
    if ("P" == this.CodeValue) { 
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0"
            ...
            <tr>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0"
                    ...
                    <tr>
                    	<th width="57">카드번호</th>
                        <td colspan="5" width="170"><comment id="_NSID_"> <object 
