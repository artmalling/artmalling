<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 회원관리 > 멤버쉽운영 > EXCEL UPLOAD 적립
 * 작 성 일    : 2010.03.29
 * 작 성 자    : jinjung.kim
 * 수 정 자    : 
 * 파 일 명    : DMBO6230.jsp
...
function gridCreate1() {
    var hdrProperies = '<FC>id={currow}     name="NO"            width=30   align=center   BgColor={if(ERR_MSG = "","white","orange")} </FC>'
                     + '<FC>id=BRCH_ID      name="가맹점"        width=90   align=center   show=false</FC>'
                     + '<FC>id=BRCH_NAME    name="가맹점명"      width=110  align=left     SumText="합계"   BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=CARD_NO      name="카드번호"      width=140  align=center   mask="XXXX-XXXX-XXXX-XXXX" BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=SS_NO        name="생년월일"      width=120  align=center   BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=RECP_NO      name="영수증번호"    width=150  align=center   BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=SALE_DT      name="영업일자"      width=80   align=center   mask="XXXX/XX/XX" BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=FLAG         name="적립유형"      width=60   align=center   BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=ADD_POINT    name="적립포인트"    width=84   align=right    SumText=@sum  BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=ERR_CNT      name="오류건수"      width=60   align=right    BgColor={if(ERR_MSG = "","white","orange")}</FC>'
                     + '<FC>id=ERR_MSG      name="오류내용"      width=230  align=left     BgColor={if(ERR_MSG = "","white","orange")}</FC>';
...
function btn_Search() {
    ...
    TR_SEARCH.Action   = "/dcs/dmbo623.do?goTo=" + goTo + parameters;  
...
function btn_Conf() {
    ...
    for (var i = 1; i <= DS_IO_MASTER.CountRow; i++) {
        if (DS_IO_MASTER.RowStatus(i) == 0) {
            DS_IO_MASTER.NameValue(i, "OP_CODE") = "U";
        }
    }
    
    var goTo = "confData";
    TR_MASTER.Action  ="/dcs/dmbo623.do?goTo="+goTo;
...
function btn_Excel() {
	for (var i = 1; i <= DS_IO_MASTER.CountRow; i++) {
        DS_IO_MASTER.RowStatus(i) = 1;
	}
    GR_MASTER.GridToExcelExtProp("HideColumn") = "{currow}";
    var ExcelTitle = "EXCEL UPLOAD 미확정자료";
...
function downFile() {    
    var strPath   = "";    
    var strFileNm = "DMBO623_EXCEL_UpLoad_form.xls";       
    if( strPath != null  ) {                          
        location.href="/dcs/jsp/fileDownload.jsp?strPath="+strPath+"&strFileNm="+strFileNm;
    }   
}
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
              <tr>
                ...
                <td>
                 ...
                 -->
                 <a href="/dcs/samplefiles/DMBO623_EXCEL_UpLoad_form.xls"> <img
