 ...
 */
 ...
 function doInit(){
	...
	//Master 그리드 세로크기자동조정  2013-07-17
     ...
     // EMedit에 초기화
     initEmEdit(EM_SFROM_DT, "YYYYMMDD", PK);            // 시작일
     initEmEdit(EM_STO_DT, 	"YYYYMMDD", PK);            // 종료일
     ...
     EM_SFROM_DT.Text = strToday;
     EM_STO_DT.Text = strToday;
 ...
 function gridCreate1(){
     var hdrProperies = '<C>id={currow}    name="NO"			width=30    Edit=none  align=center</C>'
                      ...
                      + '<C>id=SALE_AMT  name="기간누계매출"     width=120    Edit=none sumtext=@sum align=right</C>'
                      + '<C>id=ENTR_SALE_AMT  name="가입일자매출"     width=120    Edit=none sumtext=@sum align=right</C>'
                      ;
...
function btn_Search() {
    ...
    var strsFromDt       = EM_SFROM_DT.text;      // 매출시작일자
    var strsToDt         = EM_STO_DT.text;        // 매출종료일자
    ...
    var parameters =  "&strStrCd="	+encodeURIComponent(strStrCd)
				    ...
				    + "&strsFromDt=" +encodeURIComponent(strsFromDt)
				    + "&strsToDt="   +encodeURIComponent(strsToDt); 
    
    TR_MAIN.Action  = "/dcs/dmbo633.do?goTo="+goTo+parameters;  
...
function btn_Excel() {
    ...
    var strFromDt       = EM_FROM_DT.Text;         // 시작일자
    var strToDt         = EM_TO_DT.Text;           // 종료일자
    var strsFromDt       = EM_SFROM_DT.Text;         // 시작일자
    var strsToDt         = EM_STO_DT.Text;           // 종료일자
	...
	var parameters = "";
		parameters  = "점="+strStrCd                    
			    	...
			    	+ "~"+ strsToDt
			    	;
	openExcel2(GR_MASTER, "시럽가입회원list", parameters, true );
}
 ...
 function checkValidation(Gubun) {
	 switch (Gubun){
	    case "search" :
	        ...
	        if (isNull(EM_SFROM_DT.Text) ==true ) {
	            showMessage(Information, OK, "USER-1003","시작일자"); 
	            EM_FROM_DT.focus();
	            return false;
	        }
	        ...
	        if (EM_SFROM_DT.Text.replace(" ","").length != 8 ) {
	            showMessage(Information, OK, "USER-1027","시작일자","8");
	            EM_FROM_DT.focus();
	            return false;
	        }
	        ...
	        if(!checkYYYYMMDD(EM_SFROM_DT.Text)){
	        	showMessage(Information, OK, "USER-1004","시작일자");
	        	EM_FROM_DT.focus();
	            return false;
	        }
	        ...
	        if (EM_STO_DT.Text.replace(" ","").length != 8 ) {
	            showMessage(Information, OK, "USER-1027","종료일자","8");
	            EM_TO_DT.focus();
	            return false;
	        }
	        ...
	        if(!checkYYYYMMDD(EM_STO_DT.Text)){
	            showMessage(Information, OK, "USER-1004","종료일자");
	            EM_TO_DT.focus();
	            return false;
	        }
	        ...
	        if(!isBetweenFromTo(EM_SFROM_DT.Text, EM_STO_DT.Text)){
	            showMessage(INFORMATION, OK, "USER-1015"); 
	            EM_FROM_DT.focus();
	            return false;
	        }
      ...
      <tr>
        <td><table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
          <tr>
            ...
            <td>      
                  <comment id="_NSID_">
                      <object id=EM_SFROM_DT classid=<%=Util.CLSID_EMEDIT%>  width=110 tabindex=1 align="absmiddle">
                      </object>
                  </comment><script> _ws_(_NSID_);</script>
                  <img src="/<%=dir%>/imgs/btn/date.gif" id="IMG_FROM_DT" onclick="javascript:openCal('G',EM_SFROM_DT)" align="absmiddle" />
                  ~
                  <comment id="_NSID_">
                      <object id=EM_STO_DT classid=<%=Util.CLSID_EMEDIT%>  width=110 tabindex=1 align="absmiddle">
                      </object>
                  </comment><script> _ws_(_NSID_);</script>
                  <img src="/<%=dir%>/imgs/btn/date.gif" id="IMG_TO_DT" onclick="javascript:openCal('G',EM_STO_DT)" align="absmiddle" />
            </td>
            <!--  <th width="60" class="point">회원구분</th>
            <td width="130">
                    <comment id="_NSID_">
