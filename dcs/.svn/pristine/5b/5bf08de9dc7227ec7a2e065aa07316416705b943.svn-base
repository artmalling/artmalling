...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}         name="NO"               width=30        align=center</FC>'
                     ...
                     + '<FC>id=ADD_POINT        name="적립포인트"       gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=75        gte_columntype="number:0:true" gte_Summarytype="number:0:true"	align=right SumText=@sum </FC>'
                     + '<FC>id=ADD_TYPE         name="적립유형"         width=90        align=center      show=false</FC>'
                     + '<FC>id=ADD_TYPE_NM      name="적립유형"         width=90        align=left</FC>'
                     + '<FC>id=USE_POINT        name="사용포인트"       gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=75        gte_columntype="number:0:true" gte_Summarytype="number:0:true"	align=right SumText=@sum </FC>'
                     ...
                     + '<FC>id=REAL_SALE_AMT    name="실매출액"         gte_columntype="number:0:true" gte_Summarytype="number:0:true"	width=90        gte_columntype="number:0:true" gte_Summarytype="number:0:true"	align=right SumText=@sum </FC>'
                     + '<FC>id=TYPE_NAME        name="결제수단"         width=250       align=left</FC>'
                     + '<FC>id=PUMBUN_NAME      name="브랜드"           width=170       align=left</FC>'
                     ;
...
function btn_Search() {
    ...
    if(RD_COMP_PERS_FLAG.CodeValue == "P" && trim(EM_SS_NO_S.Text).length == 0 && trim(EM_CUST_ID_S.Text).length == 0 
        ...
        if (RD_COMP_PERS_FLAG.CodeValue == "P" && (trim(EM_SS_NO_S.Text).length > 0 && trim(EM_SS_NO_S.Text).length < 13)) {
...
function showMaster(){
    ...
    TR_MAIN.Action  ="/dcs/dctm304.dm?goTo="+goTo+parameters;  
            ...
            <tr>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<comment id="_NSID_"> <object
                        ...
                        classid=<%=Util.CLSID_EMEDIT%> width=104 tabindex=1
	    ...
	    <td align="right">
			<table border="0" cellspacing="0" cellpadding="0">
			  <tr>
			    ...
			    <td class="btn_r"></td>
			    <td class="btn_l"> </td>
			    <td class="btn_c"><a href="#" onclick="gourl('dmbo999.do','회원정보안내(소멸예정)','DMBO')">회원정보안내(소멸예정)</a></td>
			    <td class="btn_r"></td>
			 </tr>
			</table>                
	    </td>	
