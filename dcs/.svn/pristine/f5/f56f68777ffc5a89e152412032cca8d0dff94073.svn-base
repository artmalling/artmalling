<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 회원관리 > 회원관리 > 회원 즉시 탈퇴 등록
 * 작 성 일 : 2010.03.22
 * 작 성 자 : 장형욱
 * 수 정 자 : 
 * 파 일 명 : dctm1090.jsp
...
//     initEmEdit(EM_SUM_POINT,   "NUMBER^9^0",          READ);       //누적+발생포인트
    ...
    initComboStyle(LC_REASON_S,DS_REASON_S, "CODE^0^25,NAME^0^85", 1, PK);    //탈퇴사유
    initEmEdit(EM_REMARK,  "GEN^100",              NORMAL);       //비고             
    initComboStyle(LC_SCED_REQ_TYPE,DS_SCED_REQ_TYPE, "CODE^0^25,NAME^0^85", 1, PK);    //탈퇴요청유형
    initEmEdit(EM_REMARK_SCED_REQ_TYPE,  "GEN^100",              NORMAL);       //비고-탈퇴요청유형             
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}       name="NO"        width=30   align=center  </FC>'
                     + '<FC>id=SAVE_CHK       name="선택"       width=40   align=center   show=false   Edit={IF(FLAG="Y","true","false")}  EditStyle=CheckBox </FC>'
                     + '<FC>id=CUST_NAME      name="회원/법인명" width=110   align=left    Edit=none</FC>'
                     + '<FC>id=CUST_STAT_NM   name="회원상태"    width=80   align=center  Edit=none</FC>'
                     + '<FC>id=CARD_TYPE_NM   name="카드종류"    width=120   align=left    Edit=none</FC>'
                     + '<FC>id=CARD_NO        name="카드번호"    width=150  align=center  Edit=none mask="XXXX-XXXX-XXXX-XXXX"</FC>'                     
                     + '<FC>id=COMM_NAME1     name="카드상태"    width=86   align=center  Edit=none </FC>'
                     + '<FC>id=ISSUE_DT       name="발급일자"    width=90   align=center  Edit=none mask="XXXX/XX/XX"</FC>'
                     + '<FC>id=BRCH_NAME      name="가입경로"    width=130   align=left    Edit=none</FC>'
                     + '<FC>id=POINT          name="누적포인트 "  width=70   align=right   show=false Edit=none</FC>'
                     + '<FC>id=REASON_CD      name="*탈퇴사유"    width=85   align=left   show=false   EDITSTYLE=LOOKUP   DATA="DS_REASON_S:CODE:NAME"</FC>'    
                     + '<FC>id=REMARK         name="비고"    width=130  align=left    Edit=none show=false </FC>'
                     + '<FC>id=SCED_REQ_TYPE  name="*탈퇴요청형태"   width=90   align=left    show=false EDITSTYLE=LOOKUP   DATA="DS_SCED_REQ_TYPE:CODE:NAME"</FC>'    
                     + '<FC>id=REMARK_SCED_REQ_TYPE         name="비고(탈퇴요청)"    width=130  align=left    Edit=none show=false </FC>'
                     
                     + '<FC>id=CUST_ID        name="hidden"    width="165" show=false  Edit=none </FC>'
                     + '<FC>id=CUST_STAT_FLAG name="hidden"    width="165" show=false  Edit=none </FC>'
                     + '<FC>id=CARD_TYPE_CD   name="hidden"    width="165" show=false  Edit=none </FC>'
                     + '<FC>id=CARD_STAT_NM   name="hidden"    width="165" show=false  Edit=none </FC>'
                     + '<FC>id=ENTR_PATH_CD   name="hidden"    width="165" show=false  Edit=none </FC>';
...
function btn_Search() {
    ...
    TR_MAIN.Action  = "/dcs/dctm109.dm?goTo="+goTo+parameters;  
    ...
    if (DS_IO_MASTER.CountRow > 0 ) {
    	...
    	if(trim(EM_CUST_NAME.Text) != ""){
            showMessage(EXCLAMATION, Ok, "USER-1000", "탈퇴한 회원이므로 조회 할 수 없습니다.");
    	}else{
            setPorcCount("SELECT", DS_IO_MASTER.CountRow);
        }
    	EM_CARD_NO_S.Focus();
    }
    setComboData(LC_REASON_S,        "");
    EM_REMARK.Text = "";

    setComboData(LC_SCED_REQ_TYPE,        "");
    EM_REMARK_SCED_REQ_TYPE.Text = "";
}
...
function btn_Save() {
    ...
    if(LC_REASON_S.Text == ""){
        showMessage(EXCLAMATION, Ok, "USER-1000", "탈퇴사유 필수입력값입니다.");
        LC_REASON_S.Focus();
        return;
    }
    DS_IO_MASTER.UserStatus(1) = intRowStatus;                 
    DS_IO_MASTER.NameValue(1, "REASON_CD") = LC_REASON_S.BindColVal;
    DS_IO_MASTER.NameValue(1, "REMARK") = EM_REMARK.Text;

    if(LC_SCED_REQ_TYPE.Text == ""){
        showMessage(EXCLAMATION, Ok, "USER-1000", "탈퇴요청유형 필수입력값입니다.");
        LC_SCED_REQ_TYPE.Focus();
        return;
    }
    
    DS_IO_MASTER.UserStatus(1) = intRowStatus;                 
    DS_IO_MASTER.NameValue(1, "SCED_REQ_TYPE") = LC_SCED_REQ_TYPE.BindColVal;
    DS_IO_MASTER.NameValue(1, "REMARK_SCED_REQ_TYPE") = EM_REMARK_SCED_REQ_TYPE.Text;
    ...
    var goTo        = "saveData";
    var action      = "I";  //조회는 O
    TR_MAIN.Action  ="/dcs/dctm109.dm?goTo="+goTo;   
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td colspan="5" style="display:none" ><comment id="_NSID_"> <object
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					class="s_table">
					<tr>
						<th width="80" class="point">탈퇴사유</th>
						<td width="100"><comment id="_NSID_"> <object
							id=LC_REASON_S classid=<%=Util.CLSID_LUXECOMBO%> height=100
							width=155 tabindex=1> </object> </comment> <script> _ws_(_NSID_);</script>
						</td>
						<th width="80">비고</th>
						<td><comment id="_NSID_"> <object
							id=EM_REMARK classid=<%=Util.CLSID_EMEDIT%> width=450 align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script>
						</td>
					</tr>					
					<tr>
						<th width="80" class="point">탈퇴요청유형</th>
						<td><comment id="_NSID_"> <object
							id=LC_SCED_REQ_TYPE classid=<%=Util.CLSID_LUXECOMBO%> height=100
							width=155 tabindex=1> </object> </comment> <script> _ws_(_NSID_);</script>
						</td>
						<th width="80">비고</th>
						<td colspan="3"><comment id="_NSID_"> <object
							id=EM_REMARK_SCED_REQ_TYPE classid=<%=Util.CLSID_EMEDIT%> width=450 align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script>
						</td>
					</tr>	
				</table>
				</td>
	...
	<tr valign="top">
      <td class="PB03">
        <table width="100%" border="0" cellspacing="0" cellpadding="0"
			class="BD4A">
          <tr>
            <td><OBJECT id=GD_MASTER width="100%" height=590
