 ...
 * @caused   by 
 */
public class DMbo641DAO extends AbstractDAO {
    ...
    public int save(ActionForm form, MultiInput mi[], HttpServletRequest request) throws Exception {
        ...
        try {
			...
			// 전문번호 SEQ
			strTranNo = this.getOCBTranNo(form).get("OCB_TRAN_NO").toString();
			System.out.println ("전문번호 SEQ:" + strTranNo);
        	
			// OCBNet 전송을 위한 맵핑
			HashMap<String, String> hm = OCBNetUtil.makeForm("BE", strTranNo, strCardNo, strVaildNo,
					String.valueOf(intConvPoint), strPwdNo, strApprNo, strApprDt, strEncryptedSign);
        	...
        	// 응답 전문이 없을 때 처리
        	if (m_hash == null) {
        		throw new Exception("|[USER]상품권교환에 실패하였습니다");
        	} else {
        		strApprNo = m_hash.get("AuthNum");
        		strApprDt = m_hash.get("Authdate").substring(0, 6);
        		
        		// 전문 수신 정보를 저장한다.
        		dao.updOCBData(form, hm, m_hash);
        		
            	// 응답 전문은 정상 수신, 응답 상태 코드가 거절일 때 처리
        		if (m_hash.get("Status").equals("X")) {
            		throw new Exception("|[USER]" + m_hash.get("Message1") + " " + m_hash.get("Message2"));
            	} else {
        			//***3. 전문송수신결과에 따른 저장여부 및 메세지.
    				ret = this.saveData(form, mi, request, m_hash.get("Authdate"), m_hash.get("UniqNum"));
			...
			// OCBNet 전송을 위한 맵핑
			HashMap<String, String> hm = OCBNetUtil.makeForm("BG", strTranNo, strCardNo, strVaildNo,
					String.valueOf(intConvPoint), strPwdNo, strApprNo, strApprDt, strEncryptedSign);
    ...
    public int saveData(ActionForm form, MultiInput mi[], HttpServletRequest request, String Authdate, String UniqNum) throws Exception {
        ...
        try {
	    	...
	    	sql.setString(i++, Authdate.substring(0, 6));	// 수신일시
	    	sql.setString(i++, UniqNum);	// 전문일련번호
    ...
    public List[] searchMaster(ActionForm form) throws Exception {
        ...
        try{       
	        ...
	        sql.put(strQuery);
	        //sql.setString(1,util.encryptedStr(strCardNo));
