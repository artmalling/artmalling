...
public class DMbo999DAO extends AbstractDAO {
     ...
     * 
     */
    /*public List searchClrPoint(ActionForm form) throws Exception {   //20160727_강연식 막음 소멸포인트 일별로 변환
        List       ret  = null; 
        SqlWrapper sql  = null; 
        Service    svc  = null;
        Util       util = new Util();
        ...
        //차이 월을 구한다(종료일 - 시작일).
        RowCnt = (Integer.parseInt(strAddTDt.substring(0,4)) - Integer.parseInt(strAddFDt.substring(0,4)))*12 
        		   + Integer.parseInt(strAddTDt.substring(4,6)) - Integer.parseInt(strAddFDt.substring(4,6));
        ...
        strQuery = svc.getQuery("SEL_MONTH_MASTER_1")  + "\n";
        ...
        System.out.println("strAddFDt = "+strAddFDt.substring(0,6));
        System.out.println("strAddTDt = "+strAddTDt.substring(0,6));
        System.out.println("strCustId = "+strCustId);
        
        
        for(int j=0; j<RowCnt+1; j++){
        	strQuery += svc.getQuery("SEL_MONTH_MASTER_2")  + "\n";       	
        	sql.setString(i++, strAddFDt);  
            sql.setString(i++, ""+j);
            sql.setString(i++, strAddFDt);  
            sql.setString(i++, ""+j);
        	...
        	if(j!=RowCnt)
        	{
        		strQuery += svc.getQuery("SEL_MONTH_MASTER_3") + "\n";
        	}
        }

        strQuery += svc.getQuery("SEL_MONTH_MASTER_4") + "\n";
