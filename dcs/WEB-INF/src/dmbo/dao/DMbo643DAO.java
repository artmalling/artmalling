...
public class DMbo643DAO extends AbstractDAO {
    ...
    public List searchMaster(ActionForm form) throws Exception {
        ...
        try {
			...
			if (! is_ChkRecpAdd_Chk(svc, strRecpNo)) {  
				throw new Exception("[USER]"+ "적립되지 않은 영수증입니다. 영수증 번호를 확인해주세요." );
			} 
	...
	public int save(ActionForm form, MultiInput [] mi, String userId) throws Exception { 
		...
		try {
			...
			svc = (Service) form.getService();  
				...
				while (mi[0].next()) {  
					System.out.println(mi[0].getString("STR_CD"));
		            System.out.println(mi[0].getString("SALE_DT"));
		            System.out.println(mi[0].getString("POS_NO"));
		            System.out.println(mi[0].getString("EXE_TYPE"));
					
		            String STR_CD  = mi[0].getString("STR_CD"); //점코드
		            String SALE_DT = mi[0].getString("SALE_DT");//매출일자
		            String POS_NO  = mi[0].getString("POS_NO"); //POS번호
		            String TRAN_NO = mi[0].getString("TRAN_NO");//거래번호
		            String EXE_TYPE = mi[0].getString("EXE_TYPE");//적립 방법 11 : 일반 적립 , 13 : 사후 적립
		            ...
		            String RECP_NO_FULL = "1" + mi[0].getString("SALE_DT").substring(2,8) + mi[0].getString("POS_NO") + mi[0].getString("TRAN_NO");
		             
		            // 1. 포인트 적립 체크 
		            if (searchTrpointCount(form, STR_CD, SALE_DT, POS_NO, TRAN_NO) > 0) { 
				            ...
				            result_D = searchDetail(form , RECP_NO, mi[0].getString("BRCH_ID"));  
					...
					//3. DCS.DO_POINT_ADD DELETE		            
		            sql.close();
		            i = 1;
					sql.put(svc.getQuery("DEL_DO_POINT_ADD"));
					sql.setString(i++,RECP_NO_FULL);
					sql.setString(i++,RECP_NO_FULL);
					sql.setString(i++,RECP_NO_FULL);
					sql.setString(i++,RECP_NO_FULL);
					res = update(sql); 
					if (res <= 0) {
						throw new Exception("[USER]"+ "데이터의 적합성 문제로 인하여"
								+ "데이터 입력을 하지 못했습니다(DEL_DO_POINT_ADD).");
					}
					System.out.println("loop4");
					//4. DCS.DO_RECP_ADD DELETE  // 사후 적립된 영수증의 경우
					if ("13".equals(EXE_TYPE)) {
						sql.close();
						i = 1;
						sql.put(svc.getQuery("DEL_DO_RECP_ADD"));
						sql.setString(i++,STR_CD);
						sql.setString(i++,SALE_DT);
						sql.setString(i++,POS_NO);
						sql.setString(i++,TRAN_NO);
						res = update(sql); 
						if (res <= 0) {
							throw new Exception("[USER]"+ "데이터의 적합성 문제로 인하여"
									+ "데이터 입력을 하지 못했습니다(DEL_DO_RECP_ADD).");
						}
					}
					System.out.println("loop5");
					//5. DPS.PS_TRPOINT DELETE
					sql.close();
					i = 1;
					sql.put(svc.getQuery("DEL_PS_TRPOINT"));
					sql.setString(i++,STR_CD);
					sql.setString(i++,SALE_DT);
					sql.setString(i++,POS_NO);
					sql.setString(i++,TRAN_NO);
					...
					if (res <= 0) {
						throw new Exception("[USER]"+ "데이터의 적합성 문제로 인하여"
								+ "데이터 입력을 하지 못했습니다(DEL_PS_TRPOINT).");
					}
					System.out.println("loop6");
					//6. 회원 포인트 차감
					sql.close();
					i = 1;
					sql.put(svc.getQuery("UPD_DO_CUST_TPOINT"));
					sql.setString(i++,mi[0].getString("OCC_POINT"));
					sql.setString(i++,USER_ID);
					sql.setString(i++,mi[0].getString("CUST_ID"));
					res = update(sql); 
					if (res <= 0) {
						throw new Exception("[USER]"+ "데이터의 적합성 문제로 인하여"
								+ "데이터 입력을 하지 못했습니다(UPD_DO_CUST_TPOINT).");
					}
					System.out.println("loop7");
					//7. 적립취소 LOG 생성
					sql.close();
					i = 1;
					sql.put(svc.getQuery("INS_POINT_CANCEL_LOG"));
					sql.setString(i++,STR_CD);
					sql.setString(i++,SALE_DT);
					sql.setString(i++,POS_NO);
					sql.setString(i++,TRAN_NO);
					...
					if (res <= 0) {
						throw new Exception("[USER]"+ "데이터의 적합성 문제로 인하여"
								+ "데이터 입력을 하지 못했습니다(INS_POINT_CANCEL_LOG).");
					}
...
public int save_old(ActionForm form, MultiInput [] mi, String userId) throws Exception { 
