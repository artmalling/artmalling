 ...
 * @caused   by 
 */
public class DMbo638DAO extends AbstractDAO {
    ...
    public List searchDetail(ActionForm form) throws Exception {
        ...
        String strConvPoint = String2.nvl(form.getParam("strConvPoint"));
        String strMemoDateF = String2.nvl(form.getParam("strMemoDateF"));
        ...
        try {
            ...
            while (mi.next()) {
                if (mi.IS_INSERT()) { // 저장
                    ...
                    int i=1;
                    sql.put(svc.getQuery("INS_DO_GIFTCONV_RULE")); 
                    sql.setString(i++, mi.getString("BRCH_ID"));       	//가맹점번호
                    sql.setString(i++, mi.getString("CONV_POINT")); 	//교환포인트
                    sql.setString(i++, mi.getString("START_DT"));      	//시작일자
                    sql.setString(i++, mi.getString("END_DT"));        	//종료일자
                    sql.setString(i++, mi.getString("CONV_GIFT_AMT"));  //교환상품권금액                 
                    ...
                    int i=1;
                    sql.put(svc.getQuery("UPD_DO_GIFTCONV_RULE")); 
                    sql.setString(i++, mi.getString("END_DT"));        	//종료일자
                    sql.setString(i++, mi.getString("CONV_GIFT_AMT"));  //교환상품권금액                               
                    sql.setString(i++, userId);                         //로그인ID                    
                    sql.setString(i++, mi.getString("BRCH_ID"));       	//가맹점번호
    ...
    public int searchTermChk(ActionForm form, MultiInput mi) throws Exception {
        ...
        sql.put(svc.getQuery("SEL_DO_GIFTCONV_RULE"));
        sql.setString(1, mi.getString("BRCH_ID"));
        sql.setString(2, mi.getString("CONV_POINT"));
