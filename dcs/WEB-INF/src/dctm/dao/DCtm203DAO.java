...
public class DCtm203DAO extends AbstractDAO {
	 ...
	 * @caused by
	 */
	public List searchCustinfo(ActionForm form, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		List ret = null;
		SqlWrapper sql = null;
		Service svc = null;
		...
		try {
			...
			/*
			 * ret = util.decryptedStr(ret,3); //주민등록번호 복호화. ret =
			 * util.decryptedStr(ret,12); ret = util.decryptedStr(ret,13); ret =
			 * util.decryptedStr(ret,14); ret = util.decryptedStr(ret,15); ret =
			 * util.decryptedStr(ret,16); ret = util.decryptedStr(ret,17); ret =
			 * util.decryptedStr(ret,18); //EMAIL1 ret =
			 * util.decryptedStr(ret,19); //EMAIL2 ret =
			 * util.decryptedStr(ret,20); //EMAIL3
			 */
			System.out.println("리턴 사이트" + ret);
			// 조회끝난후 로그 쌓기.
			if (ret.size() > 0) {
				String logGubun = "S"; // S:조회 //I:입력
				String log1 = "DM_CUSTOMER"; // P_TABLE_ID 테이블명.
				String log2 = "DCTM203"; // P_PGM_ID 프로그램 ID
				String log3 = "CARD_NO=[" + strCardNo + "]" + "SS_NO=[" + strSsno + "]";// P_PK_VAL
																						// 키값
				String log4 = strQuery.replaceAll(" ", ""); // 입력시에는 P_AFT_DATA
															// 데이터, 조회시에는 쿼리.
				String log5 = sessionInfo.getUSER_ID(); // 등록자 ID
				
				System.out.println(logGubun);
	 ...
	 * @caused by
	 */
	public void addrCls(ActionForm form, MultiInput mi, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		while (mi.next()) {
			...
			String strNoclsReqYN = mi.getString("NOCLS_REQ_YN"); // 주소클렌징여부(Y:
																	// 정제 , N:
																	// 안함)
...
//			// 주소 정제 솔루션 처리 전 고객 입력 주소 저장
//			mi.set("HORI_ZIP_CD1",
//					mi.getString("HOME_ZIP_CD1")); /* 고객입력 우편번호 1 */
//			mi.set("HORI_ZIP_CD2",
//					mi.getString("HOME_ZIP_CD2")); /* 고객입력 우편번호 2 */
//			mi.set("HORI_ADDR1", strHomeAddr1); /* 고객입력 주소1(주소정세솔루션 처리 전) */
//			mi.set("HORI_ADDR2", strHomeAddr2); /* 고객입력 주소2(주소정세솔루션 처리 전) */
//			mi.set("HORI_NEW_YN",
//					mi.getString("HOME_NEW_YN")); /* 새주소, 구주소 구분 */

			// 형지 아트몰링 주석처리 주소정제솔루션 사용안함
			/*
			 * if(strNoclsReqYN.equals("Y")){ //주소정제 솔루션 returnSuccYn =
			 * zip.putAddress(strHomeAddr1,strHomeAddr2); }
			 */
			...
			if (returnSuccYn.equals("Y")) {
				...
				mi.set("HOME_ZIP_CD1",
						zip.getRevise_Zipcode1()); /* 정제 후 우편번호1 - 구주소 */
				mi.set("HOME_ZIP_CD2",
						zip.getRevise_Zipcode2()); /* 정제 후 우편번호2 */
				mi.set("HOME_ADDR1", zip.getRevise_Addr1()); /* 정제 후 구주소1 */
				mi.set("HOME_ADDR2", zip.getRevise_Addr2()); /* 정제 후 구주소2 */

				mi.set("HNEW_ZIP_CD1",
						zip.getRevise_Zipcode1()); /* 정제 후 우편번호1 - 신주소 */
				mi.set("HNEW_ZIP_CD2",
						zip.getRevise_Zipcode2()); /* 정제 후 우편번호2 */
				...
				mi.set("HNEW_ADDR1", strHnewAddr1);
				mi.set("HNEW_ADDR2", strHnewAddr2);
...
//			// 주소 정제 솔루션 처리 후고객 입력 주소 저장
//			mi.set("OORI_ZIP_CD1",
//					mi.getString("OFFI_ZIP_CD1")); /* 고객입력 우편번호 1 */
//			mi.set("OORI_ZIP_CD2",
//					mi.getString("OFFI_ZIP_CD2")); /* 고객입력 우편번호 2 */
//			mi.set("OORI_ADDR1", strOffiAddr1); /* 고객입력 주소1(주소정세솔루션 처리 전) */
//			mi.set("OORI_ADDR2", strOffiAddr2); /* 고객입력 주소2(주소정세솔루션 처리 전) */
//			mi.set("OORI_NEW_YN",
//					mi.getString("OFFI_NEW_YN")); /* 새주소, 구주소 구분 */
	 ...
	 * @caused by
	 */
	public ProcedureResultSet logSave(String logGubun, String para1, String para2, String para3, String para4,
			String para5) throws Exception {
	 ...
	 * @caused by
	 */
	public GauceDataSet saveData2(ActionForm form, MultiInput mi, GauceDataSet dSet, HttpServletRequest request,
...
//			}
			...
			/*
			 * String strSexCd = ""; //주민번호로 성별 체크 - 내국인일경우....
			 * if(mi.getString("SS_NO").length() == 6){ }else
			 * if(mi.getString("SS_NO").substring(6, 7).equals("1") ||
			 * mi.getString("SS_NO").substring(6, 7).equals("3")
			 * ||mi.getString("SS_NO").substring(6, 7).equals("5") ||
			 * mi.getString("SS_NO").substring(6, 7).equals("7")){ strSexCd =
			 * "M"; }else{ strSexCd = "F"; }
			 */
			String strCardPwdNo = mi.getString("CARD_PWD_NO"); // 카드비밀번호 //암호화
																// ###미처리
			// if(!strCardPwdNo.equals("****") &&
			// !String2.nvl(strCardPwdNo).equals("")){ //'****'가 아닐경우 암호화처리
			...
			String strHOriAddr1 = mi.getString("HORI_ADDR1"); // 고객입력 주소(주소정세솔루션
																// 처리 전)
			String strHOriAddr2 = mi.getString("HORI_ADDR2"); // 고객입력 주소(주소정세솔루션
																// 처리 전)
			String strOOriAddr1 = mi.getString("OORI_ADDR1"); // 고객입력 주소(주소정세솔루션
																// 처리 전)
			String strOOriAddr2 = mi.getString("OORI_ADDR2"); // 고객입력 주소(주소정세솔루션
																// 처리 전)
			...
			String strOffTerms1Yn = mi.getString("OFF_TERMS1_YN"); // 정보제공 동의일자
																	// 동의여부
			String strOffTerms1Dt = mi.getString("OFF_TERMS1_DT"); // 정보제공 동의일자
			String strOffTerms1Name = mi.getString("OFF_TERMS1_NAME"); // 정보제공
																		// 동의성명
			String strOffTerms2Yn = mi.getString("OFF_TERMS2_YN"); // 개인정보 제3자
																	// 제공 동의여부
			String strOffTerms2Dt = mi.getString("OFF_TERMS2_DT"); // 개인정보 제3자
																	// 제공 동의일자
			String strOffTerms2Name = mi.getString("OFF_TERMS2_NAME"); // 개인정보
																		// 제3자
																		// 제공
																		// 동의자
			String strOffTerms3Yn = mi.getString("OFF_TERMS3_YN"); // 정보제공 동의일자
																	// 동의여부
			String strOffTerms3Dt = mi.getString("OFF_TERMS3_DT"); // 정보제공 동의일자
			String strOffTerms3Name = mi.getString("OFF_TERMS3_NAME"); // 정보제공
																		// 동의성명
			String strOffTerms4Yn = mi.getString("OFF_TERMS4_YN"); // 개인정보 제3자
																	// 제공 동의여부
			String strOffTerms4Dt = mi.getString("OFF_TERMS4_DT"); // 개인정보 제3자
																	// 제공 동의일자
			String strOffTerms4Name = mi.getString("OFF_TERMS4_NAME"); // 개인정보
																		// 제3자
																		// 제공
																		// 동의자
            ...
            System.out.println("strOffiBuildHo==== "+strOffiBuildHo);
			String strOffiFloor = mi.getString("OFFI_FLOOR"); // 건물 층
            System.out.println("strOffiFloor==== " + strOffiFloor);
			String strOffiMailYn = mi.getString("OFFI_MAIL_YN"); // 우편물발송가능여부
            System.out.println("strOffiMailYn==== " + strOffiMailYn);
			String strHoriNewYn = mi.getString("HORI_NEW_YN");
            System.out.println("strHoriNewYn==== " + strHoriNewYn);
			String strOoriNewYn = mi.getString("OORI_NEW_YN");
            System.out.println("strOoriNewYn==== " + strOoriNewYn);
			String strCustGrade = mi.getString("CUST_GRADE"); // 회원등급 기본 - "21";
            System.out.println("strCustGrade==== " + strCustGrade);
			String strVipFlag = mi.getString("VIP_FLAG"); // VIP 등록구분
            System.out.println("strVipFlag==== " + strVipFlag);
			...
			String strON_TERMS5_DATE = mi.getString("ON_TERMS5_DATE"); // 매장협력사회원
																		// 일경우
																		// 매장명
																		// 컬럼추가
																		// 20150326

            System.out.println("strON_TERMS5_DATE==== "+strON_TERMS5_DATE);
            ...
            System.out.println("strEtc==== " + strEtc);
            
			if (strChksave.equals("U")) {
				psql.put("DCS.PR_PCUST_UPDATE", 171);
			} else {
				psql.put("DCS.PR_PCUST_INSERT", 170);
			}
			...
			psql.setString(i++, strOffiAddr1); // 사무실주소1 -- 고객입력 주소(주소정제솔루션 처리
												// 후)
			psql.setString(i++, strOffiAddr2); // 사무실주소2 -- 고객입력 주소(주소정제솔루션 처리
												// 후)
			...
			psql.setString(i++, strHOriAddr1); // P_HORI_ADDR1 고객입력 주소(주소정세솔루션
												// 처리 전)
			psql.setString(i++, strHOriAddr2); // P_HORI_ADDR2 고객입력 주소(주소정세솔루션
												// 처리 전)
			psql.setString(i++, strOOriAddr1); // P_OORI_ADDR1 고객입력 주소(주소정세솔루션
												// 처리 전)
			psql.setString(i++, strOOriAddr2); // P_OORI_ADDR2 고객입력 주소(주소정세솔루션
												// 처리 전)
			...
			psql.setString(i++, strON_TERMS5_DATE); // ON_TERMS5_DATE
													// --------104
			...
			if (strChksave.equals("U") && strUpdate.equals("N")) {
				psql.setString(i++, "Y"); // P_CARD_ISSUE_YN 카드발급 여부 : 등록화면에서는
											// 'Y', 조회/수정화면에서는 'N' ※ 등록기능에 추가된
											// 항목
			...
			if (!prsRet.equals("0")) {
				throw new Exception("[USER]" + "데이터의 적합성 문제로 인하여" + "데이터 입력을 하지 못했습니다.(1)");
			} else {
				...
				// 회원등급 저장
				// this.custGradeSave (form, strUserId, strBrchId, strCustId,
				// strCustGrade, strVipFlag);

				// 회원유형 저장
				// this.custTypeSave (form, strUserId, strBrchId, strCustId,
				// strCustType);
				...
				aftData = "CUST_ID=[" + strCustId + "]," // 회원ID.
						...
						+ "OFF_TERMS1_NAME=[" + strOffTerms1Name + "]," // 정보제공
																		// 동의성명
						+ "OFF_TERMS2_YN=[" + strOffTerms2Yn + "]," // 개인정보 제3자
																	// 제공 동의여부
						+ "OFF_TERMS2_DT=[" + strOffTerms2Dt + "]," // 개인정보 제3자
																	// 제공 동의일자
						+ "OFF_TERMS2_NAME=[" + strOffTerms2Name + "]," // 개인정보
																		// 제3자
																		// 제공
																		// 동의자
						+ "OFF_TERMS3_YN=[" + strOffTerms3Yn + "]," // 정보제공 동의여부
						+ "OFF_TERMS3_DT=[" + strOffTerms3Dt + "]," // 정보제공 동의일자
						+ "OFF_TERMS3_NAME=[" + strOffTerms3Name + "]," // 정보제공
																		// 동의성명
						+ "OFF_TERMS4_YN=[" + strOffTerms4Yn + "]," // 개인정보 제3자
																	// 제공 동의여부
						+ "OFF_TERMS4_DT=[" + strOffTerms4Dt + "]," // 개인정보 제3자
																	// 제공 동의일자
						+ "OFF_TERMS4_NAME=[" + strOffTerms4Name + "]," // 개인정보
																		// 제3자
																		// 제공
																		// 동의자
						...
						+ "HNEW_ZIP_CD1=[" + strHnewZipCd1 + "]," + "HNEW_ZIP_CD2=[" + strHnewZipCd2 + "],"
						+ "HNEW_ADDR1=[" + strHnewAddr1 + "]," + "HNEW_ADDR2=[" + strHnewAddr2 + "]," + "HOME_NEW_YN=["
						+ strHomeNewYn + "]," + "ONEW_ZIPCD1=[" + strOnewZipCd1 + "]," + "ONEW_ZIPCD2=[" + strOnewZipCd2
						+ "]," + "ONEW_ADDR1=[" + strOnewAddr1 + "]," + "ONEW_ADDR2=[" + strOnewAddr2 + "],"
						+ "OFFI_NEW_YN=[" + strOffiNewYn + "]," + "HORI_NEW_YN=[" + strHoriNewYn + "],"
						+ "HORI_ZIP_CD1=[" + strHoriZipCd1 + "]," + "HORI_ZIP_CD2=[" + strHoriZipCd2 + "],"
						+ "HOME_BUNJI=[" + strOffiNewYn + "]," + "OORI_ZIP_CD1=[" + strOoriZipCd1 + "],"
						+ "OORI_ZIP_CD2=[" + strOoriZipCd2 + "]," + "HOME_SIDO=[" + strHomeSido + "]," + "HOME_SIGUN=["
						+ strHomeSiGun + "]," + "HOME_GU=[" + strHomeGu + "]," + "HOME_YDONG=[" + strHomeYdong + "],"
						+ "HOME_DONGRI=[" + strHomeDongri + "]," + "HOME_BUNJI_FLAG=[" + strHomeBunjiFlag + "],"
						+ "HOME_BUNJI=[" + strHomeBunji + "]," + "HOME_GAJI_BUNJI=[" + strHomeGajiBunji + "],"
						+ "HOME_BUILD_NAME=[" + strHomeBuildName + "]," + "HOME_BUILD_DONG=[" + strHomeBuildDong + "],"
						+ "HOME_BUILD_HO=[" + strHomeBuildHo + "]," + "HOME_FOOR=[" + strHomeFloor + "],"
						+ "HOME_MAIL_YN=[" + strHomeMailYn + "]," + "OFFI_SI_DO=[" + strOffiSido + "],"
						+ "OFFI_SI_GUN=[" + strOffiSiGun + "]," + "OFFI_GU=[" + strOffiGu + "]," + "OFFI_YDONG=["
						+ strOffiYdong + "]," + "OFFI_DONGRI=[" + strOffiDongri + "]," + "OFFI_BUNJI_FLAG=["
						+ strOffiBunjiFlag + "]," + "OFFI_BUNJI=[" + strOffiBunji + "]," + "OFFI_GAJI_BUNJI=["
						+ strOffiGajiBunji + "]," + "OFFI_BUILD_NAME=[" + strOffiBuildName + "]," + "OFFI_BUILD_DONG=["
						+ strOffiBuildDong + "]," + "OFFI_BUILD_HO=[" + strOffiBuildHo + "]," + "OFFI_FLOOR=["
						+ strOffiFloor + "]," + "OFFI_MAIL_YN=[" + strOffiMailYn + "]," + "CUST_GRADE=[" + strCustGrade
						+ "]," + "VIP_FLAG=[" + strVipFlag + "]," + "TM_YN=[" + strTMYN + "]," // TM여부
						+ "MOBILE_COMP=[" + strMobileComp + "]," // 통신사
						+ "DI=[" + strDI + "]," + "CI=[" + strCI + "]," + "ENTR_CH=[" + strEntrCh + "]," + "STR_CD=["
						+ strSTR_CD + "]," + "ETC_MEMO=[" + strEtc + "]"; // 적요
	 ...
	 * @throws Exception
	 */
	public int editSsnoPro(ActionForm form, GauceDataSet dSet, HttpServletRequest request) throws Exception {
		...
		try {
			...
			String strBfSsNo = String2.nvl(form.getParam("strBfSsNo"));
			String strSsno = String2.nvl(form.getParam("strSsno"));
			...
			strQuery = svc.getQuery("INS_SSNO") + "\n";
			...
			if (res != 1) {
				...
				sql.close();
				i = 1;
				sql.put(svc.getQuery("INS_SSNO_UPD"));
				sql.setString(i++, strCustid);
				sql.setString(i++, strBfSsNo);
				sql.setString(i++, strSsno);
	 ...
	 * @throws Exception
	 */
	public String chkSsNo(ActionForm form, GauceDataSet dSet, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String ret = null;
		Map map = null;
		SqlWrapper sql = null;
		...
		sql.put(svc.getQuery("SEL_SS_NO"));
		sql.setString(i++, strSsNo);
		map = selectMap(sql);
	 ...
	 * @throws Exception
	 */
	public int editCustnmPro(ActionForm form, GauceDataSet dSet, HttpServletRequest request) throws Exception {
		...
		try {
			...
			String strCustnm = dSet.getDataRow(0).getColumnValue(1).toString();
			String strBfCustnm = dSet.getDataRow(0).getColumnValue(2).toString();

			sql.setString(i++, strCustnm);
			sql.setString(i++, strCustid);

			strQuery = svc.getQuery("INS_CUST_NAME") + "\n";
			...
			if (res != 1) {
				...
				sql.close();
				i = 1;
				sql.put(svc.getQuery("INS_CUSTNM_UPD"));
				sql.setString(i++, strCustid);
				sql.setString(i++, strBfCustnm);
				sql.setString(i++, strCustnm);
				sql.setString(i++, sessionInfo.getUSER_ID());
				res = update(sql);
			}
	 ...
	 * @throws Exception
	 */
	public int editJuso(ActionForm form, MultiInput mi[], HttpServletRequest request) throws Exception {
		...
		try {
			...
			HttpSession session = request.getSession();
			SessionInfo sessionInfo = (SessionInfo) session.getAttribute("sessionInfo");
			String strSessionId = sessionInfo.getUSER_ID();
			String strAddrFlag = String2.nvl(form.getParam("strAddrFlag"));
			String strAddrPathCd = sessionInfo.getBRCH_ID();
			String strCustid = "";
			String strAptNewYn = "";
			String strAptZipCd1 = "";
			String strAptZipCd2 = "";
			String strAptAddr1 = "";
			String strAptAddr2 = "";
			String strAptZipCd_1 = "";
			String strAptZipCd_2 = "";
			String strAptAddr_1 = "";
			String strAptAddr_2 = "";
			...
			if (strAddrFlag.equals("H")) {
				while (mi[0].next()) {
					/*
					 * strCustid = mi[0].getString("CUST_ID"); strAptNewYn =
					 * mi[0].getString("HOME_NEW_YN"); strAptZipCd1 =
					 * mi[0].getString("HOME_ZIP_CD1"); strAptZipCd2 =
					 * mi[0].getString("HOME_ZIP_CD2"); strAptAddr1 =
					 * mi[0].getString("HOME_ADDR1"); strAptAddr2 =
					 * mi[0].getString("HOME_ADDR2"); strPreNewYn =
					 * mi[0].getString("BF_HOME_NEW_YN"); strPreZipCd1 =
					 * mi[0].getString("BF_HOME_ZIP_CD1"); strPreZipCd2 =
					 * mi[0].getString("BF_HOME_ZIP_CD2"); strPreAddr1 =
					 * mi[0].getString("BF_HOME_ADDR1"); strPreAddr2 =
					 * mi[0].getString("BF_HOME_ADDR2");
					 */

					strCustid = mi[0].getString("CUST_ID");
					strAptNewYn = mi[0].getString("HOME_NEW_YN");
					strAptZipCd1 = mi[0].getString("HNEW_ZIP_CD1");
					strAptZipCd2 = mi[0].getString("HNEW_ZIP_CD2");
					strAptAddr1 = mi[0].getString("HNEW_ADDR1");
					strAptAddr2 = mi[0].getString("HNEW_ADDR2");

					strAptZipCd_1 = mi[0].getString("HOME_ZIP_CD1");
					strAptZipCd_2 = mi[0].getString("HOME_ZIP_CD2");
					strAptAddr_1 = mi[0].getString("HOME_ADDR1");
					strAptAddr_2 = mi[0].getString("HOME_ADDR2");

					strPreNewYn = mi[0].getString("BF_HOME_NEW_YN");
					strPreZipCd1 = mi[0].getString("BF_HOME_ZIP_CD1");
					strPreZipCd2 = mi[0].getString("BF_HOME_ZIP_CD2");
					strPreAddr1 = mi[0].getString("BF_HOME_ADDR1");
					strPreAddr2 = mi[0].getString("BF_HOME_ADDR2");

				}
			} else {
				while (mi[1].next()) {
					strCustid = mi[1].getString("CUST_ID");
					strAptNewYn = mi[1].getString("OFFI_NEW_YN");
					strAptZipCd1 = mi[1].getString("OFFI_ZIP_CD1");
					strAptZipCd2 = mi[1].getString("OFFI_ZIP_CD2");
					strAptAddr1 = mi[1].getString("OFFI_ADDR1");
					strAptAddr2 = mi[1].getString("OFFI_ADDR2");
					strPreNewYn = mi[1].getString("BF_OFFI_NEW_YN");
					strPreZipCd1 = mi[1].getString("BF_OFFI_ZIP_CD1");
					strPreZipCd2 = mi[1].getString("BF_OFFI_ZIP_CD2");
					strPreAddr1 = mi[1].getString("BF_OFFI_ADDR1");
					strPreAddr2 = mi[1].getString("BF_OFFI_ADDR2");
				}
			}
			...
			if (strAddrFlag.equals("H")) {
				sql.setString(i++, "N");
				sql.setString(i++, strAptNewYn);
				sql.setString(i++, strAptZipCd_1);
				sql.setString(i++, strAptZipCd_2);
				sql.setString(i++, strAptAddr_1);
				sql.setString(i++, strAptAddr2);
				sql.setString(i++, strAptZipCd1);
				sql.setString(i++, strAptZipCd2);
				sql.setString(i++, strAptAddr1);
				sql.setString(i++, strAptAddr2);
				sql.setString(i++, strSessionId);
				sql.setString(i++, strCustid);

				strQuery = svc.getQuery("UPD_HOME_JUSO") + "\n";
				// 회사
			} else {
				sql.setString(i++, "N");
				sql.setString(i++, strAptNewYn);
				...
				strQuery = svc.getQuery("UPD_OFFI_JUSO") + "\n";
			}
			...
			if (ret != 1) {
				throw new Exception("[USER]" + "데이터의 적합성 문제로 인하여" + "데이터 입력을 하지 못했습니다.(2)");
			}

			i = 1;
			sql.put(svc.getQuery("UPD_CUST_ADDR"));
			// 집
			if (strAddrFlag.equals("H")) {
				sql.setString(i++, "N");
				sql.setString(i++, strAptNewYn);
				...
				sql.setString(i++, strSessionId);
				sql.setString(i++, strCustid);
				sql.setString(i++, strAddrFlag);
				sql.setString(i++, strAddrPathCd);
				// 회사
			} else {
				sql.setString(i++, "N");
				sql.setString(i++, strAptNewYn);
			...
			if (ret != 1) {
				...
				sql.close();
				i = 1;
				sql.put(svc.getQuery("INS_JUSO_UPD"));
				sql.setString(i++, strCustid);
				sql.setString(i++, strAddrFlag);
				sql.setString(i++, strAddrPathCd);
				sql.setString(i++, "N");
				sql.setString(i++, strPreNewYn);
				sql.setString(i++, strPreZipCd1);
				sql.setString(i++, strPreZipCd2);
				sql.setString(i++, strPreAddr1);
				sql.setString(i++, strPreAddr2);
				sql.setString(i++, strPreZipCd1);
				sql.setString(i++, strPreZipCd2);
				sql.setString(i++, strPreAddr1);
				sql.setString(i++, strPreAddr2);
	...
	* @throws Exception
	*/
	public int editCardno(ActionForm form, MultiInput mi, HttpServletRequest request) throws Exception {
		...
		try {
			...
			while (mi.next()) {
				...
				if (mi.IS_INSERT()) { // 저장  
					...
					// 변경전 카드 상태 체크
					if (isCard_Chk(svc, mi, "S")) {  
						
						// 1.변경전 카드중지처리
						res = Save_Bfcard(svc, mi, userId); 
						
						if (res < 0) {
							throw new Exception( "[USER]" + "변경 전 카드 중지 처리 중 에러가 발생했습니다. "
												+ "다시 확인하시기 바랍니다. ");
						}
						// 2.변경전 카드중지이력
						res = Save_Bfcard_Loss(svc, mi, userId); 
						
						if (res < 0) {
							throw new Exception( "[USER]" + "변경 전 카드 중지 이력 처리 중 에러가 발생했습니다. "
												+ "다시 확인하시기 바랍니다. ");
						}
						
					} else { 
						throw new Exception("[USER]"+ "변경 전 카드는 카드번호를 변경 할 수 없습니다.카드 정보를 확인해주세요."); 
					}
					
					// 변경후 카드 상태 체크
					if (isCard_Chk(svc, mi, "E")) { 
						
						// 3.변경 후 카드정보 등록  
						res = Save_card_Info(svc, mi, userId); 
	
						if (res < 0) {
							throw new Exception( "[USER]" + "변경 후 카드정보 등록 처리 중 에러가 발생했습니다. "
												+ "다시 확인하시기 바랍니다. ");
						} 
						// 4.카드번호 변경 이력 처리
						res = Save_card_Upd(svc, mi, userId); 
						
						if (res < 0) {
							throw new Exception( "[USER]" + "변경 후 카드정보 변경 이력 처리 중 에러가 발생했습니다. "
												+ "다시 확인하시기 바랍니다. ");
						}  
						
					} else { 
						throw new Exception("[USER]"+ "변경 후 카드정보는 등록 할 수 없습니다. 카드정보를 확인해주세요."); 
					}
	...
	* @throws Exception
	*/
	private int Save_Bfcard(Service svc, MultiInput mi, String strUserId) throws Exception {
		String strLoc = "INS_CARD_STAT_UPD";
		int ret = 0;
		SqlWrapper sql = null;
		sql = new SqlWrapper();		
		...
		String strBfCardNo = mi.getString("BF_CARD_NO");// 변경전 카드번호 
		String strCustNm = "SYSTEM"; // 분실신고자명
		
		sql.setString(i++, strCustNm);   // 분실신고자명
		sql.setString(i++, strUserId); 	 // 수정자ID
		sql.setString(i++, strBfCardNo); // 변경전 카드번호
		ret += update(sql); 
	...
	* @throws Exception
	*/
	private int Save_Bfcard_Loss(Service svc, MultiInput mi, String strUserId) throws Exception {
		String strLoc = "INS_CARDNO_LOSS";
		int ret = 0;
		SqlWrapper sql = null;
		sql = new SqlWrapper();		
		sql.put(svc.getQuery(strLoc));
	...
	* @throws Exception
	*/
	private int Save_card_Info(Service svc, MultiInput mi, String strUserId) throws Exception {
		String strLoc = "INS_CARDNO_UPD";
		int ret = 0;
		SqlWrapper sql = null;
		sql = new SqlWrapper();		
		sql.put(svc.getQuery(strLoc));
		...
		String strCardNo = mi.getString("CARD_NO"); // 변경후 카드번호
		String strBfCardNo = mi.getString("BF_CARD_NO");// 변경전 카드번호 
		 
		sql.setString(i++, strUserId);     // 수정자ID 
		sql.setString(i++, strBfCardNo);// 변경전 카드번호
		sql.setString(i++, strCardNo);  // 변경후 카드번호 
		ret += update(sql); 
	...
	* @throws Exception
	*/
	private int Save_card_Upd(Service svc, MultiInput mi, String strUserId) throws Exception {
		String strLoc = "INS_DM_CARD_NO_UPD";
		int ret = 0;
		SqlWrapper sql = null;
		sql = new SqlWrapper();		
		sql.put(svc.getQuery(strLoc));
		...
		String strCardNo = mi.getString("CARD_NO"); // 변경후 카드번호
		String strBfCardNo = mi.getString("BF_CARD_NO");// 변경전 카드번호
		String strCustNo = mi.getString("CUST_ID"); // 회원ID 
		 
		sql.setString(i++, strCustNo);  // 회원번호 
		sql.setString(i++, strBfCardNo);// 변경전 카드번호
		sql.setString(i++, strCardNo);  // 변경후 카드번호
		sql.setString(i++, strUserId);     // 수정자ID  
		ret += update(sql);  
	...
	* @throws Exception
	*/
	private boolean isCard_Chk(Service svc, MultiInput mi, String strGbn) throws Exception {
		SqlWrapper sql = null;
		boolean ret = false;
		...
		if (strGbn.equals("S") ) {
			sql.put(svc.getQuery("SEL_BFCARD_CHK"));
			int i = 0;
		    sql.setString(++i, strBfCardNo);  
		} else {
			sql.put(svc.getQuery("SEL_CARD_CHK"));
			int i = 0;
		    sql.setString(++i, strCardNo); 
		}
		...
		if (map.get("CNT").equals("1") || map.get("CNT").equals(1)) {
			ret = true;
		} 
	...
	* @throws Exception
	*/
	public List searchChk_Card(ActionForm form) throws Exception {
		...
		strQuery = svc.getQuery("SEL_BFCARD_CHK");
		sql.put(strQuery); 		
		ret = select2List(sql);
		return ret;
	}
	...
	* @throws Exception
	*/
	public int editTelno(ActionForm form, MultiInput mi, HttpServletRequest request) throws Exception {
		...
		try {
			...
			while (mi.next()) {
				...
				if (mi.IS_INSERT()) { // 저장   
					...
					// 1.회원정보 변경
					res = Save_Dmcus(svc, mi, userId); 
					
					if (res < 0) {
						throw new Exception( "[USER]" + "회원정보 휴대번호 변경 처리 중 에러가 발생했습니다. "
											+ "다시 확인하시기 바랍니다. ");
					}
					// 2.휴대전화번호 변경 이력 
					res = Save_Dmcus_upd(svc, mi, userId); 
					
					if (res < 0) {
						throw new Exception( "[USER]" + "휴대번호 변경 이력 처리 중 에러가 발생했습니다. "
											+ "다시 확인하시기 바랍니다. ");
					} 
	...
	* @throws Exception
	*/
	private int Save_Dmcus(Service svc, MultiInput mi, String strUserId) throws Exception {
		String strLoc = "INS_TELNO_UPD";
		int ret = 0;
		SqlWrapper sql = null;
		sql = new SqlWrapper();		
		sql.put(svc.getQuery(strLoc));
		...
		//변경 후 휴대반호
		String strTelNo1 = mi.getString("AFT_MOBILE_PH1");  
		String strTelNo2 = mi.getString("AFT_MOBILE_PH2");  
		String strTelNo3 = mi.getString("AFT_MOBILE_PH3");  
		String strCustNo = mi.getString("CUST_ID"); 
		 
		sql.setString(i++, strTelNo1);      
		sql.setString(i++, strTelNo2); 
		sql.setString(i++, strTelNo3);
	...
	* @throws Exception
	*/
	private int Save_Dmcus_upd(Service svc, MultiInput mi, String strUserId) throws Exception {
		String strLoc = "INS_DM_MOBILE_UPD";
		int ret = 0;
		SqlWrapper sql = null;
		sql = new SqlWrapper();		
		sql.put(svc.getQuery(strLoc));
		...
		//변경 전 휴대반호
		String strBfTelNo1 = mi.getString("BF_MOBILE_PH1");  
		String strBfTelNo2 = mi.getString("BF_MOBILE_PH2");  
		String strBfTelNo3 = mi.getString("BF_MOBILE_PH3");   
		//변경 후 휴대반호
		String strTelNo1 = mi.getString("AFT_MOBILE_PH1");  
		String strTelNo2 = mi.getString("AFT_MOBILE_PH2");  
		String strTelNo3 = mi.getString("AFT_MOBILE_PH3"); 
		...
		sql.setString(i++, strCustNo);  //회원번호  
		sql.setString(i++, strBfTelNo1);    
		sql.setString(i++, strBfTelNo2);    
		sql.setString(i++, strBfTelNo3);  
		sql.setString(i++, strTelNo1);  
		sql.setString(i++, strTelNo2);  
		sql.setString(i++, strTelNo3);  
		sql.setString(i++, strUserId); 
	...
	* @throws Exception
	*/
	public List searchCard(ActionForm form) throws Exception {
		...
		strQuery = svc.getQuery("SEL_CARD_GRADE");
		sql.put(strQuery); 		
		ret = select2List(sql);
		return ret;
	}
