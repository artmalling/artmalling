<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 회원관리 > 회원관리 > 보너스카드발급매수등록
 * 작 성 일 : 2010.01.14
 * 작 성 자 : 김영진
 * 수 정 자 : 
 * 파 일 명 : dctm1170.jsp
...
function doInit(){
	...
	initComboStyle(LC_PREFIX,DS_O_PREFIX, "CODE^0^30,NAME^0^100", 1, PK);    //카드시작번호
	...
	getEtcCode("DS_O_CARD_GRADE", "D", "D211", "N");   // 회원등급(D011)  --> 카드등급(D211) 신규코드생성하여 별도관리
	getEtcCode("DS_O_PREFIX", "D", "D012", "N");
    ...
    registerUsingDataset("dctm117","DS_O_MASTER,DS_O_MASTER2");
...
function btn_Save() {
	...
	if(LC_PREFIX.BindColVal == ""){
		showMessage(EXCLAMATION, OK, "USER-1003",  "카드시작번호");
		LC_PREFIX.Focus();
		return false;
	}
...
*/
function saveData(){	
	TR_MAIN.Action="/dcs/dctm117.dm?goTo=saveData";
...
function delData(){	
	...
	var parameters = "&strCardCreDt="+encodeURIComponent(strCardCreDt)+"&strCardSeq="+encodeURIComponent(strCardSeq)+"&strCardCnt="+encodeURIComponent(strCardCnt);
	TR_MAIN.Action="/dcs/dctm117.dm?goTo=delData"+parameters;
    TR_MAIN.Post();  
    DS_IO_DATA.ClearData();
    bfListRowPosition = 0;
...
function showMaster(){
    ...
    var action     = "O";     
    var parameters = "&strFromDt="+encodeURIComponent(strFromDt)+"&strToDt="+encodeURIComponent(strToDt);
    TR_MAIN.Action="/dcs/dctm117.dm?goTo="+goTo+parameters;  
...
/**
 ...
 function doClick(row){
     ...
     var action     = "O";     
     var parameters = "&strCardCreDt="+encodeURIComponent(strCardCreDt)+"&strCardSeq="+encodeURIComponent(strCardSeq);
     TR_DETAIL.Action="/dcs/dctm117.dm?goTo="+goTo+parameters;  
		...
		<tr>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr>
					...
					<td COLSPAN="3"><comment id="_NSID_"> <object id=EM_CARD_CRE_DT
						classid=<%=Util.CLSID_EMEDIT%> width=70 tabindex=1
				...
				<tr>
					...
					<td width="140"><comment id="_NSID_"> <object id=LC_CARD_GRADE
						classid=<%=Util.CLSID_LUXECOMBO%> height=100 width=130
						align="absmiddle" tabindex=1> </object> </comment><script>_ws_(_NSID_);</script>
					</td>
					<th width="100" class="point">시작번호</th>
					<td><comment id="_NSID_"> <object id=LC_PREFIX
						classid=<%=Util.CLSID_LUXECOMBO%> height=100 width=130
						align="absmiddle" tabindex=1> </object> </comment><script>_ws_(_NSID_);</script>
					</td>
				</tr>
				<tr>
					...
					<td COLSPAN="3"><comment id="_NSID_"> <object id=EM_CDREG_CNT
...
<comment id="_NSID_">
    <object id=BO_I_DATA classid=<%=Util.CLSID_BIND%>>
        ...
        <param name=BindInfo
			...
			<c>col=CARD_PREFIX            ctrl=LC_PREFIX                Param=Text</c>
