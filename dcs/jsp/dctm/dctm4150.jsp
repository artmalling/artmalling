...
//     initEmEdit(EM_SUM_POINT,   "NUMBER^9^0",          READ);       //누적+발생포인트
    ...
    RD_SEARCH_GUBUN.CodeValue = '1'; //조회구분
    EM_CARD_NO_S.Focus();
    
	GD_MASTER.ColumnProp("CUST_NAME", "Show") = "true"; 
	GD_MASTER.ColumnProp("GBN", "Show") 		 = "false";  
	GD_MASTER.ColumnProp("BF_GBN1", "Show")   = "false"; 
	GD_MASTER.ColumnProp("BF_GBN2", "Show")   = "false";
	GD_MASTER.ColumnProp("AFT_GBN1", "Show")  = "false";  
	GD_MASTER.ColumnProp("AFT_GBN2", "Show")  = "true";  
 }
 
 function gridCreate1(){
    var hdrProperies = '<FC>id={currow}     name="NO"            width=30    align=center	</FC>'
                     + '<FC>id=CUST_ID      name="회원ID"         width=113   align=center	</FC>'
                     + '<FC>id=CUST_NAME    name="변경전 회원명"     width=113   align=left		</FC>' 
                     + '<FC>id=GBN          name=""     		width=120   align=left		</FC>'
                     + '<FC>id=BF_GBN1	    name=""     		width=120   align=left		</FC>'
                     + '<FC>id=BF_GBN2 		name=""     		width=120   align=left		</FC>'
                     + '<FC>id=AFT_GBN1 	name=""     		width=120   align=left		</FC>'
                     + '<FC>id=AFT_GBN2 	name="변경후 회원명"     width=120   align=left		</FC>'
                     + '<FC>id=MOD_DATE     name="수정일시"        width=160   align=center mask="XXXX/XX/XX XX:XX:XX"</FC>'
                     + '<FC>id=MOD_ID       name="수정자ID"        width=113   align=left </FC>'
                     + '<FC>id=USER_NAME    name="수정자명"        width=120   align=left</FC>'
                     ;
 ...
 */
 function btn_Search() {
    ...
    strSdt    	= EM_S_DT_S.text; 		//조회일자S
    strEdt    	= EM_E_DT_S.text; 		//조회일자E
    strSearchgbn= RD_SEARCH_GUBUN.CodeValue;//조회구분
    strCustId 	= EM_CUST_ID_S.text;	//회원id
    strCustNm 	= EM_CUST_NAME_S.text;	//회원명
    strCardNo 	= EM_CARD_NO_S.text;  	//카드번호
    strSSNo   	= EM_SS_NO_S.text;    	//생년월일
    
    var goTo        = "searchMaster";    
    var action      = "O";     
    var parameters  = "&strSdt="       + encodeURIComponent(strSdt)
                    + "&strEdt="       + encodeURIComponent(strEdt)              
                    + "&strSearchgbn=" + encodeURIComponent(strSearchgbn)              
                    + "&strCustId="    + encodeURIComponent(strCustId)
                    + "&strCardNo="    + encodeURIComponent(strCardNo)
                    + "&strSSNo="      + encodeURIComponent(strSSNo)
                    ;            
    
    TR_MAIN.Action  = "/dcs/dctm415.dm?goTo="+goTo+parameters;  
 	...
 	if(Gubun == "Search") { 
 	    if(trim(EM_SS_NO_S.Text).length == 0 && trim(EM_CUST_ID_S.Text).length == 0 
 	        ...
 	        if (trim(EM_SS_NO_S.Text).length > 0 && trim(EM_SS_NO_S.Text).length < 6) {
 	            showMessage(EXCLAMATION, OK, "USER-1000",  "생년월일는 6자리입니다.");
 	            EM_SS_NO_S.Focus();
 	            return false;
 	        }
 	    }
...
<script language=JavaScript for=RD_SEARCH_GUBUN event=onSelChange()> 

   DS_O_MASTER.ClearData();
   
   if(this.CodeValue=="2"){ 
	   GD_MASTER.ColumnProp("CUST_NAME", "Name") = "회원명";  
	   GD_MASTER.ColumnProp("GBN", "Name") 		 = "주소구분";  
	   GD_MASTER.ColumnProp("BF_GBN1", "Name")  = "변경전 우편번호"; 
	   GD_MASTER.ColumnProp("BF_GBN2", "Name")  = "변경전 주소";
	   GD_MASTER.ColumnProp("AFT_GBN1", "Name") = "변경후 우편번호"; 
	   GD_MASTER.ColumnProp("AFT_GBN2", "Name") = "변경후 주소";  
   } else if (this.CodeValue=="3" ){  
	   GD_MASTER.ColumnProp("CUST_NAME", "Name") = "회원명";  
	   GD_MASTER.ColumnProp("GBN", "Name") 		 = "";  
	   GD_MASTER.ColumnProp("BF_GBN1", "Name")  = ""; 
	   GD_MASTER.ColumnProp("BF_GBN2", "Name")  = "변경전 생년월일";
	   GD_MASTER.ColumnProp("AFT_GBN1", "Name") = ""; 
	   GD_MASTER.ColumnProp("AFT_GBN2", "Name") = "변경후 생년월일";  
   } else if (this.CodeValue=="4" ){   
	   GD_MASTER.ColumnProp("CUST_NAME", "Name") = "회원명";  
	   GD_MASTER.ColumnProp("GBN", "Name") 		 = "";  
	   GD_MASTER.ColumnProp("BF_GBN1", "Name")  = ""; 
	   GD_MASTER.ColumnProp("BF_GBN2", "Name")  = "변경전 휴대번호";
	   GD_MASTER.ColumnProp("AFT_GBN1", "Name") = ""; 
	   GD_MASTER.ColumnProp("AFT_GBN2", "Name") = "변경후 휴대번호"; 
   } else if (this.CodeValue=="5") {
	   GD_MASTER.ColumnProp("CUST_NAME", "Name") = "회원명";  
	   GD_MASTER.ColumnProp("GBN", "Name") 		 = "";  
	   GD_MASTER.ColumnProp("BF_GBN1", "Name")  = ""; 
	   GD_MASTER.ColumnProp("BF_GBN2", "Name")  = "변경전 카드번호";
	   GD_MASTER.ColumnProp("AFT_GBN1", "Name") = ""; 
	   GD_MASTER.ColumnProp("AFT_GBN2", "Name") = "변경후 카드번호"; 
   } else {
	   GD_MASTER.ColumnProp("CUST_NAME", "Name") = "변경전 회원명"; 
	   GD_MASTER.ColumnProp("GBN", "Name") 		 = "";  
	   GD_MASTER.ColumnProp("BF_GBN1", "Name")   = ""; 
	   GD_MASTER.ColumnProp("BF_GBN2", "Name")   = "";
	   GD_MASTER.ColumnProp("AFT_GBN1", "Name")  = "";  
	   GD_MASTER.ColumnProp("AFT_GBN2", "Name")  = "변경 후 회원명"; 
   }
   ...
   if(this.CodeValue=="1"){   
	   ...
	   GD_MASTER.ColumnProp("CUST_NAME", "Show") = "true"; 
	   GD_MASTER.ColumnProp("GBN", "Show") 		 = "true";  
	   GD_MASTER.ColumnProp("BF_GBN1", "Show")   = "true"; 
	   GD_MASTER.ColumnProp("BF_GBN2", "Show")   = "true";
	   GD_MASTER.ColumnProp("AFT_GBN1", "Show")  = "true";  
	   GD_MASTER.ColumnProp("AFT_GBN2", "Show")  = "true";  
	   
   } else if (this.CodeValue=="3" || this.CodeValue=="4"|| this.CodeValue=="5" ){  
	   GD_MASTER.ColumnProp("CUST_NAME", "Show") = "true"; 
	   GD_MASTER.ColumnProp("GBN", "Show") 		 = "false";  
	   GD_MASTER.ColumnProp("BF_GBN1", "Show")   = "false"; 
	   GD_MASTER.ColumnProp("BF_GBN2", "Show")   = "true";
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				...
				<table width="100%" border="10" cellpadding="0" cellspacing="0"
					class="s_table">
					<tr>
						<th width="77" class="point">조회일자</th>
						<td width="255"><comment id="_NSID_"> 
						    <object id=EM_S_DT_S
							...
							<comment id="_NSID_"> 
							<object id=EM_E_DT_S classid=<%=Util.CLSID_EMEDIT%> width=70 tabindex=1
							align="absmiddle"></object> 
							</comment> <script> _ws_(_NSID_);</script> 
							<img src="/<%=dir%>/imgs/btn/date.gif" align="absmiddle"
							onclick="javascript:openCal('G',EM_E_DT_S)" /> 
					  	</td>
					  	<th width="77" class="point">조회구분</th> 
					  	<td>
				  		  <comment id="_NSID_"> 
			                <object id="RD_SEARCH_GUBUN" classid="<%=Util.CLSID_RADIO%>" width=350 height=25 align="absmiddle">
			                  <param name="Cols" value="5">
			                  <param name="Format" value="1^회원명,2^주소,3^생년월일,4^휴대전화,5^카드번호">
			                </object> 
			              </comment><script>_ws_(_NSID_);</script>   
					  	</td>  
					</tr>   
