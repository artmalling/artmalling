<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 멤버쉽 운영 > 승인처리> 포인트 강제적립/차감
 * 작 성 일 : 2010.03.28
 * 작 성 자 : 장형욱
 * 수 정 자 : 
 * 파 일 명 : dmbo6040.jsp
...
//     initEmEdit(EM_SUM_POINT,   "NUMBER^9^0",          READ);       //누적+발생포인트
    ...
    */
    ...
    registerUsingDataset("dmbo604","DS_I_POINT");
}
...
function btn_Save() {
    ...
    if (DS_O_CUSTDETAIL.NameValue(1, "CARD_NO") == "") {
        showMessage(StopSign, OK, "USER-1000",  "유효한 카드가 존재하지 않는 회원입니다. 포인트 강제적립/차감 할 수 없습니다.");
        return;
    }
...
function showMaster(){
    ...
    TR_MAIN.Action="/dcs/dmbo604.do?goTo="+goTo+parameters;  
...
function searchTodayList(){
    ...
    TR_MAIN2.Action="/dcs/dmbo604.do?goTo="+goTo+parameters;  
...
function saveData(){
    ...
    TR_MAIN.Action="/dcs/dmbo604.do?goTo=saveData";
