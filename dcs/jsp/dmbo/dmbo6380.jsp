...
function doInit(){
    ...
    initEmEdit(EM_CONV_POINT,		"NUMBER^9^0",	PK);
    initEmEdit(EM_CONV_GIFT_AMT,	"NUMBER^9^0",	NORMAL);
    ...
    registerUsingDataset("dmbo638","DS_O_MASTER,DS_IO_DETAIL"); 
}
...
function gridCreate1(){      
    var hdrProperies = '<FC>id={currow}        		name="NO"            		width=30    align=center</FC>'
                     ...
                     + '<FC>id=CONV_POINT			name="OK캐쉬백 교환포인트"			width=150   align=right</FC>'
                     + '<FC>id=CONV_GIFT_AMT  		name="OK캐쉬백 교환상품권금액"  		width=150   align=right</FC>'
...
function btn_Search() {
	...
	if (DS_O_MASTER.IsUpdated || DS_IO_DETAIL.IsUpdated){
	    if(showMessage(EXCLAMATION, YESNO, "USER-1073") != 1 ){
	    	setTimeout("EM_CONV_POINT.Focus();",50);
	        return false;
        }else{
	        strChangFlag = true;
	    }
...
function btn_New() {
    if (DS_O_MASTER.IsUpdated || DS_IO_DETAIL.IsUpdated){
	    if(showMessage(EXCLAMATION, YESNO, "USER-1072") != 1 ){
	    	...
	    	strChangFlag = true;
	        if(DS_O_MASTER.NameValue(DS_O_MASTER.CountRow, "CONV_POINT") == ""){
...
function btn_Save() {
	...
	if(trim(EM_CONV_POINT.Text).length == 0){
        showMessage(EXCLAMATION, OK, "USER-1002",  "교환포인트");
        EM_CONV_POINT.Focus();
        return;
    }
...
function showMaster(){
    ...
    var strConvPoint  	= EM_CONV_POINT.Text;
    ...
    var parameters  = "&strMemoDateF=" + encodeURIComponent(strMemoDateF)
                    ...
                    + "&strConvPoint="+ encodeURIComponent(strConvPoint);
    TR_MAIN.Action  = "/dcs/dmbo638.do?goTo="+goTo+parameters;  
...
function showDetail(){
    ...
    var strConvPoint = DS_O_MASTER.NameValue(row ,"CONV_POINT");
    ...
    TR_DETAIL.Action  = "/dcs/dmbo638.do?goTo="+goTo+parameters;  
...
function saveDetail(){
    ...
    TR_MAIN.Action  ="/dcs/dmbo638.do?goTo="+goTo;   
...
function newData(flag) {
    ...
    EM_CONV_POINT.Enable  	= flag;
    EM_CONV_GIFT_AMT.Enable	= flag;
...
<script language=JavaScript for=DS_O_MASTER event=CanRowPosChange(row)>
    if ( strChangFlag == false && (DS_O_MASTER.IsUpdated || DS_IO_DETAIL.IsUpdated)){
        if( showMessage(QUESTION, YESNO, "USER-1074") != 1 ){
        	...
        	if(DS_O_MASTER.NameValue(row, "CONV_POINT") == "")
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			class="s_table">
            <tr>
                <th width="140" class="point">가맹점</th>
            ...
            <tr>
                <th width="140" class="point">OK캐쉬백 교환포인트</th>
                <td width="200"><comment id="_NSID_"> <object
							id=EM_CONV_POINT classid=<%=Util.CLSID_EMEDIT%> width=160
							tabindex=1></object> </comment> <script> _ws_(_NSID_);</script>
				</td>
                <th width="140">OK캐쉬백 교환상품권금액</th>
                <td><comment id="_NSID_"> <object
							id=EM_CONV_GIFT_AMT classid=<%=Util.CLSID_EMEDIT%> width=160
							tabindex=1></object> </comment> <script> _ws_(_NSID_);</script>
                </td>
            </tr>	            			
            <tr>
               <th width="150" class="point">시작일자</th>
                   <td><comment id="_NSID_"> <object
...
<object id=BD_DETAIL classid=<%=Util.CLSID_BIND%>>
	...
	<param name=BindInfo
		value='
            ...
            <c>col=CONV_POINT    	ctrl=EM_CONV_POINT    	 Param=Text</c>
            <c>col=CONV_GIFT_AMT    ctrl=EM_CONV_GIFT_AMT    Param=Text</c>
