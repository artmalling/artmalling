<!-- 
/*******************************************************************************
 * 시스템명 : 영업지원 > 고객관리 > 포인트운영 > OK캐쉬백상품권교환
 * 작  성  일  : 2010.01.14
 * 작  성  자  : 김영진
 * 수  정  자  : 
 * 파  일  명  : dmbo6410.jsp
...
function doInit(){
    ...
    initEmEdit(EM_PWD_NO_S,    "0000",                NORMAL);     //비밀번호
...
//     initEmEdit(EM_SUM_POINT,   "NUMBER^9^0", READ);       //누적+발생포인트
    ...
    registerUsingDataset("dmbo641","DS_I_POINT,DS_I_MASTER");
...
function saveData(){
    ...
    var goTo        = "chkSave";
    var action      = "I";  //조회는 O
    TR_MAIN2.Action  ="/dcs/dmbo641.do?goTo="+goTo;   
 ...
 */
 function realSave(){
    ...
    var strCardNo    = EM_CARD_NO_S.Text;
    var strPwdNo     = EM_PWD_NO_S.Text;
    var strVaildNo   = EM_VAILD_NO_S.Text;
	
    var goTo        = "save";
    var action      = "I";  //조회는 O
    var parameters  = "&strCardNo="    + encodeURIComponent(strCardNo)
                    + "&strPwdNo="     + encodeURIComponent(strPwdNo)
                    + "&strVaildNo="   + encodeURIComponent(strVaildNo);
    TR_MAIN.Action  ="/dcs/dmbo641.do?goTo="+goTo+parameters;   
...
function showMaster(){
    ...
    var parameters  = "&strCardNo="    + encodeURIComponent(strCardNo)
                    ...
                    + "&strVaildNo="   + encodeURIComponent(strVaildNo);
    TR_MAIN.Action  ="/dcs/dmbo641.do?goTo="+goTo+parameters;  
    TR_MAIN.KeyValue="SERVLET("+action+":DS_O_MASTER=DS_O_MASTER,"+action+":DS_IO_RULE=DS_IO_RULE,"+action+":DS_O_CUSTDETAIL=DS_O_CUSTDETAIL)"; //조회는 O
    TR_MAIN.Post();
}
...
function searchAmt(){
    ...
    TR_MAIN3.Action  ="/dcs/dmbo641.do?goTo="+goTo+parameters;  
...
<script language=JavaScript for=GR_MASTER event=OnExit(row,colid,olddata)>
		
    if(colid == "CONV_QTY" && DS_IO_RULE.NameValue(row, "CONV_QTY") >= 6)
    {
    	showMessage(StopSign, OK, "USER-1000",  "최대교환가능 매수는 5장 입니다.");
    	DS_IO_RULE.NameValue(row, "CONV_QTY") = 0;
    }
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td width="150">
							<comment id="_NSID_">
							<object id=EM_PWD_NO_S
							classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1>
							</object> </comment> <script> _ws_(_NSID_);</script></td>
						<th width="80">유효기간</th>
						<td>
							<comment id="_NSID_">
							<object id=EM_VAILD_NO_S
							classid=<%=Util.CLSID_EMEDIT%> width=100 tabindex=1>
							</object> </comment> <script> _ws_(_NSID_);</script>(MONTH/YEAR)
						</td>
					</tr>
				</table>
				</td>
				...
				<tr>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="s_table">
						<tr>
							...
							<td width="160"><comment id="_NSID_"> <object id=EM_POINT
								classid=<%=Util.CLSID_EMEDIT%> width=140 tabindex=1></object> </comment> <script> _ws_(_NSID_);</script>
							</td>
							<th width="80">발생포인트</th>
							<td width="160"><comment id="_NSID_"> <object id=EM_OCCURS_POINT
								classid=<%=Util.CLSID_EMEDIT%> width=140 tabindex=1></object> </comment> <script> _ws_(_NSID_);</script>
							</td>
							<th width="80">누적포인트</th>
							<td colspan="2"><comment id="_NSID_"> <object id=EM_SUM_POINT
								classid=<%=Util.CLSID_EMEDIT%> width=140 tabindex=1></object> </comment> <script> _ws_(_NSID_);</script>
							</td>
						</tr>
						<tr>
							<th width="80">상태메세지</th>
							<td colspan="6"><comment id="_NSID_"> <object id=EM_MESSAGE
								classid=<%=Util.CLSID_EMEDIT%> width=240 tabindex=1></object> </comment> <script> _ws_(_NSID_);</script>
							</td>
						</tr>
					</table>
					</td>
    ...
    <tr>
        <td class="sub_title PT07 PB02"><img
            ...
            align="absmiddle" class="PR03" /> 상품권 교환 내역 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    </tr>
    <tr valign="top">
        <td class="PB03">
        <table width="100%" border="0" cellspacing="0" cellpadding="0"
...
<object id=BD_CUSTDETAIL classid=<%=Util.CLSID_BIND%>>
	...
	<param name=BindInfo
		value='
            ...
            <c>col=MESSAGE1      ctrl=EM_MESSAGE      Param=Text</c>
        '>
</object>
</comment>
<script>_ws_(_NSID_);</script>
