...
function gridCreate1(){
     var hdrProperies = '<FC>id={currow}      			name="NO"             	width=30    align=center</FC>'
                      ...
                      + '<FC>id=TOT_SALE_AMT       		name="총매출"       		width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=CUST_SALE_AMT     		name="회원매출"     		width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=CUST_SALE_RATE     		name="구성비"   			width=80   	align=right </FC>'
                      + '<FC>id=NCUST_SALE_AMT    		name="비회원매출"		    width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=NCUST_SALE_RATE   		name="구성비"  			width=80   	align=right </FC>'
                      + '</FG>' 
                      + '<FG>id=TITLE                  	name="대비일자" 		edit=none 	align=center'
                      + '<FC>id=CMPR_DT          		name="대비일자"       	width=100   align=center mask="XXXX-XX-XX"</FC>'
                      + '<FC>id=PRE_TOT_SALE_AMT   		name="총매출"   		width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=PRE_CUST_SALE_AMT   	name="회원매출"   		width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=PRE_CUST_SALE_RATE     	name="구성비"   			width=80   	align=right </FC>'
                      + '<FC>id=PRE_NCUST_SALE_AMT  	name="비회원매출"   		width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=PRE_NCUST_SALE_RATE   	name="구성비"  			width=80   	align=right </FC>'
                      + '</FG>' 
                      + '<FC>id=ADD_CUST_SALE_AMT   	name="회원매출;증감액"		    width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=ADD_CUST_SALE_RATE     	name="회원매출;증감율"    	    width=80   	align=right </FC>'
                      + '<FC>id=ADD_NCUST_SALE_AMT      name="비회원매출;증감액"    	width=100   align=right SumText=@sum</FC>'
                      + '<FC>id=ADD_NCUST_SALE_RATE    	name="비회원매출;증감율"		width=80   </FC>'
                      ;
                      /*+ '<FC>id=TOT_DANGA   			name="총매출 객단가"     		width=100    align=right show=false</FC>'                      
                      + '<FC>id=CUST_DANGA   			name="회원매출 객단가"   		width=100    align=right show=false</FC>'
                      + '<FC>id=NCUST_DANGA   			name="비회원매출 객단가"		width=100    align=right show=false</FC>'
...
function showMaster(){
    ...
    var parameters  = "&strFromBSDate="+ encodeURIComponent(EM_FROM_BS_DT.Text)
					...
					/*
					+ "&strFromCSDate="+ EM_FROM_CS_DT.Text
					+ "&strToCSDate="+ EM_TO_CS_DT.Text
					*/
					;
    TR_MAIN.Action  ="/dcs/dbri313.db?goTo="+goTo+parameters;  
