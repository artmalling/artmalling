<!-- 
/*******************************************************************************
 * 시스템명 : 포인트카드 > 가맹점관리 > 실적조회 > 일단위 포인트 현황 조회 
 * 작 성 일    : 2010.01.14
 * 작 성 자    : 김영진
 * 수 정 자    : 
 * 파 일 명    : dbri2010.jsp
...
function doInit(){
    ...
    // EMedit에 초기화
    initEmEdit(EM_SUM_S_DT,    "YYYYMMDD",  PK);           //조회 시작기간    
    initEmEdit(EM_SUM_E_DT,    "TODAY",     PK);           //조회 종료기간
...
function gridCreate1(){
    var hdrProperies = '<FC>id={currow}      name="NO"           width=30   align=center</FC>'
                     ...
                     + '<FC>id=SUM_DT        name="조회일자"      width=76  align=center  mask="XXXX/XX/XX" SumText="합계"</FC>'
                     + '<FC>id=COMP_NO       name="사업자번호"    width=86  align=center  mask="XXX-XX-XXXXX"</FC>'
                     ...
                     + '<FC>id=RECP_POINT    name="영수증포인트"  width=86  align=right   SumText=@sum show=false</FC>'
                     + '<FC>id=CAM_POINT     name="캠페인포인트"  width=86  align=right   SumText=@sum show=false</FC>'
                     + '<FC>id=EVENT_POINT   name="이벤트포인트"  width=86  align=right   SumText=@sum show=false</FC>'
                     + '<FC>id=DCUBE_DVD_AMT name="적립분담금"  width=86  align=right   SumText=@sum</FC>'
                     + '<FC>id=BRCH_DVD_AMT  name="가맹점분담금"  width=86  align=right   SumText=@sum</FC>'
                     + '<FC>id=ADD_FEE_AMT   name="적립수수료"    width=70  align=right   SumText=@sum</FC>'
                     + '<FC>id=USE_CNT       name="사용건수"      width=56  align=right   SumText=@sum</FC>'
...
function btn_Search() {
    ...
    if(trim(EM_SUM_S_DT.Text).length == 0){          // 조회시작일
        showMessage(EXCLAMATION, OK, "USER-1001","조회기간");
        EM_SUM_S_DT.Focus();
        return;
    }
    if(trim(EM_SUM_E_DT.Text).length == 0){    // 조회 종료일
        showMessage(EXCLAMATION, OK, "USER-1001","조회기간");
        EM_SUM_E_DT.Focus();
        return;
    }
    if(EM_SUM_S_DT.Text > EM_SUM_E_DT.Text){   // 조회일 정합성
...
function showMaster(){
    ...
    strSumSDt   = EM_SUM_S_DT.Text;
    strSumEDt   = EM_SUM_E_DT.Text;
    var goTo        = "searchMaster";    
    var action      = "O";     
    var parameters  = "&strBrchId="   + encodeURIComponent(strBrchId)
                    + "&strSumSDt="   + encodeURIComponent(strSumSDt)
                    + "&strSumEDt="   + encodeURIComponent(strSumEDt);
    TR_MAIN.Action  ="/dcs/dbri201.db?goTo="+goTo+parameters;  
...
<script language=JavaScript for=EM_SUM_S_DT event=onKillFocus()>
    if(!checkDateTypeYMD(EM_SUM_S_DT)){
    	EM_SUM_S_DT.text = <%=strToMonth%>;
    }
</script>
<!-- 조회기간 End onKillFocus() -->
<script language=JavaScript for=EM_SUM_E_DT event=onKillFocus()>
    checkDateTypeYMD(EM_SUM_E_DT);
</script>
<!--*************************************************************************-->
<!--* C. 가우스 이벤트 처리 끝                                                                                                                                  *-->
<!--*************************************************************************-->
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td><comment id="_NSID_"> <object id=EM_SUM_S_DT
							classid=<%=Util.CLSID_EMEDIT%> width=70 tabindex=1
							align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script> <img
							src="/<%=dir%>/imgs/btn/date.gif" align="absmiddle"
							onclick="javascript:openCal('G',EM_SUM_S_DT)" />- <comment
							id="_NSID_"> <object id=EM_SUM_E_DT
							classid=<%=Util.CLSID_EMEDIT%> width=70 tabindex=1
							align="absmiddle"> </object> </comment> <script> _ws_(_NSID_);</script> <img
							src="/<%=dir%>/imgs/btn/date.gif" align="absmiddle"
							onclick="javascript:openCal('G',EM_SUM_E_DT)" /></td>
